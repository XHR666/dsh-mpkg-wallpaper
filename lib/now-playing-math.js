...
/* ①(2026-09-21 第17条)「卡片缩小时描述行与左右切歌键重叠」——
   实测几何（两端都对，中间那段错）：收起端 p=0 时文字块右边缘 154、传输行左边缘 162（相距 8px，
   没问题）；可 p≈0.3 时传输行从下方升进文字带，且横向比文字块右边缘更靠左
   （行 [131,235] × 文字块 [67,171]）⇒ 描述行与切歌键**真的叠在一起**（用户读数）。
   修法（纯几何，不加状态）：只要两条带在纵向相交，文字块的右边缘就必须停在传输行左边缘之前
   （留 SAY_ROW_GAP 的呼吸）—— `sayWidth` 因此取"原宽度"与"不越界宽度"的较小值。
   判据：tools/np-control-test.mjs 的 F 组按 0.01 步长全枚举 p，断言两矩形永不相交；
   并把**旧公式**喂给同一个相交判据（必须判红）作为分辨力对照。 */
const SAY_ROW_GAP = 8;
/** 传输行的矩形（{l,r,t,b}，与渲染用同一份几何；渲染器测试台若复用组件也读它）。 */
const transportRow = (p) => {
  const side = transportSide(p), lead = transportLead(p), gap = transportGap(p);
  const w = side * 2 + lead + gap * 2;
  const cx = opsX(p), cy = opsY(p);
  return { l: cx - w / 2, r: cx + w / 2, t: cy - side / 2, b: cy + side / 2 };
};
/** 文字块矩形（宽度已按"不与传输行相交"收口）。 */
const sayRect = (p) => {
  const l = sayLeft(p), w = sayWidth(p);
  return { l: l, r: l + w, t: artY(), b: artY() + artSize(p), w: w };
};
/** 两条带在纵向是否相交（横向的收口只在"纵向相交"时才需要）。
 *  注意：这里**只看纵向**，不调用 sayRect —— 后者用了收口后的宽度，会与 sayWidth 递归。 */
const sayRowBandsMeet = (p) => {
  const b = transportRow(p);
  const t = artY(), bot = artY() + artSize(p);
  return !(bot <= b.t || t >= b.b);
};
/** 旧公式（未收口）—— 只用于分辨力对照与外部复用，渲染一律走 sayWidth。 */
const sayWidthRaw = (p) => mix(94, W - PAD - (PAD + ART.s[1] + 10) - (LIKE + 10), p);
const sayWidth = (p) => {
  const base = sayWidthRaw(p);
  if (!sayRowBandsMeet(p)) return base;
  const allow = transportRow(p).l - sayLeft(p) - SAY_ROW_GAP;
  return allow > 0 ? Math.min(base, allow) : base;
};
...
/** 时间轴（**唯一事实源**）：当前媒体的时长。缺省/非法值才退回上游演示常量 `TOTAL`。
 *
 *  为什么必须带参（2026-09-21 真机）：卡片的"−剩余"与进度条填充过去无条件用 `TOTAL`（214 = 上游
 *  演示里那首曲子的秒数）。真实媒体 100.05s（1:40）时：卡片写 `−3:29`、条子播到结尾也只画到
 *  47%（100/214）⇒ 用户看到的正是「1 分 40 秒读成 3 分 30 多秒」+「点一下只跳到整条的一半」。
 *  3.11.0 修的是**数据目标**（npMediaTarget：位置/时长/seek 落到同一个元素），
 *  但**显示轴**还是这个常量 —— 于是"数据对、显示错"能长期共存。这条口径与数据面同源：
 *  显示用的时长 == `npResolveMedia()` 送进控制器的 `media.total` == 媒体元素自己的 duration。 */
const axisOf = (total) => (typeof total === "number" && isFinite(total) && total > 0 ? total : TOTAL);
/** 播放进度百分比（0..`total` 的时钟 → 0..100）。`total` 省略 = 上游演示常量（演示/纯几何测试用）。 */
const runPct = (at, total) => (clamp(at, 0, axisOf(total)) / axisOf(total)) * 100;
/** 剩余时间（负号由组件加，保证两段数字等宽）。同样以 `total` 为准（唯一事实源）。 */
const remain = (at, total) => axisOf(total) - clamp(at, 0, axisOf(total));
...
module.exports = {
  ...
  lateOf, sayLeft, sayWidth, sayWidthRaw, sayRect, transportRow, sayRowBandsMeet, SAY_ROW_GAP,
