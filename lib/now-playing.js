...
function createNowPlaying(deps) {
  ...
  const h = react.createElement;
    ...
    /* 时间轴 = **当前媒体的时长**（唯一事实源；`known` 为假才退回上游演示常量）。
       夹取、进度条填充、"−剩余"三处**共用这一个值** —— 任何一处各自用常量，就会出现
       "数据对、显示错"（真机：100.05s 的媒体被画成 214s 的轴）。判据见
       tools/np-axis-live-probe.mjs（真机）与 tools/np-control-test.mjs 的 X 组（无浏览器）。 */
    const axis = M.axisOf(known ? media.total : null);
    const at = Math.max(0, Math.min(axis, Number(q.at) || 0));
        ...
        h("span", {
          ...
          h("span", { className: "mpw_np_rail" },
            h("span", {
              ...
              style: { width: (known ? M.runPct(at, axis) : 0) + "%" },
            })),
          ...
          h("span", { className: "mpw_np_clock", style: { opacity: late } },
            ...
            h("span", null, known ? ("−" + M.clock(M.remain(at, axis))) : "--:--"))),
        h("button", {
  ...
  return {
    /* ①(2026-09-21) 纯渲染函数也对外：**显示面**（进度条填充宽度、"−剩余"文本）要能在
       无浏览器门禁里对着产出的元素树断言 —— 只测控制器状态测不到"数据对、显示错"
       （真机 bug：媒体 100.05s，卡片按上游常量 214 画）。判据：tools/np-control-test.mjs 的 X 组。 */
