...
export function installAudioBus(win, opts = {}) {
  ...
  const syncFrames = () => {
    ...
    state.frames = frames.map((f, i) => {
      ...
      if (sameOrigin && cw) { try { installAudioBus(cw, { mode: frameModeFor(state.mode), onEvent: opts.onEvent, mediaRoots: opts.mediaRoots }) } catch (e) {} }
  ...
  const api = {
    ...
    setMode(m) { state.mode = normalizeMode(m); syncFrames(); applyMute(); emit('mode', { mode: state.mode }); return state.mode },
    setMuted(v) {
      state.muted = !!v;
      syncFrames();
      applyMute();
