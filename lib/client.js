window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *  ①(新) folder= → /custom-folder（自定义目录里的 workshop 子文件夹资源）；
		 *  web=1 → /library-web（本地库目录内任意资源，网页壁纸 iframe 用）。 */
		function resolveHostUrl(image) {
			try {
				if (typeof image === "string" && image.indexOf("host:") === 0) {
					const q = image.slice(5);
					if (q.indexOf("ltoken=") >= 0) {
						if (q.indexOf("web=1") >= 0) return HOST_BASE + "/library-web" + q.replace("web=1&", "");
						return HOST_BASE + "/library-media" + q;
					}
					if (q.indexOf("custom=") >= 0) {
						if (q.indexOf("folder=") >= 0) return HOST_BASE + "/custom-folder" + q.replace("custom=1&", "");
...
`;
		...
		function MpkgSection(props) {
			...
			const applyWallFromList = (w) => {
				...
				// ①(新) 自定义目录 workshop 子文件夹：网页壁纸 → iframe；场景 → 预览图
			...
			const applyCustomWeb = (w) => {
				commit({
					webUrl: "host:?custom=1&folder=" + encodeURIComponent(w.name) + "&file=" + encodeURIComponent(w.media || "index.html"),
					source: w.title, mpkgKey: w.key, mpkgName: w.title,
					fromMpkg: false, converted: "web", slot: null,
					info: undefined, timeVideos: undefined, timeConfig: undefined, activeSlot: null,
					image: undefined
				}, true);
				setMpkgMeta({ name: w.title, key: w.key, info: null, entryName: w.media || "index.html", slot: null });
				setHint(t("lib.applied") + "：" + w.title);
			};
			// ①(新) 应用自定义目录里的场景壁纸（暂用预览图；静态帧提取后续替换）
			const applyCustomScenePreview = (w) => {
				const pv = w.preview || "preview.jpg";
				commit({
					image: "host:?custom=1&folder=" + encodeURIComponent(w.name) + "&file=" + encodeURIComponent(pv),
					source: w.title, mpkgKey: w.key, mpkgName: w.title,
					fromMpkg: false, converted: "gif", slot: null,
					info: undefined, timeVideos: undefined, timeConfig: undefined, activeSlot: null,
					webUrl: undefined
				}, true);
				setMpkgMeta({ name: w.title, key: w.key, info: null, entryName: pv, slot: null });
				setHint(t("lib.applied") + "：" + w.title);
			};
			...
			const applyLibraryWallpaper = (wp) => {
				// ①(新) Steam 库网页壁纸 → iframe（/library-web 服务目录内全部资源）
				if (wp.type === "web") {
					if (!wp.media) { showError(t("lib.sceneOnly")); return; }
					const file = String(wp.media).split(/[\\/]/).pop();
					commit({
						webUrl: "host:?ltoken=" + wp.ltoken + "&web=1&file=" + encodeURIComponent(file),
				...
				// ①(新) Steam 库场景壁纸 → 预览图（静态帧提取后续接入）
				if (wp.type === "scene") {
					const file = String(wp.preview || "").split(/[\\/]/).pop() || "preview.jpg";
					commit({
						image: "host:?ltoken=" + wp.ltoken + "&web=1&file=" + encodeURIComponent(file),
						source: wp.title, mpkgKey: "library|" + wp.ltoken, mpkgName: wp.title,
						fromMpkg: false, converted: "gif", slot: null,
						info: undefined, timeVideos: undefined, timeConfig: undefined, activeSlot: null,
