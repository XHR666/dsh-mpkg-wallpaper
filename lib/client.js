			...
			const SpeedBar = (props) => {
				...
				return h("div", { className: "mpw_speedBar", ref,
					...
					SPEED_LEVELS.map((rate, i) => h("div", { key: rate, style: {
						position: "absolute", top: "50%", marginTop: -9, left: "calc(" + (6 + i / (SPEED_LEVELS.length - 1) * (100 - 12)) + "% - 1px)",
						width: 1, height: 18, background: "color-mix(in srgb, var(--dsw-alias-border-l2, #444b5c) 70%, transparent)"
					} })),
					// 填充进度：与当前档位标记严格对齐（i/(N-1)），不再多出 1/3（之前误用 (i+1)/N）
					...
					// 当前档位标记
					h("div", { style: { position: "absolute", top: "50%", marginTop: -8, left: "calc(" + (6 + idx / (SPEED_LEVELS.length - 1) * (100 - 12)) + "% - 7px)", width: 14, height: 14, borderRadius: "50%", background: "var(--dsw-alias-brand-primary, #4f8cff)", border: "2px solid var(--dsw-alias-bg-base, #0e1420)" } })
				);
			};
			...
			return h("div", { className: "mpw_row" }, [
				...
				rotEditor && rotEdit ? h(MaskPortal, { onClick: () => setRotEditor(false) }, [
					h("div", { className: "mpw_dialog", style: { width: "min(560px, calc(100vw - 32px))" }, onClick: (ev) => ev.stopPropagation() }, [
						...
						h("input", { className: "mpw_input", type: "text", style: { width: "100%", boxSizing: "border-box", height: 46, fontSize: 15, marginBottom: 8 }, placeholder: t("rot.namePh"), value: rotEdit.name, onChange: (ev) => setRotEdit(Object.assign({}, rotEdit, { name: ev.target.value })) }),
						// ①(修正) 顺序/随机放这里；轮换间隔移到列表选择下方单独设置（用户要求）
						h("div", { className: "mpw_inline", style: { marginBottom: 8 } }, [
							...
							h("span", { className: "mpw_hint" }, t("rot.intervalHint"))
						]),
						h("p", { className: "mpw_hint" }, t("rot.pickHint") + "（" + wallList.length + "）"),
						// ①(修正) 勾选改整行点击（原生 checkbox 在移动端点击不可靠，用户实测不能选）
