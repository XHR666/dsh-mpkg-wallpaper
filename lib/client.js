...
/* ①(修正) 悬浮效果开启时，rail 右缘**不向右溢出**：收起态悬浮的右 margin -12px
   会让 rail 视觉超出入侵 DSH 中心列，而 better-sidebar 底部面板 left 按 grid 轨道
   测量（不含 margin）→ rail 右缘与面板左缘重叠 12px（用户实测：悬浮+收起+底部
   面板展开重叠）。加一条 clamp，rail 更窄但右缘不越界，面板不重叠。 */
body[data-mpw-float] [data-sidebar-collapsed] [class*="sidebarCol"] {
