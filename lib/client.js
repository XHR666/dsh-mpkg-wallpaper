...
`;
			...
			// 采用开源社区标准方案 **mix-blend-mode: difference**（白色文字与背景做差混合）：
			// 文字经过深色背景 → 自动变白；经过浅色背景 → 自动变深（背景反色）。
			// 这是"文字智能适配背景"的标准做法（iCSS #169），无需逐字采样。
			// 只作用普通文本段落，排除 code/pre（代码块保留原色，避免反色混乱）。
			if (section.aquaTextEnhance !== void 0 ? !!section.aquaTextEnhance : false) {
				css += `
/* ── 深底文字可读增强（实验，默认关）：mix-blend-mode difference 自动反色 ── */
body[data-mpw-aqua-text] [data-conversation-scroll] :is(p, li, h1, h2, h3, h4, td, th, blockquote, dd, dt) {
