...
function readSection() {
		...
		function applyFromStorageInner() {
			...
			try {
				...
				if (rsBlurOn) document.body.setAttribute("data-mpw-rsblur", ""); else document.body.removeAttribute("data-mpw-rsblur");
		...
		function mpwDiagCollect() {
			...
			const sels = {
				dialogs: '[role="dialog"], [role="alertdialog"], [class*="_overlay_"], [class*="_modal_"], [class*="_dialog_"], [class*="_settings_"]',
				...
				leftSide: 'aside, [class*="_sidebar_"]'
			};
			const out = { at: new Date().toISOString(), url: location.href, viewport: [innerWidth, innerHeight],
				body: pick(document.body), html: pick(document.documentElement), groups: {} };
			for (const [k, sel] of Object.entries(sels)) {
				try { out.groups[k] = Array.from(document.querySelectorAll(sel)).slice(0, 6).map(el => ({ self: pick(el), chain: chain(el) })); }
			...
			if (localStorage.getItem('mpwdiag') !== '0') {
				...
				try {
					...
					const mo = new MutationObserver(() => {
						...
						const hasUI = document.querySelector('[role="dialog"], [class*="_overlay_"], [class*="_modal_"], [class*="_menu_"], [role="menu"], [class*="_settings_"]');
						if (!hasUI) return;
...
html body:not(:has([class*="_overlay_"])):not(:has([class*="_modal_"])):not(:has([class*="_portal_"])) [data-sidebar-right-panel],
html body:not(:has([class*="_overlay_"])):not(:has([class*="_modal_"])):not(:has([class*="_portal_"])) [data-sidebar-right-float-host],
html body:not(:has([class*="_overlay_"])):not(:has([class*="_modal_"])):not(:has([class*="_portal_"])) [data-mpw-rs],
html body:not(:has([class*="_overlay_"])):not(:has([class*="_modal_"])):not(:has([class*="_portal_"])) [data-dockkit-strip],
html body:not(:has([class*="_overlay_"])):not(:has([class*="_modal_"])):not(:has([class*="_portal_"])) [data-dockkit-float] {
...
body:not([data-mpw-rsblur]) [data-sidebar-right-panel], body:not([data-mpw-rsblur]) [data-sidebar-right-float-host],
body:not([data-mpw-rsblur]) [data-dockkit-strip], body:not([data-mpw-rsblur]) [data-dockkit-float] {
  -webkit-backdrop-filter: none !important;
  backdrop-filter: none !important;
}
`;
...
html body:not([data-mpw-sblur-off]):not([data-mpw-header-blur-off]):not(:has([class*="_overlay_"])):not(:has([class*="_modal_"])):not(:has([class*="_portal_"])) .pI_x6G_sidebarCol,
html body:not([data-mpw-sblur-off]):not([data-mpw-header-blur-off]):not(:has([class*="_overlay_"])):not(:has([class*="_modal_"])):not(:has([class*="_portal_"])) [class*="sidebarCol"]:not([data-sidebar-right-panel]),
html body:not([data-mpw-sblur-off]):not([data-mpw-header-blur-off]):not(:has([class*="_overlay_"])):not(:has([class*="_modal_"])):not(:has([class*="_portal_"])) .wSkVaW_header {
	backdrop-filter: blur(${unifyAmount}px) !important;
	-webkit-backdrop-filter: blur(${unifyAmount}px) !important;
}
html body:has([class*="_overlay_"]), html body:has([class*="_modal_"]), html body:has([class*="_portal_"]) {
	--mpw-overlay-open: 1;
}
/* 全局兜底：任何 fixed overlay/modal 打开时，栏与标题栏的 backdrop-filter 一律关闭
   （backdrop-filter 会把栏变成 fixed 弹窗的 containing block → 弹窗尺寸被压缩进侧栏）。 */
html body:has([class*="_overlay_"]) .pI_x6G_sidebarCol,
html body:has([class*="_overlay_"]) [class*="sidebarCol"],
html body:has([class*="_overlay_"]) .wSkVaW_header,
html body:has([class*="_modal_"]) .pI_x6G_sidebarCol,
html body:has([class*="_modal_"]) [class*="sidebarCol"],
html body:has([class*="_modal_"]) .wSkVaW_header,
html body:has([class*="_overlay_"]) [data-mpw-rs],
html body:has([class*="_modal_"]) [data-mpw-rs] {
	backdrop-filter: none !important;
	-webkit-backdrop-filter: none !important;
}
`;
...
`;
		...
		function MpkgSection(props) {
			...
			return h("div", { className: "mpw_row mpw_glassHost" }, [
				...
				(section.image || section.webUrl) ? h("div", { className: "mpw_field" }, [
					(section.source || (mpkgMeta && mpkgMeta.name))
						? h("p", { className: "mpw_hint" }, [
							(section.source || mpkgMeta.name),
							(section.entryName || (mpkgMeta && mpkgMeta.entryName)) ? " · " + t("mpkg.using") + "：" + (section.entryName || mpkgMeta.entryName)
				...
				toggleRow(t("sidebarBlur"), t("sidebarBlur.desc"), "sidebarBlur", DEFAULT_SIDEBAR_BLUR,
					!(section.sidebar !== void 0 ? !!section.sidebar : DEFAULT_SIDEBAR)
						|| (section.unifyTint !== void 0 ? !!section.unifyTint : DEFAULT_UNIFY_TINT),
					(section.unifyTint !== void 0 ? !!section.unifyTint : DEFAULT_UNIFY_TINT)),
				h("div", { style: {
				...
				(section.unifyTint !== void 0 ? !!section.unifyTint : DEFAULT_UNIFY_TINT)
					...
					: null,
				(section.sidebar !== void 0 ? !!section.sidebar : DEFAULT_SIDEBAR) === false
					? h("p", { className: "mpw_hint" }, t("sidebarBlur.needReveal"))
					: null,
				...
				// ①(修正) aquaMask（统一雾）已挪到「界面统一」tab 第 2 类——这里不再重复
		...
		const zh = {
			...
			"sec.unify": "界面统一",
			"sec.unify.desc": "整屏界面外观统一管理，两类效果互不干扰：①统一虚化=壁纸模糊度+白雾厚度（侧边栏/标题栏整体毛玻璃）；②统一雾=全屏色调雾罩（所有表面共享一种雾色/浓度）。开启后「界面虚化」里的侧边栏磨砂被接管",
			"sec.unifyFog": "统一雾（色调）",
			"sec.unifyFog.desc": "全屏纯色雾罩：所有表面（侧边栏/标题栏/聊天区/按钮）共享一种雾色与浓度，让整屏色调统一；关 = 保持原有分区配色",
			"sec.show": "透出壁纸",
			...
			"sidebarBlur.needReveal": "需先开启「侧边栏透出壁纸」才能使用侧边栏磨砂（受侧边栏透出控制）",
			...
			const check = () => {
				try {
					...
					const isFixedOverlay = (el) => {
						try { return getComputedStyle(el).position === 'fixed' } catch { return false }
					};
					const hasDlg = Array.from(document.querySelectorAll('[class*="_overlay_"], [class*="_modal_"]')).some(isFixedOverlay)
						|| Array.from(document.querySelectorAll('[role="dialog"], [role="alertdialog"], [class*="_dialog_"], [class*="_settings_"]')).some(isFullDlg)
