window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function hideWebPanel(frame) {
			try {
				...
				try {
					...
					const obs = new MutationObserver(() => { try { hidePanel(); } catch {} });
		...
		function applyFromStorage() {
			...
			// ①(修正) 放宽门控：只要 webUrl 存在就优先显示 web（converted 只是辅助标记，
			// 不同导入路径可能没写对/丢失——用户实测 clear 后导入星野，converted 丢失导致
			// 走了 img 路径 → iframe 空白）。有 webUrl 就一定是 web 壁纸。
			if (section.webUrl) {
				if (enabled) {
			...
			if (typeof image === "string" && image.indexOf("host:") === 0) {
				...
				if (img.src !== image) {
					...
					// ①(诊断) clear 后导入星野不显示：输出当前 section 的关键字段，定位 web 分支为何未触发
					try { console.log("[dsh-mpkg-wallpaper][诊断] else分支 section:", JSON.stringify({ webUrl: section.webUrl, converted: section.converted, image: image, enabled })); } catch {}
				}
			}
			if (image) {
...
`;
		...
		function applyInner(ctx) {
			...
			// ①(修正) 清 webUrl 时**同步清 converted/source/fromMpkg/mpkgKey**——否则留下
			// `converted:"web"` 但无 webUrl 的半残状态：apply() 每次被调用（重挂载/刷新/RTC
			// 重连）都会命中刚导入的 webUrl 并抹掉它，而 converted 残留 → applyFromStorage
			// 被误导走 img 路径 → iframe 空白（用户实测：clear 后导入星野不显示）。
			try {
				const bootS = readSection();
				if (bootS && bootS.webUrl) {
					writeSection(Object.assign({}, bootS, {
						webUrl: undefined, converted: "", source: "", fromMpkg: false, mpkgKey: "", mpkgName: ""
					}), true);
				}
			} catch {}
