...
`;
		...
		function MpkgSection(props) {
			...
			const clearBg = () => {
				idbDel("bg");
				const s = readSection();
				writeSection(Object.assign({}, s, { image: "", source: "", fromMpkg: false, converted: "", mpkgKey: "", mpkgName: "", info: undefined, slot: null, timeVideos: undefined, timeConfig: undefined, activeSlot: null, timeSrc: null }), true);
				setSection(readSection());
				setMpkgMeta(null);
				applyFromStorage();
			};
		...
		function applyInner(ctx) {
			...
			// 按 timeSrc 按需提取，单槽峰值 ~50MB；手动时段覆盖按钮也走这里）
			const swapTimeSlot = (slot) => {
				const s = readSection();
