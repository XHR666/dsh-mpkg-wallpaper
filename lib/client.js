...
// ①(2026-09-12 根因修复) 字符串字段归一化：持久化状态里 image 可能是布尔 true，
		...
		function showVideoEdge(url, video) {
			...
			/* ①(NP-3) 静音设置项 → 视频元素（Edge canvas 路线同样要落） */
			try { applyVideoMute(); } catch {}
			edgeDrawFrame();
		...
		function showVideoEl(url) {
			...
			/* ①(NP-3) 静音设置项 → 视频元素（此前只有建 DOM 时的硬编码 true，再没人改过它）。 */
			try { applyVideoMute(); } catch {}
			video.style.display = "";
		...
		function applyVideoMute() {
			...
			try {
				const s = readSection();
				const want = (s.mute !== void 0 ? !!s.mute : true) || npAudioOwns();
...
`;
		...
		/** 单个音轨的字节 URL（宿主 /raw，支持 Range —— 长音频可拖动）。 */
		function npTrackUrl(scope, trackPath) {
			if (!scope || !trackPath) return "";
			const file = String(trackPath).split(/[\\/]/).filter(Boolean).pop() || "";
			if (!file) return "";
			if (scope.mode === "custom") {
		...
		function npTrackList(section) {
			...
			return { scope: scope, key: key, tracks: cached.tracks };
		}
		function npCurrentTrack(section) {
			const list = npTrackList(section);
			if (!list) return null;
			const i = Math.max(0, Math.min(list.tracks.length - 1, npTrackIdx));
			return { scope: list.scope, key: list.key, tracks: list.tracks, index: i, track: list.tracks[i] };
		}
		...
		function npResolveMedia(section) {
			...
			const out = {
				...
				total: null, playing: false, muted: s.mute !== void 0 ? !!s.mute : true, volume: 1,
			...
			if (vid) {
				...
				out.muted = !!vid.muted;
				out.volume = typeof vid.volume === "number" ? vid.volume : 1;
				...
				out.byline = hasAudio === false ? mpwT("np.noAudio")
					: (hasAudio === true ? (id.name + " · " + mpwT("np.src.video")) : id.name);
			...
			if (cur) {
				...
				out.canPlay = true;
				out.canVolume = true;
				out.canPrev = n > 1;                     /* 只有一条 ⇒ 禁用（不假装有第二首） */
				out.canNext = n > 1;
				out.hasAudio = true;
				if (npAudio) {
		...
		function npFetchTracks(section) {
			...
			try {
				fetch(url).then((r) => (r && r.ok ? r.json() : null)).then((d) => {
					...
					npMediaCache = { key: key, at: Date.now(), data: { count: d && d.count || 0, tracks: tracks } };
					...
					npMediaCache = { key: key, at: Date.now(), data: { count: 0, tracks: [] } };
				});
			} catch (e) {}
		}
		...
		function npAudioError() {
			...
			const url = npTrackUrl(cur.scope, cur.track && cur.track.path);
		 ...
		 */
		function npTransport(op, section) {
			const s = section || readSection();
			try {
				if (op === "play" || op === "pause") {
					const vid = npActiveVideo(s);
					if (vid) {
						if (op === "play") { const p = vid.play(); if (p && p.catch) p.catch(() => {}); }
						else vid.pause();
					} else {
						...
						if (list && list.tracks.length) {
							if (op === "play") {
								...
								if (a && !String(a.getAttribute("src") || "")) npLoadTrack(true);
								else if (a) { const p = a.play(); if (p && p.catch) p.catch(() => {}); }
							} else if (npAudio) npAudio.pause();
						} else {
					...
					npApplyMute();
					try { fetch(HOST_BASE + "/media-audio", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ muted: next }) }).catch(() => {}); } catch {}
				}
			} catch (e) { mpwErr("npTransport", e); }
		...
		function npEnsureCtl() {
			...
			try {
				const mod = MPW_NP.createNowPlaying({
					...
					log: console,
					onTransport: (op) => npTransport(op, readSection()),
				});
		...
		function applyNowPlaying(section) {
			...
			try { const vid = npActiveVideo(s); if (vid) npWatchVideo(vid); } catch (e) {}
		...
		try {
			...
			globalThis.__mpwNpTest = {
				...
				trackUrl: (section, p) => { try { return npTrackUrl(npAudioScope(section || readSection()), p) } catch (e) { return "ERR " + (e && e.message || e) } },
				...
				transport: (op, section) => { try { npTransport(op, section || readSection()); return true } catch (e) { return String(e && e.message || e) } },
				audio: () => npAudio,
				idx: () => npTrackIdx,
				...
				seedTracks: (section, tracks) => {
					try {
						const s = npAudioScope(section || readSection());
						npMediaCache = { key: npScopeKey(s), at: Date.now(), data: { count: (tracks || []).length, tracks: tracks || [] } };
						npTrackIdx = 0;
