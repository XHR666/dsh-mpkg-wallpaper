	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			if (typeof image === "string" && image.indexOf("host:") === 0) {
				...
				const hostUrl = resolveHostUrl(image);
			...
			try {
				...
				// ①(新) 液态玻璃（elysia395 方案）：glassColor 非空或 alpha>0 即生效（去掉总开关）
				if ((section.glassColor && /^#[0-9a-fA-F]{6}$/.test(section.glassColor)) || (section.glassAlpha !== void 0 && section.glassAlpha > 0))
					document.body.setAttribute("data-mpw-glass", "");
				else document.body.removeAttribute("data-mpw-glass");
...
`;
			...
			// ①(新) 液态玻璃（参考 elysia395 的 glassColor/glassAlpha）：
			// 玻璃底色 + 透明度，作用于**整个设置界面**（DSH 设置弹窗 + 插件卡片 + 本插件弹窗）。
			// ①(修正) 去掉 glassWindow 总开关（用户方案）；玻璃色默认中性灰半透明
			//（用户实测白底发白 → 改为类似左侧设置栏的灰半透明）。
			if ((section.glassColor && /^#[0-9a-fA-F]{6}$/.test(section.glassColor)) || (section.glassAlpha !== void 0 && section.glassAlpha > 0)) {
				const gc = section.glassColor && /^#[0-9a-fA-F]{6}$/.test(section.glassColor)
					? section.glassColor
					: (document.body && document.body.hasAttribute("data-ds-dark-theme") ? "#1a2030" : "#f2f4f8");
				const ga = Math.max(0, Math.min(60, section.glassAlpha !== void 0 ? section.glassAlpha : 10));
				css += `
/* ── 液态玻璃（elysia395 方案）：整个设置界面玻璃化 ── */
body[data-mpw-glass] {
	--mpw-glass-color: ${gc};
	--mpw-glass-alpha: ${ga}%;
}
body[data-mpw-glass] .mpw_glassHost,
body[data-mpw-glass] .mpw_dialog {
	background: color-mix(in srgb, var(--mpw-glass-color) var(--mpw-glass-alpha), transparent) !important;
...
`;
			...
			// ①(新) 配色（accent）：按钮/滑条/选中/链接/发送键跟随自定义品牌色
			//（参照 elysia395 的 accent；放末尾保证覆盖 aquaInk 的品牌色）
			// ①(修正) 与「主题颜色」分工：accent 管品牌交互元素；themeColor 管表面 tint。
			if (section.accent && /^#[0-9a-fA-F]{6}$/.test(section.accent)) {
				css += `
/* ── 配色（accent）：品牌交互元素统一为自定义色（空 = DSH 默认蓝） ── */
body[data-mpw-accent] {
	--dsw-alias-brand-primary: var(--mpw-accent-color, #3964fe);
	--dsw-alias-brand-text: var(--mpw-accent-color, #3964fe);
	--dsw-alias-button-info-fill: var(--mpw-accent-color, #3964fe);
	--dsw-alias-button-info-hover: var(--mpw-accent-color, #3964fe);
	--dsw-alias-state-business-primary: var(--mpw-accent-color, #3964fe);
	--dsw-alias-interactive-bg-selected: color-mix(in srgb, var(--mpw-accent-color, #3964fe) 16%, transparent);
}
`;
			}
			// ①(新) 主题颜色：控制 侧边栏/标题栏/新会话按钮/设置弹窗 的表面 tint
			//（用户方案：主题颜色管整体底色，配色管交互元素）
			if (section.themeColor && /^#[0-9a-fA-F]{6}$/.test(section.themeColor)) {
				css += `
/* ── 主题颜色：侧边栏/标题栏/新会话/设置弹窗 底色 tint（空 = 不启用） ── */
body[data-mpw-theme] {
	--dsw-alias-specific-sidebar-fill: color-mix(in srgb, var(--mpw-theme-color, #3964fe) 30%, var(--dsw-alias-bg-base, #0e1420));
}
body[data-mpw-theme] .pI_x6G_sidebarCol,
body[data-mpw-theme] .hHd-Xa_root {
	background-color: color-mix(in srgb, var(--mpw-theme-color, #3964fe) 30%, var(--dsw-alias-bg-base, #0e1420)) !important;
}
body[data-mpw-theme] .wSkVaW_header {
	background-color: color-mix(in srgb, var(--mpw-theme-color, #3964fe) 30%, var(--dsw-alias-bg-base, #0e1420)) !important;
}
body[data-mpw-theme] .hHd-Xa_newSession {
	background-color: color-mix(in srgb, var(--mpw-theme-color, #3964fe) 30%, var(--dsw-alias-bg-base, #0e1420)) !important;
}
/* ①(修正) header 内部按钮（session log / 标准模式）透明：不再出现单独矩形底 */
body[data-mpw-theme] .wSkVaW_header button,
body[data-mpw-theme] .wSkVaW_header [class*="button"],
body[data-mpw-theme] .wSkVaW_header [class*="logo"],
body[data-mpw-theme] .wSkVaW_header [class*="mode"] {
	background: transparent !important;
	box-shadow: none !important;
}
body[data-mpw-theme] .wSkVaW_header button:hover,
body[data-mpw-theme] .wSkVaW_header [class*="button"]:hover {
	background: color-mix(in srgb, var(--dsw-alias-interactive-bg-hover) 60%, transparent) !important;
}
`;
			}
		...
		function MpkgSection(props) {
				...
				try {
					...
					if (!d.ok) { showError(t("lib.dirFail") + String(d.error || "")); }
						...
						setWallList((prev) => {
							...
							const keys = new Set((prev || []).map((w) => w.key));
							return fresh.filter((w) => !keys.has(w.key)).concat(prev || []);
						});
					}
				} catch (err) { showError(t("lib.fail") + String(err && err.message || err)); }
				setLibBusy(false);
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					glassColor: "", glassAlpha: 12, glassWindow: false, accent: "",
			...
			// ⑳(新) 视频倍速分段条：6 段竖线分隔，点击/拖拽直接定位到对应档位
			const SpeedBar = (props) => {
				const { value, onChange } = props;
				const ref = react.useRef(null);
				const dragging = react.useRef(false);
				const PAD = 12; // 左右内边距（轨道/刻度/球/进度/标签共用同一坐标系）
				const idx = Math.max(0, SPEED_LEVELS.indexOf(value));
				const pos = (i) => "calc(" + PAD + "px + " + (i / (SPEED_LEVELS.length - 1) * (100 - PAD * 2)) + "%)";
				const pick = (clientX) => {
					const el = ref.current;
					if (!el) return;
					const rect = el.getBoundingClientRect();
					// ①(修正) 用**轨道内坐标**（减去左右 PAD）映射档位——之前用全容器宽，
					// 刻度在内缩区，点击映射偏右错一档（点 1.25 得 1x、2 只得 1.5x，用户实测）
					const ratio = Math.max(0, Math.min(1, (clientX - rect.left - PAD) / (rect.width - PAD * 2)));
					const i2 = Math.min(SPEED_LEVELS.length - 1, Math.round(ratio * (SPEED_LEVELS.length - 1)));
					onChange(SPEED_LEVELS[i2]);
				};
				return h("div", { className: "mpw_speedBar" }, [
					// ①(重做) 视觉条：轨道/进度/竖线/球，可拖拽（轨道坐标 12px 内边距）。
					// 数字改为**下方独立等宽按钮行**（点击精确设置）——彻底消除"点数字错档"
					// 和"2x 后面空一截"（之前绝对定位标签 + 冒泡覆盖反复出问题）。
					h("div", { ref,
						style: { position: "relative", height: 24, margin: "8px 4px 0", touchAction: "none", cursor: "pointer" },
						onPointerDown: (ev) => { dragging.current = true; try { ev.target.setPointerCapture && ev.target.setPointerCapture(ev.pointerId); } catch {} pick(ev.clientX); },
						onPointerMove: (ev) => { if (dragging.current) pick(ev.clientX); },
						onPointerUp: () => { dragging.current = false; },
						onPointerCancel: () => { dragging.current = false; },
						onClick: (ev) => pick(ev.clientX) },
						h("div", { style: { position: "absolute", left: PAD, right: PAD, top: "50%", height: 6, marginTop: -3, borderRadius: 3, background: "color-mix(in srgb, var(--dsw-alias-interactive-bg-hover, #333a4a) 60%, transparent)" } }),
						h("div", { style: { position: "absolute", left: PAD, top: "50%", height: 6, marginTop: -3, borderRadius: 3, background: "var(--dsw-alias-brand-primary, #4f8cff)", width: "calc(" + (idx / (SPEED_LEVELS.length - 1) * (100 - PAD * 2)) + "% - 0px)" } }),
						SPEED_LEVELS.map((rate, i) => h("div", { key: rate, style: {
							position: "absolute", top: "50%", marginTop: -9, left: "calc(" + PAD + "px + " + (i / (SPEED_LEVELS.length - 1) * (100 - PAD * 2)) + "% - 1px)",
							width: 1, height: 18, background: "color-mix(in srgb, var(--dsw-alias-border-l2, #444b5c) 70%, transparent)"
						} })),
						h("div", { style: { position: "absolute", top: "50%", marginTop: -8, left: "calc(" + PAD + "px + " + (idx / (SPEED_LEVELS.length - 1) * (100 - PAD * 2)) + "% - 7px)", width: 14, height: 14, borderRadius: "50%", background: "var(--dsw-alias-brand-primary, #4f8cff)", border: "2px solid var(--dsw-alias-bg-base, #0e1420)" } })
					),
					// ①(重做) 等宽数字按钮行：6 个占满整行，点击精确设置，当前档高亮
					h("div", { style: { display: "flex", gap: 2, margin: "6px 4px 0" } },
						SPEED_LEVELS.map((rate) => {
							const on = rate === value;
							return h("button", { key: rate, type: "button",
								style: { flex: "1 1 0", padding: "6px 0", fontSize: 12, borderRadius: 8, cursor: "pointer",
			...
			return h("div", { className: "mpw_row mpw_glassHost" }, [
				...
				h("div", { className: "mpw_field" }, [
					...
					h(SpeedBar, { value: section.playbackRate !== void 0 ? section.playbackRate : DEFAULT_PLAYBACK_RATE, onChange: (v) => commit({ playbackRate: v }, true) }),
				...
				// ①(新) 液态玻璃（参考 elysia395 的配色/玻璃颜色/玻璃透明度/设置窗口玻璃 4 控制）
				h("div", { className: "mpw_section" }, t("glass.title")),
				h("p", { className: "mpw_hint" }, t("glass.desc")),
				// ①(修正) 去掉「设置窗口液态玻璃」总开关（用户方案：玻璃色/透明度设置即生效）
				// 配色（accent，品牌交互元素——与主题颜色分工）
				h("div", { className: "mpw_field" }, [
				...
				// 玻璃颜色（glassColor）
				h("div", { className: "mpw_field" }, [
					h("label", { className: "mpw_label" }, t("glass.color")),
					h("div", { className: "mpw_inline" }, [
						h("button", {
							className: "mpw_colorSwatch", "data-mpw-picker-anchor": "glassColor", type: "button",
							style: { background: (section.glassColor && /^#[0-9a-fA-F]{6}$/.test(section.glassColor) ? section.glassColor : "#ffffff") },
							onClick: () => openPicker("glassColor", section.glassColor)
						}),
						...(AQUA_PRESETS.map((hex) => h("button", {
							className: "mpw_presetSwatch", type: "button", title: hex,
							style: { background: hex }, onClick: () => commit({ glassColor: hex }, true)
						}))),
						h("button", { className: "mpw_miniBtn", type: "button", onClick: () => commit({ glassColor: "" }, true) }, t("glass.reset"))
					]),
					h("p", { className: "mpw_hint" }, t("glass.color.hint"))
				]),
				// 玻璃透明度（glassAlpha 0-60%，设了即生效）
				sliderRow(t("glass.alpha"), "glassAlpha", 0, 60, "%", 5, 12),
