window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function showWebEl(url) {
			...
			applyWebMute(frame);
			// ①(修正) **observer 挂载必须在新文档 onload 之后**——改 src 后同步挂只能拿到
			// 旧 contentDocument，新文档永远挂不上（评审硬问题①）。onload 里挂
			// webMediaObserve + hideWebPanel + 静音/倍速（此时新文档就绪）。
			try {
				frame.onload = () => {
		...
		function hideWebPanel(frame) {
			try {
				...
				const containsCanvas = (el) => {
					try {
						// ①(修正) 保守护栏：遇到可能是壁纸宿主/包裹整棵 DOM 的祖先就停，防误藏整棵树。
						// 仅认 canvas/#main 有风险（纯 DOM 渲染的 web 壁纸无 canvas 时可能整棵被藏→白屏）；
						// 所以 #app / 含 >3 子节点或宽度近全屏的容器也视为"宿主导航级"，不藏。
						if (el.id === "main" || el.id === "app" || el.id === "root" || el.tagName === "BODY" || el.tagName === "HTML") return true;
						if (el.querySelector && el.querySelector("canvas, #main")) return true;
						// 含大量子节点/全屏尺寸 → 疑似主机容器，不藏
						if (el.childElementCount > 4) return true;
						const r = el.getBoundingClientRect && el.getBoundingClientRect();
						if (r && r.width > window.innerWidth * 0.9) return true;
						return false;
					} catch { return true; }
				};
				const hidePanelChain = () => {
					...
					if (b) {
						// ①(修正) 只向上藏极少数层（面板通常嵌套不深）；每层先过 containsCanvas 护栏。
		...
		function applyFromStorage() {
			...
			if (typeof image === "string" && image.indexOf("host:") === 0) {
				...
				// 原来走 showImageEl 才卸；但 image=="" 时 img.src===image 相等会跳过
				// showImageEl → iframe 永不卸载。这里直接兜底卸载。
...
`;
			...
			// ①(新) PR3：自定义灰色字颜色（fontColorGray，独立于 aqua）——开则所有
			// 次级/三级/弱化灰字 + 分隔线/边框灰跟随自定义颜色（17 个灰色 token 全覆盖）。
			// 用 body 级 CSS 变量直接写（不依赖 overrideTokens API），任何环境都生效。
			if (section.fontColorGray !== void 0 ? !!section.fontColorGray : false) {
				const gc = section.fontColorGrayColor && /^#[0-9a-fA-F]{6}$/.test(section.fontColorGrayColor) ? section.fontColorGrayColor : "#9aa4b2";
				css += `
/* ── PR3: 自定义灰色字颜色（fontColorGray，独立开关） ── */
body {
	--dsw-alias-label-secondary: ${gc} !important;
	--dsw-alias-label-tertiary: ${gc} !important;
	--dsw-alias-label-caption: ${gc} !important;
	--dsw-alias-label-dimmed: ${gc} !important;
	--dsw-alias-label-quaternary: ${gc} !important;
	--dsw-alias-label-primary-bluish: ${gc} !important;
	--dsw-alias-label-primary-dimmed: ${gc} !important;
	--dsw-alias-label-primary-foreground: ${gc} !important;
	--dsw-alias-label-inverse: ${gc} !important;
	--dsw-alias-label-primary-inverted: ${gc} !important;
	--dsw-alias-label-error: ${gc} !important;
	--dsw-alias-line-secondary: ${gc} !important;
	--dsw-alias-separator-primary: ${gc} !important;
	--dsw-alias-border-secondary: ${gc} !important;
	--dsw-alias-state-warn-label: ${gc} !important;
}
`;
			}
...
`;
		...
		function MpkgSection(props) {
			...
			react.useEffect(() => {
				const anyOpen = previewModal || errorModal || dirPick || updConfirm || updChoose || conflictModal;
				try {
				...
				(section.fontColorGray !== void 0 ? !!section.fontColorGray : false)
					? h("div", { className: "mpw_field" }, [
						...
						h("div", { className: "mpw_inline" }, [
							...
							h("button", { className: "mpw_miniBtn", type: "button", onClick: () => commit({ fontColorGrayColor: "#9aa4b2" }, true) }, t("aquaColorReset"))
						]),
		...
		const zh = {
			...
			"fontColorGray.desc": "开：所有次级/三级/弱化灰字与分隔线/边框灰跟随自定义颜色（默认暖灰 #9aa4b2）；关：用主题默认灰",
			"fontColorGray.color": "灰字颜色",
			"fontColorGray.hint": "覆盖 label-secondary/tertiary/caption/dimmed/quaternary/primary-bluish 及分隔线/边框/警告灰（约 17 个灰色 token），独立于 Aqua 开关，任何主题下生效",
		...
		const en = {
			...
			"fontColorGray.desc": "On: all secondary/tertiary/dimmed gray text and separator/border grays follow a custom color (default warm gray #9aa4b2); Off: theme default gray",
			"fontColorGray.color": "Gray text color",
			"fontColorGray.hint": "Overrides label-secondary/tertiary/caption/dimmed/quaternary/primary-bluish and separator/border/warning grays (~17 gray tokens); independent of Aqua, works in any theme",
