...
/* ①(修正) 主题颜色(themeColor)：默认(空)=DSH 原色；手动选了才 tint 侧边栏/标题栏底色
   (第4项: 主题颜色/配色默认不启用, 手动选才切换) */
...
`;
		...
		function MpkgSection(props) {
			...
			return h("div", { className: "mpw_row mpw_glassHost" }, [
				...
				h("p", { className: "mpw_hint" }, t("aquaTint.manual")),
		...
		const zh = {
			...
			"aquaTint.manual": "想手动选色？用外观 tab 的「遮罩自定义色」取色盘（优先于自动采样）",
		...
		const en = {
			...
			"aquaTint.manual": "Want to pick a color manually? Use the \"Custom mask color\" picker in the Appearance tab (takes precedence over auto-sampling)",
