...
function mpwTrace(ev, data) {
	try {
		...
		/* ①(2026-09-18 用户裁定)「dsh-better-sidebar 适配」总开关默认**开**：底部面板悬浮适配
		   （圆角外壳 + 单层裁切 + 零边距 + strip 挪进面板）已经真机定案，默认关 = 没人看得见。
		   存量用户（从没显式设过）由 mpwNormalizeSection 的迁移块抬到 true；**手动关过的绝不覆盖**
		   （写入口打 bsCompatUserSet 标记，见 writeSection）。 */
		const DEFAULT_BS_COMPAT = true;
		/* ①(NP-1 2026-09-19 用户第 1 条「now playing 挂载到 dsh 设置上面 有一个开关启用是否挂载
		   左边栏收起就隐藏」) Now playing 挂到 DSH 左侧栏「设置」入口**上方**。
		   ①(NP-3 2026-09-19) **默认改成开**（本条要求）：组件是"壁纸声音"的唯一落点，
		   默认关 = 用户导入带音频的壁纸后永远看不到/听不到它（现场正是如此）。
		   配套的"别抢别人的位置"见 lib/now-playing.js 的让位逻辑（occupantOf / data-mpw-np-yield）：
		   宿主同一位置已经有**别的插件**注入的元素时，我们**不挂/撤下**并留一行可读 warn，
		   而不是叠着画。 */
		const DEFAULT_NP_NOW_PLAYING = true;
		/* ①(NP-4 2026-09-19 真机修复) 音量电平（0..100）。
		   为什么要有它：NP-3 的"第四个键"只是**静音开关**（布尔 muted），全仓**没有任何**
		   能改电平的落点（真机读数：section 里与音量有关的键 0 个）⇒ 用户现场
		   "打开之后也不能调整它开启的音量"。默认 100：与 `HTMLMediaElement.volume` 的默认值
		   相同，也与 web 帧主音量的缺省（web-wallpaper.js 的 masterVolume=1）相同
		   ⇒ **默认档逐字节零行为变化**（判据见 tools/np-control-test.mjs 的 E 组）。 */
		const DEFAULT_NP_VOLUME = 100;
		/* ①(NP-5 2026-09-20) 卡片的**播放/暂停意图**要持久化（页面刷新后不许自动换成播放）。
		   写入口径：**只有用户显式操作**（点暂停/播放键）才写这个键；
		   `npPrimedMuted`/`npSoundBlocked`/hidden/省电/自动对齐这些**内部状态一律不写**（否则串味）。 */
		const DEFAULT_NP_PAUSED = false;
		/* ①(NP-4)② 「播放/暂停是否同时控制壁纸的播放/暂停」联动开关，默认**开**。
		   默认必须是开：改动之前 NP 的播放/暂停/进度就是直接落在壁纸媒体元素上的
		   （视频档：`vid.play()/pause()`；见 npTransport）——默认关就等于改掉了既有行为，
		   而本条明确要求"不许在做这件事时改变既有默认行为"。
		   语义（口径见 docs/NOW-PLAYING-DSH.md §7.8）：
		     开 ⇒ 传输动作同时驱动壁纸自己的媒体 + 我们自己的播放器；
		     关 ⇒ 只驱动我们自己的播放器（壁纸的画面播放不受控件影响）；
		          壁纸媒体是**唯一**声源时（视频档没有旁挂音频）控件如实禁用，不假装。 */
		const DEFAULT_NP_LINK_WALLPAPER = true;
		/* ⑤(2026-09-20 真机修复)「省电·页面隐藏/切页暂停」的**默认档改成 true**：
		   用户现场第 2 条要的是"切页就静音"，而这是浏览器的基本礼仪（后台标签不该出声），
		   不是可选优化 ⇒ 默认做，用户显式关掉才不做（显式关过的不迁移，见 writeSection 的
		   与 bsCompat 同款的"用户动过"标记）。 */
		const DEFAULT_POW_PAUSE_HIDDEN = true;
		...
		/* ①(2026-09-19 设置持久化轮) 落盘收尾闸门：boot 的第一次 applyFromStorage() 跑在宿主
		   `GET /settings` **之前**（旧写法会在那一刻把残缺档同时写进 localStorage + PUT 给宿主，
		   即"两处一起写残"）。`mpwBootPending=true` 期间 writeSection 只置脏、不落盘；
		   宿主设置合并完（或 3.5s 超时）由 mpwBootSettle() 补落一次。 */
		let mpwBootPending = false;
		let mpwStoreDirty = false;
		let mpwBootTimer = 0;
		...
		function loadSection() {
			// ①(2026-09-19 设置持久化轮) 统一走 mpwLoadLocalRaw：元键（时间戳/自愈标记）**不进 section**
			//   —— 面板、buildCss、导出、字段集合比较看到的永远是"用户档"本身。
			try { return mpwStripMeta(mpwLoadLocalRaw()) } catch { return {} }
		}
		 ...
		 *  ①(2026-09-19 设置持久化轮) 合并口径改成**显式裁决**（旧写法 `Object.assign({}, host, local)`
		 *  只是"本地有的用本地"，两处谁新完全没判）：
		 *    · 两份各带自己的写入时刻（`__mpwLocalAt` / `__mpwHostAt`）⇒ 新者优先；
		 *    · 再走三路合并 ⇒ 只有一方动过的字段不会被另一方抹掉（字段级，不是整档级）；
		 *    · 源字段（image/webUrl/sceneKey）粘性 ⇒ 谁都没带就保持原样，绝不因为"这次没提"而消失。
		 *  合并结果**同时**回写两处（旧写法只写 localStorage，宿主那份半残档永远修不回来）。 */
		function initHostSettings() {
			// ①(2026-09-19 设置持久化轮) **一次性**：apply() 会因 RTC 重连/热重注入重跑，而每一次都会
			//   重新 GET 一次宿主设置。旧实现里重复的 GET 只是重复合并（幂等，代价小）；现在合并带
			//   "两处存储时间戳裁决 + 回写两处"，重复跑会让"后到的那次 GET"拿着**更旧**的宿主快照
			//   参与裁决（极端情况下把刚写下的本地新值判给宿主）。所以这一轮明确只跑一次。
			if (window.__mpwHostInitOnce) return;
			window.__mpwHostInitOnce = true;
			try {
				fetch(HOST_BASE + "/settings", { method: "GET" }).then(async (r) => {
					if (!r.ok) { try { mpwBootSettle("host-get-not-ok") } catch {} return }
					const d = await r.json();
					if (!(d && d.ok)) { try { mpwBootSettle("host-not-ok") } catch {} return }
					hostSettingsOk = true;
					if (!d.settings || typeof d.settings !== "object") { try { mpwBootSettle("host-empty") } catch {} return }
					if (sectionDirty) {
						// 用户已改 → 不做整档替换；但仍要**补齐**宿主独有的字段（否则用户这次改的那一笔
						// 会带着"缺字段的档"写回宿主，把宿主的字段一起抹掉）。
						try {
							const rec2 = mpwReconcileStores(sectionCache || {}, d.settings);
							const carry = {};
							for (const k of MPW_SRC_KEYS) {
								if (mpwVal(sectionCache || {}, k) !== void 0 && mpwVal(rec2.merged, k) === void 0) carry[k] = mpwVal(sectionCache, k);
							}
							sectionCache = mpwNormalizeSection(mpwMergeSection(rec2.merged, carry));
						} catch (e) { mpwErr("initHostSettings(sectionDirty 合并)", e) }
						try { mpwBootSettle("dirty-skip") } catch {}
						applyFromStorage();
						return;
					}
					const localRaw = mpwLoadLocalRaw();
					const rec = mpwReconcileStores(localRaw, d.settings);
					// 源字段粘性：宿主/本地谁都没带源时，绝不让合成把源抹掉（只有显式 null 才删）。
					const localS = mpwStripMeta(localRaw);
					const carry = {};
					for (const k of MPW_SRC_KEYS) {
						if (mpwVal(localS, k) !== void 0 && mpwVal(rec.merged, k) === void 0) carry[k] = mpwVal(localS, k);
					}
					sectionCache = mpwNormalizeSection(mpwMergeSection(rec.merged, carry));
					try {
						mpwTrace("settings:reconcile", {
							winner: rec.winner, newer: rec.newer, reason: rec.reason,
							localKeys: rec.localKeys.length, hostKeys: rec.hostKeys.length,
							halfArchive: !!rec.half,
							srcKeys: MPW_SRC_KEYS.map((k) => (rec.merged[k] === void 0 ? "-" : (rec.merged[k] ? "set" : "empty")))
						});
					} catch {}
					// ①(2026-09-19) 合并结果**两处都落**（同一次写：本地 + 宿主），这样"半残档"能被合成档覆盖：
					//   ①(2026-09-17 持久化轮) 仍然走**唯一落盘入口** mpwPersistSection：host 只存它自己的字段
					//   （HOST_SKIP_KEYS 明确跳过 image），合并串一旦超 256KB 旧写法就是"拒写 + 不落 IDB"，
					//   等于每次刷新都白丢一次壁纸选择。现在超顶自动把 image 落 IDB 并留 idb:img 哨兵。
					mpwStoreDirty = true;
					try { mpwBootSettle("host-merged") } catch (e) { mpwErr("initHostSettings(落盘)", e) }
					applyFromStorage();
				}).catch(() => { hostSettingsOk = false; try { mpwBootSettle("host-error") } catch {} });
				// 宿主不可用/超时也不能把用户永久卡在"不落盘"：3.5s 后按 bootstrap 那次合并结果收尾。
				try {
					if (!mpwBootTimer) mpwBootTimer = setTimeout(() => { mpwBootTimer = 0; try { mpwBootSettle("timeout") } catch {} }, 3500);
				} catch {}
			} catch { hostSettingsOk = false; try { mpwBootSettle("throw") } catch {} }
		}
...
// ①(2026-09-12 根因修复) 字符串字段归一化：持久化状态里 image 可能是布尔 true，
		...
		function mpwNormalizeSection(s0) {
			...
			/* ①(2026-09-18 用户裁定) **bsCompat 默认值迁移**：只抬"从没显式设过"的存量用户 ——
			   `bsCompatUserSet === true`（用户在面板里动过）⇒ **一字不动**，他手动关的就保持关；
			   否则（无标记）⇒ 采用新默认 `DEFAULT_BS_COMPAT`（= true）。
			   为什么必须有标记而不是"看 bsCompat 是否 === false"：`commit()` 走的是
			   `writeSection(Object.assign({}, readSection(), patch))`（整段合并后落盘），
			   所以**每次**保存都会把 `bsCompat` 一起写进去 ⇒ 光看值分不出"用户关的"和"旧默认带下来的"。
			   迁移不写标记（用户仍然"从没设过"），只有他真去动开关时才打（见 writeSection）。 */
			const bsMigrate = s0.bsCompatUserSet === true ? null : DEFAULT_BS_COMPAT;
			const bsNeeds = bsMigrate !== null && s0.bsCompat !== bsMigrate;
			/* ⑤(2026-09-20) **powPauseHidden 默认档迁移**（与上面 bsCompat 完全同一个口径）：
			   这一条从"可选省电"变成"浏览器基本礼仪"（后台标签不许出声）⇒ 新默认 true。
			   读过盘/被保存过就会带上旧默认的 `false`，所以判据必须是"用户**动过**这个开关的标记"，
			   而不是"值是不是 false"；没标记（从没设过）才迁移，动过的用户一字不动。 */
			const phMigrate = s0.powPauseHiddenUserSet === true ? null : DEFAULT_POW_PAUSE_HIDDEN;
			const phNeeds = phMigrate !== null && s0.powPauseHidden !== phMigrate;
			if (!need && !bsNeeds && !phNeeds) return s0;
			const o = Object.assign({}, s0);
			if (bsNeeds) { o.bsCompat = bsMigrate; try { console.log("[dsh-mpkg-wallpaper] bsCompat 迁移：从没显式设过 ⇒ 采用新默认 " + bsMigrate + "（手动关过的用户不受影响）") } catch (e) {} }
			if (phNeeds) { o.powPauseHidden = phMigrate; try { console.log("[dsh-mpkg-wallpaper] powPauseHidden 迁移：从没显式设过 ⇒ 采用新默认 " + phMigrate + "（切页静音是浏览器基本礼仪；手动关过的用户不受影响）") } catch (e) {} }
			const bad = [];
		...
		function readSection() {
			if (sectionCache) {
				// ①(2026-09-19 设置持久化轮) **读口一律不见元键**：内部缓存为了落盘会带
				//   `__mpwLocalAt` 这类元键（时间戳/自愈标记），但"用户档"的读数口（面板/buildCss/
				//   导出/字段集合比较/测试的逐字段 diff）必须看不到它们 —— 否则元键会被当成
				//   "用户的设置项"参与比较（真机上表现为"莫名多出一个键"，测试里表现为假红）。
				const clean = mpwStripMeta(sectionCache);
				if (clean !== sectionCache) sectionCache = clean;
				return sectionCache;
			}
		...
		function writeSection(next, instant) {
			// ①(2026-09-19 设置持久化轮) **一次写不得静默抹掉字段** —— 见上面"唯一契约"块。
			//   三道护栏，顺序有讲究：
			//   ① mpwStickySource：调用方给的 next 若**没带**源字段（image/webUrl/sceneKey），
			//      而当前档里有 ⇒ 从当前档带过来（"本次没改到壁纸" ≠ "把壁纸抹掉"）。
			//      显式清空（""）与显式删除（null）都不受影响，照旧生效（clearBg 就是显式清空）。
			//   ② 与旧 sectionCache 合并：任何 `undefined` 的键都不覆盖既有值。
			//   ③ 时间戳：本次写被打上"本地写时刻"；若这次写同时会 PUT 到宿主，则宿主那份带"宿主写时刻"。
			//      两处存储各自带时间戳 ⇒ 启动时能裁决"谁新"（旧实现两处互相覆盖，没有口径）。
			try {
			...
			} catch (e) { mpwErr("writeSection(合并不替换)", e); }
			...
			// ①(2026-09-18) bsCompat：**只有值真的变了**才认为"用户显式设过"。
			//   注意不能照抄上面 headerFrost 那种"键存在即打标"的写法 —— commit() 传进来的是
			//   **整段合并后**的 section，键永远存在，那样写会让任何一次保存都打上标记，
			//   于是存量用户永远迁移不到新默认。这里与"当前生效值"比，变了才算用户动过。
			try {
				if (next && next.bsCompat !== void 0) {
					const prevEff = (() => { try { return readSection().bsCompat } catch (e) { return void 0 } })();
					if (prevEff !== void 0 && !!next.bsCompat !== !!prevEff) next.bsCompatUserSet = true;
					else if (prevEff === void 0) next.bsCompatUserSet = true;
				}
			} catch {}
			/* ⑤(2026-09-20) powPauseHidden 的"用户动过"标记（同上：只有值真的变了才算动过，
			   否则每次 commit 写整段都会打上标记 ⇒ 存量用户永远迁移不到新默认）。 */
			try {
				if (next && next.powPauseHidden !== void 0) {
					const prevPh = (() => { try { return readSection().powPauseHidden } catch (e) { return void 0 } })();
					if (prevPh !== void 0 && !!next.powPauseHidden !== !!prevPh) next.powPauseHiddenUserSet = true;
					else if (prevPh === void 0) next.powPauseHiddenUserSet = true;
				}
			} catch {}
			sectionCache = next;
			sectionDirty = true;
			...
			persistTimer = setTimeout(() => {
				// ①(2026-09-19 设置持久化轮) **启动收尾前不落盘**：boot 的 applyFromStorage() 跑在宿主
				//   `GET /settings` **之前**，那一刻内存里的档最不完整（宿主带回来的字段还没合并）。
				//   旧写法会在这个窗口把"只剩本地的残档"同时 PUT 给宿主 + 写进 localStorage ⇒
				//   **两处一起写残**（这就是"一次无关操作把设置抹掉"能长期存在的原因）。
				//   现在：脏了就记着，等两处存储合并完（或 3.5s 超时）再落一次盘。
				if (mpwBootPending) { mpwStoreDirty = true; return }
				mpwFlushStore();
			}, instant ? 0 : 250);
		}
		/** 把当前 sectionCache 落进两处存储（写序号消歧 + 时间戳 + 宿主整档同步）。
		 *  ①②③ 与旧实现的差别：① 启动收尾前不落盘；② 两处各带自己的时间戳；③ 宿主 PUT 失败不再
		 *  "乐观置 true"掩盖问题（下次仍会重试，语义不变）。 */
		function mpwFlushStore() {
			mpwStoreDirty = false;
			...
			const __at = Date.now();
			/* ①(2026-09-19) 时间戳只进**落盘用的那一份**：`readSection()` 是"用户档"的读数口，
			   元键一旦进了 sectionCache，面板/导出/字段集合比较就会看到 `__mpwLocalAt`
			   （真机无害但会污染"逐字段 diff"这类判据）⇒ 这里换成一个新对象，不动缓存本体。 */
			try { sectionCache = Object.assign({}, mpwStripMeta(sectionCache || {}), { [MPW_STAMP_LOCAL]: __at }) } catch {}
					...
					try {
						...
						if (persisted && typeof persisted === "object" && (persisted.image === "idb:img" || persisted.image === "")) {
							...
							if (mpwIsDataUrl(cur && cur.image) && cur.image !== persisted.image) {
								...
								try { sectionCache[MPW_STAMP_LOCAL] = __at } catch {}
							}
						}
					} catch {}
				}).catch(() => {});
			...
			// ①(2026-09-19 设置持久化轮) 宿主那份带 `__mpwHostAt`（它自己的写时刻）——
			//   两处存储各有各的时间戳，启动时才能裁决"谁新"（旧写法两处互相覆盖、没有口径）。
			try {
				const payload = {};
				const src = sectionCache || {};
				for (const k of Object.keys(src)) {
					if (HOST_SKIP_KEYS.includes(k)) continue;
					if (k === MPW_STAMP_LOCAL) continue;
					payload[k] = src[k];
				}
				payload[MPW_STAMP_HOST] = __at;
		...
		/** boot 收尾（宿主设置已合并 / 或超时）：把延迟的那一次落盘补上，并把 boot 置为收尾。 */
		function mpwBootSettle(source) {
			if (!mpwBootPending) return false;
			mpwBootPending = false;
			try { mpwTrace("settings:boot-settled", { source: source || "", dirty: !!mpwStoreDirty, at: Date.now() }) } catch {}
			if (mpwStoreDirty) { try { mpwFlushStore() } catch (e) { mpwErr("mpwFlushStore(boot 收尾)", e) } }
			// 自愈排在最后：两处存储都就位之后再判断"是不是半残档"，避免把"还没读回来"误判成"丢了"。
			try { mpwHealSchedule() } catch {}
			return true;
		}
		...
		/* ═══════════════════════════════════════════════════════════════════════════════════
		   ①(2026-09-19 设置持久化轮 · 真机「壁纸字段会丢」) 「一次写不得静默抹掉字段」的**唯一契约**
		   ───────────────────────────────────────────────────────────────────────────────────
		   现场（:3080 真机探针，tools/settings-persist-live-probe.mjs）：
		     两处存储都是半残档 —— `mpkgKey:"custom|3582362359"` 还在，`image`/`webUrl` **两个都没了**
		     ⇒ buildCss 的 hasImage=false ⇒ `.mpw-bgWrap{display:none}` ⇒ 用户的壁纸不显示。
		   结构缺陷（与本条修法一一对应）：
		     ⓐ 两处存储**都没有**"整段替换会抹字段"的护栏：
		        · 客户端 writeSection 把 `sectionCache` 整段（减 HOST_SKIP_KEYS）PUT 给宿主；
		        · 宿主 PUT /settings 把请求体**整串覆写** settings.json（lib/index.js 的
		          `writeFileSync(tmp, JSON.stringify(settings)); renameSync`）。
		        任何一次"内存里已经不完整"的保存都会把**两处**一起写残。
		     ⓑ 壁纸选择类字段（image/webUrl/sceneKey）**只有 localStorage 一份**（宿主明确跳过 image）：
		        localStorage 一丢，选择就永久没了 —— 这就是"半残档"能长期存在的原因。
		     ⓒ 读侧没有裁决：`initHostSettings` 的 `Object.assign({}, host, local)` 只按"谁在本地"
		        覆盖，两处的**新旧**没有任何口径；启动时的 `applyFromStorage()` 还跑在宿主 GET **之前**，
		        那一刻看到的档最不完整（"先隐藏壁纸层、再等宿主回来"）。
		     ⓓ 没有任何自愈：`mpkgKey` 明明还在（"custom|3582362359" 足以重新推出源），
		        但读侧只会照单全收地按"无源"处理 → 静默隐藏壁纸层。
		   本块给出三条不变量（都由 tools/settings-persist-test.mjs 机器断言）：
		     Ⅰ **合并不替换**：`mpwMergeSection(base, patch)` —— patch 里 `undefined` 的键**不覆盖**
		        base；要删必须写 `null`（显式删除的唯一语义）。
		     Ⅱ **源字段粘性**：`mpwStickySource(base, next)` —— next 里没有源字段而 base 有 ⇒ 从 base
		        带过来；只有显式删除（null）才允许清空。注意"清壁纸"路径（clearBg）本来就把
		        `mpkgKey` 一起写成 `""` ⇒ 清理语义不受影响。
		     Ⅲ **两处存储裁决**：每个档带一份内嵌时间戳（`__mpwLocalAt` / `__mpwHostAt`），
		        启动时按"新者优先 + 三路合并"合成，且**网络未回来之前不落盘**（脏标记延迟到 boot 收尾）。
		   诚实边界：时间戳是**同机时钟**（两处存储都在同一台机器上写），不做跨设备信任判断；
		   没有时间戳的旧档一律按"保守"处理（宁可保留字段，也不猜谁新）。 */
		/** 各存储的写入时刻（内嵌进档里，随档一起持久化 —— 两处存储各写各的，不互相覆盖）。 */
		const MPW_STAMP_LOCAL = "__mpwLocalAt";
		const MPW_STAMP_HOST = "__mpwHostAt";
		/** 半残档标记（持久留痕；诊断与探针据此判"这条档被自愈过/自愈推不出来"）。 */
		const MPW_SRC_LOST_KEY = "__mpwSrcLost";
		/** 元键：只管诊断/自愈记账，**永远不进 section**（面板/apply/CSS 都看不到它们）。 */
		const MPW_META_KEYS = [MPW_STAMP_LOCAL, MPW_STAMP_HOST, MPW_SRC_LOST_KEY];
		/** 壁纸「源」字段：buildCss 的 `hasImage = image || webUrl` 唯一依据这三个。 */
		const MPW_SRC_KEYS = ["image", "webUrl", "sceneKey"];

		/** 主机原始读（含元键）；**所有**读侧（readSection/loadSection 调用者）都从这里走。 */
		function mpwLoadLocalRaw() {
			try {
		...
		/** 去掉元键后的"用户档"（面板、CSS、导出看到的永远是这一份）。 */
		function mpwStripMeta(s0) {
			if (!s0 || typeof s0 !== "object") return {};
			try {
				let need = false;
				for (const k of MPW_META_KEYS) if (s0[k] !== void 0) { need = true; break }
				if (!need) return s0;
				const o = Object.assign({}, s0);
				for (const k of MPW_META_KEYS) delete o[k];
				return o;
			} catch { return s0 }
		}
		function mpwVal(s, k) {
			try { return s && typeof s === "object" ? s[k] : void 0 } catch { return void 0 }
		}
		function mpwHasVal(s, k) { return mpwVal(s, k) !== void 0 }
		/** 键序确定化：字段集合比较/落盘 diff 都要可复现（调试与测试都按这个序）。 */
		function mpwSortedKeys(o) {
			try { return Object.keys(o || {}).sort() } catch { return [] }
		}
		/** 不变量Ⅰ：合并不替换。patch 里 `undefined` 的键**不覆盖** base；`null` = 显式删除。 */
		function mpwMergeSection(base, patch) {
		...
		/** 不变量Ⅱ：源字段粘性 —— 新档没带源、旧档有 ⇒ 从旧档带过来（只有 null 才算显式清空）。 */
		function mpwStickySource(base, next) {
			try {
				const out = next && typeof next === "object" ? next : {};
				if (!base || typeof base !== "object") return out;
				for (const k of MPW_SRC_KEYS) {
					if (mpwVal(out, k) !== void 0) continue;                 // 新档明确给了（含 ""/null 语义已在 merge 里处理）
					const b = mpwVal(base, k);
					if (b !== void 0 && b !== null) out[k] = b;
				}
				return out;
			} catch { return next }
		}
		/* ═══════════════════════════════════════════════════════════════════════════════════
		   ②(2026-09-20 真机修复)「换壁纸 = **成套重写**源字段」
		   ───────────────────────────────────────────────────────────────────────────────────
		   真机现场（用户档，:3080）：`mpkgKey="custommpkg|小鸟游星野01_04.mpkg"`、`source="bgcs_abydos03.mp4"`、
		   `converted="mp4"`，而 `webUrl` 还留着**上一个 web 档**（`folder=3326873240`，换过 customDir 后
		   该 folder 已不可解析 ⇒ `/raw` 404）⇒ 壁纸层按"webUrl 优先"先挂 web 帧、404、再回退 scene，
		   回退也拿不到首帧 ⇒ 黑屏 + 破图图标 + 日志框闪一下；点"清除壁纸"后**壁纸被重新加载一遍**
		   （名字/类型显示"无"，画面还在）。
		   结构缺陷：所有换档/清空调用点都用 `undefined` 表达"这个字段不属于新壁纸"，而不变量Ⅰ明确
		   规定 `undefined` = **不覆盖**（`mpwStickySource` 还会主动把它从旧档带回来）⇒ 旧源字段永远删不掉。
		   修法：换档/清空时把这组"壁纸身份字段"里写成 `undefined` 的键转成 `null`（唯一删除语义）。
		   —— 只转这一组（下面 8 个），**不碰** `info`/`propEdits`/`timeVideos` 这类"跨壁纸共享"的键
		   （把整个 propEdits 删掉会连别的壁纸的属性编辑一起丢）。 */
		/** ②(2026-09-20 审计第 1 条) 换档补全：**成套重写**（不属于新选择的源字段显式置空）+ 自动补
		 *  `srcRoot`/`srcDirPath`。规则只有两条，覆盖真机出现过的全部混态：
		 *    · 本次只给 webUrl（web 档）⇒ `image/sceneKey` 一并清空；
		 *    · 本次只给 image（媒体/静态帧档）⇒ `webUrl/sceneKey` 一并清空（**渲染器档两个都会给**，
		 *      所以不会被误清 —— 那条路同时写 webUrl + sceneKey）。
		 *  调用方不需要知道这些细节：`commit()` 是面板侧唯一入口，这里补全即可（外观类 patch 不含
		 *  任何源字段 ⇒ 一个字节都不动）。 */
		function mpwSwitchPatchFull(patch) {
			try {
				if (!patch || typeof patch !== "object") return patch;
				const p = Object.assign({}, patch);
				const hasWeb = p.webUrl !== void 0 && p.webUrl !== null && p.webUrl !== "";
				const hasImg = p.image !== void 0 && p.image !== null && p.image !== "";
				const touchesSrc = ["webUrl", "image", "sceneKey", "mpkgKey", "converted", "source", "mpkgName"].some((k) => Object.prototype.hasOwnProperty.call(p, k));
				if (touchesSrc) {
					if (hasWeb && !hasImg) { if (p.image === void 0) p.image = null; if (p.sceneKey === void 0) p.sceneKey = null }
					if (hasImg && !hasWeb) { if (p.webUrl === void 0) p.webUrl = null; if (p.sceneKey === void 0) p.sceneKey = null }
				}
				// 来源类别/来源目录（自描述字段）：能从 key 或形状推就推，推不出保持原样（不瞎写）
				let root = p.srcRoot;
				if (root === void 0) {
					const k = String(p.mpkgKey || "");
					root = /^custommpkg\|/.test(k) ? "container" : /^custom\|/.test(k) ? "custom" : /^library\|/.test(k) ? "library" : null;
					if (!root && touchesSrc) {
						const sh = mpwSrcShapeOf(hasWeb ? p.webUrl : (hasImg ? p.image : ""));
						root = (sh && sh !== "external") ? sh : null;
					}
					if (root) p.srcRoot = root;
				}
				if (p.srcRoot === "custom") {
					if (p.srcDirPath === void 0) { try { p.srcDirPath = String((readSection() || {}).customDirPath || "") } catch (e) { p.srcDirPath = null } }
				} else if (p.srcRoot !== void 0) {
					if (p.srcDirPath === void 0) p.srcDirPath = null;
				}
				return mpwNormalizeSwitchPatch(p);
			} catch (e) { return mpwNormalizeSwitchPatch(patch) }
		}
		const MPW_SWITCH_SRC_KEYS = ["image", "webUrl", "sceneKey", "source", "mpkgKey", "mpkgName", "converted", "fromMpkg", "srcRoot", "srcDirPath"];
		function mpwNormalizeSwitchPatch(patch) {
			try {
				if (!patch || typeof patch !== "object") return patch;
				let out = patch, copied = false;
				for (const k of MPW_SWITCH_SRC_KEYS) {
					if (!Object.prototype.hasOwnProperty.call(patch, k)) continue;
					if (patch[k] !== void 0) continue;
					if (!copied) { out = Object.assign({}, patch); copied = true }
					out[k] = null;
				}
				return out;
			} catch { return patch }
		}
		/** 两处存储的新旧（毫秒）；读不到/不是有限数 → null（"未知"，按保守策略处理）。 */
		function mpwStampOf(s, which) {
			try {
				const v = s && s[which === "host" ? MPW_STAMP_HOST : MPW_STAMP_LOCAL];
				return (typeof v === "number" && isFinite(v) && v > 0) ? v : null;
			} catch { return null }
		}
		/** 半残档判据：**有来源线索（mpkgKey/非空 source）却没有源字段**。
		 *  这是"用户点了壁纸却看不到"的唯一特征；干净的"从没选过壁纸"档（mpkgKey 为空）**不算**。
		 *  ②(2026-09-20 真机修复) 加上**类型自相矛盾**这一条：档里写着"我是一张 mp4/scene/web"，
		 *  可那个类型需要的字段却不在（`converted="mp4"` 而 `image` 缺 / `converted="scene"` 而
		 *  `sceneKey` 缺 / `converted="web"` 而 `webUrl` 缺）—— 这种档**同样**是半残档。
		 *  真机现场：`converted="mp4"` + `image` 缺 + `webUrl` 是上一个 web 档的残留 ⇒ 旧判据只数
		 *  `image/webUrl/sceneKey` 三兄弟里有没有"任何一个非空"，`webUrl` 一非空就判"不是半残"⇒
		 *  自愈**不介入**，而挂载又按 webUrl 走 ⇒ 黑屏且永不自愈。 */
		function mpwSrcMissing(s) {
			const src = MPW_SRC_KEYS.some((k) => typeof mpwVal(s, k) === "string" && mpwVal(s, k));
			const clue = (typeof mpwVal(s, "mpkgKey") === "string" && mpwVal(s, "mpkgKey")) ||
				(typeof mpwVal(s, "source") === "string" && mpwVal(s, "source"));
			if (!src && !!clue) return true;
			// 类型自相矛盾（⚠ 只在声明了类型时才判，避免把"没写 converted"的旧档误判成半残）
			const decl = typeof mpwVal(s, "converted") === "string" ? String(mpwVal(s, "converted")) : "";
			const need = decl === "web" ? "webUrl" : decl === "scene" ? "sceneKey" : (decl === "mp4" || decl === "gif" || decl === "video" || decl === "image") ? "image" : null;
			if (need && !mpwVal(s, need)) return true;
			return false;
		}
		/* ══════════════════════════════════════════════════════════════════════════════════
		   ①(2026-09-20 真机修复)「设置里当前壁纸的预览框对 web 档是空的」
		   ──────────────────────────────────────────────────────────────────────────────────
		   真机现场（用户素材：`dd/3580207945`，project.json: `type:"web"`、`file:"index.html"`、
		   `preview:"preview.gif"`，Vite 构建：index.html + assets/ 三件套）：
		     设置面板「当前壁纸」那一行的预览框**什么都没有**（不是破图，是空）。
		   根因：预览分支只认三种形态 —— mpkg 容器预览 / 视频首帧 / `section.image`。
		   web 档的 `image` 是空串（webUrl 与 image 互斥），`mpkgKey` 也不是 `.mpkg`
		   ⇒ 三个分支全落空 ⇒ 走最后那句 `t("none")` 提示（框里自然什么都没有）。
		   修法：给 web 档补一条**候选链**（纯函数产出，逐个 onError 前进，耗尽才落占位）：
		     ① mpkg 容器预览（既有行为，容器档仍走它）；
		     ② web 档所在**目录**里的预览图：`preview.gif/jpg/png/webp`、`loading.webp`
		        （WE 约定 `project.json.preview`，语料里绝大多数 web 包都带 preview.gif）；
		     ③ 视频首帧 / `image`（既有行为）。
		   诚实边界：`project.json.preview` 里写**非**这几个约定名的包，本候选链取不到
		   —— 这时显示的是类型占位文字（"web"），**绝不显示破图图标**（判据见门禁）。
		   ══════════════════════════════════════════════════════════════════════════════════ */
		function mpwThumbCandidates(s) {
			const out = [];
			try {
				const host = HOST_BASE;
				const converted = String(mpwVal(s, "converted") || "");
				const img = typeof mpwVal(s, "image") === "string" ? mpwVal(s, "image") : "";
				const webUrl = typeof mpwVal(s, "webUrl") === "string" ? mpwVal(s, "webUrl") : "";
				const srcName = String(mpwVal(s, "source") || mpwVal(s, "entryName") || "");
				const keyRaw = String(mpwVal(s, "mpkgKey") || "");
				const afterBar = keyRaw.indexOf("|") >= 0 ? keyRaw.split("|")[1] : keyRaw;
				const folderFromKey = afterBar && afterBar.indexOf("/") >= 0 ? afterBar.split("/")[0] : "";
				const fileFromKey = afterBar && afterBar.indexOf("/") >= 0 ? afterBar.split("/").pop() : afterBar;
				// ① mpkg 容器预览（服务端解容器取 preview.*；既有行为，保持第一位）
				if (fileFromKey && /\.mpkg$/i.test(fileFromKey)) {
					out.push({ kind: "img", why: "mpkg-preview",
						src: host + "/custom-mpkg-preview?" + (folderFromKey ? "folder=" + encodeURIComponent(folderFromKey) + "&" : "") + "file=" + encodeURIComponent(fileFromKey) });
				}
				// ② web 档目录内的预览图（目录 id 从 mpkgKey / webUrl / image 里推）
				const webId = (() => {
					try {
						let m = /^custom\|([^/|]+)/.exec(keyRaw); if (m) return { base: "/custom-folder/", id: m[1] };
						m = /^library\|([^|]+)/.exec(keyRaw); if (m) return { base: "/library-web/", id: m[1] };
						m = /\/custom-folder\/([^/?#]+)\//.exec(webUrl); if (m) return { base: "/custom-folder/", id: decodeURIComponent(m[1]) };
						m = /\/library-web\/([^/?#]+)\//.exec(webUrl); if (m) return { base: "/library-web/", id: decodeURIComponent(m[1]) };
						m = /[?&]folder=([^&#]+)/.exec(webUrl || img); if (m) return { base: "/custom-folder/", id: decodeURIComponent(m[1]) };
					} catch (e) {}
					return null;
				})();
				if (webId && webId.id && !/\.(mpkg|pkg)$/i.test(webId.id) && (converted === "web" || !!webUrl)) {
					for (const name of ["preview.gif", "preview.jpg", "preview.png", "preview.webp", "preview.jpeg", "loading.webp"]) {
						out.push({ kind: "img", why: "web-dir-preview", src: host + webId.base + encodeURIComponent(webId.id) + "/" + name });
					}
				}
				// ③ 视频首帧 / 图片（既有行为）
				if (img) {
					if (converted === "mp4" || /\.(mp4|webm|mkv|mov|m4v)$/i.test(srcName)) out.push({ kind: "video", why: "video-first-frame", src: resolveHostUrl(img) });
					else out.push({ kind: "img", why: "image", src: resolveHostUrl(img) });
				}
			} catch (e) {}
			return out;
		}
		/** ① 预览候选链的推进器（挂在 <img> 的 onError 上）：下一个候选 / 耗尽则显示占位。
		 *  状态存在**元素自己身上**（data-mpw-thumb-idx），与 React 重渲染无冲突、也不需要 hooks。 */
		function mpwThumbNext(ev) {
			try {
				const el = ev && ev.target;
				if (!el || !el.getAttribute) return;
				let list = [];
				try { list = JSON.parse(el.getAttribute("data-mpw-thumb-list") || "[]") } catch (e) { list = [] }
				let i = Number(el.getAttribute("data-mpw-thumb-idx") || 0) + 1;
				while (i < list.length) {
					if (list[i]) { el.setAttribute("data-mpw-thumb-idx", String(i)); el.setAttribute("src", list[i]); return }
					i++;
				}
				el.setAttribute("data-mpw-thumb-failed", "1");
				try { el.style.display = "none" } catch (e) {}
				try {
					const ph = el.parentElement && el.parentElement.querySelector ? el.parentElement.querySelector("[data-mpw-thumb-ph]") : null;
					if (ph) { ph.style.display = ""; try { ph.removeAttribute("hidden") } catch (e) {} }
				} catch (e) {}
			} catch (e) {}
		}
		/* ══════════════════════════════════════════════════════════════════════════════════
		   ②(2026-09-20 真机修复·审计第 2 条)「**武装前先验活**：不可用的源绝不交给 iframe/媒体元素」
		   ──────────────────────────────────────────────────────────────────────────────────
		   真机现场：源不可用时（404），`/custom-folder/<x>/index.html` 返回的
		   `{"ok":false,"error":"not found"}` 被浏览器**原样渲染进 iframe** ⇒ 用户看到的"壁纸"是
		   一坨 JSON；`<img>` 的那条路则是左上角一个破图图标；`<video>` 是黑屏。
		   修法（两层，各管一半）：
		     ① **服务端**（lib/index.js）：文档类请求（.html / Sec-Fetch-Dest: iframe|document）
		        的 404/403 改回**极小的空 HTML**，绝不回 JSON —— 从根上断掉"JSON 当壁纸"；
		     ② **客户端**（本块）：挂载后做一次**轻量验活**（`Range: bytes=0-0` 的 GET）。
		        只有宿主**明确回了 `{ok:false}`**（真机那条 404 的形态）才算"源不可用"
		        —— 网络异常/无应答一律不判死（不允许一次探测抖动把好壁纸判没）。
		        不可用 ⇒ `data-mpw-bg-error="<status> <url>"` + `window.__mpwBgArmError`
		        + 壁纸层**隐藏**（露出主题纯色，绝不显示破图/JSON）+ 面板一行人话。
		   可判定状态：`#mpw-bgWrap[data-mpw-bg-error]`、`window.__mpwBgArmError`、`/diag` 的
		   `bg-arm-error` 信标、以及"`document.body.innerText` 不含 `\"ok\":false`"这条探针断言。
		   ══════════════════════════════════════════════════════════════════════════════════ */
		const mpwArmProbeCache = new Map();   // url → {ok, status, at}（同 URL 只探一次，避免每次 apply 都打）
		const mpwArmHinted = new Map();       // url → 上次提示时刻（同一条错误不刷面板）
		/** 源不可用：可判定状态 + 层隐藏 + 人话提示（**唯一落点**，任何路径都不许各写各的）。 */
		function mpwBgArmErrorSet(kind, url, status, detail) {
			try {
				const st = { kind: String(kind || ""), url: String(url || "").slice(0, 400), status: Number(status) || 0, detail: String(detail || "").slice(0, 200), at: Date.now() };
				try { window.__mpwBgArmError = st } catch (e) {}
				const { wrap } = (typeof bgElements === "function" ? bgElements() : {});
				if (wrap && wrap.setAttribute) {
					try { wrap.setAttribute("data-mpw-bg-error", String(st.status || 0) + " " + st.url) } catch (e) {}
					// 层保持纯色/占位：**隐藏整层**（内联 display 赢过类规则；恢复时由 clear 摘掉）
					try { wrap.style.setProperty("display", "none") } catch (e) {}
				}
				try { mpwTrace("bg-arm-error", st); } catch (e) {}
				try { mpwWebDiag("bg-arm-error", st); } catch (e) {}
				const lastAt = mpwArmHinted.get(st.url) || 0;
				if (Date.now() - lastAt > 15000) {
					mpwArmHinted.set(st.url, Date.now());
					const where = (() => { try { const d = String((readSection() || {}).customDirPath || ""); return d ? ("；当前自定义目录：" + d + (String((readSection() || {}).srcDirPath || "") && String(readSection().srcDirPath) !== d ? "（该壁纸记录来自：" + readSection().srcDirPath + "）" : "")) : "" } catch (e) { return "" } })();
					try {
						mpwPersistEmit("该壁纸源不可用（HTTP " + (st.status || "?") + "）：" + st.url + where
							+ "。已保持占位背景（不把错误页当壁纸渲染）—— 请在「壁纸」页重新选一次，或检查「自定义本地壁纸目录」是否指向该壁纸所在目录。");
					} catch (e) {}
				}
				return st;
			} catch (e) { return null }
		}
		/** 源重新可用/换档成功：清掉不可用状态与"层隐藏"（幂等，每次 apply 都会调）。 */
		function mpwBgArmErrorClear() {
			try {
				const { wrap } = (typeof bgElements === "function" ? bgElements() : {});
				if (wrap && wrap.removeAttribute) {
					try { wrap.removeAttribute("data-mpw-bg-error") } catch (e) {}
					try { wrap.style.removeProperty("display") } catch (e) {}
				}
				if (window.__mpwBgArmError) { try { window.__mpwBgArmError = null } catch (e) {} }
			} catch (e) {}
		}
		/** 轻量验活：只有**宿主明确回 `{ok:false}`** 才算不可用（其余一律放行 ⇒ 不允许误杀）。 */
		function mpwArmProbe(url) {
			try {
				let u = String(url || "");
				try { if (u.indexOf(location.origin) === 0) u = u.slice(String(location.origin).length) } catch (e) {}
				if (!u || u.indexOf(HOST_BASE) !== 0) return Promise.resolve({ ok: true, skipped: "not-host-url" });
				const cached = mpwArmProbeCache.get(u);
				if (cached && Date.now() - cached.at < 20000) return Promise.resolve(cached);
				const rec = (v) => { try { mpwArmProbeCache.set(u, Object.assign({ at: Date.now() }, v)) } catch (e) {} return v };
				return fetch(u, { method: "GET", headers: { Range: "bytes=0-0" }, cache: "no-store" }).then((r) => {
					const status = Number((r && r.status) || 0);
					if (r && r.ok) return rec({ ok: true, status: status, url: u });
					return Promise.resolve(r && r.json ? r.json().catch(() => null) : null).then((body) => {
						if (body && body.ok === false) return rec({ ok: false, status: status, url: u, error: String(body.error || "").slice(0, 120) });
						return rec({ ok: true, status: status, url: u, unknown: true });
					});
				}).catch(() => rec({ ok: true, url: u, unknown: true }));
			} catch (e) { return Promise.resolve({ ok: true, unknown: true }) }
		}
		/** 挂载后的**后台验活**：坏 ⇒ 交给 onBad（调用方决定卸谁）。 */
		function mpwVerifyArm(kind, url, onBad) {
			try {
				mpwArmProbe(url).then((v) => {
					if (v && v.ok === false) { try { onBad && onBad(v) } catch (e) {} }
				}).catch(() => {});
			} catch (e) {}
		}
		/* ══════════════════════════════════════════════════════════════════════════════════
		   ②(2026-09-20 真机修复·审计第 1 条)「**形状必须跟着条目来源走**」
		   ──────────────────────────────────────────────────────────────────────────────────
		   真机两次现场（同一类）：
		     · `webUrl="host:?custom=1&folder=3580207945&file=index.html"` 而 `3580207945` 是
		       **库根（allwallpaper/dd）下的目录名**、当前 customDir 却是另一处 ⇒ 404（JSON 渲染成壁纸）；
		     · `mpkgKey="custommpkg|…mpkg"`（容器）+ `image="host:?token=…"` ✓ + `webUrl="host:?custom=…"` ✗
		       —— 三个字段里混了两种来源形状，`webUrl` 是上一档残留却"抢先武装"。
		   三类来源各有唯一形状（与本仓既有契约逐字一致，见 applyCustomWebReal / applyLibraryWallpaper）：
		     custom    （自定义目录 / 子目录）：`host:?custom=1&folder=<目录名>&file=<文件>`
		     library   （本地库 token）      ：`host:?ltoken=<token>&web=1&file=<文件>`（场景另有 &scene=1）
		     container （上传/导入的 mpkg）  ：`host:?token=<token>&index=<下标>`（+ &offset=）
		   本块把"条目 → 源字段"绑成一个纯函数：形状只由 `srcRoot` 决定，**调用点再也无法拼错**；
		   同时写入 `srcRoot` / `srcDirPath` 两个自描述字段（换档时一起成套重写，见 MPW_SWITCH_SRC_KEYS）。
		   ══════════════════════════════════════════════════════════════════════════════════ */
		const MPW_SRC_ROOTS = ["custom", "library", "container", "external"];
		/** 源字段的"形状"分类（唯一口径；`host:` 标记与展开后的 URL 都认）。 */
		function mpwSrcShapeOf(url) {
			try {
				const u = String(url || "");
				if (!u) return null;
				if (/[?&]ltoken=/.test(u)) return "library";
				if (/[?&]token=/.test(u)) return "container";
				if (/[?&]custom=1/.test(u) || /[?&]folder=/.test(u)) return "custom";
				return "external";   // 直链 http(s)（渲染器 URL 等）
			} catch { return null }
		}
		/** 从 `mpkgKey` 反推来源类别（没有 key 的旧档 ⇒ null = 不做一致性裁决）。 */
		function mpwSrcRootOfKey(key) {
			try {
				const k = String(key || "");
				if (/^custommpkg\|/.test(k)) return "container";
				if (/^custom\|/.test(k)) return "custom";
				if (/^library\|/.test(k)) return "library";
				return null;
			} catch { return null }
		}
		/** 条目 → 完整源字段（含 srcRoot/srcDirPath）。`entry` 是列表项（三种列表同一套字段名）。 */
		function mpwSourceFieldsFor(entry, ctx) {
			const e = entry || {};
			const c = ctx || {};
			const out = {
				image: null, webUrl: null, sceneKey: null, source: null, mpkgKey: null, mpkgName: null,
				converted: null, fromMpkg: false, srcRoot: null, srcDirPath: null
			};
			try {
				const root = e.srcRoot || (e.ltoken ? "library" : (e.token !== void 0 ? "container" : "custom"));
				out.srcRoot = root;
				const kind = String(e.kind || e.type || "").toLowerCase();
				const file = String(e.file || e.media || "").split(/[\\/]/).pop();
				if (root === "library") {
					if (!e.ltoken) return out;
					out.mpkgKey = "library|" + e.ltoken;
					out.mpkgName = String(e.title || e.ltoken);
					out.source = file || "scene.pkg";
					if (kind === "web" || /\.(html?|xhtml)$/i.test(file)) {
						out.webUrl = "host:?ltoken=" + encodeURIComponent(e.ltoken) + "&web=1&file=" + encodeURIComponent(file || "index.html") + (c.compat ? "" : "&shim=1");
						out.converted = "web";
					} else if (kind === "scene" || /\.(pkg|mpkg)$/i.test(file)) {
						out.image = "host:?ltoken=" + encodeURIComponent(e.ltoken) + (file ? "&file=" + encodeURIComponent(file) : "") + "&scene=1";
						out.converted = "gif";
					} else {
						out.image = "host:?ltoken=" + encodeURIComponent(e.ltoken) + (file ? "&file=" + encodeURIComponent(file) : "");
						out.converted = kind === "video" ? "mp4" : "gif";
					}
					return out;
				}
				if (root === "container") {
					if (e.token === void 0 || e.index === void 0) return out;
					out.mpkgKey = "custommpkg|" + String(e.file || e.name || e.title || "");
					out.mpkgName = String(e.title || e.file || e.name || "");
					out.source = String(e.entryName || e.source || "");
					out.image = "host:?token=" + encodeURIComponent(e.token) + "&index=" + Number(e.index) + (e.offset ? "&offset=" + Number(e.offset) : "");
					out.converted = e.isMp4 === false ? "gif" : "mp4";
					return out;
				}
				if (root === "custom") {
					const folder = String(e.folder || e.name || c.folder || "");
					if (!folder) return out;
					out.mpkgKey = "custom|" + folder + (file ? "/" + file : "");
					out.mpkgName = String(e.title || folder);
					out.source = file || folder;
					out.srcDirPath = String(c.dir || (readSection() || {}).customDirPath || "");
					if (kind === "web" || /\.(html?|xhtml)$/i.test(file)) {
						out.webUrl = "host:?custom=1&folder=" + encodeURIComponent(folder) + "&file=" + encodeURIComponent(file || "index.html") + (c.compat ? "" : "&shim=1");
						out.converted = "web";
					} else if (kind === "scene" || /\.(pkg|mpkg)$/i.test(file)) {
						out.image = "host:?custom=1&folder=" + encodeURIComponent(folder) + (file ? "&file=" + encodeURIComponent(file) : "") + "&scene=1";
						out.converted = "gif";
					} else {
						out.image = "host:?custom=1&folder=" + encodeURIComponent(folder) + (file ? "&file=" + encodeURIComponent(file) : "");
						out.converted = kind === "video" || /\.(mp4|webm|mov|m4v|mkv)$/i.test(file) ? "mp4" : "gif";
					}
					return out;
				}
			} catch (err) { mpwErr("mpwSourceFieldsFor", err) }
			return out;
		}
		/** 源字段自洽裁决：形状与来源类别不符 ⇒ **不许武装**（宁可隐藏 + 明确提示，也不糊一坨错误页）。
		 *  @returns {{ok:boolean, why:string, root:string|null, shapes:object}} */
		function mpwSrcConsistent(section) {
			const s = section || {};
			const root = MPW_SRC_ROOTS.indexOf(String(mpwVal(s, "srcRoot") || "")) >= 0
				? String(mpwVal(s, "srcRoot")) : (mpwSrcRootOfKey(mpwVal(s, "mpkgKey")) || null);
			const shapes = { webUrl: mpwSrcShapeOf(mpwVal(s, "webUrl")), image: mpwSrcShapeOf(mpwVal(s, "image")) };
			if (!root) return { ok: true, why: "no-root", root: null, shapes: shapes };
			const bad = [];
			for (const k of ["webUrl", "image"]) {
				const sh = shapes[k];
				if (!sh) continue;
				if (sh === "external") continue;                 // 渲染器/直链：形状由 URL 自身决定
				if (root === "container" && k === "image" && sh === "container") continue;
				if (root === "custom" && sh === "custom") continue;
				if (root === "library" && sh === "library") continue;
				if (root === "container" && k === "webUrl") { bad.push(k + "=custom-shape-on-container"); continue }
				bad.push(k + "=" + sh + " vs root=" + root);
			}
			return { ok: bad.length === 0, why: bad.join(","), root: root, shapes: shapes };
		}
		/** ②(2026-09-20 真机修复) **挂载分支的唯一裁决**（纯函数：无 DOM、无 fetch，门禁可逐形态断言）。
		 *
		 *  为什么需要一条独立判据：`applyFromStorage` 旧写法是"`if (section.webUrl)` 一律走 web"，
		 *  而真机现场恰好是"`converted="mp4"` + `image` 缺 + `webUrl` 是**上一个** web 档的残留"
		 *  ⇒ 壁纸层先挂 web 帧（`/raw` 404）→ 场景看门狗打 `mpw-scene-fallback` → 回退也拿不到首帧
		 *  ⇒ 黑屏 + 左上角破图图标 + 日志框闪一下（级联故障的第一跳）。
		 *  规则（顺序即规格）：
		 *    1. 没有 webUrl ⇒ 有 image 就是媒体档，否则"无源"；
		 *    2. `converted==="web"` ⇒ 真网页壁纸，走 web（旧行为）；
		 *    3. webUrl 是**场景渲染器**（带 `pkgurl=`）⇒ 走 web（渲染器路径仍要 iframe）；
		 *    4. 有 image（说明这是一个媒体/静态帧档）⇒ **忽略残留 webUrl** 走媒体档；
		 *    5. `image` 缺但档案自相矛盾（`mpwSrcMissing`：声明了类型却没有对应字段）⇒ **谁也不挂**
		 *       （`none`：壁纸层隐藏 + 明确提示，由自愈去补源），绝不用一张必 404 的旧 web 档糊上去；
		 *    6. 其余（只有 webUrl、也没有类型声明）⇒ 走 web（历史档兼容）。
		 *  @returns {{mount:'web'|'media'|'none', reason:string}} */
		function mpwPickMount(s) {
			const webUrl = typeof mpwVal(s, "webUrl") === "string" ? String(mpwVal(s, "webUrl")) : "";
			const image = typeof mpwVal(s, "image") === "string" ? String(mpwVal(s, "image")) : "";
			const converted = typeof mpwVal(s, "converted") === "string" ? String(mpwVal(s, "converted")) : "";
			if (!webUrl) return { mount: image ? "media" : "none", reason: image ? "media-only" : "no-source" };
			// ②(2026-09-20 审计第 1 条) **形状与来源不符 ⇒ 这个 webUrl 是残留**（如 `mpkgKey=custommpkg|…`
			//   却带着 `custom=1&folder=…`）⇒ 直接按媒体档走，不去挂那张必 404 的 web 帧。
			const webShape = mpwSrcShapeOf(webUrl);
			const keyRoot = mpwSrcRootOfKey(mpwVal(s, "mpkgKey"));
			const declaredRoot = MPW_SRC_ROOTS.indexOf(String(mpwVal(s, "srcRoot") || "")) >= 0 ? String(mpwVal(s, "srcRoot")) : null;
			/* ②(2026-09-20 真机复测) `mpkgKey` 是**权威身份**（每次换档都成套重写它）⇒ 它说话时
			   一律以它为准；`srcRoot` 只在没有 key 的老档上兜底（真机现场：`srcRoot` 残留成 custom
			   而 key 是 custommpkg ⇒ 一致性裁决被带偏，残留 webUrl 又抢到了挂载权）。 */
			const shapeRoot = keyRoot || ((declaredRoot && declaredRoot !== "external") ? declaredRoot : null);
			const shapeMismatch = !!(webShape && shapeRoot && webShape !== "external" && webShape !== shapeRoot);
			if (!shapeMismatch && converted === "web") return { mount: "web", reason: "converted-web" };
			/* ②(2026-09-20 真机复测) 渲染器 URL（带 `pkgurl=`）只有**在场景档**上才是身份的一部分：
			   真机黑屏现场是 `converted="mp4"`（刚换的容器视频）+ 上一个场景的渲染器 webUrl ⇒
			   旧写法把它当"场景渲染器档"挂上去（404 → 看门狗回退 → 破图 + 黑屏）。
			   现在：只有 `converted==="scene"`（或有 sceneKey 且没有 image）才认它是场景渲染器。 */
			const rendererUrl = /[?&]pkgurl=/.test(webUrl);
			if (!shapeMismatch && rendererUrl && (converted === "scene" || (!image && !converted))) return { mount: "web", reason: "scene-renderer" };
			if (shapeMismatch && !image) return { mount: "none", reason: "stale-shape(" + webShape + "!=" + shapeRoot + ")" };
			if (image) return { mount: "media", reason: shapeMismatch ? "stale-shape-has-image" : "stale-webUrl-has-image" };
			if (mpwSrcMissing(s)) return { mount: "none", reason: "half-archive" };
			return { mount: "web", reason: "webUrl-only" };
		}
		/** 不变量Ⅲ（半边）：**补空缺、不覆盖** —— 保留 `win` 已有的每一个键的值，
		 *  只把 `lose` 里 `win` **没有**的键补进来。
		 *  ⚠ 这里连踩过两次坑（都由 B1/B4 断言抓出来，故单独成函数 + 明确语义）：
		 *    · 用 `mpwMergeSection(lose, win)` ⇒ win 的键逐个覆盖 lose ⇒ 变成"整档赢"，
		 *      等于退化成旧写法的整档覆盖语义（字段级裁决名存实亡）；
		 *    · 用"谁值不同就取另一份"⇒ 两边都有的键仍然由**一方**决定，同样是整档赢。
		 *  只有"以赢家为底、输家只填空缺"才同时满足：两端都有的键由时间戳裁决 + 只有一端有的键一定保留。 */
		function mpwOverlayFill(win, lose) {
			const out = Object.assign({}, (win && typeof win === "object") ? win : {});
			if (!lose || typeof lose !== "object") return out;
			for (const k of Object.keys(lose)) {
				if (out[k] !== void 0) continue;          // 赢家已有 ⇒ 一律用赢家的
				const v = lose[k];
				if (v === void 0 || v === null) continue; // 输家的"缺/显式删"都不该补进赢家
				out[k] = v;
			}
			return out;
		}
		/** 不变量Ⅲ：启动时把两处存储合成一份 —— 新者优先（各自的内嵌时间戳），
		 *  输家只补赢家没有的键（`mpwOverlayFill`），源字段另有粘性保护（谁都没有时不抹）。 */
		function mpwReconcileStores(localIn, hostIn) {
			const local = mpwStripMeta(localIn || {});
			const host = mpwStripMeta(hostIn || {});
			const lAt = mpwStampOf(localIn, "local");
			const hAt = mpwStampOf(hostIn, "host");
			const localKeys = mpwSortedKeys(local), hostKeys = mpwSortedKeys(host);
			let winner = "local", newer = "local", reason = "";
			if (lAt !== null && hAt !== null) {
				if (lAt > hAt) { winner = "local"; newer = "local"; reason = "local-newer(" + (lAt - hAt) + "ms)" }
				else if (hAt > lAt) { winner = "host"; newer = "host"; reason = "host-newer(" + (hAt - lAt) + "ms)" }
				else { winner = "local"; newer = "local"; reason = "same-ms" }
			} else if (lAt !== null && hAt === null) { winner = "local"; newer = "local"; reason = "host-unstamped" }
			else if (hAt !== null && lAt === null) { winner = "host"; newer = "host"; reason = "local-unstamped" }
			else {
				// 两处都没时间戳（升级上来的旧档）：本地**有而宿主没有**的键按本地（宿主明确跳过 image），
				// 其余按"字段更全的那一份"，并如实标 conservative —— 没有依据就不假装知道谁新。
				winner = (localKeys.length >= hostKeys.length) ? "local" : "host";
				newer = "unknown";
				reason = "no-stamps(local=" + localKeys.length + " keys, host=" + hostKeys.length + " keys)";
			}
			/* 裁决后合成：**赢家已有的键一律用赢家的值**，输家只补赢家没有的键（见 mpwOverlayFill）。 */
			const win = winner === "local" ? local : host;
			const lose = winner === "local" ? host : local;
			let merged = mpwOverlayFill(win, lose);
			// 源字段粘性 + 一致性标记：两处都没有源、却都有来源线索 ⇒ 半残档（交给自愈）
			merged = mpwStickySource(local, merged);
			merged = mpwStickySource(host, merged);
			const half = mpwSrcMissing(local) || mpwSrcMissing(host);
			if (half) { try { merged[MPW_SRC_LOST_KEY] = { at: Date.now(), keys: mpwSortedKeys(merged).filter((k) => MPW_SRC_KEYS.includes(k)) } } catch {} }
			else if (merged[MPW_SRC_LOST_KEY] !== void 0) { try { delete merged[MPW_SRC_LOST_KEY] } catch {} }
			return { merged, winner, newer, reason, local, host, lAt, hAt, localKeys, hostKeys, half };
		}
		/** 自愈记账（面板可见 + 诊断可查 + 探针可读）：与 mpwPersistState 同风格，不静默。 */
		const MPW_HEAL = { attempted: 0, healed: 0, failed: 0, last: null };
		let mpwHealTimer = 0, mpwHealRunning = false, mpwHealSig = "";
		let mpwBackupSource = null;   // mpw_settings_backup 里那份源（懒读，读不到 = null）
		/** 从 mpw_settings_backup（面板被误重置时的备份）里取"最后一次已知的源"。 */
		function mpwBackupSrc() {
			if (mpwBackupSource !== null) return mpwBackupSource;
			mpwBackupSource = false;   // 只尝试一次
			try {
				const raw = localStorage.getItem('mpw_settings_backup');
				if (!raw) return mpwBackupSource;
				const o = JSON.parse(raw);
				if (!o || typeof o !== "object") return mpwBackupSource;
				const out = {};
				for (const k of MPW_SRC_KEYS.concat(["mpkgKey"])) if (typeof o[k] === "string" && o[k]) out[k] = o[k];
				if (MPW_SRC_KEYS.some((k) => out[k])) mpwBackupSource = out;
			} catch {}
			return mpwBackupSource;
		}
		function mpwHostJson(url, opts) {
			return fetch(HOST_BASE + url, opts).then((r) => (r && r.ok ? r.json() : null));
		}
		/* ②(2026-09-20 真机修复·根因 B)「宿主侧 customDir 与客户端 customDirPath 失同步」
		   现场：客户端档里 customDirPath 是 A（wallpaperE/小鸟游星野），宿主侧还停在 B（dd，用户换过目录）
		   ⇒ 所有 `folder=<A 下的子目录>` 的请求被拼到 B 下 ⇒ **ENOENT 500**
		   （音轨扫描：`GET /custom-scene-audio?refs=0&folder=小鸟游星野` → 500，错误里的路径是旧根）。
		   修法（**只在失败时补一次**，happy path 零额外请求）：任何 folder 类宿主请求失败后
		   先 POST /custom-dir 把宿主目录同步成客户端那一份，再重试一次；仍失败才如实报失败。
		   为什么不"每次请求前先同步"：那会给每个扫描/推导都加一次 POST（真机与既有门禁 C20
		   都要求"能推出来时**少一次网络**"）。 */
		let mpwDirSynced = "";        // 最近一次**成功**同步给宿主的目录（留证/诊断用）
		/** POST /custom-dir：一次调用同时完成"宿主目录同步"与"目录清单获取"（宿主侧就是这样实现的）。 */
		function mpwSyncCustomDir() {
			try {
				const dir = String((readSection() || {}).customDirPath || "");
				if (!dir) return Promise.resolve(null);
				return fetch(HOST_BASE + "/custom-dir", {
					method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ dir: dir })
				}).then((r) => (r && r.ok ? r.json() : null)).then((d) => {
					if (d && d.ok) { mpwDirSynced = dir; mpwTrace("custom-dir:synced", { dir: dir }); }
					else mpwTrace("custom-dir:sync-failed", { dir: dir });
					return d || null;
				}).catch(() => null);
			} catch { return Promise.resolve(null) }
		}
		/** 宿主 JSON 请求 + **失败即同步目录并重试一次**（根因 B 的唯一落点）。 */
		function mpwHostJsonRetryDir(url, opts) {
			return mpwHostJson(url, opts).then((d) => {
				if (d && d.ok) return d;
				return mpwSyncCustomDir().then((sd) => {
					if (!sd || !sd.ok) return d || null;
					mpwTrace("dir-resync-retry", { url: String(url).slice(0, 80) });
					return mpwHostJson(url, opts);
				}).catch(() => d || null);
			}).catch(() => null);
		}
		/** 推断的落点必须**真的存在**：custom 系列先问宿主目录清单（"能推就推、推不出就明说"）。 */
		function mpwCustomDirListing() {
			try {
				const dir = (readSection() || {}).customDirPath;
				if (typeof dir !== "string" || !dir) return Promise.resolve(null);
				// 这一次 POST 同时把宿主目录同步好（同一个调用，不多一次网络）
				return mpwSyncCustomDir();
			} catch { return Promise.resolve(null) }
		}
		function mpwPickVideo(files) {
			const vids = (files || []).filter((f) => f && f.type === "video");
			const named = vids.find((f) => /mid-?autumn|hoshino|中秋|星野/i.test(String(f.name || "") + " " + String(f.title || "")));
			return (named || vids[0] || null);
		}
		function mpwAppliedFrom(f, folder) {
			const media = String((f && f.media) || "");
			if (!media) return null;
			if (f.type === "video" || f.type === "mp4") return { image: "host:?custom=1&folder=" + encodeURIComponent(folder) + "&file=" + encodeURIComponent(media), converted: "mp4" };
			if (f.type === "web") return { webUrl: "host:?custom=1&folder=" + encodeURIComponent(folder) + "&file=" + encodeURIComponent(media || "index.html") + "&shim=1", converted: "web" };
			if (f.type === "scene") return { image: "host:?custom=1&folder=" + encodeURIComponent(folder) + "&scene=1", converted: "gif" };
			if (/\.(gif|png|jpe?g|webp)$/i.test(media)) return { image: "host:?custom=1&folder=" + encodeURIComponent(folder) + "&file=" + encodeURIComponent(media), converted: "gif" };
			return { image: "host:?custom=1&folder=" + encodeURIComponent(folder) + "&file=" + encodeURIComponent(media), converted: "mp4" };
		}
		/** 按 mpkgKey 反推源（无浏览器可测的纯逻辑；宿主只读、不写）。
		 *  支持前三类 key（本轮真机现场就是 `custom|<folder>` 这一类）：
		 *    `custom|<folder>`                    → 目录清单里那条（有 media 就用它）
		 *    `custom|<folder>/<file>`             → 直接拼
		 *    `custommpkg|<folder>/<file>` / `<file>` → 走 /custom-mpkg 换一个**新鲜 token**
		 *    `library|<ltoken>` / `library|<ltoken>|<file>` → web 走 /library-web、其它走 /library-scene-*
		 *  推不出来（目录里没有这个 folder / 宿主不可用 / 清单为空）→ 返回 null，调用方必须**明确提示**。 */
		function mpwDeriveSourceFromKey(key) {
			const k = String(key || "");
			if (!k) return Promise.resolve(null);
			if (k.indexOf("custommpkg|") === 0) {
				const rest = k.slice("custommpkg|".length);
				...
				// ②(2026-09-20 真机修复·根因 B) `/custom-mpkg` 也在**宿主侧 customDir** 下解析 file
				//   ⇒ 失败时（宿主目录还是旧的那一份）同步目录并重试一次，否则会出现
				//   "手工调 200、自愈里 404"这种假失败（现场：heal 推不出源，壁纸层永久 display:none）。
				return mpwHostJsonRetryDir("/custom-mpkg?" + q).then((d) => {
					...
					return {
						...
						source: sel.name, entryName: sel.entryName || "", converted: sel.isMp4 ? "mp4" : "gif", mpkgName: file
					};
				}).catch(() => null);
			}
			if (k.indexOf("custom|") === 0) {
				const rest = k.slice("custom|".length);
				const slash = rest.indexOf("/");
				if (slash > 0) {
					const folder = rest.slice(0, slash), file = rest.slice(slash + 1);
					if (!file) return Promise.resolve(null);
					const isVid = /\.(mp4|webm|mov)$/i.test(file);
					return Promise.resolve({
						image: "host:?custom=1&folder=" + encodeURIComponent(folder) + "&file=" + encodeURIComponent(file),
						source: file, mpkgName: folder, converted: isVid ? "mp4" : "gif"
					});
				}
				const folder = rest;
				if (!folder) return Promise.resolve(null);
				return mpwCustomDirListing().then((d) => {
					if (!d || !d.ok || !Array.isArray(d.files)) return null;
					const me = d.files.find((f) => f && f.folder && String(f.name) === folder);
					if (!me) return null;
					let f = me;
					/* 同一目录里有多个媒体（图片/视频/网页）时用户当时选的是哪一个没有记录 —— 这时**不猜**：
					   只在"唯一候选"（或视频中能唯一命名命中的那条）时才推导，否则如实报"推不出"。 */
					if (me.type === "unknown" || me.type === "image") {
						const cands = d.files.filter((x) => x && x.folder && String(x.name) === folder);
						if (cands.length > 1) return null;
						f = cands[0] || me;
					}
					const pick = (me.type === "video" && !me.media) ? mpwPickVideo(d.files.filter((x) => x && x.folder && String(x.name) === folder)) : me;
					const out = mpwAppliedFrom(pick || f, folder);
					if (!out) return null;
					out.source = pick && (pick.title || pick.media) || folder;
					out.mpkgName = (pick && pick.title) || folder;
					return out;
				}).catch(() => null);
			}
			if (k.indexOf("library|") === 0) {
				const rest = k.slice("library|".length);
				const parts = rest.split("|");
				const ltoken = parts[0], file = parts[1] || "";
				if (!ltoken) return Promise.resolve(null);
				if (file && /\.(html?|js)$/i.test(file)) {
					return Promise.resolve({
						webUrl: "host:?ltoken=" + encodeURIComponent(ltoken) + "&web=1&file=" + encodeURIComponent(file) + "&shim=1",
						source: file, mpkgName: "library|" + ltoken, converted: "web"
					});
				}
				return Promise.resolve({
					image: "host:?ltoken=" + encodeURIComponent(ltoken) + (file ? "&file=" + encodeURIComponent(file) : "") + "&scene=1",
					source: file || "scene.pkg", mpkgName: "library|" + ltoken, converted: "gif"
				});
			}
			return Promise.resolve(null);
		}
		/** 半残档自愈（同步返回"这一轮有没有改内存里的档"；网络/异步部分在后台补齐后再 apply 一次）。
		 *  ②(2026-09-20 真机修复) 去重口径从"**同一把 key 只试一次**"改成"**同一把 key + 同一个
		 *  源字段状态**只试一次"：真机上"webUrl 被清空"这件事本身**没有改变 mpkgKey**，
		 *  旧口径于是把这次（唯一一次可能成功的）自愈也挡掉了 —— 现场就是"清掉 webUrl 后
		 *  壁纸层 display:none，而 image 永远补不回来"。源字段状态一变就允许再试；总数仍有上限。 */
		const MPW_HEAL_MAX_ATTEMPTS = 6;
		function mpwHealSigOf(s) {
			try {
				return MPW_SRC_KEYS.map((k) => {
					const v = mpwVal(s, k);
					return k + "=" + (v === void 0 ? "-" : v === null ? "null" : String(v) ? "set" : "empty");
				}).join(",");
			} catch { return "?" }
		}
		function mpwHealHalfArchive() {
			...
			try {
				const cur = readSection();
				if (!cur || !mpwSrcMissing(cur)) { mpwHealSig = ""; return false }
				const key = String(cur.mpkgKey || "");
				if (!key) return false;
				const sig = key + "|" + mpwHealSigOf(cur);
				if (mpwHealSig === sig && MPW_HEAL.attempted > 0) return false;   // 同一状态只试一次
				if (MPW_HEAL.attempted >= MPW_HEAL_MAX_ATTEMPTS) return false;     // 总量上限（防抖环）
				// 迟到那次已经把源写回来了 ⇒ 本档不是半残档（时间戳裁决晚到的场景）
				if (MPW_HEAL.last && MPW_HEAL.last.ok && MPW_HEAL.last.key === key && mpwVal(cur, "image") !== void 0) return false;
				const bak = mpwBackupSrc();
				if (bak && MPW_SRC_KEYS.some((sk) => typeof bak[sk] === "string" && bak[sk])) {
					const next = mpwMergeSection(cur, bak);
					writeSection(next, true);
					changed = true;
					MPW_HEAL.attempted++;
					MPW_HEAL.healed++;
					MPW_HEAL.last = { key, ok: true, from: "backup", at: Date.now() };
					mpwTrace("settings:heal", { key, from: "backup" });
					mpwPersistEmit("检测到壁纸选择只剩来源线索（" + key + "）→ 已从本地备份 " + "mpw_settings_backup" + " 恢复壁纸源，壁纸层恢复显示。");
					return true;
				}
				mpwHealSig = sig;
				MPW_HEAL.attempted++;
				MPW_HEAL.last = { key, ok: false, from: "pending", at: Date.now() };
				mpwTrace("settings:heal-attempt", { key, sig: mpwHealSigOf(cur) });
				Promise.resolve(mpwDeriveSourceFromKey(key)).then((found) => {
					/* 自愈必须**幂等且只作用于这一档**：期间用户换了壁纸/清了壁纸（mpkgKey 变了）就不许写回。 */
					let nowKey = "";
					try { nowKey = String(readSection().mpkgKey || "") } catch {}
					if (nowKey !== key) { try { console.warn("[dsh-mpkg-wallpaper] 自愈完成时壁纸已切换，丢弃迟到结果:", key); } catch {} return }
					if (found) {
						writeSection(mpwMergeSection(readSection(), found), true);
						MPW_HEAL.healed++;
						MPW_HEAL.last = { key, ok: true, from: "derive", at: Date.now() };
						mpwTrace("settings:heal", { key, from: "derive" });
						mpwPersistEmit("检测到壁纸选择只剩来源线索（" + key + "）→ 已按包/目录元数据推出壁纸源并恢复显示。");
						try { applyFromStorage() } catch (e) { mpwErr("applyFromStorage(自愈后重放)", e) }
					} else {
						MPW_HEAL.failed++;
						MPW_HEAL.last = { key, ok: false, from: "derive-failed", at: Date.now() };
						mpwTrace("settings:heal-failed", { key });
						/* 推不出 ⇒ **明确提示**，不静默隐藏壁纸层（这是本条与旧行为的根本区别）。 */
						mpwPersistEmit("壁纸选择记录只剩来源线索（" + key + "），且按包/目录元数据**推不出**具体素材"
							+ "（目录里找不到、或该目录有多个候选）。请在「壁纸」页重新选一次壁纸 —— 已保留其余全部设置。");
					}
				}).catch((e) => {
					MPW_HEAL.failed++;
					MPW_HEAL.last = { key, ok: false, from: "error", at: Date.now() };
					mpwErr("半残档自愈", e);
				});
			} catch (e) { mpwErr("mpwHealHalfArchive", e) }
			return changed;
		}
		/** boot 之后补一次自愈（不抢"用户已改过设置"的语义：只在档仍半残时生效）。 */
		function mpwHealSchedule() {
			try {
				if (mpwHealTimer) return;
				mpwHealTimer = setTimeout(() => {
					mpwHealTimer = 0;
					try { mpwHealRunning = true; if (mpwHealHalfArchive()) { try { applyFromStorage() } catch {} } } catch (e) { mpwErr("mpwHealSchedule", e) }
					finally { mpwHealRunning = false }
				}, 1200);
			} catch {}
		}
		...
		function showVideoEdge(url, video) {
			...
			/* ①(NP-4) 起播顺序：**先 muted 起播，拿到播放权之后再按设置应用 muted/volume**。
			   旧写法在这里先 `applyVideoMute()` 再 `play()` ⇒ 那是一次"有声自动播放"，
			   没有用户手势时被浏览器直接拒（真机读数：NotAllowedError，见 npPrimePlay 的注释）
			   ⇒ 视频永远 paused、画面冻住、也没有声音。顺序是这条 bug 的全部根因。 */
			try { npPrimePlay(video); } catch {}
			edgeDrawFrame();
		...
		function showImageEl() {
			...
			/* C(2026-09-20 真机修复) **切离视频档必须真的停住它**：
			   旧写法只 `removeAttribute("src")` —— 而按 HTML 规范，**移除 src 属性不会停止正在播放的媒体**
			   （元素仍在 DOM 里、`display:none` 也照样出声），必须 `pause()` 且 `load()` 把管线重置。
			   这正是"已切掉 web/视频档、什么都没动却又响一下"的第一嫌疑点：
			   隐藏但仍在播的 `<video>` 不在任何"可见媒体"的筛选里，而任何一次"取消静音"的瞬态
			   （起播/转码/电平重放）都会让它出声几声。 */
			try { if (!video.paused) video.pause() } catch (e) {}
			try { video.removeAttribute("src"); if (video.load) video.load() } catch (e) {}
			...
			video.style.display = "none";
			img.style.display = "";
			/* ②(2026-09-20 审计第 2 条) 图片档也要验活：源不可用时浏览器画的是**破图图标**
			   （用户现场"左上角破图图标"就是这条），而 JSON body 也一样会被 img 判为解码失败。
			   验活失败 ⇒ 撤 src + 记状态 + 层隐藏（绝不显示破图）。 */
			try {
				const u = String(img.getAttribute && img.getAttribute("src") || "");
				if (u) mpwVerifyArm("image", u, (v) => {
					try { img.removeAttribute("src") } catch (e) {}
					mpwBgArmErrorSet("image", v.url, v.status, v.error || "host returned ok:false");
				});
			} catch (e) {}
		}
		function showVideoEl(url) {
			npOpTrace("showVideoEl", null, String(url || "").slice(0, 48));
			const { img, video, frame, canvas, wrap } = bgElements();
			if (!img || !video || !wrap) { npOpTrace("showVideoEl", null, "no-elements"); return; }
			...
			try { if (video.getAttribute('src') !== url) { video.src = url; npSetUserPaused(video, false) } } catch {}
			...
			// ①(NP-4) 门控**在 npPrimePlay 里**（它自己判 wallUserPaused/powPaused），
			//   并且把"静音/电平落点"挪到**起播成功之后**（顺序根因见 npPrimePlay 注释）。
			try { npPrimePlay(video); } catch {}
			/* ②(2026-09-20 审计第 2 条) 视频档同样验活：源不可用 ⇒ 撤 src + 记状态 + 层隐藏
			   （否则是一块黑屏，用户分不清"视频坏了"和"壁纸就是这个颜色"）。 */
			try {
				const u = String(url || "");
				if (u) mpwVerifyArm("video", u, (v) => {
					try { video.pause() } catch (e) {}
					try { video.removeAttribute("src") } catch (e) {}
					try { video.load && video.load() } catch (e) {}
					mpwBgArmErrorSet("video", v.url, v.status, v.error || "host returned ok:false");
				});
			} catch (e) {}
		}
		/**
		 ...
		 *  ①(NP-3) 我们自己的曲目播放器在放音时**强制静音帧内**（见 npAudioOwns）：壁纸目录里
		 *  那首歌由控件放，帧内那份再放一遍就是两个声音叠着。播放器没在放 ⇒ 完全按设置走（零行为变化）。 */
		function applyWebMute(frame) {
			try {
				const s = readSection();
				/* ④(2026-09-20) 帧内静音 = 设置项静音 ∨ **卡片把帧内音频挡住了**（暂停/我们自己在放音）。
				   旧写法只看 `npAudioOwns()`（我们**正在放**）⇒ 用户按下卡片暂停的那一刻
				   npAudioOwns() 变假、帧内立刻解除静音 ⇒ 壁纸自带的 BGM 继续响（真机现场第 ④ 条）。 */
				const mute = (s.mute !== void 0 ? !!s.mute : true) || npFrameSoundBlocked();
				if (frame) frame.muted = mute;
				// ①(2026-09-16 I 项) shim 通道：帧内静音由 shim 执行（不透明源读不到帧内元素）
				if (frame && mpwWebShimUrl(frame.getAttribute("src"))) {
					webShimCall(frame, { op: "policy", muted: mute, speed: npPolicySpeed(), volume: npWantVolume() });
				}
				const doc = frame && frame.contentDocument;
				if (doc) {
		...
		/** ①(NP-3) 视频壁纸的静音落点：`video.muted` 从建 DOM 那刻起被写死 true、之后**再没有**
		 *  任何代码按静音设置给它赋值 ⇒ 现场"声音控制打不开、一直是静音状态"（视频有音轨
		 *  —— 用户当前那张 mp4 实测 `ffprobe`: h264 + **aac 音轨**，20.0s）。
		 *  这里让既有设置项 `mute` 真正管住视频元素；NP 控件的静音键走的是同一个设置项。 */
		function applyVideoMute() {
			const { video } = bgElements();
			if (!video) return false;
			try {
				const s = readSection();
				const want = npWantMuted();   // C4：唯一口径（含手势闸）
				if (video.muted !== want) video.muted = want;
			} catch {}
			return true;
		}
		/* ══════════════════════════════════════════════════════════════════════════
		   ①(NP-4 2026-09-19 真机修复)「壁纸声音」的电平 + 起播顺序
		   真机读数（用户当前档 `custom|3582362359`：video + mp4 内 aac，设置 mute=false）：
		     · 加载后 `<video>` = {paused:true, muted:false, volume:1, currentTime:0}
		     · 全程 `play()` **只被调用过一次**，`muted:false`，promise
		       **rejected NotAllowedError**（"The play method is not allowed by the user agent…"），
		       而旧写法在 .catch 里把它吞了（lib/client.js 的 showVideoEl 尾部）
		   ⇒ 视频永远没起播：画面冻在第一帧、**也没有任何声音**，于是静音开关"开/关都听不到声音"。
		   根因是**顺序**：旧写法先 `applyVideoMute()`（把 muted 写成 false）再 `play()`
		   —— 那一刻这是一次"有声自动播放"，没有用户手势时被浏览器直接拒；
		   先以 muted 起播（浏览器允许）拿到播放权，再取消静音才是对的顺序。
		   下面三件事一起做，缺一条就还剩一半：①起播顺序；②失败**不静默**（状态可查 + 首次手势重试）；
		   ③**电平**落点（NP-3 只有布尔 muted，全仓没有改 volume 的地方）。
		   ══════════════════════════════════════════════════════════════════════════ */
		/** ①(NP-4) 传输/起播动作的**统一轨迹**（最近 32 条）。真机排障唯一能看到"动作走到哪一步"的地方；
		    只记账，零行为影响（探针读它，见 tools/np-media-live-probe.mjs 的 I/K 组）。 */
		function npOpTrace(op, value, did) {
			try {
				const a = window.__mpwNpOps || (window.__mpwNpOps = []);
				a.push({ op: op, value: value === void 0 ? null : value, at: Date.now(), did: String(did || "") });
				if (a.length > 32) a.shift();
			} catch (e) {}
		}
		let npPlayBlocked = null;      // {name,message,at,retryMuted} —— 起播被拒的可查原因（null = 没被拒）
		/** 起播被拒的原因**挂到 window 上**（真机探针/控制台可查；写失败不影响任何行为）。
		    为什么必须可查：这条 bug 的修前形态就是"被拒了但没人知道"（`.catch(() => {})`）——
		    一个只在闭包里存在的错误等于没有错误。 */
		function npMarkPlayBlocked(v) {
			npPlayBlocked = v;
			try { window.__mpwNpPlayBlocked = v; } catch (e) {}
		}
		/* C4(2026-09-20 真机修复)「**首次用户手势之前一律保持 muted**」
		   真机（家长 4 分钟审计，48/48 拍）：`#mpw-bgVideo { muted=false, volume=0.33, paused=false }`
		   连续循环播放，`__mpwNpOps = ["… muted=true paused=false", "already-playing"]`
		   ⇒ 我们的顺序是"muted 起播 → **稍后** `npApplyMute` 恢复设置值(mute=false)"，
		   于是声音在"加载完成之后某一刻"才出现 —— 用户主观上就是"我没动它，它自己开始放了"。
		   契约（与本仓既有"没有用户手势不许出声"一致）：**没被用户交互过 ⇒ 一直 muted**；
		   第一次 pointerdown/keydown（或卡片上的显式操作）⇒ 解除，此后完全按设置走。
		   可判定状态：`window.__mpwNpSoundAllowedBy`（'gesture' | 'card' | null）+ `npGestureSeen`。 */
		let npGestureSeen = false;     // C4：用户至少交互过一次（解除"必须 muted"）
		let npGestureArmed = false;    // 首次用户手势重试只装一次（不常驻监听）
		let npPrimedMuted = false;     // 正在用"muted 起播"兜着（这一段里 muted 由重试持有，不被状态校准撤销）
		let npPrimedMutedTimer = 0;    // 上面那段的到期兜底（1.5s）
		/* ── ①(NP-4) 浏览器自动播放策略的**第二半**（真机读数逼出来的）──────────────────
		   第一半：没有用户手势时"有声"自动播放会被 **reject**（NotAllowedError）——见 npPrimePlay。
		   第二半（更隐蔽）：**"muted 起播成功之后再取消静音"同样不被允许** —— Firefox 在那一刻
		   直接把元素**停住**（paused=true）：既不 reject 任何 promise，也不调用 `video.pause()`
		   （所以包 pause/load 都抓不到它）。真机读数（用户那张档，页面从未交互）：
		     ops = [showVideoEl, prime:"calling-play muted=true", prime-ok:"played"]
		     而 video = {paused:true, currentTime:0, muted:false}
		   ⇒ 画面冻住、也没有任何声音；静音开关怎么拨都不响（因为**根本没在放**）。
		   正确做法（浏览器规则就是这么定的，不是绕过它）：
		     · 页面还没被交互过 ⇒ 以 muted 保持画面在动，并把"声音被策略挡住"记成可查状态；
		     · 用户**第一次**真的点了页面（pointerdown/keydown 任一）⇒ 立刻取消静音并续播（声音到位）；
		     · 这一段里 muted 由本状态持有（`npSoundBlocked` ⇒ npWantMuted() 为 true），
		       否则"取消静音 → 被停 → 再取消静音"会自己转成一个环。 */
		let npWantPlay = false;        // 我们**想要**它在放（用来把"被策略停住"与"用户自己暂停"分开）
		/* ①(NP-4) 真机回归的修法：`applyNowPlaying` 里那次"补起播"（刷新后没人再调 play 了）
		   会把**用户刚用卡片按下的暂停**在 60ms 后解开（真机读数：点播放键 ⇒ paused false→false，
		   因为暂停被延迟 apply 又续上了）。所以"用户按过暂停"必须是一条独立状态：
		     · npTransport 的 pause 分支置真，play 分支置假；
		     · npPrimePlay 在它为真时**直接返回**（和 wallUserPaused/powPaused 同一条闸）；
		     · 切壁纸/换源（applyNowPlaying 走新 section）时清零。 */
		/* ①(NP-4) "用户亲手按过暂停"这条状态记在**元素自己身上**（`video.__mpwUserPaused`），
		   不记模块变量：元素是这件事的天然所有者（换源即清零），而且单元门禁里每个用例一份新夹具时
		   模块变量会跨夹具串味（实测：上一个用例的暂停把下一个用例的静音判据带红）。 */
		function npUserPausedOf(video) { try { return !!(video && video.__mpwUserPaused) } catch (e) { return false } }
		function npSetUserPaused(video, on) { try { if (video) video.__mpwUserPaused = !!on } catch (e) {} }
		let npSoundBlocked = false;    // 浏览器不允许"没有用户手势就出声"（已如实记录并保持画面在动）
		let npSoundReason = null;      // 可查原因（window.__mpwNpSoundBlocked）
		/** 设置项 `npVolume`（0..100）→ 0..1。缺省/非法 ⇒ 1（与 elements 的默认值一致 ⇒ 零行为变化）。 */
		function npWantVolume() {
			let raw;
			try { raw = readSection().npVolume; } catch (e) { return 1 }
			const n = Number(raw);
			if (raw === void 0 || raw === null || !isFinite(n)) return 1;
			return Math.max(0, Math.min(1, n / 100));
		}
		/** 静音落点（唯一口径）：设置项 `mute`，或我们的播放器正在放音（内容重复）。 */
		function npWantMuted() {
			let s = {};
			try { s = readSection() } catch (e) {}
			/* C4(2026-09-20) **没有用户手势 ⇒ 一律 muted**：不允许"加载后若干秒自己从 muted 翻成可听"
			   （真机 48/48 拍的可听播放就是这么来的）。手势/卡片显式操作之后才按设置走。 */
			if (!npGestureSeen) return true;
			if (npPrimedMuted) return true;   // muted 起播兜底期间不许被状态校准撤销（NP-4 的既有语义）
			/* ①(NP-4) 策略挡住出声时，muted 由本状态持有（见 npSoundBlocked 的注释：不这样做，
			   "取消静音 ⇒ 被浏览器停住 ⇒ 再取消静音"会变成一个自己转的环）。 */
			return npSoundBlocked || ((s.mute !== void 0 ? !!s.mute : true) || npAudioOwns());
		}
		/** C4：用户手势/卡片显式操作 ⇒ 解除"必须 muted"（并留痕，卡片据此显示"点一下才会出声"）。 */
		function npGestureUnlock(by) {
			try {
				if (npGestureSeen) return false;
				npGestureSeen = true;
				try { window.__mpwNpSoundAllowedBy = String(by || "gesture") } catch (e) {}
				npOpTrace("gesture-unlock", String(by || ""), "unmute-allowed");
				applyVideoMute();
				try { npApplyMute() } catch (e) {}
				try { npApplyFrameMute() } catch (e) {}
				return true;
			} catch (e) { return false }
		}
		/* ══════════════════════════════════════════════════════════════════════════════════
		   C(2026-09-20 真机修复)「切页后偶尔响一下」的**唯一闸门**
		   ──────────────────────────────────────────────────────────────────────────────────
		   用户现场（**非 web 档**，容器视频 `image=host:?token=…&index=0` → `/media?token=…`）：
		     「切到别的浏览器页面/标签后，仍会出声；而且是间歇的：有时响一下、有时不响、
		       过一会儿又响一下」。
		   根因链（我们自己的**重试/补起播**链在 hidden 之后仍然会跑）：
		     · `npPrimePlay` 是唯一的起播入口，它的闸门只有 `wallUserPaused / powPaused /
		       npUserPausedOf(video)`；而 `powPaused` 只在**省电·页面隐藏**那条开关打开时才为真；
		     · 后台标签里 `setTimeout` 被节流但**仍会触发** ⇒ 任何一次 `applyFromStorage →
		       applyNowPlaying`（48ms/60ms 延迟重放、转码轮询 10s、源补挂 1.2s 复核、
		       进度轮询、`mpw:wallpaused` 事件驱动的重放…）都会走到"补起播"那一行
		       ⇒ 起播一次、被我们下一轮暂停、再起播一次 —— 听感正是"过一会儿又响一下"。
		   修法（三层一起，缺一层就还剩一半）：
		     ① `mpwHiddenAudioBlock()`：**唯一判据**（开关开 且 页面隐藏）；
		     ② 所有起播入口（`npPrimePlay`、手势重试、延迟重放里的补起播）都先过这道闸；
		     ③ 隐藏时把能控的都停住 + 静音（`pauseWallpaperVideo` 已覆盖 video/帧内/我们自己的
		        audio；帧内跨源只能发意图，见 web 帧那条通道）。
		   语义与开关：复用既有 `powPauseHidden`（默认已改 true + 只迁移"从没设过"的存量档）——
		   它显式关掉时本闸门一并失效（用户要的逃生门；见文档"开关语义"）。
		   ══════════════════════════════════════════════════════════════════════════════════ */
		function mpwHiddenPauseOn() {
			try {
				const s = readSection();
				return s.powPauseHidden !== void 0 ? !!s.powPauseHidden : DEFAULT_POW_PAUSE_HIDDEN;
			} catch (e) { return DEFAULT_POW_PAUSE_HIDDEN }
		}
		function mpwHiddenAudioBlock() {
			try {
				if (!mpwHiddenPauseOn()) return false;
				if (powHiddenNow) return true;
				return !!(typeof document !== "undefined" && document && document.hidden === true);
			} catch (e) { return false }
		}
		/* ══════════════════════════════════════════════════════════════════════════════════
		   C3(2026-09-20 真机修复)「**换档即断开旧音源**」：一条真凶级漏洞
		   ──────────────────────────────────────────────────────────────────────────────────
		   现场：用户「我已经把那个 web 壁纸切掉了，他却还能播放他的声音」；且**刷新页面后立刻**会响。
		   代码链（修前）：
		     · `npFetchTracks` 只在**目录/网页音频**这条路上有"清掉我们自己的 `<audio>`"那一步
		       （而且是在 fetch **成功**的 `.then` 里）；
		     · `npScanUrl()` 为空（容器档/无目录档）时它**直接 return**，缓存命中时也直接 return；
		     · `npDropAudio()` 只在"NP 开关被关掉"那一条路上被调用；
		     ⇒ 从一张**有曲目清单且在播**的档，切到**无清单的档**（容器视频档就是）时，
		       上一张壁纸的 `<audio>` **继续在放**（它只受 `npAudioOwns`/`npApplyMute` 的时序影响，
		       而切档那一刻没人再按它）——听感就是"切掉了还在放它的声音"。
		   修法（单一事实源 + 切换即归零）：
		     · `npSourceKind(s)`：当前音源**唯一口径**（`video | tracks | frame | none`）；
		     · `npSourceId(s)`：音源身份（类别 + 清单作用域键）；
		     · `applyNowPlaying` **入口**先算身份，与上一轮不同 ⇒ `npAudioHardReset()`
		       （`pause()` + 去掉 `src` + `load()`；不依赖任何 fetch 成功）。
		   ══════════════════════════════════════════════════════════════════════════════════ */
		/** ①(NP-5) 「用户的暂停意图」**唯一写入口**（只有用户显式操作才会走到这里）。
		 *  为什么独立成函数：这条键必须与"我们的内部暂停"严格分开 —— `npPrimedMuted`（muted 起播兜底）、
		 *  `npSoundBlocked`（浏览器策略）、hidden/省电暂停、联动对齐，**一个都不许写进来**。 */
		function npPersistPaused(v) {
			try {
				const s = readSection();
				const now = !!v;
				if ((s.npPaused !== void 0 ? !!s.npPaused : DEFAULT_NP_PAUSED) === now) return false;
				writeSection(Object.assign({}, s, { npPaused: now }), true);
				try { mpwAuditPush("np-persist-paused", npActiveVideo(s) || npAudio, { value: now }); } catch (e) {}
				try { npOpTrace("persist-paused", now ? "pause" : "play", "user-intent"); } catch (e) {}
				return true;
			} catch (e) { return false }
		}
		/** ①(NP-5) 按**持久化意图**落实可听状态（刷新/换档后第一件事；不依赖任何 fetch）。
		 *  语义按当时的联动开关翻译：开 ⇒ 整体 `pause()`（画面也停）；关 ⇒ **只静音音轨**（画面继续）。 */
		function npApplyPersistedPause(section) {
			try {
				const s = section || readSection();
				const userPaused = s.npPaused !== void 0 ? !!s.npPaused : DEFAULT_NP_PAUSED;
				npCardPaused = userPaused;
				if (!userPaused) return false;
				const link = npLinkOn(s);
				const vid = npActiveVideo(s);
				if (vid) {
					if (link) { try { npSetUserPaused(vid, true) } catch (e) {} try { vid.pause() } catch (e) {} }
					else { try { vid.muted = true } catch (e) {} }
					try { npOpTrace("apply-persisted-pause", link ? "pause" : "mute-only", "video"); } catch (e) {}
				}
				try { if (npAudio) { npAudio.pause(); npAudio.muted = true } } catch (e) {}
				try { mpwAuditPush("apply-persisted-pause", vid || npAudio, { link: link, userPaused: true }); } catch (e) {}
				return true;
			} catch (e) { return false }
		}
		function npSourceKind(s) {
			try {
				if (npActiveVideo(s)) return "video";
				if (npTrackList(s)) return "tracks";
				if (npAudioScope(s)) return "tracks";        // 清单还没到，但作用域在（下一步会去拉）
				return (s && s.webUrl) ? "frame" : "none";
			} catch (e) { return "none" }
		}
		function npSourceId(s) {
			try { return npSourceKind(s) + "|" + (npScopeKey(npAudioScope(s)) || "") } catch (e) { return "none|" }
		}
		/** 旧音源**硬归零**：不依赖 fetch、不依赖时序（切换前一律先执行）。 */
		function npAudioHardReset(why) {
			try {
				if (npAudio) {
					const wasPlaying = !npAudio.paused;
					try { npAudio.pause() } catch (e) {}
					try { npAudio.removeAttribute("src") } catch (e) {}
					try { npAudio.muted = true } catch (e) {}      // 归零期间连"取消静音"的瞬态都不许出声
					try { if (npAudio.load) npAudio.load() } catch (e) {}
					if (wasPlaying) { try { npOpTrace("audio-reset", why || "", "was-playing"); } catch (e) {} }
				}
				npCardPaused = false;
				npFramePausedByUs = false;
				try { mpwAuditPush("audio-reset", npAudio, { why: String(why || "") }); } catch (e) {}
			} catch (e) {}
		}
		/** ①(NP-4) 联动的唯一判据（②）：开 ⇒ 传输动作同时驱动壁纸媒体；关 ⇒ 只驱动我们的播放器。 */		/** ①(NP-4) 联动的唯一判据（②）：开 ⇒ 传输动作同时驱动壁纸媒体；关 ⇒ 只驱动我们的播放器。 */		/** ①(NP-4) 联动的唯一判据（②）：开 ⇒ 传输动作同时驱动壁纸媒体；关 ⇒ 只驱动我们的播放器。 */
		function npLinkOn(section) {
			const s = section || {};
			return s.npLinkWallpaper !== void 0 ? !!s.npLinkWallpaper : DEFAULT_NP_LINK_WALLPAPER;
		}
		/** ①(NP-4) 「电平被别人显式设过」的唯一判据：section 里有 `npVolume` 这个键。
		 *  为什么要有这一条（而不是"反正默认 100 等于 1，随便写"）：
		 *    · web 帧那条 shim 通道（lib/web-wallpaper.js 的 `policy.volume` → setMasterVolume）
		 *      **只要字段出现就装钩子**（包 HTMLMediaElement.prototype.volume 的 setter + 包
		 *      window.Audio）——那是作者脚本能观察到的环境变化；
		 *    · 同源兼容模式直写 `els[i].volume` 会把作者自己设的音量**覆盖成 1**。
		 *  两条都只在"用户真的动过电平"之后才允许发生 ⇒ 默认档（从没设过 npVolume）
		 *  **一个元素、一个字段都不碰**，与改动前逐字节同行为（判据见 tools/np-control-test.mjs A11/A12）。 */
		function npVolumeSet() {
			try {
				const raw = readSection().npVolume;
				return raw !== void 0 && raw !== null && isFinite(Number(raw));
			} catch (e) { return false }
		}
		/** ①(NP-4) 电平落点：视频元素 / 我们自己的播放器 / web 帧（同源直写 + shim 主音量）。
		 *  shim 那条通道是**既有**的（实机口径 = 作者页面内音量 × 宿主主音量，见 lib/web-wallpaper.js）。
		 *  `force` = 调用方明确要落（拖完音量条那一下）；否则只在用户设过电平时才写。 */
		function npApplyVolume(force) {
			const vol = npWantVolume();
			const touch = force === true || npVolumeSet();
			if (touch) {
				try { const v = bgElements().video; if (v && v.volume !== vol) v.volume = vol; } catch (e) {}
				try { if (npAudio && npAudio.volume !== vol) npAudio.volume = vol; } catch (e) {}
			}
			try {
				const frame = bgElements().frame;
				if (frame) {
					if (mpwWebShimUrl(frame.getAttribute("src"))) {
						const msg = { op: "policy", muted: npWantMuted(), speed: npPolicySpeed() };
						if (touch) msg.volume = vol;      /* 没设过 ⇒ 不带这个键（钩子不装，零环境变化） */
						webShimCall(frame, msg);
					}
					/* 兼容模式（同源）：帧内元素直接写。沙箱（不透明源）读不到 ⇒ 只有上面 shim 那一条。 */
					const doc = touch ? frame.contentDocument : null;
					if (doc) {
						const els = doc.querySelectorAll("audio,video");
						for (let i = 0; i < els.length; i++) { try { if (els[i].volume !== vol) els[i].volume = vol } catch (e) {} }
					}
				}
			} catch (e) {}
			npReportAudio();
		...
		function npPolicySpeed() {
			try {
				const r = readSection().playbackRate;
				return (typeof r === "number" && r >= 0.5 && r <= 2) ? r : 1;
			} catch (e) { return 1 }
		}
		/** ①(NP-4) 把"我们知道的音频状态"如实报到宿主 `/media-audio`（WP-1 契约）。
		 *  节流 250ms：拖动音量时每帧都 POST 就是打自己的接口打到卡。 */
		let npReportAt = 0, npReportTimer = 0;
		function npReportAudio(now) {
			const send = () => {
				npReportAt = Date.now();
				try {
					const s = readSection();
					const vid = npActiveVideo(s);
					const muted = npWantMuted();
					const body = {
						muted: muted, volume: npWantVolume(),
						hasAudio: vid ? npVideoAudio(vid) : (npAudio ? true : null),
						source: vid ? "video" : (npAudio ? "tracks" : (bgElements().frame ? "web" : null)),
					};
					fetch(HOST_BASE + "/media-audio", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) }).catch(() => {});
				} catch (e) {}
			};
			const since = Date.now() - npReportAt;
			if (now === true || since >= 250) { send(); return; }
			if (npReportTimer) return;
			try { npReportTimer = setTimeout(() => { npReportTimer = 0; send(); }, 250 - since); } catch (e) {}
		}
		/** ①(NP-4) 起播顺序的唯一入口（showVideoEl / showVideoEdge / 首帧恢复都走它）。
		 *  规则：要起播就先 muted 起播，**拿到播放权之后**再按设置应用 muted/volume。
		 *  失败 ⇒ 记 canPlay 的诚实状态 + 装一次性手势重试（首次点击/按键时自动补一次）。 */
		function npPrimePlay(video) {
			if (!video) { npOpTrace("prime", null, "no-video"); return false; }
			try { mpwAuditPush("np-prime", video, { stage: "enter" }); } catch (e) {}
			/* C(2026-09-20) **hidden 期间任何起播都拒绝**（含我们自己的重试/补起播；见 mpwHiddenAudioBlock） */
			if (wallUserPaused || powPaused || npUserPausedOf(video) || npCardPaused || mpwHiddenAudioBlock()) {
				npApplyMute(); npApplyVolume();
				npOpTrace("prime", null, "gated: wallUserPaused=" + wallUserPaused + " powPaused=" + powPaused
					+ " userPaused=" + npUserPausedOf(video) + " hiddenBlock=" + mpwHiddenAudioBlock());
				return false;
			}
			/* 已经在放 = 没有起播可偷，而且**不许在这里再校准静音**：
			   起播那一刻的元素状态就是对的（上面那条路已经保证），而"每次 apply 都按设置重写"
			   会踩到另一条既有竞态 —— `npTransport` 末尾那次 60ms 的延迟 apply 可能落在
			   "设置缓存被一次更早发出的 GET /settings 的应答覆盖回旧值"之后（真机读数：
			   localStorage=false 而 readSection()=true）⇒ 元素被按**旧值**重新静音，
			   用户看到"开关明明关了却还是没声音"。静音/电平各自有明确入口：
			   静音键（npTransport 的 mute 分支）与音量拖动（volume 分支）+ 切壁纸时的
			   applyNowPlaying。这里只起播，不改状态。 */
			if (!video.paused) { npOpTrace("prime", null, "already-playing"); return false; }
			npWantPlay = true;
			npOpTrace("prime", null, "calling-play muted=" + video.muted);
			/* **第一次就用设置里的静音状态去 play()**：有用户手势（或浏览器本身允许）时这是
			   最正确的一条 —— 元素状态从头到尾等于设置，一个字节都不偷。
			   旧写法反过来：先无条件 muted=true 再 play()，然后指望某个回调把 muted 恢复成设置值
			   ⇒ 回调不来（promise 不被 resolve / 元素被换掉 / 那一帧整页 reload）元素就**永久停在
			   muted=true**：设置里写着 mute=false，用户却什么都听不到（"开/关都听不到声音"）。
			   ⇒ 只有被拒时才退到"muted 起播"重试一次（见 npPlayRetryMuted），并且那次有兜底纠偏。 */
			let p = null;
			try { p = video.play(); } catch (e) {
				npMarkPlayBlocked({ name: String(e && e.name || "Error"), message: String(e && e.message || "").slice(0, 200), at: Date.now(), retryMuted: false });
				npArmGestureRetry();
				return false;
			}
			/* 起播成功只清"被拒"标记 + 落一次电平：**不在这里重写 muted** —— 起播用的就是设置里的
			   静音状态，重写一次只会把"设置缓存被更早的 GET 应答覆盖回旧值"这条既有竞态带进来
			   （真机读数见 npPrimePlay 里那段注释）。 */
			const done = () => { npMarkPlayBlocked(null); npApplyVolume(); npOpTrace("prime-ok", null, video.paused ? "resolved-but-paused" : "played"); };
			if (p && typeof p.then === "function") {
				p.then(done, (err) => { npPlayRetryMuted(video, err) });
				/* ①(NP-4) "promise 一直不落"也是一种结局：留一条痕，别让人对着 paused=true 猜。
				   3s 后若既没 ok 也没 reject 的记录，就记一条 pending（只记账，不改行为）。 */
				try {
					setTimeout(() => {
						try {
							const a = window.__mpwNpOps || [];
							const last = a[a.length - 1] || {};
							if (last.did === "calling-play muted=" + false || /^calling-play/.test(String(last.did || ""))) {
								npOpTrace("prime-pending", null, "3s 后 promise 仍未落（paused=" + video.paused + "）");
							}
						} catch (e) {}
					}, 3000);
				} catch (e) {}
		...
		/** 有声起播被浏览器拒 ⇒ **只重试一次** muted 起播（浏览器一律允许 muted 自动播放）。
		 *  muted 这一次是"元素状态 == 设置"的**唯一例外**，所以配两条兜底纠偏，缺一不可：
		 *   ① 媒体的 `playing` 事件（npWatchVideo 里接的）—— 真的出画/出声了就恢复设置值；
		 *   ② 1.5s 一次性兜底（`npPrimedMuted` 到期即清）—— 连 playing 都不来也不许一直静音。
		 *  两条路都会把 `npPrimedMuted` 清掉，恢复之后 `npApplyMute` 才允许把 muted 写成设置值
		 *  （否则重试那一下会被"状态校准"当场撤销，等于没重试）。 */
		function npPlayRetryMuted(video, err) {
			npOpTrace("prime-reject", null, String(err && err.name || "Error"));
			npMarkPlayBlocked({
				name: String(err && err.name || "Error"),
				message: String(err && err.message || "").slice(0, 200), at: Date.now(), retryMuted: true,
			});
			let retried = false;
			try {
				video.muted = true;
				const p2 = video.play();
				retried = true;
				/* ①(NP-4) 重试**也要留结果**：不静默的判据不能只管第一次。
				   第一次被拒时那个 promise rejection 曾被 `.catch(() => {})` 吞掉 —— 同一个坑不踩第二次。 */
				if (p2 && typeof p2.then === "function") {
					p2.then(
						() => { try { window.__mpwNpRetryPlay = "ok" } catch (e) {} },
						(e2) => { try { window.__mpwNpRetryPlay = String(e2 && e2.name || "Error") } catch (e) {} });
				} else { try { window.__mpwNpRetryPlay = "ok" } catch (e) {} }
			} catch (e) { try { window.__mpwNpRetryPlay = String(e && e.name || "Error") } catch (e2) {} }
			npOpTrace("prime-retry", null, retried ? "muted-play" : "throw");
			npPrimedMuted = retried;
			if (retried) {
				try { clearTimeout(npPrimedMutedTimer); } catch (e) {}
				try { npPrimedMutedTimer = setTimeout(() => { npPrimedMuted = false; npApplyMute(); npApplyVolume(); }, 1500); } catch (e) {}
			}
			try {
				console.warn("[dsh-mpkg-wallpaper] 壁纸视频的有声起播被浏览器拒绝（自动播放策略）: "
					+ npPlayBlocked.name + " — " + npPlayBlocked.message
					+ (retried ? "；已退回 muted 起播（成功出画后恢复静音设置）" : "")
					+ "；首次点击/按键页面时还会再补一次。读数: "
					+ JSON.stringify({ muted: video.muted, volume: video.volume, readyState: video.readyState }));
			} catch (e) {}
			npArmGestureRetry();
			return false;
		}
		/** 起播被拒 ⇒ 装**一次性**的首次手势重试（pointerdown/keydown，capture，事件驱动、自卸）。 */
		function npArmGestureRetry() {
			if (npGestureArmed) return;
			npGestureArmed = true;
			const once = () => {
				try { document.removeEventListener("pointerdown", once, true); } catch (e) {}
				try { document.removeEventListener("keydown", once, true); } catch (e) {}
				npGestureArmed = false;
				if (!npPlayBlocked && !npSoundBlocked) return;
				if (mpwHiddenAudioBlock()) { npOpTrace("gesture-retry", null, "gated:hidden"); return; }   // C: 隐藏时不补起播
				/* ①(NP-4) 用户真的交互了 ⇒ 声音限制解除：清掉"被挡住"状态，取消静音并续播。 */
				npSoundBlocked = false; npSoundReason = null;
				npGestureUnlock("gesture");   // C4：这一次交互同时解除"必须 muted"（此后按设置）
				try { window.__mpwNpSoundBlocked = null; window.__mpwNpSoundAllowedBy = "gesture"; } catch (e) {}
				try { const v = npActiveVideo(readSection()) || bgElements().video; if (v) { npApplyMute(); npApplyVolume(); npPrimePlay(v); } } catch (e) {}
			};
		...
		function applyWebSpeed(frame) {
			try {
				...
				if (frame && mpwWebShimUrl(frame.getAttribute("src"))) {
					webShimCall(frame, { op: "policy", muted: s.mute !== void 0 ? !!s.mute : true, speed: rate, volume: npWantVolume() });
				}
				const doc = frame && frame.contentDocument;
				if (!doc) return;
		...
		//
		//   ①(WP-2 2026-09-19) 本段补齐的两件事（都只改父页侧，帧内沙箱一字未动）：
		//     · 指针消息带上 DOM 口径的 `button`/`buttons`（位掩码，含右/中键）/`pointerType`/`pressure`：
		//       **修复了拖拽被当成点击** —— 旧实现的移动消息不带掩码，帧内看到"1→0 跳变"，
		//       于是拖拽的第一个 pointermove 就派发 pointerup + click（真机表现为"拖不动"）；
		//     · 新增触摸通道（`touchstart/move/end/cancel` → `op:'touch'`）：真机上作者脚本的
		//       TouchEvent 监听器（本机 8 张 web 语料里 5 张有）此前**一个都收不到**。
		//
		...
		//   所以**不要**在这里手改逻辑：改 lib/web-interaction.js，然后按测试里给出的方式重新生成这一段
		//   （两边逐字节比对，漂移即报红）。
...
(function (win) {
    ...
    function make() {
      ...
      var buttons = 0, lastX = null, lastY = null, touchSig = "";
      ...
      var api = {
        ...
        arm: function (m) {
          ...
          on = true; started = t; last = t; buttons = 0; lastX = null; lastY = null; touchSig = "";
          if (m === "full" || m === "pointer") mode = m;
          return true;
        },
        disarm: function () {
          if (!on) return false;
          on = false; started = 0; last = 0; buttons = 0; lastX = null; lastY = null; touchSig = "";
          return true;
        },
        /* 有一次真实注入（指针/滚轮/键盘/触摸）→ 续期。注意与 touchEvent 不是一回事 */
        ...
        /* 指针：buttons 是 DOM 位掩码（1 左/2 右/4 中）的**边缘**；位置没变就不发（父页按事件频率推送）。
           info 里的 button/pointerType/pointerId/isPrimary/pressure 都是**加法**：旧帧内 shim
           只读 x/y/buttons/mods（bit0=左），所以左键点击链逐字节不变。 */
        pointer: function (ev, rect, iw, ih, down, info) {
          if (!on) return null;
          var p = point(ev, rect, iw, ih);
          if (!p) return null;
          var o = info || {};
          var kind = o.kind === "button" || o.kind === "cancel" ? o.kind : "move";
          var next = kind === "cancel" ? 0 : (o.buttons === undefined || o.buttons === null ? (down ? BTN : 0) : (num(o.buttons, 0) | 0));
          if (kind === "button" && next === buttons) return null;
          if (kind === "move" && lastX !== null && lastX === p.x && lastY === p.y) return null;
          buttons = next; lastX = p.x; lastY = p.y; last = nowMs();
          var msg = { mpw: MSG, op: "pointer", x: p.x, y: p.y, inside: p.inside, buttons: next, mods: mods(ev),
            button: kind === "move" ? -1 : (isFinite(Number(o.button)) ? Number(o.button) | 0 : -1),
            phase: kind === "cancel" ? "cancel" : (kind === "move" ? "move" : ((next & BTN) ? "down" : "up")) };
          var pm = o.pointerType !== undefined ? o.pointerType : ev.pointerType;
          var pi = o.pointerId !== undefined ? o.pointerId : ev.pointerId;
          var pp = o.isPrimary !== undefined ? o.isPrimary : ev.isPrimary;
          var pf = o.pressure !== undefined ? o.pressure : ev.pressure;
          if (typeof pm === "string" && pm) msg.pointerType = pm.slice(0, 16);
          if (isFinite(Number(pi))) msg.pointerId = Number(pi) | 0;
          if (pp !== undefined) msg.isPrimary = !!pp;
          if (pf !== undefined && isFinite(Number(pf))) msg.pressure = Math.max(0, Math.min(1, Number(pf)));
          if (kind === "cancel") msg.cancel = true;
          return msg;
        },
        /* 触摸（①WP-2 新增）：touches = 屏上全部触点 / changed = 本条消息涉及的触点。
           坐标逐点换算（与 pointer 同一口径）；move 且两个列表逐字节相同就不发（静止 ≠ 在动）。 */
        touchEvent: function (ev, rect, iw, ih, phase) {
          if (!on || !ev) return null;
          var ph = (phase === "start" || phase === "move" || phase === "end" || phase === "cancel") ? phase : "";
          if (!ph) return null;
          function conv(list) {
            var out = [], n = Math.min(num(list && list.length, 0), TOUCH_LIMIT);
            for (var i = 0; i < n; i++) {
              var t = list[i], p = point(t, rect, iw, ih);
              if (!p) continue;   /* 坐标坏掉的触点丢掉，不污染整条消息 */
              out.push({ id: num(t && t.identifier, 0) | 0, x: p.x, y: p.y, inside: p.inside,
                rx: num(t && t.radiusX, 1) || 1, ry: num(t && t.radiusY, 1) || 1,
                force: Math.max(0, Math.min(1, num(t && t.force, 0.5))) });
            }
            return out;
          }
          var touches = conv(ev.touches), changed = conv(ev.changedTouches);
          if (!touches.length && !changed.length && ph !== "end" && ph !== "cancel") return null;
          /* 去重按**坐标签名**（与 phase 无关）：start 之后坐标没动的 touchmove 一律不发 */
          var sig = JSON.stringify(changed) + "|" + JSON.stringify(touches);
          var same = sig === touchSig;
          touchSig = sig;
          if (ph === "move" && same) return null;
          var primary = changed[0] || touches[0] || null;
          last = nowMs();
          return { mpw: MSG, op: "touch", phase: ph,
            x: primary ? primary.x : null, y: primary ? primary.y : null, inside: primary ? primary.inside : false,
            touches: touches, changed: changed, count: touches.length, mods: mods(ev),
            button: ph === "move" ? -1 : 0, buttons: touches.length ? BTN : 0, pointerType: "touch" };
        },
        /* 滚轮：delta 原样透传（与视口尺寸无关，不做缩放）；坐标可缺省（键盘/手势） */
        ...
        /* 键盘**只在 full 档**注入：pointer 档下作者收不到按键，也就不会误吞用户的输入 */
        ...
        /* 必须拦下的键：Ctrl/Cmd+R/W/T/N/Q/L/P、F5/F11/F12、Backspace、Tab（不拦会把宿主页面刷掉/导航走） */
        ...
        /* 1s 心跳式看护：到期自动退出（防"用户忘了退出 → 宿主界面被永久劫持"） */
...
`;
		...
		function mpwWebIxWire() {
			...
			const send = (ev, kind, arg) => {
				...
				let msg = null;
				try {
					if (kind === "move") msg = api.pointer(ev, g.rect, g.iw, g.ih, false, ixPointerInfo(ev, "move"));
					else if (kind === "down") msg = api.pointer(ev, g.rect, g.iw, g.ih, true, ixPointerInfo(ev, "button"));
					else if (kind === "up") msg = api.pointer(ev, g.rect, g.iw, g.ih, false, ixPointerInfo(ev, "button"));
					else if (kind === "cancel") msg = api.pointer(ev, g.rect, g.iw, g.ih, false, ixPointerInfo(ev, "cancel"));
					else if (kind === "wheel") msg = api.wheel(ev, g.rect, g.iw, g.ih);
					else if (kind === "key") msg = api.key(ev, !!arg);
					else if (kind === "touch") msg = api.touchEvent(ev, g.rect, g.iw, g.ih, arg);
				} catch {}
				if (!msg) return false;
				try { frame.contentWindow.postMessage(msg, "*"); } catch {}
				return true;
			};
			// ①(WP-2) 指针事件的 DOM 字段 → 协议 info。三条要点：
			//   · `buttons` 是**位掩码**（1 左 / 2 右 / 4 中），必须**每次事件都带**：移动事件里不带的话，
			//     帧内会看到"掩码从 1 跳到 0"⇒ 拖拽的第一个 pointermove 就派发 pointerup + click
			//     （历史实现正是如此：拖拽被当成点击，且被静默吞掉，没有任何报错）；
			//   · `button` 在移动/悬停类事件里必须是 -1（W3C 哨兵，见 lib/web-interaction.js 的注释）；
			//   · pointerType/pointerId/isPrimary/pressure 是给触屏/笔作者脚本的（帧内旧 shim 忽略它们）。
			const ixPointerInfo = (ev, kind) => ({
				kind: kind,
			...
			/** 把"释放一切"发给帧内（不合成 click）：失焦/取消/离开都走这里，否则作者卡在按下态。 */
			const releaseAll = () => { try { const f = mpwWebIxFrame(); if (f && api.isOn()) api.blur(f) } catch {} };
			// 指针：move/down/up/cancel 都接；up 也挂在 window（拖拽出舞台再松开不能丢，否则作者卡在按下态）
			const onMove = (ev) => { try { send(ev, "move") } catch {} };
			const onDown = (ev) => { try { send(ev, "down") } catch {} };
			const onUp = (ev) => { try { send(ev, "up") } catch {} };
			// ①(WP-2) 指针**取消**：触屏上浏览器接管手势、或系统弹窗抢走指针时会发 pointercancel；
			//   先 releaseAll（帧内抬起全部按键、不合成 click），再补一条带 cancel 标记的指针消息
			//   （帧内新实现据此能区分"取消"与"抬起"；旧 shim 已经在 0 掩码上，不会重复发事件）。
			const onCancel = (ev) => {
				try {
					if (!api.isOn()) return;
					releaseAll();
			...
			// ①(WP-2) 触摸：`touchstart/touchmove/touchend/touchcancel` → `op:'touch'` → 帧内真 TouchEvent。
			//   为什么和指针两条都要：真机上浏览器对同一根手指**同时**发 pointer* 与 touch*，
			//   两条消息各自喂各自的作者监听器（pointer 那条喂 click/drag，touch 那条喂 TouchEvent），
			//   互不重复（我们只监听 pointer* 与 touch*，从不监听 compat mouse*）。
			const onTouch = (phase) => (ev) => { try { send(ev, "touch", phase) } catch {} };
			const onLeave = () => { try { releaseAll() } catch {} };
			...
			window.addEventListener("touchstart", onTouch("start"), { capture: true, passive: true });
			window.addEventListener("touchmove", onTouch("move"), { capture: true, passive: true });
			window.addEventListener("touchend", onTouch("end"), { capture: true, passive: true });
			window.addEventListener("touchcancel", (ev) => {
				try {
					if (!api.isOn()) return;
					send(ev, "touch", "cancel");
					releaseAll();
				} catch {}
		...
		function webShimArm(frame) {
			try {
				...
				frame.__mpwShimCaps = null;
				frame.__mpwSandboxFallback = null;
		...
		/** ③(2026-09-20) 该错误是不是"沙箱策略挡住了作者脚本"（而不是作者自己的 bug）。
		 *  只认可归因到**不透明源/沙箱**的信号；普通 TypeError 之类绝不触发降级。 */
		function mpwWebLooksPolicyBlocked(d) {
			try {
				const msg = String((d && d.message) || "") + " " + String((d && d.kind) || "");
				return /SecurityError|sandbox|opaque|not allowed|denied|Failed to construct 'Worker'|Blocked|insecure|Operation is insecure/i.test(msg);
			} catch { return false }
		}
		/** ③(2026-09-20) 沙箱档 → 兼容档的**一次性**降级（与 webShimArm 的"shim 没报到"兜底同一手法：
		 *  去掉 `mpwshim=1` 标记 + 换成兼容属性集 + 重载；只做一次，绝不来回切）。
		 *  可判定状态：`frame.__mpwShimFellBack`（既有）+ `frame.__mpwSandboxFallback`（新增，含原因）
		 *  + `window.__mpwWebSandboxFallback` + `/diag` 的 `sandbox-fallback` 信标 + 面板提示。 */
		function mpwWebSandboxFallback(why, detail) {
			try {
				const frame = bgElements().frame;
				if (!frame) return false;
				if (frame.__mpwSandboxFallback) return false;              // 只一次
				const u = String(frame.getAttribute("src") || "");
				if (!mpwWebShimUrl(u)) return false;                       // 只在沙箱档上降级
				frame.__mpwSandboxFallback = { why: String(why || ""), detail: String(detail || "").slice(0, 300), at: Date.now() };
				frame.__mpwShimFellBack = true;
				if (frame.__mpwShimTimer) { clearTimeout(frame.__mpwShimTimer); frame.__mpwShimTimer = 0; }
				try { window.__mpwWebSandboxFallback = frame.__mpwSandboxFallback } catch (e) {}
				const bare = u.replace(/([?&])mpwshim=1(?:&|$)/, "$1").replace(/([?&])mpwmute=[^&]*/g, "$1").replace(/([?&])mpwspeed=[^&]*/g, "$1").replace(/([?&])mpwpause=[^&]*/g, "$1").replace(/[?&]$/, "");
				mpwWebDiag("sandbox-fallback", { why: String(why || ""), detail: String(detail || "").slice(0, 200), url: bare.slice(0, 200), caps: frame.__mpwShimCaps || null });
				try { console.warn("[dsh-mpkg-wallpaper] 沙箱模式被浏览器策略挡住（" + why + "：" + String(detail || "").slice(0, 120) + "）→ 自动切到兼容模式重载一次"); } catch (e) {}
				try { mpwPersistEmit("这张网页壁纸在**沙箱模式**下被浏览器策略挡住（" + why + "）→ 已自动切到**兼容模式**重载。" + "沙箱模式隔离更强，但这张壁纸依赖沙箱里不可用的浏览器能力（如帧内 Worker）。"); } catch (e) {}
				try { frame.setAttribute("sandbox", MPW_WEB_COMPAT_ATTR); } catch (e) {}
				if (!bare || bare === u) return false;
				try { frame.src = bare; } catch (e) {}
				return true;
			} catch (e) { return false }
		}
		...
		function webShimPushPolicy(frame) {
			try {
				const s = readSection();
				webShimCall(frame, { op: "policy", muted: s.mute !== void 0 ? !!s.mute : true, speed: npPolicySpeed(), volume: npWantVolume() });
					...
					try {
						...
						if (d.op === "ready" || d.op === "pong") {
							if (frame) {
								...
								if (frame.__mpwShimTimer) { clearTimeout(frame.__mpwShimTimer); frame.__mpwShimTimer = 0; }
								// ③(2026-09-20) 帧内能力自证（不透明源下父页读不到帧内 DOM ⇒ 只能由 shim 上报）
								if (d.caps && typeof d.caps === "object") {
									frame.__mpwShimCaps = d.caps;
									try { window.__mpwShimCaps = d.caps } catch (e) {}
								}
							}
							if (d.op === "ready") mpwWebDiag("shim-ready", { v: Number(d.v) || 0, href: String(d.href || "").slice(0, 200), caps: d.caps || null });
							return;
						}
						if (d.op === "error") {
							const info = mpwWebSrcFromUrl(frame && frame.getAttribute("src"));
							try { console.warn("[dsh-mpkg-wallpaper] 网页壁纸脚本错误（已兜住）:", String(d.kind || "error"), String(d.message || ""), String(d.stack || "").split("\n")[1] || ""); } catch {}
							mpwWebDiag("script-error", { kind: String(d.kind || "").slice(0, 40), message: String(d.message || "").slice(0, 300), key: info && info.key, caps: (frame && frame.__mpwShimCaps) || null });
							/* ③(2026-09-20 真机修复) **沙箱档被浏览器策略挡住 ⇒ 一次性降级到兼容档（同源）**。
							   为什么需要：作者脚本（Live2D/Pixi 类）常用 blob-URL Worker / OffscreenCanvas，
							   而沙箱帧是不透明源 ⇒ 这些调用直接抛 SecurityError，页面**白屏/黑底**，
							   父页却因为"shim 已握手成功"而毫不知情（现有兜底只管"shim 没报到"）。
							   判据只认**策略类**错误（SecurityError / sandbox / Worker / opaque / denied …），
							   普通作者脚本 bug 不触发降级（沙箱的隔离价值不能被一次无关报错换掉）。 */
							if (mpwWebLooksPolicyBlocked(d)) {
								mpwWebSandboxFallback("script-" + String(d.kind || "error"), String(d.message || ""));
							}
							return;
						}
					} catch {}
		...
		function applyFromStorageInner() {
			...
			try {
				...
				if (looksReset && bak) {
					...
					if (parsed && typeof parsed === 'object' && (parsed.sidebarAlpha !== void 0 || parsed.unifyTint !== void 0 || parsed.float !== void 0)) {
						writeSection(Object.assign({}, parsed, { enabled: cur.enabled !== false },
							// ①(2026-09-18) 备份里**显式**带了 bsCompat ⇒ 那是用户的选择，打标记别让迁移覆盖它
							(parsed.bsCompat !== void 0 ? { bsCompatUserSet: true } : {})), true);
						console.warn('[dsh-mpkg-wallpaper] 检测到设置被误重置 → 已从 mpw_settings_backup 还原');
					}
				}
			} catch {}
			/* ①(2026-09-19 设置持久化轮) **半残档自愈**：`mpkgKey` 在、`image`/`webUrl` 都缺 ⇒
			   先试着把源推回来（能推就在**渲染之前**推，本帧就不是 display:none），推不出则明确提示。
			   放在这里（而不是只放 boot 定时器里）的原因：CSS 生成就在下面几行，晚一步就多一帧隐藏。 */
			try { if (mpwHealHalfArchive()) { try { mpwTrace("settings:heal-before-render", { key: readSection().mpkgKey }) } catch {} } } catch (e) { mpwErr("applyFromStorageInner(自愈)", e) }
			const sectionRaw2 = normalizeSection(readSection());
			const enabled = sectionRaw2.enabled !== void 0 ? !!sectionRaw2.enabled : DEFAULT_ENABLED;
			// ④ 总开关关闭 → 视为无背景（恢复默认外观）
			const section = enabled ? sectionRaw2 : Object.assign({}, sectionRaw2, { image: "" });
			const { img, video, wrap } = bgElements();
			if (!img || !video || !wrap) return;
			const image = section.image || "";
			// ②(2026-09-20 真机修复) 挂载分支由**判据函数**决定，不再"有 webUrl 就走 web"：
			//   真机现场（converted=mp4 + image 缺 + webUrl 是上一个 web 档的残留）旧写法先挂 web 帧
			//   → /raw 404 → 场景看门狗打 mpw-scene-fallback → 回退也拿不到首帧 ⇒ 黑屏 + 破图图标。
			const mountPlan = mpwPickMount(section);
			// ②(2026-09-20) 每次 apply 先清"源不可用"状态（本轮挂载成功就保持干净；失败时下面再置位）
			try { mpwBgArmErrorClear(); } catch (e) {}
			if (mountPlan.mount !== "web" && section.webUrl) {
				try { mpwTrace("mount:ignore-stale-webUrl", { reason: mountPlan.reason, cls: String(section.converted || "") }); } catch {}
				try { mpwWebDiag("stale-webUrl", { reason: mountPlan.reason, converted: String(section.converted || ""), hasImage: !!image, key: String(section.mpkgKey || "").slice(0, 120) }); } catch {}
			}
			// ①(新) 网页壁纸：webUrl 优先（iframe 全屏），与 image 互斥。
			// ①(修正) 在 return 前刷新 buildCss —— web 壁纸 image 为 ""，buildCss 需按 webUrl
			// 生成可见规则（否则 .mpw-bgWrap{display:none} 把 iframe 整个藏掉）。
			if (section.webUrl && mountPlan.mount === "web") {
				if (enabled) {
					let armUrl = "";
					try { armUrl = String(resolveHostUrl(section.webUrl) || ""); } catch (e) { armUrl = "" }
					try { showWebEl(armUrl || section.webUrl); } catch (e) { mpwErr("showWebEl(挂载 web 壁纸)", e); }
					/* ②(2026-09-20) 挂载后验活：宿主明确回 `{ok:false}`（真机那条 404）⇒ 卸掉 iframe
					   （否则 404 的 JSON body 会一直显示成"壁纸"）+ 记状态 + 面板提示 + 层隐藏。 */
					try {
						mpwVerifyArm("web", armUrl, (v) => {
							try { teardownWebFrame(); } catch (e) {}
							mpwBgArmErrorSet("web", v.url, v.status, v.error || "host returned ok:false");
						});
					} catch (e) {}
				} else {
			...
			try { applyFrostInline(section); startFrostObserver(); } catch {}
				...
				/* ①(NP-3 2026-09-19 真机) **web 壁纸这条路径提前 return，把 NP 那一次 apply 整段跳过**
				   （下面共享尾里的 applyNowPlaying 只在非 web 路径跑得到）⇒ 用户"用 web 类壁纸"
				   时的两个现场症状直接由此而来：
				     · 刷新/切到 web 壁纸之后控件**根本不挂**（本探针 E2/H1 就是这条，np=0）；
				     · 控件即使挂着，状态也永远不按当前壁纸刷新（播放/静音落在旧源上）。
				   共享刷新尾里已有的每一项都在这里补过（时钟/Aqua/玻璃/省电/rail），NP 也补上。 */
				try { applyNowPlaying(section); } catch (e) { mpwErr("applyNowPlaying(web)", e); }
				/* ①(NP-3) 同一类漏项：悬浮效果的 **body 门控属性**也在下面那条共享尾里
				   （`data-mpw-float`，见"悬浮效果：控制 body 属性门控"）⇒ web 壁纸期间开了悬浮
				   也不生效（buildCss 里的悬浮规则全靠这个属性开门）。这里一并补上。 */
				try {
			...
			// ②(2026-09-20 真机修复) 走"非 web"分支时，若档案里还留着 webUrl（历史残留），
			//   必须**把 web 帧真的卸掉**：否则旧 iframe 继续加载（/raw 404 刷屏）、
			//   还会在场景看门狗回退时抢到 `mpw-scene-fallback` 那张必坏的静态帧。
			if (section.webUrl) {
				try { teardownWebFrame(); } catch (e) { mpwErr("teardownWebFrame(stale-webUrl)", e); }
			}
			if (section.converted === "scene" && section.sceneKey) {
				if (enabled && sceneComposite && sceneComposite.key === section.sceneKey) {
					/* ①(NP-3) 这条 return 同样会跳过 NP 那一次 apply ⇒ 先补上再走（同 web 分支的理由）。 */
					let npShown = false;
					try { showSceneEl(sceneComposite); npShown = true; } catch {}
					if (npShown) {
						try { applyNowPlaying(section); } catch (e) { mpwErr("applyNowPlaying(scene)", e); }
						return;
					}
				}
			...
			if (typeof image === "string" && image.indexOf("host:") === 0) {
				...
				/* ②(2026-09-20 真机复测) **无源 = 把媒体元素也真的卸掉**。
				   旧写法只在 `img.src !== image` 时走清理（`showImageEl()` 里面才 `video.removeAttribute("src")`）；
				   真机上"本来就没有 img.src、image 也是空串"⇒ 两边相等 ⇒ 走 else 只打一行日志 ⇒
				   **`<video>` 的旧 src 留着**：层虽然隐藏了，但解码器/网络请求还在（用户点"清除壁纸"
				   之后后台仍在拉那个 mp4）。判据：清空后 `video.src` 必须为空。 */
				if (!image) {
					try { img.removeAttribute("src") } catch {}
					try { video.pause() } catch {}
					try { video.removeAttribute("src"); if (video.load) video.load() } catch {}
					console.log("[dsh-mpkg-wallpaper] 背景清空: 卸载 img/video 源");
					showImageEl();
			...
			// ①(NP-1) Now playing（开关 npNowPlaying，默认关）：开则挂到左侧栏「设置」入口上方，
			//   关则纯拆除（节点 + 观察者 + 渲染根全清）。关的时候这一行只是 setEnabled(false)，不建任何东西。
			try { applyNowPlaying(section); } catch (e) { mpwErr("applyNowPlaying", e); }
		}
		...
		/* ═══ MPW-DIAG-SUBSYS-BEGIN ═══
		   ①(2026-09-18 MASTER-TODO §5 第 3 项 / P0-3 sibling)「诊断自证闭环」
		   ───────────────────────────────────────────────────────────────────────
		   现状问题：诊断 payload 里各子系统状态是"随手塞进去的字段"——有的在 `reveal`、有的在
		   `rail`、有的在 `sceneHealth`，**读不到就整段消失**（`catch { out.x = {err} }` 或干脆没有），
		   拿到一份 diag 也说不清"这个字段是从哪个 DOM 节点/哪个状态读的、是没读到还是真没有"。
		   本块把它变成**声明式字段表** + 三条硬约束：
		     ① 每个字段都带 `provenance`（来源：哪个 DOM 节点 / 哪个内部状态）；
		     ② 字段**一定出现在 payload 里**：读不到 ⇒ `value:null` + `degraded:{reason,...}`；
		        部分子来源读不到 ⇒ `degraded:{reason:"partial", partial:[…]}`。绝不静默省略、绝不静默残缺；
		     ③ 字段表可被测试枚举（`__mpwDiagTest.spec()`）⇒ "字段被删/被改名"是断言而不是肉眼。
		   字段清单/来源/怎么发/落在哪/上限：docs/DIAGNOSTICS.md。 */
		const MPW_DIAG_SCHEMA = "mpw-subsystems/1";
		/* 一次 read() 期间收集"读不到的子来源"（soft() 里 push）⇒ 字段级 degraded 能区分
		   "整个字段读不到"（value:null）与"字段读到了但有几处子来源读不到"（partial）。 */
		let __mpwDiagIssues = [];
		/** 软读取：抛错/undefined 都变成 issues 里的一条 + 默认值（**不冒泡、不静默**）。 */
		function soft(label, fn, dflt) {
			try {
				const v = fn();
				if (v === void 0) { __mpwDiagIssues.push(label + ":undefined"); return dflt; }
				return v;
			} catch (e) { __mpwDiagIssues.push(label + ":" + String((e && e.message) || e).slice(0, 80)); return dflt; }
		}
		/** **主来源**读取：失败必须让字段变 value:null + degraded ——
		 *  否则会出现"读不到"被伪装成"没有这个子系统"（例如元素查不到 ⇒ {present:false}）这种**静默错误的字段**。 */
		function strict(label, fn) {
			try { return fn(); }
			catch (e) { const err = new Error(label + ":" + String((e && e.message) || e).slice(0, 120)); err.__mpwDiagPrimary = true; throw err; }
		}
		/** 单个字段的取值包装：异常/空值都变成 degraded，字段本身永不缺席。 */
		function mpwDiagFieldValue(f) {
			const base = { provenance: f.provenance };
			__mpwDiagIssues = [];
			try {
				const v = f.read();
				const issues = __mpwDiagIssues.slice(0, 8);
				if (v === void 0 || v === null) {
					return Object.assign({ value: null, degraded: { reason: "unreadable:empty", partial: issues } }, base);
				}
				return Object.assign({ value: v, degraded: issues.length ? { reason: "partial", partial: issues } : null }, base);
			} catch (e) {
				return Object.assign({ value: null, degraded: { reason: String((e && e.message) || e).slice(0, 200), kind: "throw", partial: __mpwDiagIssues.slice(0, 8) } }, base);
			}
		}
		const mpwHasDom = () => !!(typeof document !== "undefined" && document && document.body);
		const mpwCs = (el, pseudo) => (el ? soft("getComputedStyle", () => getComputedStyle(el, pseudo), null) : null);
		const mpwTrim = (v, n) => String(v === void 0 || v === null ? "" : v).slice(0, n || 200);
		const mpwProp = (cs, name) => (cs ? soft("getPropertyValue(" + name + ")", () => cs.getPropertyValue(name), "") : null);
		/** 壁纸类型/路径（**用户的第一个问题永远是"你在放哪个文件"**） */
		function mpwDiagWallpaper() {
			const sec = strict("readSection()", () => (typeof readSection === "function" ? readSection() : null)) || {};
			const el = soft("bgElements()", () => bgElements(), {}) || {};
			const inner = sec.source || sec.entryName || "";
			const fromName = (() => {
				const m = /\.([a-z0-9]+)(?:$|[?#])/i.exec(String(inner || sec.image || ""));
				return m ? m[1].toLowerCase() : "";
			})();
			const type = sec.converted === "mp4" ? "video"
				: sec.converted === "web" ? "web"
				: sec.converted === "scene" ? "scene"
				: sec.converted === "gif" ? "gif"
				: (fromName === "mpkg" || fromName === "pkg" ? "scene" : (fromName || (sec.image ? "image" : "none")));
			const cur = el.img ? (el.img.currentSrc || el.img.src) : (el.video ? (el.video.currentSrc || (el.video.getAttribute && el.video.getAttribute("src"))) : null);
			return {
				type,
				hasSource: !!(sec.image || sec.webUrl || sec.video),
				converted: mpwTrim(sec.converted, 16),
				mpkgName: mpwTrim(sec.mpkgName, 120),
				innerFile: mpwTrim(inner, 160),
				// 路径可能很长（custom 目录/创意工坊目录），只留尾部 160 字符（定位够用，又不撑爆 payload）
				imagePath: mpwTrim(sec.image, 160),
				webUrl: mpwTrim(sec.webUrl, 200),
				sceneKeyTail: mpwTrim(sec.sceneKey, 60),
				dom: {
					wrap: el.wrap ? mpwTrim(el.wrap.className, 80) : null,
					img: !!el.img, video: !!el.video, frame: !!el.frame, canvas: !!el.canvas,
					mediaCurrentSrc: mpwTrim(cur, 160),
				},
			};
		}
		/** 网页壁纸 shim 是否注入（帧内 shim 不报到 ⇒ 作者 API 全失效，历史表现"网页壁纸半坏"） */
		function mpwDiagShim() {
			const el = strict("bgElements()", () => bgElements()) || {};
			const f = el.frame;
			const src = f ? mpwTrim(f.getAttribute && f.getAttribute("src"), 400) : "";
			return {
				isWebWallpaper: !!f,
				// shim 是否**被要求**注入（src 上带 mpwshim=1）
				shimUrl: /(^|[?&])mpwshim=1(&|$)/.test(src),
				shimOk: f ? !!f.__mpwShimOk : null,
				shimFellBack: f ? !!f.__mpwShimFellBack : null,
				// ③(2026-09-20) 沙箱档降级与帧内能力自证（真机排障第一现场）
				sandboxFallback: f && f.__mpwSandboxFallback ? f.__mpwSandboxFallback : null,
				shimCaps: f && f.__mpwShimCaps ? f.__mpwShimCaps : null,
				sandbox: f ? mpwTrim(f.getAttribute && f.getAttribute("sandbox"), 160) : null,
				frameSrcTail: mpwTrim(src, 160),
				// 帧内是否报到过（沙箱下父页读不到帧内 DOM，只能看宿主侧 token/握手状态）
				sandboxDiag: soft("mpwSandboxDiag()", () => mpwSandboxDiag(), null),
			};
		}
		/** 视频解码状态（用户报"黑屏/没声音/不循环"时第一眼要看的） */
		function mpwDiagVideo() {
			const el = strict("bgElements()", () => bgElements()) || {};
			const v = el.video;
			if (!v) return { present: false };
			const st = soft("window.__mpwWallpaperState", () => window.__mpwWallpaperState || null, null);
			return {
				present: true,
				readyState: v.readyState, networkState: v.networkState,
				paused: !!v.paused, ended: !!v.ended, muted: !!v.muted, loop: !!v.loop,
				playbackRate: v.playbackRate, currentTime: Number(v.currentTime || 0).toFixed(2),
				...
				// 0×0 + readyState<2 通常就是"解码没起来"（编码不支持/转码没完成）
				decoding: (v.videoWidth || 0) > 0 ? "ok" : (v.readyState >= 2 ? "metadata-only" : "not-decoded"),
				error: soft("video.error", () => (v.error ? { code: v.error.code, message: mpwTrim(v.error.message, 120) } : null), null),
				// 宿主侧三态（direct/cached/transcode，见 mpwWallpaperStateSet）+ <video> 上的标记
				hostState: { state: st ? (st.state || null) : null, detail: mpwTrim(st && st.detail, 120), attr: mpwTrim(v.getAttribute && v.getAttribute("data-mpw-wp-state"), 24) },
			};
		}
		/** §5 第 1 项的表面 token 是否真的解析出值（磨砂/主题一致性出问题一眼看出是哪一层断的） */
		function mpwDiagSurfaceTokens() {
			const names = ["--mpw-surface-frost-top", "--mpw-surface-side", "--mpw-surface-panel", "--mpw-surface-panel-frost", "--mpw-rail-ink", "--mpw-rail-halo", "--mpw-hdr-blur"];
			const csBody = mpwHasDom() ? soft("getComputedStyle(body)", () => getComputedStyle(document.body), null) : null;
			const csRoot = csBody ? null : strict("getComputedStyle(documentElement)", () => getComputedStyle(document.documentElement));
			const out = {};
			for (const n of names) {
				// 空字符串 = 该 token 在当前档位没定义（**不是**读失败）⇒ 如实记为空串
				const v = csBody ? mpwProp(csBody, n) : (csRoot ? mpwProp(csRoot, n) : null);
				out[n] = v === null ? null : mpwTrim(v, 80);
			}
			return out;
		}
		/** 诊断字段表：id → {provenance, read}。加字段只加在这里 + docs/DIAGNOSTICS.md 的表。 */
		const MPW_DIAG_FIELDS = [
			{ id: "frost", provenance: "mpwHeaderFrostDiag()（JS 侧 hdrFrostState）+ .wSkVaW_header / .mpw-hdrFrost 的 computed backdrop-filter/backgroundColor", read: () => { const f = strict("mpwHeaderFrostDiag()", () => mpwHeaderFrostDiag()) || {}; return { injected: !!f.injected, px: f.px, wanted: f.wanted, reason: mpwTrim(f.reason, 200), hostHasHeader: f.hostHasHeader, headerTranslucent: f.headerTranslucent, computed: f.computed || null, state: f.state ? { reason: mpwTrim(f.state.reason, 120), watch: f.state.watch || null } : null } } },
			{ id: "sidebarAffected", provenance: "[class*=\"sidebarCol\"] / .hHd-Xa_root / [data-slot=\"sidebar\"] 的 computed backgroundColor+backdropFilter + body 上 data-mpw-sblur-off / data-mpw-rsblur / data-mpw-bs-version 门控 + --dsw-specific-sidebar-fill 解析值", read: () => {
				const el = mpwHasDom() ? strict('querySelector(sidebar)', () => document.querySelector('[class*="sidebarCol"], .hHd-Xa_root, [data-slot="sidebar"]')) : null;
				const cs = mpwCs(el);
				const body = mpwHasDom() ? document.body : null;
				const csBody = mpwCs(body);
				return {
					found: !!el,
					className: el ? mpwTrim(el.className, 100) : null,
					background: cs ? mpwTrim(cs.backgroundColor, 60) : null,
					backdropFilter: cs ? (cs.backdropFilter === "none" ? "none" : "set") : null,
					ourGates: body ? {
						sblurOff: soft("body.hasAttribute", () => body.hasAttribute("data-mpw-sblur-off"), null),
						rsblur: mpwTrim(soft("body.getAttribute(rsblur)", () => body.getAttribute("data-mpw-rsblur"), ""), 8),
						bsVersion: mpwTrim(soft("body.getAttribute(bs-version)", () => body.getAttribute("data-mpw-bs-version"), ""), 16),
					} : null,
					// 我们唯一覆盖的宿主 token 现在解析成什么（覆盖是否生效、有没有被别的东西改回去）
					sidebarFillToken: mpwProp(csBody, "--dsw-specific-sidebar-fill"),
				};
			} },
			{ id: "railAffected", provenance: ".eGxaPq_mark::before computed background/boxShadow/rect + body[data-mpw-rail-ink] 门控 + html 上 --mpw-rail-* 内联值", read: () => {
				const rail = mpwHasDom() ? strict("querySelector(rail)", () => document.querySelector('.eGxaPq_mark, [class*="eGxaPq_mark"]')) : null;
				if (!rail) return { found: false, ours: false };
				const cs = mpwCs(rail, "::before");
				const root = mpwCs(mpwHasDom() ? document.documentElement : null);
				const body = mpwCs(mpwHasDom() ? document.body : null);
				const b = soft("rail.getBoundingClientRect", () => { const r = rail.getBoundingClientRect(); return [Math.round(r.x), Math.round(r.y), Math.round(r.width), Math.round(r.height)] }, null);
				return {
					found: true,
					ours: soft("body.hasAttribute(rail-ink)", () => !!(document.body && document.body.hasAttribute("data-mpw-rail-ink")), null),
					markBeforeBackground: cs ? mpwTrim(cs.backgroundColor, 40) : null,
					markBoxShadow: cs ? mpwTrim(cs.boxShadow, 60) : null,
					rect: b,
					// 可见性判据（"条看不见"最常见的两个原因：宽高为 0 / 被祖先裁掉）
					zeroSize: !!(b && (b[2] === 0 || b[3] === 0)),
					clipAncestors: soft("rail 祖先 overflow 扫描", () => { const res = []; for (let n = rail.parentElement, i = 0; n && i < 4; n = n.parentElement, i++) { const c = getComputedStyle(n); if (c && c.overflow !== "visible") res.push(mpwTrim(c.overflow + "@" + (n.className || n.tagName), 40)) } return res }, []),
					tokens: { mpwRailInk: mpwProp(root, "--mpw-rail-ink"), mpwRailHalo: mpwProp(root, "--mpw-rail-halo"), dswBorderL4: mpwProp(body, "--dsw-alias-border-l4") },
				};
			} },
			{ id: "wallpaper", provenance: "readSection()（持久化字段 converted/source/mpkgName/image/webUrl/sceneKey）+ #mpw-bgWrap / #mpw-bgImg / #mpw-bgVideo / iframe.mpw-webFrame 实际 DOM", read: mpwDiagWallpaper },
			{ id: "shim", provenance: "#mpw-bgWrap iframe.mpw-webFrame 的 src（含 mpwshim=1?）+ sandbox 属性 + frame.__mpwShimOk / __mpwShimFellBack（webShimArm 维护）+ mpwSandboxDiag()", read: mpwDiagShim },
			{ id: "video", provenance: "#mpw-bgVideo 的 readyState / networkState / error / videoWidth + data-mpw-wp-state 属性 + window.__mpwWallpaperState（转码三态）", read: mpwDiagVideo },
			{ id: "surfaceTokens", provenance: "getComputedStyle(document.body) 上 --mpw-surface-* / --mpw-rail-* / --mpw-hdr-blur 的解析值（§5 第 1 项 SSOT）", read: mpwDiagSurfaceTokens },
			{ id: "sceneHealth", provenance: "mpwSceneWdState() 看门狗 + window.__mpwSceneHealth（渲染器 mpw-health 最近消息）+ 信标可用性", read: () => { const wd = strict("mpwSceneWdState()", () => mpwSceneWdState()); return { wd: wd, health: soft("__mpwSceneHealth", () => __mpwSceneHealth, null), beaconOk: soft("__mpwSceneBeaconOk", () => __mpwSceneBeaconOk, null), lowmem: soft("mpwSceneLowMem()", () => mpwSceneLowMem(), null) } } },
		];
		/** 组装 subsystems（字段一定在；读不到 = value:null + degraded；部分读不到 = degraded.partial）。 */
		function mpwDiagSubsystems() {
			const fields = {};
			for (const f of MPW_DIAG_FIELDS) fields[f.id] = mpwDiagFieldValue(f);
			return { schema: MPW_DIAG_SCHEMA, fieldCount: MPW_DIAG_FIELDS.length, fields: fields };
		}
		/** 单份 payload 的**字节上限**（客户端侧）：超了就按"先砍可选段、再截断超长字符串"的顺序瘦身，
		 *  并写 `truncated:{reason,capBytes,…}` —— 绝不静默丢字段（见 docs/DIAGNOSTICS.md「上限」）。
		 *  宿主侧另有"≤50 个 / ≤32MB 最旧先删"（lib/index.js 的 pruneDiagDir）。 */
		const MPW_DIAG_MAX_BYTES = 512 * 1024;
		const MPW_DIAG_OPTIONAL = ["groups", "thumbSources"];
		function mpwDiagFit(data, maxBytes) {
			const cap = Number(maxBytes) > 0 ? Number(maxBytes) : MPW_DIAG_MAX_BYTES;
			const size = (o) => { try { return JSON.stringify(o).length } catch { return -1 } };
			let bytes = size(data);
			if (bytes < 0) return { data: { at: new Date().toISOString(), fitError: "circular" }, bytes: 0, truncated: { reason: "unserializable" } };
			let truncated = null;
			for (const k of MPW_DIAG_OPTIONAL) {
				if (bytes <= cap) break;
				if (!(k in data)) continue;
				const before = bytes;
				try { delete data[k] } catch { try { data[k] = null } catch {} }
				bytes = size(data);
				truncated = Object.assign(truncated || {}, { reason: "payload-over-cap", capBytes: cap, removed: (truncated && truncated.removed ? truncated.removed + "," : "") + k, savedBytes: before - bytes });
			}
			if (bytes > cap) {
				// 还超 ⇒ 截断超长字符串字段（**保结构、保字段名**，值显式标注被截断）
				for (const k of Object.keys(data)) {
					if (bytes <= cap) break;
					if (typeof data[k] !== "string" || data[k].length < 2000) continue;
					data[k] = data[k].slice(0, 2000) + "…[truncated]";
					bytes = size(data);
					truncated = Object.assign(truncated || {}, { reason: "payload-over-cap", capBytes: cap, truncatedStringField: k });
				}
			}
			if (truncated) data.truncated = truncated;
			return { data: data, bytes: size(data), truncated: truncated };
		}
		/* ═══ MPW-DIAG-SUBSYS-END ═══ */
		function mpwDiagCollect() {
			...
			// ①(2026-09-18 §5 第3项) 子系统自证段：字段**一定在**，读不到就是 value:null + degraded
			//   （不静默省略 —— 这是"一轮定位"的前提：看到 null 就知道是"读不到"而不是"没问题"）。
			try { out.subsystems = mpwDiagSubsystems(); }
			catch (e) { out.subsystems = { schema: MPW_DIAG_SCHEMA, fieldCount: 0, fields: {}, collectError: String((e && e.message) || e) }; }
			return out;
		}
		/** 一键诊断上报（用户点一次）：采集 → 单份字节上限瘦身 → POST /diag。
		 *  返回 `{ok:true,file,bytes,truncated}` 或 `{ok:false,error,bytes,truncated,data}`（data 给离线兜底用）。 */
		async function mpwDiagSendOnce(why) {
			const data = mpwDiagCollect();
			try { data.why = why || "manual" } catch {}
			const fit = mpwDiagFit(data, MPW_DIAG_MAX_BYTES);
			try {
				const r = await fetch(HOST_BASE + "/diag", { method: "POST", body: JSON.stringify(fit.data) });
				const j = await r.json().catch(() => null);
				if (r.ok && j && j.ok) return { ok: true, file: mpwTrim(j.file, 400), bytes: fit.bytes, truncated: fit.truncated };
				return { ok: false, error: "host-" + r.status + (j && j.error ? ":" + mpwTrim(j.error, 160) : ""), bytes: fit.bytes, truncated: fit.truncated, data: fit.data };
			} catch (e) {
				return { ok: false, error: String((e && e.message) || e), bytes: fit.bytes, truncated: fit.truncated, data: fit.data };
			}
		}
		/** 离线兜底：把同一份 payload 下载成 .json 文件（宿主不可用/无网络时用户也能把状态发回来）。 */
		function mpwDiagDownload(data) {
			try {
				if (typeof Blob !== "function" || !URL || typeof URL.createObjectURL !== "function") return { ok: false, error: "blob-unavailable" };
				const name = "mpw-diag-" + new Date().toISOString().replace(/[:.]/g, "-") + ".json";
				...
				return { ok: true, name: name, bytes: blob.size };
			} catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
		}
		/** 设置页「一键诊断上报」按钮的唯一入口：先 POST；宿主不可用就**下载 JSON**（离线可用）。 */
		async function mpwDiagManual() {
			const r = await mpwDiagSendOnce("manual");
			if (r.ok) return r;
			const d = mpwDiagDownload(r.data);
			return { ok: false, sent: false, error: r.error, bytes: r.bytes, truncated: r.truncated, download: d };
		}
		...
		/* ══════════════════════════════════════════════════════════════════════════════════
		   ⑥(2026-09-20 真机修复)「设置面板被压缩进左侧栏」的**从结构上不可能**版本
		   ──────────────────────────────────────────────────────────────────────────────────
		   真机读数（:3080，1440×900，本插件全部按用户档开启）：
		     · `VOzbGW_overlay`(position:fixed, 1440×900) 的 **祖先链里有 `pI_x6G_sidebarCol`**
		       （overlay 是侧栏 `hHd-Xa_settingsArea` 的后代，靠 fixed 逃出侧栏）：
		       `VOzbGW_overlay → … → hHd-Xa_footArea → … → pI_x6G_sidebarCol`
		     · 基线（守卫在）：`panel = {x:320,w:800}`、`sidebarCol.backdropFilter = none`
		     · **摘掉 body 的 `data-mpw-sblur-off`** ⇒ `sidebarCol.backdropFilter = blur(30px) saturate(1.4)`
		       ⇒ `panel = {x:13,w:254}` —— 面板被 flex 压缩进左栏（侧栏本身 x=12..268）。
		       （同一条读数在源码注释里早有记录："panel 宽 254 而非 800"，见 §H 块注释。）
		   根因：`backdrop-filter` 让元素成为 fixed 后代的 **containing block**；我们给
		   `[class*="sidebarCol"]` 的 backdrop-filter 规则（!important）落在 overlay 的祖先上。
		   唯一防线是 `data-mpw-sblur-off` 这个 body 属性，而它由 MutationObserver + 可见性判定驱动
		   （动画/重渲染期间 overlay 高度或 opacity 未达阈值 ⇒ 属性被摘 ⇒ 模糊瞬间回来 ⇒ 面板被压扁），
		   且 `:has()` 兜底在 Firefox 上对 backdrop-filter 的层叠不可靠（源码 §E 注释自己写了）。
		   本块的修法（三层，缺一层就还剩一半）：
		     Ⅰ `mpwSyncLayerHosts()`：**按结构**（谁真的是弹层的祖先）给祖先打 `data-mpw-holds-layer`，
		        并自愈摘除失效标记 —— 与"弹层是否可见/是否动画中"无关，不依赖 `:has()`；
		     Ⅱ 所有会给这些节点加 `backdrop-filter` 的规则一律 `:not([data-mpw-holds-layer])`；
		     Ⅲ 给**我们自己注入**的兼容类（`sidebarCol`）加自愈：弹层容器（或其祖先）绝不允许带它，
		        否则它会吃到 `[class*="sidebarCol"]` 的 !important 规则（把弹层变成侧栏）。
		   判据（夹具驱动真代码 + 真机探针）：
		     · 弹层容器不得带 `sidebarCol`（`data-mpw-side-mark` 也不得留）；
		     · `data-mpw-holds-layer` 落在 overlay 的每一个祖先上（含 `sidebarCol` 本体）；
		     · 面板 rect 不得落进侧栏 rect（`panel.left >= sidebar.right - 4` 且面板宽 ≥ 700）。
		   ══════════════════════════════════════════════════════════════════════════════════ */
		const MPW_LAYER_HOST_ATTR = "data-mpw-holds-layer";
		/** 我们注入的兼容类标记（**只有我们打的**才允许自愈摘除；宿主自带的类名一律不碰）。 */
		const MPW_SIDE_MARK_ATTR = "data-mpw-side-mark";
		/** 弹层/浮层容器候选（与既有弹层判定同一套锚点，另加"面板型"类名）。 */
		const MPW_LAYER_SEL = '[class*="_overlay"], [class*="_modal"], [role="dialog"], [role="alertdialog"], [aria-modal="true"]';
		/** 弹层容器的**类名/角色**特征：这些节点绝不允许被我们打上侧栏类（会吃到侧栏 !important 规则）。 */
		const MPW_LAYERISH_CLASS_RE = /(overlay|_modal|modal_|dialog|sheet|drawer|popover|panel|menu|portal)/i;
		/** 该节点自己是不是"弹层/面板型"容器（不看子树，只看自身）。 */
		function mpwLooksLikeLayer(el) {
			try {
				if (!el || el.nodeType !== 1) return false;
				const role = String(el.getAttribute && (el.getAttribute("role") || "")).toLowerCase();
				if (role === "dialog" || role === "alertdialog" || role === "menu" || role === "tooltip" || role === "listbox") return true;
				if (el.getAttribute && String(el.getAttribute("aria-modal") || "") === "true") return true;
				const cls = String((el.className && el.className.baseVal !== void 0 ? el.className.baseVal : el.className) || "");
				if (MPW_LAYERISH_CLASS_RE.test(cls)) return true;
				// backdrop-filter 非 none：这节点已经是一个"面板/弹层容器"（含我们自己的 dialogBlur 落点），
				// 给它或其祖先追加布局类会同时改变 containing block / 层叠，绝不允许。
				const cs = getComputedStyle(el);
				if (cs && cs.backdropFilter && cs.backdropFilter !== "none") return true;
			} catch {}
			return false;
		}
		/** 该节点自身是弹层，或**子树里有**弹层容器（⇒ 它绝不能成为 containing block 的受害者，也不能被打侧栏类）。 */
		function mpwHoldsLayer(el) {
			try {
				if (!el || el.nodeType !== 1) return false;
				if (el.hasAttribute && el.hasAttribute(MPW_LAYER_HOST_ATTR)) return true;
				if (mpwLooksLikeLayer(el)) return true;
				return !!(el.querySelector && el.querySelector(MPW_LAYER_SEL));
			} catch { return false }
		}
		/** 同步 `data-mpw-holds-layer`：给每个"开着的弹层"的**全部祖先**打标记，并摘掉不再成立的标记。 */
		function mpwSyncLayerHosts() {
			try {
				const want = new Set();
				const all = Array.from(document.querySelectorAll(MPW_LAYER_SEL));
				for (const el of all) {
					let open = false;
					try {
						const cs = getComputedStyle(el);
						open = !(cs.display === "none" || cs.visibility === "hidden" || Number(cs.opacity) === 0);
					} catch {}
					if (!open) continue;
					let n = el;
					let guard = 0;
				...
				const have = Array.from(document.querySelectorAll("[" + MPW_LAYER_HOST_ATTR + "]"));
				let added = 0, removed = 0;
				for (const el of want) { if (!el.hasAttribute(MPW_LAYER_HOST_ATTR)) { try { el.setAttribute(MPW_LAYER_HOST_ATTR, ""); added++ } catch {} } }
				for (const el of have) { if (!want.has(el)) { try { el.removeAttribute(MPW_LAYER_HOST_ATTR); removed++ } catch {} } }
				if (added || removed) mpwTrace("layer-host:sync", { added: added, removed: removed, overlays: all.length });
				return { added: added, removed: removed };
			} catch (e) { return { added: 0, removed: 0, error: String((e && e.message) || e) } }
		}
		/** ⑥ 自愈：把我们**注入**的侧栏类从"弹层容器 / 弹层祖先"上摘掉（宿主自带的类名绝不碰）。 */
		function mpwUntagInjectedCompat(reason) {
			try {
				const marked = Array.from(document.querySelectorAll("[" + MPW_SIDE_MARK_ATTR + "]"));
				let n = 0;
				for (const el of marked) {
					const unsafe = mpwHoldsLayer(el);
					if (!unsafe) continue;
					try { el.classList.remove("sidebarCol"); el.removeAttribute(MPW_SIDE_MARK_ATTR); n++ } catch {}
					try { mpwTrace("compat-untag", { reason: String(reason || ""), cls: String(el.className || "").slice(0, 80) }); } catch {}
					try { mpwWebDiag("compat-untag", { reason: String(reason || ""), tag: el.tagName, cls: String(el.className || "").slice(0, 120) }); } catch {}
				}
				return n;
			} catch { return 0 }
		}
		...
		const COMPAT_BRIDGES = [
			...
			// ④(2026-09-20 真机修复) 第 4 个元素 = **这一条是不是右栏/dock 桥**（显式声明）。
			//   旧写法用 `/sidebar-right|dockkit/.test(sel)` 从**选择器文本**猜 —— 而 aside 桥的选择器里
			//   带着 `:not([class*="_sidebar-right_"])`，于是左栏桥被误判成右栏桥：`rightSidebarBlur`
			//   关掉时它会把真侧栏的 `sidebarCol` **摘掉**，且永不给我们注入的类打"是我们打的"标记
			//   （标记缺失 ⇒ 自愈无从下手）。判据见 tools/wallpaper-lifecycle-test.mjs 的 E4/E6。
			['[data-sidebar-right-panel], [data-sidebar-right-float-host], [data-dockkit-strip], [data-dockkit-float]', 'mpw-rs-mark', 'mark', true],
			// ⑥(2026-09-20 真机修复) `aside` 收窄：**弹层/面板/抽屉/菜单/侧滑**一律排除。
			//   为什么：我们打的 `sidebarCol` 会被 `[class*="sidebarCol"]`（含 !important 的
			//   backdrop-filter / 背景色 / z-index）命中 ⇒ 一旦某个宿主版本把设置面板/抽屉做成
			//   `<aside role="dialog">` 或 `aside[class*="_panel_"]`，它就会变成"侧栏"，
			//   并因为 backdrop-filter 而成为 fixed 弹层的 containing block（真机实测：面板被压成 254px）。
			//   排除项是**角色/类名**判据（Firefox 对 :has()+backdrop-filter 的层叠不可靠，
			//   所以不用 :has）；JS 侧还有 mpwHoldsLayer() 兜第二道。
			['aside:not([role="dialog"]):not([role="alertdialog"]):not([role="presentation"]):not([role="menu"]):not([role="tooltip"]):not([aria-modal="true"]):not([class*="overlay"]):not([class*="Overlay"]):not([class*="modal"]):not([class*="Modal"]):not([class*="panel"]):not([class*="Panel"]):not([class*="sheet"]):not([class*="Sheet"]):not([class*="drawer"]):not([class*="Drawer"]):not([class*="popover"]):not([class*="Popover"]):not([class*="dialog"]):not([class*="Dialog"]):not([class*="menu"]):not([class*="Menu"]), [data-sidebar-left], [class*="_sidebar_"]:not([class*="_sidebar-right_"])', 'sidebarCol', 'sideonly', false],
			['header, [class*="_header_"]', 'wSkVaW_header', 'wide', false],
			['[class*="_floatHeader_"]', 'wSkVaW_header', 'wide', false]
		];
		...
		function applyCompatBridges(section) {
			try {
				...
				// ⑥(2026-09-20) 先同步"弹层祖先"标记，再自愈我们已经打错的类 —— 顺序有讲究：
				//   标记是本轮判据的输入，必须先算；自愈必须在打新类之前（否则这一帧又是错的）。
				try { mpwSyncLayerHosts(); } catch {}
				for (const [sel, cls, mode, rightFlag] of COMPAT_BRIDGES) {
					const isRight = rightFlag !== void 0 ? !!rightFlag : /sidebar-right|dockkit/.test(sel);
					...
					document.querySelectorAll(sel).forEach((el) => {
						...
						if (mode === 'sideonly') {
							...
							// ⑥(2026-09-20) **布局类只给真侧栏**：弹层容器、或子树里含弹层的节点、
							//   以及任何已经带 backdrop-filter 的面板（自身可能成为 containing block）
							//   一律不打。真机事故：面板被压成 254px 宽就是这条类被吃到侧栏规则上。
							if (mpwHoldsLayer(el)) {
								try { mpwTrace("compat-skip-layer", { tag: el.tagName, cls: String(el.className || "").slice(0, 80) }); } catch {}
								return;
							}
						}
						if (isRight && !rightOn) el.classList.remove(cls); else el.classList.add(cls);
						// ⑥ 记录"这个类是**我们**打的"：只有带标记的节点才允许被自愈摘除（宿主自带类名绝不碰）。
						if (!isRight) { try { el.setAttribute(MPW_SIDE_MARK_ATTR, "") } catch {} }
					});
				}
				// ⑥ 打完全部类之后**再自愈一次**：MutationObserver 路径下可能出现"先打类、弹层随后挂载"
				//   （打类那一刻 mpwHoldsLayer 还看不出来）⇒ 每次重算都收口，标记不会被粘住。
				try { mpwUntagInjectedCompat("apply-compat-bridges"); } catch {}
			} catch (e) {}
		}
		...
		// ═══════════════════════════════════════════════════════════════════
		//  🎵(NP-1 2026-09-19 用户第 1 条) Now playing：挂到 DSH 左侧栏「设置」入口**上方**。
		//   · 开关 `npNowPlaying`（①(NP-3) 默认**开**）在「壁纸设置」tab 里；
		//   · 组件与数学在 lib/now-playing.js / lib/now-playing-math.js（上面的生成区逐字节内联）；
		//   · 左侧栏收起 ⇒ 组件自己隐藏（判据/阈值见 lib/now-playing.js 的 NP_COLLAPSE_MAX_W 注释）；
		//   · 数据源**不伪造**：四种情形与"做不到"的清单见 docs/NOW-PLAYING-DSH.md §4；
		//   · ①(NP-3) 传输行就是「壁纸声音」的 UI（上一首/下一首/播放暂停/静音 + 进度），
		//     不另开设置项；**任何传输动作都不重载壁纸**（不碰 iframe src / 不调 showWebEl）。
		// ═══════════════════════════════════════════════════════════════════
		let npCtl = null;               // 控制器单例（window.__mpwNowPlaying 也指向它，便于真机排障）
		let npMediaToken = 0;           // 数据源请求序号（迟到的应答直接丢）
		let npMediaCache = { key: "", at: 0, data: null };
		/* ①(NP-3) 我们自己的曲目播放器：壁纸目录里**真实存在**的音频文件用真 <audio> 播。
		   为什么不让 iframe 里的壁纸自己放：实测（用户现场 `3646392375`）那些文件在壁纸自己的
		   脚本里**一次都没被引用**（`grep backgroundmuisc` = 0 命中）—— 它们只是躺在目录里，
		   所以"谁都不放"才是本来的行为；控件要放的是这些文件，而不是去猜壁纸内部实现。 */
		let npAudio = null;
		let npTrackIdx = 0;
		/* ①(NP-1) 模块级取词口（applyInner 里赋成 `ctx.locale.bind(NS)`；缺省回退键名）。 */
		let mpwT = (k) => k;

		/** 当前壁纸的身份（readSection 的规范化字段；见 lib/client.js 的 MPW_STR_KEYS）。 */
		function npWallpaperIdentity(section) {
			const s = section || {};
			const name = String(s.mpkgName || s.title || s.folderName || "").trim();
			if (s.webUrl && s.converted === "web") return { kind: "web", name: name || "Web wallpaper", key: String(s.mpkgKey || s.webUrl) };
			if (s.folderName) return { kind: "custom", name: name || String(s.folderName), key: "dir:" + String(s.folderName) };
			if (s.mpkgKey) return { kind: "library", name: name || String(s.mpkgKey), key: "lib:" + String(s.mpkgKey) };
			if (s.image) return { kind: "image", name: name || "Wallpaper", key: "" };
			return { kind: "none", name: "", key: "" };
		}

		/**
		 * ①(NP-3) **音轨清单的作用域**：当前壁纸的音频文件在宿主的哪个目录里。
		 * 纯函数（无 DOM/无 fetch），门禁逐形态断言（tools/np-media-test.mjs 的 A 组）。
		 *
		 * 为什么需要它（真机 bug 2 的根因之一）：NP-1 只认 `section.folderName` 与
		 * `section.mpkgKey` 两种形态，而**自定义目录里的壁纸根本不写 folderName**：
		 *   · web 壁纸（applyCustomWebReal）：`mpkgKey = "custom|<folder>"`、
		 *     `webUrl = "host:?custom=1&folder=<folder>&file=index.html"`；
		 *   · 视频/场景壁纸（custom 列表项）：`mpkgKey = "custom|<folder>"`、
		 *     `image = "host:?custom=1&folder=<folder>&file=..."`。
		 * 旧写法于是拼出 `library-scene-audio?ltoken=custom|3646392375` ⇒ 宿主 404 ⇒ 清单恒 0 条
		 * ⇒ 目录里那首 `backgroundmuisc.mp3` **从来没有接到控件**（用户现场第 2 条）。
		 * @returns {{mode:'custom'|'library', folder?:string, ltoken?:string, src:string}|null}
		 */
		function npAudioScope(section) {
			const s = section || {};
			const q = (u, k) => {
				try {
					const m = new RegExp("[?&]" + k + "=([^&#]*)").exec(String(u || ""));
					return m && m[1] ? decodeURIComponent(m[1]) : "";
				} catch (e) { return "" }
			};
			/* ②(2026-09-20 真机修复·根因 B) folder 必须是**目录名**：容器档（`custommpkg|<文件>.mpkg`）
			   的 key 里那一段是**文件名**，拿它当 folder 去问 `/custom-scene-audio` 只会得到
			   ENOENT 500（真机现场：`folder=小鸟游星野01_04.mpkg`）。这类档没有可扫描的目录 ⇒
			   如实返回 null（"没有清单作用域"），由调用方按"清单为空/接不上"处理，绝不发必 500 的请求。 */
			const isDirName = (v) => {
				const t = String(v || "");
				if (!t) return false;
				return !/\.(mpkg|pkg|mp4|webm|mov|m4v|mkv|html?|xhtml|gif|png|jpe?g|webp|bmp|ogg|mp3|wav|m4a|flac)$/i.test(t);
			};
			const customScope = (folder, src) => (isDirName(folder) ? { mode: "custom", folder: String(folder), src: src } : null);
			if (s.folderName) return customScope(s.folderName, "folderName");
			const key = String(s.mpkgKey || "");
			let m = /^custom\|(.+)$/.exec(key);
			if (m) return customScope(String(m[1]).split("/")[0], "mpkgKey");
			m = /^custommpkg\|(.+)$/.exec(key);
			if (m) return customScope(String(m[1]).split("/")[0], "mpkgKey");
			m = /^library\|(.+)$/.exec(key);
			if (m) return { mode: "library", ltoken: m[1], src: "mpkgKey" };
			const folder = q(s.webUrl, "folder") || q(s.image, "folder");
			if (folder) return customScope(folder, s.webUrl ? "webUrl" : "image");
			const lt = q(s.webUrl, "ltoken") || q(s.image, "ltoken");
			if (lt) return { mode: "library", ltoken: lt, src: s.webUrl ? "webUrl" : "image" };
			return null;
		}
		function npScopeKey(scope) {
			if (!scope) return "";
			return scope.mode === "custom" ? ("custom|" + scope.folder) : ("library|" + scope.ltoken);
		}
		/** 清单路由（宿主既有扫描路由，见 docs/AUDIO-TRACK-SPEC.md / AUDIO-SCAN-FAST.md）。 */
		function npScanUrl(scope) {
			if (!scope) return "";
			if (scope.mode === "custom") {
				return scope.folder ? (HOST_BASE + "/custom-scene-audio?refs=0&folder=" + encodeURIComponent(scope.folder)) : "";
			}
			return scope.ltoken ? (HOST_BASE + "/library-scene-audio?refs=0&ltoken=" + encodeURIComponent(scope.ltoken)) : "";
		}
		/** 单个音轨的字节 URL。
		 *  ①(NP-4 2026-09-19 真机修复) **子目录路径**：旧写法无条件取基名
		 *  （`path.split(/[\\/]/).pop()`）⇒ 音轨在子目录里时恒 404、在包内时恒 403。
		 *  真机读数（宿主两条路由的实测响应，见 docs/NOW-PLAYING-DSH.md §7.8 的清单）：
		 *    · `raw?custom=1&folder=3744579963&file=CH0200_MemorialLobby_1_1.ogg`
		 *      → **404**（清单里那条真实路径是 `assets/audio/CH0200_MemorialLobby_1_1.ogg`）
		 *    · `raw?...&file=assets%2Faudio%2F…ogg` → **403**（/raw 的 `file` 只接受**基名**，
		 *      见 lib/index.js 的 `bad()` 判据：`file.includes('/')` 即拒绝）
		 *    · `custom-folder/3744579963/assets/audio/CH0200_MemorialLobby_1_1.ogg`
		 *      → **200 / audio/ogg**（prefix 路由 `/custom-folder/<folder>/<rest…>` 支持嵌套）
		 *  ⇒ 有子目录时改走 **path 式**前缀路由（与网页壁纸的相对资源同一条）；
		 *  单段路径**保持旧 URL 逐字节不变**（根目录音轨零回归：`3646392375/backgroundmuisc.mp3`、
		 *  3580207945 那一档的 6 条都实测能放）。
		 *  包内音频（`source==='pkg'`，文件在 `scene.pkg` 里）**没有任何字节通道**：
		 *  `/custom-folder` 404（不在磁盘上）、`/raw` 403、`/custom-media` 403、
		 *  `/custom-scene-frame` 回的是 png。⇒ 返回空串，调用方按"只列清单、不能播"如实标（不假装）。 */
		function npTrackUrl(scope, trackPath, src) {
			if (!scope || !trackPath) return "";
			if (String(src || "") === "pkg") return "";
			const segs = String(trackPath).split(/[\\/]/).filter((x) => x && x !== "." && x !== "..");
			const file = segs.length ? segs[segs.length - 1] : "";
			if (!file) return "";
			const enc = segs.map(encodeURIComponent).join("/");
			if (scope.mode === "custom") {
				if (segs.length === 1) {
					return HOST_BASE + "/raw?custom=1&folder=" + encodeURIComponent(scope.folder) + "&file=" + encodeURIComponent(file);
				}
				return HOST_BASE + "/custom-folder/" + encodeURIComponent(scope.folder) + "/" + enc;
			}
			if (segs.length === 1) {
				return HOST_BASE + "/raw?ltoken=" + encodeURIComponent(scope.ltoken) + "&file=" + encodeURIComponent(file);
			}
			return HOST_BASE + "/library-web/" + encodeURIComponent(scope.ltoken) + "/" + enc;
		}

		/**
		 * ①(NP-3) 当前**真的在放**的那个壁纸 `<video>`；不是就返回 null。
		 *
		 * 真机 bug（用户现场第 4/5/6 条的同一个根因）：`ensureBgDom()` 在页面加载时就建了一个
		 * `#mpw-bgVideo`，**所有壁纸类型下它都在 DOM 里**（不用视频时 `display:none`、没有 src）。
		 * NP-1 的判定是 `document.querySelector("#mpw-bgWrap video, .mpw-bgVideo, video#mpw-bgVideo")`
		 * ⇒ **永远命中这个空壳**：于是 web 壁纸下"播放"调的是空 video（`play()` 被浏览器拒、`paused`
		 * 压根不动）、"静音"读的是它的 `muted`（恒 true）⇒ 现场"点播放键没用、默认锁在暂停"
		 * 与"声音控制打不开、一直是静音状态"。
		 * 判据（三档，缺一不可）：有 src（currentSrc/属性） + 不是 display:none + 当前 section
		 * 确实是视频类壁纸（不是 web/图片）。
		 */
		function npActiveVideo(section) {
			const s = section || {};
			let video = null;
			try { video = bgElements().video || null; } catch (e) { video = null }
			...
			if (s.webUrl) return null;                       /* web 壁纸：那个 video 不是壁纸本体 */
			const conv = String(s.converted || "");
			const img = String(s.image || "");
			const isVid = conv === "mp4" || conv === "webm" || /\.(mp4|webm|mov|m4v)([?#]|$)/i.test(img);
			return isVid ? video : null;
		}
		/** ①(NP-3) 浏览器**能不能判断**这个视频有没有音轨：true / false / null（不知道就不猜）。
		 *  Firefox 用 `mozHasAudio`（loadedmetadata 之后有效）；Chromium 系用 `audioTracks`；
		 *  两条都没有 ⇒ null（"不知道"，界面按"有音轨但可能没声音"处理，不编造）。 */
		function npVideoAudio(video) {
			if (!video) return null;
			try { if (typeof video.mozHasAudio === "boolean") return video.mozHasAudio } catch (e) {}
			try { const at = video.audioTracks; if (at && typeof at.length === "number") return at.length > 0 } catch (e) {}
			return null;
		}

		/** ①(NP-3) 当前壁纸的曲目清单（同步读缓存；拉取在 npFetchTracks）。 */
		/* ══════════════════════════════════════════════════════════════════════════════════
		   A(2026-09-20 真机修复)「交互音/角色语音不得进播放器清单」
		   ──────────────────────────────────────────────────────────────────────────────────
		   用户原话：「部分 web 壁纸是可交互的，交互里面的角色会播放特定的声音 …… 把这个声音文件从
		   音频播放里**排除掉**，不然我音频播放放的是角色的语音，而不是背景音」。
		   真机语料（`dd/3580207945`，L2D 碧蓝档案 8K … 星野(中秋)）逐文件证据：
		     · `BGM.wav` —— `assets/index-CQUR_vLf.js:916427` 处
		       `f.src="BGM.wav",f.volume=e.value.bgmVolume, … await f.play()` ⇒ **背景音**（循环播放、
		       音量取自 `bgmVolume`）；
		     · `1-1.wav` / `2.wav` / `3.wav` / `4.wav` / `5.wav` —— 只出现在 `loadJson.json` 的
		       `SettingModel` / `EventInfos` 里（= Live2D 的 **talk/语音**参数与事件表；目录里对应
		       `1-1.wav.ogg`…`5.wav.ogg`）⇒ **交互音/语音**（由作者的点击/触摸处理器按需播放）。
		   分类器（纯函数，逐条规则都要能给出"依据 + 命中位置"）：
		     ① 命中"交互语义"附近的引用（voice/talk/event/click/touch/pointer/dialog/se）⇒ voice；
		     ② 命中"背景语义"附近的引用（bgm/background/music/loop/autoplay）⇒ bgm；
		     ③ 文件名模式：`bgm|background|music` ⇒ bgm；`voice|talk|se_|click|touch|dialog` ⇒ voice；
		     ④ 数字对话式命名（`1-1.wav`、`2.mp3`）且出现在 `loadJson.json`（Live2D 设置/事件表）⇒ voice；
		     ⑤ 其余 ⇒ **unknown（默认保留为背景音）** —— 判不准时**不擅自丢用户的音频**
		        （丢错的代价是"壁纸没声音"，留下的代价只是清单里多一条），可用 `?npvoice=drop`
		        （判不准的一律丢）或 `?npvoice=keep`（一条都不丢）覆盖。
		   ══════════════════════════════════════════════════════════════════════════════════ */
		const MPW_VOICE_RE = /(voice|talk|event|click|touch|pointer|dialog|\bse\b|se_|soundeffect)/i;
		const MPW_BGM_RE = /(bgm|background|music|loop|autoplay)/i;
		/** `?npvoice=keep|drop`（缺省 = 只按分类器丢 voice，unknown 保留）。 */
		function mpwVoiceMode() {
			try {
				const v = new URLSearchParams(location.search).get("npvoice");
				if (v === "keep" || v === "drop") return v;
			} catch (e) {}
			return "auto";
		}
		/** 逐文件分类（纯函数；`texts` = `[{name, text}]`，`files` = 音频文件名列表）。 */
		function mpwClassifyWebAudio(input) {
			const files = ((input && input.files) || []).map((f) => String(f));
			const texts = (input && input.texts) || [];
			const out = [];
			for (const f of files) {
				const base = String(f).split(/[\\/]/).pop();
				let kind = "unknown", why = "", at = "";
				/* 引用的**候选写法**：目录里的名字常带双扩展名（`1-1.wav.ogg`），而作者源码里写的是
				   去掉最后一层扩展名的那个（真机语料：`loadJson.json` 里写 `1-1.wav`）。
				   由长到短逐个试，命中即用（不许拿 1 个字符的短名去乱撞）。 */
				const stem = base.replace(/(?:\.(?:ogg|mp3|wav|m4a|flac|opus|aac))+$/i, "");
				const needles = [base, base.replace(/\.(?:ogg|mp3|m4a|flac|opus|aac)$/i, ""), stem].filter((n, i, a) => n && n.length >= 2 && a.indexOf(n) === i);
				// ①② 引用上下文：**看谁离得更近**（同一段里同时出现 bgmVolume 与 talkVolume 是常态，
				//   按"谁先匹配"会把 `BGM.wav` 误判成交互音 —— 真机语料第一次跑就是这么错的）。
				for (const t of texts) {
					const text = String((t && t.text) || "");
					for (const needle of needles) {
						let idx = text.indexOf(needle);
						while (idx >= 0) {
							const ctx = text.slice(Math.max(0, idx - 120), idx + needle.length + 120);
							const rel = needle.length;
							const near = (re) => { let best = Infinity, m2; const rx = new RegExp(re.source, "gi"); while ((m2 = rx.exec(ctx))) { const d = Math.abs(m2.index - idx + 120); if (d < best) best = d } return best };
							const dVoice = near(MPW_VOICE_RE), dBgm = near(MPW_BGM_RE);
							if (dBgm < Infinity || dVoice < Infinity) {
								kind = (dBgm <= dVoice) ? "bgm" : "voice";
								why = (kind === "bgm") ? "ref-in-background-context" : "ref-in-interaction-context";
								at = String(t.name || "") + ":" + (text.slice(0, idx).split("\n").length);
								break;
							}
				...
				// ③ 文件名模式
				if (kind === "unknown") {
					if (MPW_BGM_RE.test(base)) { kind = "bgm"; why = "name-pattern-bgm" }
					else if (MPW_VOICE_RE.test(base)) { kind = "voice"; why = "name-pattern-voice" }
				}
				// ④ Live2D 设置/事件表里的数字对话式命名
				if (kind === "unknown" && /^\d+(-\d+)?\./.test(base)) {
					const inSetting = texts.some((t) => {
						if (!/loadJson|setting|event/i.test(String((t && t.name) || ""))) return false;
						const txt = String((t && t.text) || "");
						return needles.some((n) => txt.indexOf(n) >= 0);
					});
					if (inSetting) { kind = "voice"; why = "live2d-talk-table" }
				}
				if (!why) why = "default-keep";
				out.push({ file: String(f), base: base, kind: kind, why: why, at: at });
			}
			return out;
		}
		/** 分类结果 → 清单过滤（唯一落点；`mode` 见 mpwVoiceMode）。 */
		function mpwFilterVoiceTracks(tracks, table, mode) {
			const m = mode || mpwVoiceMode();
			if (m === "keep") return { tracks: tracks || [], dropped: [], table: table || [] };
			const drop = new Set();
			for (const c of table || []) {
				if (c.kind === "voice") drop.add(c.base);
				else if (m === "drop" && c.kind === "unknown") drop.add(c.base);
			}
			const kept = [], dropped = [];
			for (const t of tracks || []) {
				const b = String((t && t.path) || "").split(/[\\/]/).pop();
				if (drop.has(b)) dropped.push(t); else kept.push(t);
			}
			return { tracks: kept, dropped: dropped, table: table || [] };
		}
		/** web 档的"文本语料"：入口 HTML + 它引用的 js/json（最多 6 个、每个截断 200KB）。 */
		function mpwWebAudioTexts(section) {
			try {
				const s = section || readSection();
				const webUrl = resolveHostUrl(String(s.webUrl || ""));
				const src = mpwWebSrcFromUrl(webUrl);
				if (!src) return Promise.resolve([]);
				const dirBase = HOST_BASE + (src.src === "library" ? "/library-web/" : "/custom-folder/") + encodeURIComponent(src.id) + "/";
				const grab = (name) => fetch(dirBase + name, { cache: "no-store" }).then((r) => (r && r.ok ? r.text() : "")).catch(() => "");
				const texts = [];
				const entry = (/([^/?#]+\.[a-z0-9]+)(?:[?#]|$)/i.exec(webUrl) || [])[1] || "index.html";
				return grab(entry).then((html) => {
					if (html) {
						texts.push({ name: entry, text: String(html).slice(0, 200000) });
						const refs = [];
						try {
							const re = /(?:src|href)\s*=\s*["']([^"']+\.(?:js|json|mjs))["']/gi;
							let m2;
							while ((m2 = re.exec(html)) && refs.length < 6) refs.push(String(m2[1]).replace(/^\.\//, ""));
						} catch (e) {}
						for (const r of refs) texts.push({ name: r, text: "" });   // 先占位（顺序可读）
						return Promise.all(refs.map((r) => grab(r))).then((bodies) => {
							for (let i = 0; i < refs.length; i++) texts[1 + i] = { name: refs[i], text: String(bodies[i] || "").slice(0, 200000) };
							return Promise.all([grab("loadJson.json"), grab("project.json")]).then((extra) => {
								if (extra[0]) texts.push({ name: "loadJson.json", text: String(extra[0]).slice(0, 200000) });
								if (extra[1]) texts.push({ name: "project.json", text: String(extra[1]).slice(0, 200000) });
				...
				}).catch(() => texts.filter((t) => t.text));
			} catch (e) { return Promise.resolve([]) }
		}
		function npTrackList(section) {
			const scope = npAudioScope(section || readSection());
			if (!scope) return null;
			const key = npScopeKey(scope);
			const cached = npMediaCache.data;
			if (!cached || npMediaCache.key !== key || !cached.tracks || !cached.tracks.length) return null;
			return { scope: scope, key: key, tracks: cached.tracks, source: String(cached.source || "") };
		}
		function npCurrentTrack(section) {
			const list = npTrackList(section);
			if (!list) return null;
			const i = Math.max(0, Math.min(list.tracks.length - 1, npTrackIdx));
			return { scope: list.scope, key: list.key, tracks: list.tracks, source: list.source, index: i, track: list.tracks[i] };
		}
		function npAudioOwns() {
			/* ①(NP-3) 我们的播放器在放音时，web 壁纸帧内的那份声音要让位（同一首会被放两遍）。 */
			try { return !!(npAudio && !npAudio.paused && (npAudio.currentSrc || npAudio.getAttribute("src") || npAudio.src)) } catch (e) { return false }
		}
		/* ══════════════════════════════════════════════════════════════════════════════════
		   ④(2026-09-20 真机修复)「卡片的静音/暂停必须**真的管住壁纸自带的音频**」
		   ──────────────────────────────────────────────────────────────────────────────────
		   真机现场（用户原话）：
		     · "把音乐卡片**暂停**掉，它**还是会播放音频**" —— 卡片的暂停只落到**我们的**播放器
		       （`npAudio.pause()`），而帧内那份作者自带的 BGM 一路没被碰过；
		     · "把卡片**打开**，它会优先播卡片里的音频（壁纸自带的背景音乐会因卡片开启而关掉）"
		       —— 这一半 NP-3 已经做了（`npAudioOwns()` ⇒ 强制静音帧内）。
		   ⇒ 缺的是**另一半**：卡片处于"没在放"的状态时，帧内音频也必须安静/停住。
		   判据（唯一口径，可查）：
		     `npFrameSoundBlocked()` = 卡片开着 且（用户按过暂停 ∨ 我们的播放器正在放音）。
		   语义边界（每条都有理由，改错就会多管闲事）：
		     · 卡片**没开**（npNowPlaying=false）⇒ 完全不介入（零行为变化）；
		     · 卡片暂停 ⇒ 帧内**暂停 + 静音**（双保险：作者可能用 WebAudio，pause() 管不到）；
		     · 卡片播放 ⇒ 恢复**我们暂停的那一部分**（只撤销我们做的暂停，绝不去 play() 作者自己
		       停着的媒体 —— 那会把作者的设计改掉）；
		     · 我们自己的播放器在放音（同一首会被放两遍）⇒ 帧内只静音、不暂停（既有 NP-3 行为）。
		   可判定状态：`#mpw-bgWrap[data-mpw-np-sound]` + `window.__mpwNpFrameSound`。 */
		/* ①(NP-5) `npCardPaused` 的**初值来自持久化**（`npPaused`）：刷新后"用户按过暂停"必须还在。 */
		let npCardPaused = (() => { try { const s0 = readSection(); return s0.npPaused !== void 0 ? !!s0.npPaused : DEFAULT_NP_PAUSED } catch (e) { return DEFAULT_NP_PAUSED } })();
		let npLinkPrev = null;         // 上一次应用时的联动开关值（用于识别"关→开"并对齐，见 mpwLinkAlign）
		let npSrcIdPrev = null;        // C3：上一轮的音源身份（变了就把旧音源硬归零，见 npAudioHardReset）
		let npCardPausedKey = "";      // 上面那条状态属于哪张壁纸（换壁纸即清零，见 applyNowPlaying）
		let npFramePausedByUs = false; // 帧内媒体的暂停是**我们**按下的（决定恢复时该不该 play）
		/* ══════════════════════════════════════════════════════════════════════════════════
		   ①(NP-4 补 2026-09-20 真机修复)「联动开关关闭」的**正确语义**（用户实测是坏的）
		   ──────────────────────────────────────────────────────────────────────────────────
		   用户原话：「关闭状态下它的暂停播放用不了 —— 如果我在**暂停状态下**把开关关闭，那这个壁纸
		   就不能动起来；如果我在**播放状态下**把开关关闭，那它还是可以继续播放，就是**不能暂停**了」。
		   规格（本块把它钉成一条纯状态机 + 一条对齐动作）：
		     · 关的那一刻**不许改变壁纸当前的播放状态**（暂停中就保持暂停、播放中就继续播放）；
		     · 关掉之后卡片的播放/暂停只控**它自己的音频**（曲目档照常能播能停），并如实显示
		       canPlay/canSeek 与副标题"仅控声音，不影响壁纸"；
		     · 重新打开 ⇒ **按卡片当前状态对齐**（卡片在放就恢复壁纸、卡片暂停就压住壁纸），
		       绝不出现"卡片显示在放、壁纸停着"这种两个状态打架。
		   @returns {'none'|'resume'|'pause'} 本次开关切换要对壁纸做的动作 */
		function mpwLinkAlign(linkOn, prevLinkOn, cardPaused) {
			if (linkOn === prevLinkOn) return "none";     // 没切换 ⇒ 一个字节都不碰
			if (!linkOn) return "none";                   // 关：保持壁纸当下状态（用户现场那条）
			return cardPaused ? "pause" : "resume";       // 开：对齐到卡片当前状态
		}
		function npFrameSoundBlocked() {
			try {
				const s = readSection();
				const on = s.npNowPlaying !== void 0 ? !!s.npNowPlaying : DEFAULT_NP_NOW_PLAYING;
				if (!on) return false;
				if (npCardPaused) return true;
				return npAudioOwns();
			} catch (e) { return false }
		}
		/** ④ 把"帧内音频现在归谁管"如实发布到 DOM/全局（探针与设置面板都读它，不猜）。 */
		function npPublishFrameSound(reason) {
			try {
				const blocked = npFrameSoundBlocked();
				const reason2 = !blocked ? "open" : (npCardPaused ? "card-paused" : "np-audio-owns");
				const st = { blocked: blocked, reason: reason2, pausedByUs: npFramePausedByUs, cardPaused: npCardPaused, owns: npAudioOwns(), at: Date.now() };
				try { window.__mpwNpFrameSound = st; } catch (e) {}
				try {
					const wrap = bgElements().wrap;
					if (wrap && wrap.setAttribute) {
						wrap.setAttribute("data-mpw-np-sound", blocked ? "blocked" : "open");
						wrap.setAttribute("data-mpw-np-sound-reason", reason2);
					}
				} catch (e) {}
				return st;
			} catch (e) { return null }
		...
		/**
		 * 数据源：**只报我们真的知道的东西**。
		 * 情形① 视频壁纸：页面里**当前在放**的那个 <video>（自带音轨）⇒ 播放/暂停/进度/静音真能控。
		 * 情形② 壁纸目录里**真实存在**的音频文件（web/自定义/场景目录）⇒ 我们自己的 <audio> 播，
		 *        上一首/下一首按清单顺序（环形），进度/时长来自媒体元素本身。
		 * 情形③ web 壁纸（目录里没有音频）：帧内那份声音只有"静音"这一条真通道 ⇒
		 *        canVolume=true / canPlay=false，并在副标题里说清（不假装能暂停帧内的 WebAudio）。
		 * 情形④ 无源：空闲态（曲名 = 未在播放），点播放**不做假动作**（面板一行 + console 一行）。
		 */
		function npResolveMedia(section) {
			const s = section || {};
			const id = npWallpaperIdentity(s);
			const link = npLinkOn(s);
			const vol = npWantVolume();
			const out = {
				kind: "none", title: "", byline: id.name, cover: "",
				total: null, playing: false, muted: s.mute !== void 0 ? !!s.mute : true, volume: vol,
				canPlay: false, canVolume: false, canPrev: false, canNext: false, hasAudio: null, source: "",
				/* ①(NP-4) ②③ 新增的三个派生量（控制器据此决定键能不能按、带子能不能拖）。 */
				canSeek: false, link: link, listOnly: false, trackIndex: 0, trackCount: 0,
			};
			// 封面：当前壁纸的缩略图（真实数据；拿不到就留空 ⇒ 走渐变兜底，与上游一致）
			if (s.image && /^(data:|blob:|idb:|https?:|\/)/.test(String(s.image))) out.cover = String(s.image);
			// ① 视频壁纸本体（**只认当前在放的那个**，见 npActiveVideo 的真机 bug 注释）
			const vid = npActiveVideo(s);
			if (vid) {
				out.kind = "video";
				out.source = "video";
				out.title = id.name || "Video wallpaper";
				out.playing = npCardPaused ? false : !vid.paused;   // ①(NP-5) 用户意图优先（link 关时画面在动也显示"暂停"）
				out.muted = !!vid.muted;
				out.volume = vol;
				if (isFinite(vid.duration) && vid.duration > 0) out.total = Math.round(vid.duration);
				const hasAudio = npVideoAudio(vid);
				out.hasAudio = hasAudio;
				/* ①(NP-4)② 联动关 ⇒ 播放/暂停**不许**碰壁纸媒体。视频档的声源就是这个视频本身
				   ⇒ 关掉联动之后没有任何可控声源：如实禁用（不假装按得动），并说清是哪一条挡住的。 */
				/* ①(NP-4 补) 联动关 ⇒ 卡片的播放/暂停**仍然可用**，但它控的是**声音**（静音/取消静音），
				   不控画面 ⇒ canPlay 恒 true；canSeek 仍要求联动开（进度条是画面的事）。 */
				out.canPlay = true;
				out.linkSoundOnly = !link;
				/* 明确知道"没有音轨"时静音键禁用（如实显示无音轨）；不知道就给用户按（可能真是静音了） */
				out.canVolume = hasAudio !== false;
				out.canSeek = link && typeof out.total === "number" && out.total > 0;
				/* ①(NP-4)④ 视频档的"曲目"就是容器里那一条音轨：把 1/1 如实写出来
				   （用户看到的是"清单里只有一条"，而不是"按钮莫名其妙是灰的"）。
				   1/1 说的是**媒体条数**（进度条的主人只有一个），所以 hasAudio 判不出来
				   （Firefox 有 mozHasAudio、Chromium 那套读不到 ⇒ null）时照样成立、照样写。 */
				out.trackCount = hasAudio === false ? 0 : 1;
				out.trackIndex = out.trackCount ? 1 : 0;
				out.byline = hasAudio === false ? mpwT("np.noAudio")
					: (id.name + " · " + mpwT("np.src.video") + " · 1/1");
				if (!link) out.byline = mpwT("np.note.linkOffSoundOnly");
				out.canPrev = false; out.canNext = false;   /* 单个媒体，没有曲目清单 */
				return out;
			}
			// ② 壁纸目录里真实存在的音频文件 ⇒ 我们自己的播放器（曲目清单有顺序）
			const cur = npCurrentTrack(s);
			if (cur) {
				const n = cur.tracks.length;
				const mime = cur.track && cur.track.mime ? String(cur.track.mime) : "audio";
				const fname = String(cur.track && cur.track.path || "").split(/[\\/]/).filter(Boolean).pop() || id.name;
				/* ①(NP-4)④ 包内音频（scan 的 source==='pkg'，文件在 scene.pkg 里）：清单是真的，
				   但宿主**没有任何路由**能把它的字节取出来（实测 /custom-folder 404、/raw 403、
				   /custom-media 403）⇒ 只列清单、如实标 disabled + 副标题写清原因。 */
				const listOnly = String(cur.source || "") === "pkg";
				out.kind = "track";
				out.source = "tracks";
				out.title = fname;
				out.listOnly = listOnly;
				out.trackCount = n;
				out.trackIndex = cur.index + 1;
				out.byline = id.name + " · " + mime + (n > 1 ? (" · " + (cur.index + 1) + "/" + n) : "")
					+ (listOnly ? (" · " + mpwT("np.note.pkgListOnly")) : "")
					/* ①(NP-4 补) 联动关闭 ⇒ 如实写明"只控声音、不影响壁纸"（用户第 B 条的可见半边） */
					+ (!link ? (" · " + mpwT("np.note.linkOff")) : "");
				out.canPlay = !listOnly;
				out.canVolume = true;
				out.canPrev = n > 1 && !listOnly;        /* 只有一条 ⇒ 禁用（不假装有第二首） */
				out.canNext = n > 1 && !listOnly;
				out.hasAudio = true;
				if (npAudio) {
					out.playing = !npAudio.paused;
					out.muted = !!npAudio.muted;
					if (isFinite(npAudio.duration) && npAudio.duration > 0) out.total = Math.round(npAudio.duration);
					else if (npAudio.readyState === 0) out.total = null;
				}
				/* 我们自己的播放器一直归控件管（它就是"声音"本身）⇒ 与联动开关无关。 */
				out.canSeek = typeof out.total === "number" && out.total > 0;
				return out;
			}
			// ③ web 壁纸（目录里没有独立音频文件）：帧内那份声音只有静音这条真通道
			if (id.kind === "web") {
				out.kind = "web";
				out.source = "web";
				out.title = id.name;
				out.byline = id.name + " · " + mpwT("np.src.web");
				out.canVolume = true;
				out.canPlay = false;
				return out;
			}
			// ④ 时/场景/库里的壁纸：清单还没到或真的没有 ⇒ 只显示，不假装
			if (id.kind === "library" || id.kind === "custom") {
				out.kind = "track";
				out.title = id.name;
				out.byline = mpwT("np.src.track");
				return out;
			}
			return out;
		}

		/** 拉一次音轨清单（命中缓存就不拉）。任何失败都吞掉并把结果记成 count:0 —— 不猜。 */
		function npFetchTracks(section) {
			const s = section || {};
			const scope = npAudioScope(s);
			const url = npScanUrl(scope);
			/* C3：**这两条 return 是修前的真凶现场** —— 无扫描 URL（容器档/无目录档）时直接返回，
			   旧 `<audio>` 没人管 ⇒ 继续放上一张壁纸的歌。现在两条路都先收口。 */
			if (!url) {
				if (npAudio && (!npAudio.paused || npAudio.getAttribute("src"))) npAudioHardReset("no-scan-url");
				return;
			}
			const key = npScopeKey(scope);
			if (npMediaCache.key === key && npMediaCache.data) return;
			const seq = ++npMediaToken;
			/* ②(2026-09-20 真机修复·根因 B) folder 类扫描失败 ⇒ 同步宿主 customDir 后**重试一次**。
			   只在"**像是目录解析错**"的失败上重试（500 / ENOENT / no such file / not a directory）：
			   · 真机现场正是 500 ENOENT（宿主目录还停在旧的 dd）⇒ 重试后 200 + 清单；
			   · 普通的 404（路由/清单本来就没有）**不重试** ⇒ happy path 与"真的没有清单"两条路
			     都不多发一次请求（既有门禁 C20 的"少一次网络"仍然成立）。 */
			const grab = () => fetch(url).then((r) => {
				const st = Number((r && r.status) || 0);
				return Promise.resolve(r && r.ok ? r.json() : null).then((d) => ({ st: st, d: d }));
			}).then((res) => {
				const d = res && res.d;
				if (d && d.ok) return d;
				const msg = String((d && d.error) || "");
				const dirLooksWrong = res && res.st === 500 || /ENOENT|no such file|not a directory/i.test(msg);
				if (!dirLooksWrong) return d || null;
				return mpwSyncCustomDir().then((sd) => (sd && sd.ok
					? fetch(url).then((r2) => (r2 && r2.ok ? r2.json() : null)).catch(() => null)
					: (d || null)));
			}).catch(() => null);
			try {
				grab().then((d) => {
					if (seq !== npMediaToken) return;            // 迟到的应答丢掉（壁纸已经换了）
					let tracks = (d && d.tracks) || [];
					/* A(2026-09-20) web 档：把"交互音/角色语音"从清单里**排除**（分类器 + 依据见文档）。
					   为什么在这里做：清单来自宿主目录扫描（只看得见文件名），而"这个名字是背景音还是
					   交互音"只有**作者源码**说得清（真机语料：`BGM.wav` vs `loadJson.json` 里的 1-1…5）。 */
					const isWebWall = String(s.converted || "") === "web" || !!s.webUrl;
					if (isWebWall && tracks.length) {
						return mpwWebAudioTexts(s).then((texts) => {
							if (seq !== npMediaToken) return;
							const table = mpwClassifyWebAudio({ files: tracks.map((t) => String(t && t.path || "")), texts: texts });
							const res = mpwFilterVoiceTracks(tracks, table, mpwVoiceMode());
							try { window.__mpwNpAudioClass = { mode: mpwVoiceMode(), table: table, kept: res.tracks.length, dropped: res.dropped.map((t) => String(t.path || "")), at: Date.now() } } catch (e) {}
							try { mpwWebDiag("np-audio-class", { kept: res.tracks.length, dropped: res.dropped.length, mode: mpwVoiceMode(), table: table.slice(0, 24) }); } catch (e) {}
							npMediaCache = { key: key, at: Date.now(), data: { count: res.tracks.length, tracks: res.tracks, source: String(d && d.source || ""), dropped: res.dropped.length } };
							if (npTrackIdx >= res.tracks.length) npTrackIdx = 0;
							try { if (npAudio) { npAudio.pause(); npAudio.removeAttribute("src") } } catch (e) {}
							applyNowPlaying(readSection());
						}).catch(() => {});
					}
					npMediaCache = { key: key, at: Date.now(), data: { count: d && d.count || 0, tracks: tracks, source: String(d && d.source || "") } };
					if (npTrackIdx >= tracks.length) npTrackIdx = 0;
					/* 壁纸换了 ⇒ 我们自己的播放器必须跟着换源（否则会放着上一张壁纸的歌） */
					try { if (npAudio) { npAudio.pause(); npAudio.removeAttribute("src"); } } catch (e) {}
					applyNowPlaying(readSection());
				}).catch(() => {
					if (seq !== npMediaToken) return;
					npMediaCache = { key: key, at: Date.now(), data: { count: 0, tracks: [], source: "" } };
				});
			} catch (e) {}
		}

		/** ①(NP-3) 造/取我们的 <audio>（挂在 body 上：不进 React 管的子树，也不占布局）。 */
		function npEnsureAudio() {
			if (npAudio) return npAudio;
			try { mpwAuditPush("np-ensure-audio", null, {}); } catch (e) {}
			try {
				npAudio = document.createElement("audio");
				npAudio.setAttribute("data-mpw-np-audio", "");
				npAudio.setAttribute("preload", "metadata");
				npAudio.style.cssText = "position:absolute;width:0;height:0;opacity:0;pointer-events:none";
				const on = (k, fn) => { try { if (npAudio.addEventListener) npAudio.addEventListener(k, fn) } catch (e) {} };
				on("timeupdate", () => { try { npSyncAudio() } catch (e) {} });
				on("loadedmetadata", () => { try { npSyncAudio() } catch (e) {} });
				on("play", () => { try { npSyncAudio() } catch (e) {} });
				on("pause", () => { try { npSyncAudio() } catch (e) {} });
				on("volumechange", () => { try { npSyncAudio() } catch (e) {} });
				on("ended", () => { try {
					const list = npTrackList(readSection());
					if (list && list.tracks.length > 1) npStepTrack(1, true);
					else if (npAudio) { npAudio.currentTime = 0; const p = npAudio.play(); if (p && p.catch) p.catch(() => {}); }
				} catch (e) {} });
				on("error", () => { try { npAudioError() } catch (e) {} });
				(document.body || document.documentElement).appendChild(npAudio);
			} catch (e) { npAudio = null; mpwErr("npEnsureAudio", e); }
			return npAudio;
		}
		function npDropAudio() {
			try { if (npAudio) { try { mpwAuditPush("np-drop-audio", npAudio, {}) } catch (e) {} npAudio.pause(); npAudio.removeAttribute("src"); npAudio.remove(); } } catch (e) {}
			npAudio = null;
		}
		/** ①(NP-3) 盯住**当前那个壁纸 video** 的媒体事件：元数据/时长/播放/静音/进度变化时重解一次。
		 *  为什么必须有（真机读数）：`mozHasAudio` 要等 `loadedmetadata` 之后才是真值；插件 apply
		 *  那一刻它还是 false ⇒ 我们记下"无音轨"、把静音键**禁掉**，之后视频加载完了也没人重解
		 *  ⇒ 用户看到"声音控制打不开（灰的）"（探针 A8/B3/B4 就是这个：元素已报 mozHasAudio=true，
		 *  控件却还拿着旧快照 canVolume=false）。顺带把进度/时长也接上（视频壁纸的 rail 真的会走）。 */
		function npWatchVideo(video) {
			if (!video || video.__mpwNpWired) return false;
			try {
				video.__mpwNpWired = true;
				const sync = () => { try { npSyncVideo() } catch (e) {} };
				for (const ev of ["loadedmetadata", "durationchange", "play", "pause", "volumechange", "timeupdate"]) {
					try { video.addEventListener(ev, sync) } catch (e) {}
				}
				/* ①(NP-4) **谁把它停了**：真机上出现过"起播成功（promise resolve）之后又被暂停"
				   （读数：prime-ok → paused=true）。只记账不改行为：给 pause() 包一层，把调用栈记进轨迹。
				   包装保留原语义（返回原返回值、参数原样透传）。 */
				/* 同一条纪律：`load()` 也会把元素打回 paused=true —— 一起记账（谁重置的要知道）。 */
				try {
					if (!video.__mpwLoadTraced && typeof video.load === "function") {
						video.__mpwLoadTraced = true;
						const origLoad = video.load.bind(video);
						video.load = function () {
							try {
								const st = String(new Error().stack || "").split("\n").slice(2, 6)
									.map(function (x) { return (x.trim().split(" ")[1] || "?").split("/").pop() }).join("<");
								npOpTrace("video.load", null, st.slice(0, 180));
							} catch (e) {}
				...
				try {
					if (!video.__mpwPauseTraced && typeof video.pause === "function") {
						video.__mpwPauseTraced = true;
						const origPause = video.pause.bind(video);
						video.pause = function () {
							try {
								const st = String(new Error().stack || "").split("\n").slice(2, 6)
									.map(function (x) { return (x.trim().split(" ")[1] || "?").split("/").pop() }).join("<");
								npOpTrace("video.pause", null, st.slice(0, 180));
							} catch (e) {}
							return origPause.apply(video, arguments);
						};
					}
				} catch (e) {}
				/* ①(NP-4) `playing` = 真的出画/出声了。起播成功之后才把**静音与电平**落到元素上
				   （顺序修法的收尾：muted 起播 → playing → 按设置取消静音），并且清掉"被拒"状态。 */
				/* ①(NP-4) **被策略停住**的识别点：`pause` 事件 + 我们本来想让它放 + 用户没按过暂停
				   + 还没真正放起来（currentTime 很小）⇒ 是浏览器把"没有手势的有声播放"停掉了。
				   处理：退回 muted 让画面继续动，把原因记成可查状态，并装一次性手势重试补声音。 */
				try {
					video.addEventListener("pause", () => {
						try {
							if (wallUserPaused || powPaused) return;
							if (!npWantPlay) return;
							if (!(video.currentTime < 1.5)) return;    /* 放起来之后才停（播完/切源）⇒ 不关我们的事 */
							if (npSoundBlocked) return;
							npSoundBlocked = true;
							npSoundReason = { at: Date.now(), currentTime: +video.currentTime.toFixed(2), muted: video.muted, hint: "没有用户手势时浏览器不允许有声播放（Firefox 会直接把它停住）" };
							try { window.__mpwNpSoundBlocked = npSoundReason; } catch (e) {}
							npOpTrace("audio-blocked", null, "退回 muted 保持画面；首次手势后补声音");
							try { video.muted = true; } catch (e) {}
							try {
								const p = video.play();
								if (p && typeof p.then === "function") {
									p.then(() => { npOpTrace("audio-blocked-replay", null, "ok muted=" + video.muted + " paused=" + video.paused) },
										(e) => { npOpTrace("audio-blocked-replay", null, "rejected:" + String(e && e.name || "Error")) });
								} else { npOpTrace("audio-blocked-replay", null, "no-promise") }
							} catch (e) { npOpTrace("audio-blocked-replay", null, "threw:" + String(e && e.name || "Error")) }
							npArmGestureRetry();
						} catch (e) {}
					});
				} catch (e) {}
				try {
					video.addEventListener("playing", () => {
						/* 真的出画/出声了 ⇒ "muted 起播"那一段到此结束，把静音/电平恢复成设置值。 */
						try { npPrimedMuted = false; clearTimeout(npPrimedMutedTimer); } catch (e) {}
						try { npPlayBlocked = null; npApplyMute(); npApplyVolume(); } catch (e) {}
						sync();
					});
				} catch (e) {}
				sync();
		...
		/** 视频壁纸状态 → 控件（媒体事件驱动；与 npSyncAudio 同一条口径）。 */
		function npSyncVideo() {
			if (!npCtl) return;
			try {
				const s = readSection();
				const vid = npActiveVideo(s);
				if (!vid) return;
				npCtl.setMedia(npResolveMedia(s));
				if (isFinite(vid.currentTime)) npCtl.setProgress(Math.max(0, vid.currentTime));
			} catch (e) { mpwErr("npSyncVideo", e) }
		}
		/** 我们的播放器状态 → 控件（唯一入口：媒体事件、切曲、apply 都走它）。 */
		function npSyncAudio() {
			if (!npCtl) return;
			try {
				npCtl.setMedia(npResolveMedia(readSection()));
				if (npAudio && isFinite(npAudio.currentTime)) npCtl.setProgress(Math.max(0, npAudio.currentTime));
			} catch (e) { mpwErr("npSyncAudio", e) }
		}
		/** 源加载失败：如实显示 + 一次 blob 兜底重试（宿主 /raw 的 content-type 是 octet-stream）。 */
		let npAudioBlobTried = "";
		function npAudioError() {
			let cur = null;
			try { cur = npCurrentTrack(readSection()) } catch (e) { cur = null }
			if (!cur || !npAudio) return;
			const url = npTrackUrl(cur.scope, cur.track && cur.track.path, cur.source);
			if (!url || npAudioBlobTried === url) {
				try { console.warn("[dsh-mpkg-wallpaper] np: 音轨加载失败（宿主 /raw 或解码不支持）: " + url) } catch (e) {}
				try { if (npCtl) { npCtl.setMedia({ playing: false, canPlay: false, byline: mpwT("np.note.decode") }); npCtl.setProgress(0); } } catch (e) {}
				return;
			}
			npAudioBlobTried = url;
			/* blob 兜底：把扫描清单里报的 mime 原样带上（宿主 /raw 一律 octet-stream，
			   部分浏览器据此拒绝播放）。只在文件不大时做（内存是本机的硬约束）。 */
			const size = Number(cur.track && cur.track.size) || 0;
			/* 上限 32MiB（位移写法）：本仓门禁 persist-test 有一条**逐行正则**守卫，
			   禁止"图片阈值"类字节数写成乘法字面量（乘法写法会被它当成命中）。 */
			if (!(size > 0) || size > (32 << 20)) return;
			try {
				fetch(url).then((r) => (r && r.ok ? r.blob() : null)).then((b) => {
					if (!b || !npAudio) return;
					let obj = null;
					try { obj = URL.createObjectURL(new Blob([b], { type: String(cur.track.mime || "audio/mpeg") })) } catch (e) { obj = null }
					if (!obj) return;
					npAudio.src = obj;
					const p = npAudio.play(); if (p && p.catch) p.catch(() => {});
				}).catch(() => {});
			} catch (e) {}
		}
		/** 换曲（dir=+1 下一首 / -1 上一首；**按清单顺序**，环形）。 */
		function npStepTrack(dir, autoplay) {
			const list = npTrackList(readSection());
			if (!list || !list.tracks.length) return false;
			const n = list.tracks.length;
			npTrackIdx = ((npTrackIdx + (dir > 0 ? 1 : -1)) % n + n) % n;
			npLoadTrack(autoplay !== false);
			return true;
		}
		/** 把当前曲目装进播放器（不动壁纸、不动 iframe）。 */
		function npLoadTrack(play) {
			const cur = npCurrentTrack(readSection());
			if (!cur) return false;
			const a = npEnsureAudio();
			if (!a) return false;
			const url = npTrackUrl(cur.scope, cur.track && cur.track.path, cur.source);
			if (!url) return false;
			npAudioBlobTried = "";
			try {
				if (String(a.getAttribute("src") || "") !== url) a.setAttribute("src", url);
				const st = readSection();
				a.muted = !!(st.mute !== void 0 ? !!st.mute : true);
				if (npVolumeSet()) a.volume = npWantVolume();   /* ①(NP-4)① 用户设过电平才写（默认不碰） */
				if (play) { const p = a.play(); if (p && p.catch) p.catch(() => {}); }
			} catch (e) { mpwErr("npLoadTrack", e) }
			npApplyFrameMute();
			npSyncAudio();
			return true;
		}
		/** 静音设置 → 真实元素（视频壁纸 / 我们自己的 audio / web 帧）。
		 *  真机 bug：NP-1 的静音键只 `saveSection({mute})`，**没有任何元素生效**（视频从建 DOM 那刻起
		 *  就是 `muted = true` 且再没被赋值过）⇒ 现场"声音控制打不开、一直是静音状态"。 */
		function npApplyMute() {
			try { mpwAuditPush("np-apply-mute", npActiveVideo(readSection()) || npAudio, {}); } catch (e) {}
			const s = readSection();
			/* 唯一口径（含 C4 手势闸 / npSoundBlocked / npPrimedMuted / 设置 / 我们在放音）：
			   旧写法在这里自己拼条件 ⇒ 绕过了"手势前必须 muted"那道闸（D1c 抓到）。 */
			const mute = npWantMuted();
			try { const v = npActiveVideo(s); if (v && v.muted !== mute) v.muted = mute; } catch (e) {}
			/* 壁纸 `<video>` 本体也要写（`npActiveVideo` 只认"当前真的在放的那个"⇒ 暂停中的元素
			   不会被它命中；而"静音开关"必须对**那一个元素**生效，否则用户点静音/取消静音时
			   看不到任何变化 —— D1b 抓到的就是这个缺口）。 */
			try { const v0 = bgElements().video; if (v0 && v0.muted !== mute) v0.muted = mute } catch (e) {}
			try { if (npAudio && npAudio.muted !== mute) npAudio.muted = mute } catch (e) {}
			try { applyVideoMute(); } catch (e) {}
			npApplyFrameMute();
		}
		/** web 帧的静音：跟随设置；我们的播放器在放音时**强制静音帧内**（同一首别放两遍）。
		 *  ①(NP-3) 实现只有一处：`applyWebMute`（它已经含 npAudioOwns 那条）。 */
		function npApplyFrameMute() {
			try { applyWebMute(bgElements().frame); } catch (e) {}
			/* ④(2026-09-20) **暂停那一半**：卡片暂停 ⇒ 帧内媒体真的停住（不只静音）。
			   为什么静音还不够：作者常用 WebAudio / 自己实现的播放器，`muted` 属性管不到它们；
			   `pause()` 是唯一对所有形态都成立的"别出声"落点。恢复时**只撤销我们按下的暂停**
			   （`npFramePausedByUs`），绝不去 play() 作者自己停着的媒体。 */
			try {
				const blocked = npFrameSoundBlocked();
				if (blocked && npCardPaused) { pauseWebFrame(); npFramePausedByUs = true; }
				else if (!blocked && npFramePausedByUs) { npFramePausedByUs = false; resumeWebFrame(); }
			} catch (e) {}
			try { npPublishFrameSound("npApplyFrameMute"); } catch (e) {}
		}

		/** 传输动作的真实落点（能做到的才接线；做不到的控制器已经拦下并给说明）。
		 *  ①(NP-3) 语义（全部**不碰壁纸本体**：不改 iframe src、不调 showWebEl/applyFromStorage）：
		 *   play/pause  视频壁纸 ⇒ video.play()/pause()；有曲目清单 ⇒ 我们自己的 audio
		 *               （清单还没拉到就先拉，拉到了再放 —— 不静默失败）
		 *   prev/next   清单里的上一条/下一条（环形）
		 *   mute        既有设置项 `mute` + **真的落到元素**（video/audio/帧内 + 宿主 /media-audio 上报）
		 */
		function npTransport(op, section, value) {
			const s = section || readSection();
			const link = npLinkOn(s);
			/* ①(NP-4) 传输动作的**可查轨迹**（最近 32 条：op / value / 有没有真的落到某个元素）。
			   为什么要留它：真机上"点了没反应"这一类问题，界面与设置都看不出"动作到底走到哪一步"
			   （修前那次 NotAllowedError 就是这么消失的）。它只记账，零行为影响，排障与探针都读它。 */
			let did = "";
			const trace = () => npOpTrace(op, value, did || "no-target");
			try {
				/* ①(MEDIA-1 接线) **系统媒体优先**：卡片现在显示的是系统曲目时，按钮必须控系统播放器
				   （否则用户按"下一首"，动的是壁纸而不是他耳机里那首歌）。能力位先判：够不着就不发命令。 */
				if (npSystemMediaActive() && ["play", "pause", "restart", "next", "prev"].includes(op)) {
					const last = npSysLast || {};
					const capOk = (op === "next" ? last.canNext !== false : (op === "prev" ? last.canPrev !== false : true));
					if (capOk) {
						did = "system:" + op;
						trace();
						npSystemControl(op);
					...
					did = "system:no-cap:" + op;
					trace();
					return false;
				}
				if (op === "play" || op === "pause") {
					const vid = npActiveVideo(s);
					/* ①(NP-4)② 联动开 ⇒ 壁纸媒体一起动；关 ⇒ **一个字节都不碰壁纸**（画面不受控件影响）。 */
					if (vid && link) {
						if (op === "play") { npCardPaused = false; npPersistPaused(false); npSetUserPaused(vid, false); npPrimePlay(vid); did = "video.play"; }
						else { npWantPlay = false; npCardPaused = true; npPersistPaused(true); npSetUserPaused(vid, true); vid.pause(); did = "video.pause"; }
						npApplyFrameMute();
					} else if (vid && !link) {
						/* ①(NP-4 补 2026-09-20) **联动关 ⇒ 卡片只控声音**（用户第 B 条的原始诉求）：
						   暂停 = **静音那条音轨**（画面继续动），播放 = 取消静音；
						   `pause()/play()` 这种"整体暂停"只在联动打开时做。 */
						npGestureUnlock("card");
						if (op === "play") { npCardPaused = false; npPersistPaused(false); try { vid.muted = false } catch (e) {} did = "video.unmute(link-off)"; }
						else { npCardPaused = true; npPersistPaused(true); try { vid.muted = true } catch (e) {} did = "video.mute(link-off)"; }
						npOpTrace("link-off-transport", op, did);
						npApplyFrameMute();
					} else {
						const list = npTrackList(s);
						if (list && list.tracks.length) {
							if (op === "play") {
								npCardPaused = false; npPersistPaused(false);
								const a = npEnsureAudio();
								/* 还没装源（清单刚到/换过壁纸）⇒ 先装当前曲目再放 */
								if (a && !String(a.getAttribute("src") || "")) { npLoadTrack(true); did = "audio.load+play"; }
								else if (a) { const p = a.play(); if (p && p.catch) p.catch(() => {}); did = "audio.play"; }
							} else if (npAudio) { npCardPaused = true; npPersistPaused(true); npAudio.pause(); did = "audio.pause"; }
						} else if (!vid) {
							/* 清单还没到：拉一次，到位后由 applyNowPlaying 把键的状态刷新（不假装按过了） */
							npFetchTracks(s);
							did = "fetch-tracks";
						} else {
							/* ①(NP-4) 有壁纸媒体但**联动关** ⇒ 这里就是"一个字节都不碰"的那条路。 */
							did = "skip(link-off)";
						}
						/* ①(NP-3) 播放状态变了 ⇒ 帧内静音策略要重算（我们在放音时帧内强制静音） */
						npApplyFrameMute();
					}
				} else if (op === "next" || op === "prev") {
					did = npStepTrack(op === "next" ? 1 : -1, true) ? "step" : "step(nolist)";
				} else if (op === "restart") {
					/* ①(NP-3) 保留 op（兼容旧调用/排障），但 UI 里的"回到开头"已换成上一首/下一首。 */
					const vid = npActiveVideo(s);
					if (vid) { try { vid.currentTime = 0 } catch (e) {} }
					else if (npAudio) { try { npAudio.currentTime = 0 } catch (e) {} }
				} else if (op === "seek") {
					/* ①(NP-4)③ 拖动落点：`value` 是**相对当前媒体时长**的 0..1（控制器算好送进来）。
					   口径（与②的联动同一条，判据见 docs/NOW-PLAYING-DSH.md §7.8）：
					     · 联动开 ⇒ **真的落到当前媒体的 currentTime**（壁纸媒体 + 我们自己的播放器，
					       谁在放就落谁；视频档就是那个 <video>）；
					     · 联动关 ⇒ 只落到**我们自己的播放器**（"只影响声音"）；壁纸媒体一个字节都不碰。
					   轨道只有一个所有者（npResolveMedia 视频/曲目两条各自 return），所以
					   这里的 ratio 不会出现"同一根带子对应两个时长"的歧义。 */
					const r = Math.max(0, Math.min(1, Number(value) || 0));
					const vid = npActiveVideo(s);
					if (vid && link && isFinite(vid.duration) && vid.duration > 0) {
						try { vid.currentTime = r * vid.duration; did = "video.seek=" + vid.currentTime.toFixed(2) } catch (e) {}
					} else if (vid && !link) { did = "skip(link-off)" }
					if (npAudio && isFinite(npAudio.duration) && npAudio.duration > 0) {
						try { npAudio.currentTime = r * npAudio.duration; did += (did ? "+" : "") + "audio.seek=" + npAudio.currentTime.toFixed(2) } catch (e) {}
					}
				} else if (op === "volume") {
					/* ①(NP-4)① 电平（0..1）：写设置项 `npVolume`（0..100）并**真的落到元素**
					   （video / 我们自己的播放器 / web 帧 + shim 主音量）。与静音键是两件事：
					   静音键动的是既有 `mute` 开关，这里动的是电平。 */
					const level = Math.max(0, Math.min(1, Number(value) || 0));
					saveSection({ npVolume: Math.round(level * 100) });
					npApplyVolume(true);           /* 用户明确拖了电平 ⇒ 无论默认与否都落到实处 */
					did = "volume=" + Math.round(level * 100);
				} else if (op === "mute") {
					try { npGestureUnlock("card") } catch (e) {}   // C4：卡片上的显式操作 = 用户意图
					// ①(NP-1) 静音走**既有**设置项（mute）：这样视频/web 两条路径都由既有接线负责，
					//   不在这里另写一套；同时把状态如实报到宿主 media-audio 契约（WP-1 新增）。
					// ①(NP-3) 关键补齐：写完设置**必须真的落到元素**（下面这行 npApplyMute()）。
					// ①(NP-4) 静音是**壁纸级**设置（设置面板里就有这一行），不受联动开关管：
					//   关掉联动只影响"传输键要不要驱动壁纸的播放/进度"，不改变"静音"这条既有语义。
					const next = !(s.mute !== void 0 ? !!s.mute : true);
					saveSection({ mute: next });
					npApplyMute();
					did = "mute=" + next;
				}
			} catch (e) { mpwErr("npTransport", e); did = "error:" + String(e && e.message || e).slice(0, 60); }
			trace();
			setTimeout(() => { try { applyNowPlaying(readSection()); } catch {} }, 60);
		}

		/* ══════════════════════════════════════════════════════════════════════════════════
		   ①(MEDIA-1 接线 2026-09-20)「系统媒体」补充路径：把宿主读到的**系统正在播放**喂给卡片
		   ──────────────────────────────────────────────────────────────────────────────────
		   为什么是**另一条**路径（而不是塞进 `npResolveMedia`）：那个函数是**同步**的、解的是"**壁纸自己的**
		   媒体/音频轨"（封面来自 preview、标题来自 project.json）。系统媒体（MPRIS/SMTC）是**异步**的外部世界，
		   两者可以同时存在（壁纸在放自己的 BGM 时，Spotify 也在放）。口径：
		     · 宿主 `GET /media-session` 回 `available:false`（如本机无会话总线）⇒ **不接管**，卡片继续显示壁纸自己的媒体
		       —— 这是本机的**正常**结局，不是失败（诚实降级）；
		     · `available:true` ⇒ 用系统曲目覆盖标题/副标题/封面/时长，进度按 position 走；
		       `canNext/canPrev/canPlay` 决定卡片按钮的可用性；封面 `file://` 走宿主代理 `/media-art?player=`。
		   轮询纪律（本机是手机，这条不是可选项）：**≥2000ms**、只在开关打开且页面可见时跑、开关关掉立刻停。 */
		const NP_SYS_POLL_MS = 2000;          // 有系统媒体时：2s 一次（进度条要跟得上）
		const NP_SYS_IDLE_MS = 30000;         // **本机常态**（没有会话总线）：30s 一次
		/* 为什么不是死板的 `setInterval(2000)`：一次 `snapshot()` = 宿主机上**起一个进程**。本机（手机）
		   没有会话总线 ⇒ 每 2s 起一个必然查不到东西的进程是纯浪费（用户点名过资源纪律）。
		   所以用**自适应延时**：拿到系统媒体 ⇒ 2s；不可用/失败 ⇒ 30s 慢慢探。 */
		let npSysTimer = 0, npSysLast = null, npSysFails = 0, npSysProxyWarned = false, npSysDelay = NP_SYS_POLL_MS;
		function npSystemMediaStop() {
			try { if (npSysTimer) { clearTimeout(npSysTimer); npSysTimer = 0 } } catch (e) {}
		}
		/** 定时器工厂：**有 `unref` 就 unref**（Node/门禁环境）。
		 *  为什么必须这样：门禁把 client.js 跑在桩 DOM 里，断言跑完后**进程要能自己退出**——
		 *  一个挂着的 30s 定时器会让事件循环一直有活干 ⇒ 整条门禁"跑完了却不结束"（实测踩到：
		 *  `now-playing-test` 卡到 600s 超时）。浏览器里没有 `unref`，这一行等于不存在。 */
		function npSysTimerSet(fn, ms) {
			let t = null;
			try { t = setTimeout(fn, ms) } catch (e) { return null }
			try { if (t && typeof t.unref === "function") t.unref() } catch (e) {}
			return t;
		}
		/** 排下一次探测（单链，不叠定时器；`npSysTimer` 同时是"在跑"的哨兵）。 */
		function npSystemMediaSchedule(ms) {
			try {
				npSystemMediaStop();
				npSysDelay = Math.max(NP_SYS_POLL_MS, Math.min(NP_SYS_IDLE_MS, Number(ms) || NP_SYS_POLL_MS));
				npSysTimer = npSysTimerSet(() => { npSysTimer = 0; try { npSystemMediaTick() } catch (e) {} }, npSysDelay);
			} catch (e) {}
		}
		/** 把宿主快照映射到卡片（`kind:'system'` 只是个信息字段：组件侧对 `media.kind` 零分支）。 */
		function npApplySystemMedia(d) {
			try {
				const ctl = npEnsureCtl();
				if (!ctl || !d || d.available !== true) return false;
				const cover = d.artUrlKind === 'file'
					? (HOST_BASE + '/media-art?player=' + encodeURIComponent(String(d.player || '')))
					: (d.artUrl || '');
				if (d.artUrlKind === 'file' && !npSysProxyWarned) { npSysProxyWarned = true; try { mpwTrace("media-session:art-proxy", { player: d.player }) } catch (e) {} }
				ctl.setMedia({
					kind: 'system',
					title: String(d.title || ''),
					byline: [d.artist, d.album].filter(Boolean).join(' · ') || String(d.player || ''),
					cover: cover, total: d.duration ? Math.round(Number(d.duration) / 1000) : null,
					playing: !!d.playing, canPlay: !!d.canPlay, canNext: !!d.canNext, canPrev: !!d.canPrev,
					canVolume: true, system: true, player: String(d.player || ''), adapter: String(d.adapter || ''),
				});
				if (d.position !== null && d.position !== undefined) ctl.setProgress(Math.round(Number(d.position) / 1000));
				npSysLast = { at: Date.now(), player: String(d.player || ''), title: String(d.title || ''), playing: !!d.playing, canNext: !!d.canNext, canPrev: !!d.canPrev, canPlay: !!d.canPlay };
				return true;
			} catch (e) { mpwErr("npApplySystemMedia", e); return false }
		}
		/** 一次拉取 + 落卡（轮询的每一步都走它；`?describe=1` 不带 —— 那是诊断用的）。 */
		async function npRefreshSystemMedia() {
			try {
				const ctl = npEnsureCtl();
				if (!ctl || !ctl.isEnabled()) { npSystemMediaStop(); return null }
				const r = await fetch(HOST_BASE + '/media-session', { cache: 'no-store' });
				if (!r.ok) throw new Error('HTTP ' + r.status);
				const d = await r.json();
				npSysFails = 0;
				if (d && d.available === true) { npApplySystemMedia(d); return d }
				/* 不可用：**不接管**（卡片继续显示壁纸自己的媒体），但把原因留痕 —— 排障时一眼看到"为什么没接上"。 */
				try { window.__mpwSystemMedia = { available: false, reason: d && d.reason, at: Date.now() } } catch (e) {}
				return d || null;
			} catch (e) {
				npSysFails++;
				try { window.__mpwSystemMedia = { available: false, reason: 'fetch:' + String(e && e.message || e).slice(0, 80), fails: npSysFails, at: Date.now() } } catch (e2) {}
				return null;
			}
		}
		/** 一拍：不可见就**不发请求**（只把下一次排后一点）；可见才拉一次，并按结果决定节奏。 */
		async function npSystemMediaTick() {
			try {
				const ctl = npEnsureCtl();
				if (!ctl || !ctl.isEnabled()) { npSystemMediaStop(); return null }
				if (document.visibilityState !== 'visible') { npSystemMediaSchedule(NP_SYS_IDLE_MS); return null }
				const d = await npRefreshSystemMedia();
				if (npSysFails >= 3) { npSystemMediaSchedule(NP_SYS_IDLE_MS); return d }   // 连续失败：降到慢探（不再骚扰宿主，也不彻底停 ⇒ 后端起来了还能自己接上）
				npSystemMediaSchedule(d && d.available === true ? NP_SYS_POLL_MS : NP_SYS_IDLE_MS);
				return d;
			} catch (e) { npSystemMediaSchedule(NP_SYS_IDLE_MS); return null }
		}
		/** 起轮询（幂等）：第一次立刻探一拍，之后由 `npSystemMediaTick` 自适应续期。 */
		function npSystemMediaStart() {
			try {
				const ctl = npEnsureCtl();
				if (!ctl || !ctl.isEnabled()) return false;
				npSysFails = 0;
				npSysTimerSet(() => { try { npSystemMediaTick() } catch (e) {} }, 0);
				npSysTimer = npSysTimerSet(() => { npSysTimer = 0; try { npSystemMediaTick() } catch (e) {} }, NP_SYS_POLL_MS);
				return true;
			} catch (e) { return false }
		}
		/** 系统媒体是否是当前数据源（供传输动作改道；`npSysLast` 只记"真的接上过"）。 */
		function npSystemMediaActive() { try { return !!(npSysLast && Date.now() - npSysLast.at < 5 * NP_SYS_POLL_MS) } catch (e) { return false } }
		/** 传输动作改道：系统媒体在场时，卡片按钮控制**系统播放器**（能力位先判，够不着就不发）。 */
		async function npSystemControl(op) {
			try {
				const map = { play: 'playpause', pause: 'playpause', restart: 'playpause', next: 'next', prev: 'prev' };
				const sysOp = map[op];
				if (!sysOp) return { ok: false, reason: 'unsupported-op' };
				const last = npSysLast || {};
				const r = await fetch(HOST_BASE + '/media-control', {
					method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ op: sysOp }),
				});
				const d = await r.json().catch(() => null);
				try { npOpTrace('system-control', sysOp, (d && d.ok ? 'ok' : 'fail:' + String(d && d.reason || r.status)) + ' player=' + last.player) } catch (e) {}
				setTimeout(() => { try { npRefreshSystemMedia() } catch (e) {} }, 250);   // 控制完立刻回读一次状态
				return d || { ok: false, reason: 'bad-reply' };
			} catch (e) { return { ok: false, reason: 'fetch:' + String(e && e.message || e).slice(0, 60) } }
		}

		let npSlotAction = null;        // 宿主 slot `sidebar.footer.action` 的占用组件

		/** 控制器**单例**（幂等）：slot 组件与 apply 路径共用同一个。 */
		function npEnsureCtl() {
			if (npCtl) return npCtl;
			try {
				const mod = MPW_NP.createNowPlaying({
					math: MPW_NP_MATH,
					react: react,
					createRoot: (node) => {
						try { return require("react-dom/client").createRoot(node); } catch (e) { mpwErr("np createRoot", e); return null; }
					},
					t: (k) => { try { return mpwT(k); } catch (e) { return k; } },
					doc: document, win: window,
					log: console,
					onTransport: (op, value) => npTransport(op, readSection(), value),
				});
				npCtl = mod.createController();
				npSlotAction = mod.createSlotAction(npCtl);
				/* ①(NP-3) 真机排障用的**单例计数**：>1 就说明"控制器被造了两份"（叠节点/状态错位
				   这类问题的第一现场）。与 window.__mpwNowPlaying 同一处赋值，零行为影响。 */
				try {
					window.__mpwNpCtlSeq = (window.__mpwNpCtlSeq || 0) + 1;
					window.__mpwNowPlaying = npCtl;
					window.__mpwNowPlayingSlotAction = npSlotAction;
				} catch {}
			} catch (e) { mpwErr("npEnsureCtl", e); npCtl = null; npSlotAction = null; }
			return npCtl;
		}

		/**
		 * 宿主 slot `sidebar.footer.action` 的占用组件（**位置由宿主给**，我们不碰它的 DOM 结构）。
		 * 判据出处：该 slot 的出口 `div[data-slot="sidebar.footer.action"]` 在 `.hHd-Xa_footerActions` 里，
		 * 而 `footerActions` 与 `settingsArea` 是 `footArea` 的两个孩子 ⇒ 结构上**正在「设置」入口前面**。
		 * 开关关 ⇒ 返回 null：我们自己一个节点都不产生（那个出口 div 是宿主渲染的，不是我们注入的）。
		 */
		function MpwNowPlayingSlotAction(props) {
			let on = false;
			try {
				const s = readSection();
				on = s.npNowPlaying !== void 0 ? !!s.npNowPlaying : DEFAULT_NP_NOW_PLAYING;
			} catch (e) {}
			if (!on) return null;
			const ctl = npEnsureCtl();
			if (!ctl || !npSlotAction) return null;
			return react.createElement(npSlotAction, props || {});
		}

		/** 开关的落点（applyFromStorageInner 每次都会调用）。 */
		function applyNowPlaying(section) {
			const s = section || readSection();
			const on = s.npNowPlaying !== void 0 ? !!s.npNowPlaying : DEFAULT_NP_NOW_PLAYING;
			const ctl = npEnsureCtl();
			if (!ctl) return;
			if (!on) {
				...
				npDropAudio();
				npMediaCache = { key: "", at: 0, data: null };
				npMediaToken++;
				npSystemMediaStop();   // ①(MEDIA-1 接线) 开关关掉 ⇒ 系统媒体轮询也要停（不留后台定时器）
				return;
			}
			ctl.setEnabled(true);
			/* C3(2026-09-20) **换档即断开旧音源**（在任何解析/拉清单之前，且不依赖 fetch 成功）：
			   真机：从"有曲目清单且在播"的档切到"无清单"的容器档 ⇒ 旧 `<audio>` 继续放（用户："切掉了还在放他的声音"）。 */
			try {
				const idNow = npSourceId(s);
				if (npSrcIdPrev !== null && idNow !== npSrcIdPrev) npAudioHardReset("source-changed:" + npSrcIdPrev + "->" + idNow);
				npSrcIdPrev = idNow;
			} catch (e) {}
			/* ④(2026-09-20) 换了壁纸 ⇒ 卡片暂停状态（以及"我们暂停过帧内"的记账）一起清零：
			   否则上一张壁纸按下的暂停会把新壁纸的帧内音频永久压住。 */
			try {
				const wk = npScopeKey(npAudioScope(s)) || npWallpaperIdentity(s).key || "";
				if (wk !== npCardPausedKey) {
					npCardPausedKey = wk;
					/* ①(NP-5) 换档**不再无条件清零**：`npPaused` 是用户的意图（持久化），
					   换档/刷新后必须继续成立（旧写法在这里把"用户按过暂停"抹成 false ——
					   那正是"刷新后它把暂停自动换成播放"的最后一环）。 */
					try { const s0 = readSection(); npCardPaused = s0.npPaused !== void 0 ? !!s0.npPaused : DEFAULT_NP_PAUSED } catch (e) { npCardPaused = false }
					npFramePausedByUs = false;
				}
			} catch (e) {}
			/* ①(NP-5) **按持久化的用户意图落实可听状态**（刷新/换档后第一件事；不依赖任何 fetch） */
			try { npApplyPersistedPause(s); } catch (e) {}
			/* ①(NP-4) 换了壁纸/换了源 ⇒ "用户按过暂停"清零（新源按自动播放的规则重新起播）。 */
			/* ①(NP-4) 这里**不**再判"源变了没有"：`video.getAttribute('src')` 是**解析后**的 URL
			   （`/api/mpkg-wallpaper/custom-folder/…`），而 `section.image` 是 `host:?…` 形态，
			   两者恒不相等 ⇒ 每次 apply 都会把"用户按过暂停"清掉、60ms 后又把播放续上
			   （真机读数：`pause → video.pause` 之后紧跟 `prime: calling-play`）。
			   换源时清零这件事由 `showVideoEl` 在**真的重设 src 那一行**做（那里才拿得到真相）。 */
			/* ①(NP-3) 视频壁纸：先把媒体事件接上（元数据到了要重解，否则音轨判定会一直停在旧值）。
			   ①(NP-4) 起播门控也在这里补一次：`showVideoEl` 只在**设置变化**那条路径上跑，
			   而"刷新页面"那条路径上视频是被 `applyFromStorage` 装进来的 —— 不在这里补，
			   刷新之后就没有人再调 play()（真机读数：全程只调用过一次 play()，还被拒）。 */
			try {
				const vid = npActiveVideo(s);
				if (vid) {
					npWatchVideo(vid);
					/* C(2026-09-20) hidden 期间**不做补起播**（真机"过一会儿又响一下"就是这条路径；
					   npPrimePlay 内部还有一道同样的闸，这里是显式留痕，便于 __mpwNpOps 归因） */
					if (vid.paused && !npPlayBlocked && !mpwHiddenAudioBlock()) npPrimePlay(vid);
					else if (vid.paused && mpwHiddenAudioBlock()) npOpTrace("apply-prime", null, "skipped:hidden");
				}
			} catch (e) {}
			/* ①(NP-4)② 联动开关落点（只写一个可查询属性 + 影响媒体源的 canPlay/canSeek 派生量）。 */
			try { ctl.setLink(npLinkOn(s)); } catch (e) {}
			/* ①(NP-4 补)「联动 关→开」按卡片当前状态**对齐**壁纸（关→不动，见 mpwLinkAlign）。
			   真机现场：暂停状态下关掉开关 ⇒ 之后卡片怎么按壁纸都起不来（用户第 B 条）。 */
			try {
				const linkNow2 = npLinkOn(s);
				const act = mpwLinkAlign(linkNow2, npLinkPrev, npCardPaused);
				if (act !== "none") {
					const vid2 = npActiveVideo(s);
					if (vid2) {
						if (act === "resume") { npSetUserPaused(vid2, false); npWantPlay = true; try { npPrimePlay(vid2) } catch (e) {} }
						else { npSetUserPaused(vid2, true); npWantPlay = false; try { vid2.pause() } catch (e) {} }
						npOpTrace("link-align", act, "video");
					} else if (act === "pause") { try { if (npAudio) npAudio.pause() } catch (e) {} npOpTrace("link-align", act, "audio") }
					else { try { if (npAudio && !npCardPaused) { const p = npAudio.play(); if (p && p.catch) p.catch(() => {}) } } catch (e) {} npOpTrace("link-align", act, "audio") }
					try { npApplyFrameMute() } catch (e) {}
				}
				npLinkPrev = linkNow2;
			} catch (e) {}
			/* ①(NP-4 补) 联动关闭时如实告知"卡片只控声音"（曲目档/帧档的副标题） */
			try {
				if (!npLinkOn(s)) {
					const mm = npResolveMedia(s);
					if (mm && mm.byline && !/linkOff|仅控声音/.test(String(mm.byline))) { /* 文案在 npResolveMedia 里统一加，见下 */ }
				}
			} catch (e) {}
			/* ①(NP-4)① 电平：刷新/切壁纸后把 `npVolume` 补落到真实元素上。 */
			try { npApplyVolume(); } catch (e) {}
			ctl.setMedia(npResolveMedia(s));
			/* ①(MEDIA-1 接线 2026-09-20) 壁纸自己的媒体先落卡（同步、不变），**再**异步补系统媒体：
			   宿主回 `available:false`（本机无会话总线）⇒ 什么都不覆盖，卡片还是壁纸自己的标题/预览。 */
			try { npSystemMediaStart(); } catch (e) {}
			/* ①(NP-3) 清单/播放器：有目录作用域才拉清单；清单到了之后把当前曲目装进播放器
			   （**只装源、不自动播放** —— 浏览器自动播放策略 + "不替用户按播放"两条都要求这样）。 */
			try {
				if (ctl.isEnabled()) {
					if (!npTrackList(s)) npFetchTracks(s);
					/* 清单已经有 ⇒ 把当前曲目装进播放器（幂等：同一条不会重设 src，**不自动播放**） */
					else npLoadTrack(false);
				}
			} catch (e) { mpwErr("applyNowPlaying(tracks)", e) }
		}
		/** 静态渐变兜底背景（视频不可用时）。 */
		...
		let webFramePausedByUs = false; // ⑤ 帧内媒体的暂停是**我们**按下的（恢复时只撤销我们做的）
		let powHidNpWasPlaying = false; // ⑤ 隐藏前"我们自己的播放器"是否在放（决定回来要不要续播）
		let powHidWallWasPlaying = false; // C(2026-09-20) 隐藏前**壁纸媒体**是否在播（同上："按原状态"续播）
		/** 当前壁纸视频暂停（停住画面）。 */
		...
		function pauseWebFrame() {
			try {
				...
				if (doc) {
					const els = doc.querySelectorAll("video,audio");
					for (let i = 0; i < els.length; i++) { try { if (!els[i].paused) { els[i].pause(); n++ } } catch {} }
				}
				// ⑤(2026-09-20) 记账：只有"我们按下的暂停"才允许由 resumeWebFrame 撤销
				//   （旧写法恢复时会 play() **作者自己**停着的媒体 —— 省电档默认打开后这会变成常态问题）。
				webFramePausedByUs = true;
				return n;
			} catch { return 0 }
		}
		...
		function resumeWebFrame() {
			try {
				const { frame } = bgElements();
				if (!frame) return;
				if (!webFramePausedByUs) return;          // ⑤ 我们没停过 ⇒ 一个字节都不碰
				webFramePausedByUs = false;
		...
		function pauseWallpaperVideo() {
			try {
				...
				/* C(2026-09-20) 记下"暂停之前壁纸在不在播"：恢复时**按原状态**续播
				   （原本就暂停的绝不自作主张起播 —— 用户要的是"切页静音"，不是"回来自己开始放"）。 */
				try { powHidWallWasPlaying = !!(video && !video.paused && video.src) } catch (e) {}
				if (video && !video.paused) video.pause();
				if (frame && frame.style.display !== "none") pauseWebFrame();
				/* ⑤(2026-09-20) **我们自己的播放器**也要停：曲目档的声源是 `npAudio`，
				   旧写法只 pause 壁纸 video/帧内媒体 ⇒ 切页后卡片里的歌**继续响**
				   （用户现场第 2 条："via 浏览器切到别的页面/标签时声音不暂停"）。 */
				try {
					powHidNpWasPlaying = !!(npAudio && !npAudio.paused && (npAudio.currentSrc || npAudio.getAttribute("src") || npAudio.src));
					if (powHidNpWasPlaying && npAudio) npAudio.pause();
				} catch (e) {}
				/* 静音兜底：pause() 对 WebAudio 类作者实现无效 ⇒ 帧内再压一层静音（恢复时由 applyWebMute 还原） */
				try { if (frame) { frame.muted = true } } catch (e) {}
				try { webShimCall(frame, { op: "policy", muted: true, speed: npPolicySpeed() }) } catch (e) {}
				/* C(2026-09-20) 壁纸 `<video>` 也**强制 muted**：即使某条路径（转码重试/起播/电平重放）
				   在 hidden 期间把它 play() 起来，也一个字节都出不去（恢复时由 npApplyMute 还原）。 */
				try { if (video) video.muted = true } catch (e) {}
				try { npPublishFrameSound("power-pause"); } catch (e) {}
			} catch {}
		}
		/** 当前壁纸视频恢复播放（用户手动暂停时恢复——由调用方判断, 见 toggleWallPause）。 */
		/** @param {boolean} force 用户**显式**要求恢复（点「播放壁纸」）时传 true；
		 *  省电/可见性恢复那条路不传 ⇒ 只有"暂停前确实在播"才续播（C 的"按原状态"）。 */
		function resumeWallpaperVideo(force) {
			try {
				const s = readSection();
				if (!(s.enabled !== void 0 ? !!s.enabled : true)) return;
				const { video, frame } = bgElements();
				if (video && video.paused && video.src && (force === true || powHidWallWasPlaying) && !mpwHiddenAudioBlock()) {
					try { const p = video.play(); if (p && p.catch) p.catch(() => {}); } catch {}
				}
				// ⑤ 只撤销**我们**按下的暂停（webFramePausedByUs 记账；作者自己停着的媒体不碰）
				if (frame && frame.style.display !== "none") resumeWebFrame();
				/* ⑤ 静音回落到设置/卡片口径（不能把"卡片暂停"那条压住的静音也一起放开） */
				try { npApplyFrameMute(); } catch (e) {}
				/* ⑤ 隐藏前在放才续播；隐藏前就是暂停的绝不自作主张播放 */
				try {
					if (powHidNpWasPlaying && npAudio && npAudio.paused && !npCardPaused) {
						const p = npAudio.play(); if (p && p.catch) p.catch(() => {});
					}
					powHidNpWasPlaying = false;
				} catch (e) {}
				powHidWallWasPlaying = false;
			} catch {}
		}
		...
		function updatePowerPause() {
			try {
				const s = readSection();
				const hiddenOn = s.powPauseHidden !== void 0 ? !!s.powPauseHidden : DEFAULT_POW_PAUSE_HIDDEN;
				const blurOn = s.powPauseBlur !== void 0 ? !!s.powPauseBlur : false;
				const battOn = s.powPauseBattery !== void 0 ? !!s.powPauseBattery : false;
				let hiddenNow = powHiddenNow;
				try { if (!hiddenNow && typeof document !== "undefined" && document && document.hidden === true) hiddenNow = true } catch (e) {}
				const shouldPause = (hiddenOn && hiddenNow) || (blurOn && !powFocusedNow) || (battOn && powBattery && powBattery.charging === false);
		...
		/** 注册省电监听（一次性）。
		 *  ⑤(2026-09-20 真机修复) 切页/隐藏页**必须**把我们的音频与帧内音频一起暂停 —— 这是浏览器
		 *  的基本礼仪（不是"省电"这种可选优化），所以 `powPauseHidden` 的默认档改成 **true**，
		 *  且这里的落点补齐了三处旧写法漏掉的：
		 *    · 我们自己的 `<audio>`（曲目档）：旧写法只 pause 壁纸 video + 帧内媒体；
		 *    · 帧内媒体的暂停记账（恢复时只撤销我们按下的暂停，见 webFramePausedByUs）；
		 *    · `pagehide/pageshow`（bfcache 切页不一定派发 visibilitychange）；
		 *    · 恢复时按**原状态**续播（隐藏前在放才续播，隐藏前就是暂停的绝不自作主张播放）。 */
		function setupPowerSave() {
			...
			// ⑤ pagehide/pageshow：bfcache / 切标签在部分浏览器不派发 visibilitychange
			try {
				window.addEventListener("pagehide", () => { powHiddenNow = true; updatePowerPause(); });
				window.addEventListener("pageshow", () => {
					try { powHiddenNow = document.hidden === true } catch (e) { powHiddenNow = false }
					updatePowerPause();
				});
			} catch {}
		...
		function setWallPausedByUserState(paused, source) {
			try {
				if (source === "user") {
					...
					else resumeWallpaperVideo(true);   // C: 用户显式点"播放壁纸" ⇒ 强制恢复（不受"原状态"限制）
		...
		try {
			globalThis.__mpwPersist = {
				...
				/* ①(2026-09-19) "部分档写入"的真实入口：`writeSection` 拿到**没带源字段**的 next 时，
				   粘性护栏必须把源保住（真机上 `initHostSettings` 的宿主合成结果、`mpwMarkPersistFail`
				   的留痕写都会经过这条路）。暴露它才能把这条护栏变成断言而不是"看代码觉得对"。 */
				writePartial: (next) => { try { writeSection(next || {}, true); return true } catch (e) { return String(e && e.message || e) } },
				onMsg: (fn) => mpwPersistOnMsg(fn),
				takePending: () => mpwPersistTakePending(),
				failKey: MPW_PERSIST_FAIL_KEY,
				limits: { lsMax: MPW_LS_MAX_BYTES, spill: MPW_LS_SPILL_BYTES },
				key: STORE_KEY,
				/* ①(2026-09-19 设置持久化轮) "一次写不得静默抹掉字段"这条契约的测试入口
				   （tools/settings-persist-test.mjs 用；生产不引用）。暴露的都是**真实现**：
				     merge/sticky/reconcile/srcMissing —— 纯函数（合并不替换 / 源字段粘性 / 两处裁决）
				     deriveFromKey  —— 按 mpkgKey 反推源（真走宿主只读路由）
				     heal           —— 半残档自愈（同步部分 + 后台推导）
				     bootSettle     —— boot 收尾（把延迟的那次落盘补上）
				     write/commit   —— **面板同一条写路径**（writeSection 的三道护栏 + 时间戳 + 宿主 PUT）
				     stores         —— 两处存储的原始读数（含元键），供"逐字段 diff"断言 */
				merge: (base, patch) => mpwMergeSection(base, patch),
				sticky: (base, next) => mpwStickySource(base, next),
				reconcile: (local, host) => mpwReconcileStores(local || {}, host || {}),
				srcMissing: (s) => mpwSrcMissing(s || readSection()),
				deriveFromKey: (key) => mpwDeriveSourceFromKey(key || readSection().mpkgKey),
				heal: () => { try { return mpwHealHalfArchive() } catch (e) { return String(e && e.message || e) } },
				healState: () => Object.assign({}, MPW_HEAL, { last: MPW_HEAL.last ? Object.assign({}, MPW_HEAL.last) : null, sig: mpwHealSig }),
				healReset: () => { mpwHealSig = ""; MPW_HEAL.attempted = 0; MPW_HEAL.healed = 0; MPW_HEAL.failed = 0; MPW_HEAL.last = null; mpwBackupSource = null; return true },
				bootSettle: (src) => mpwBootSettle(src || "test"),
				bootPending: () => !!mpwBootPending,
				storeDirty: () => !!mpwStoreDirty,
				flushStore: () => { try { mpwFlushStore(); return true } catch (e) { return String(e && e.message || e) } },
				commit: (patch) => {
					try {
						const next = mpwMergeSection(readSection(), patch || {});
						writeSection(next, true);
						return next;
					} catch (e) { return String(e && e.message || e) }
				},
				lsRaw: () => { try { return mpwLoadLocalRaw() } catch { return {} } },
				localStamp: () => mpwStampOf((() => { try { return mpwLoadLocalRaw() } catch { return {} } })(), "local"),
				metaKeys: MPW_META_KEYS.slice(),
				srcKeys: MPW_SRC_KEYS.slice(),
				stampKeys: { local: MPW_STAMP_LOCAL, host: MPW_STAMP_HOST },
				srcLostKey: MPW_SRC_LOST_KEY,
			};
		} catch {}
		...
		/* ══════════════════════════════════════════════════════════════════════════════════
		   C2(2026-09-20 真机修复)「音频审计」：把每一次**出声**留痕并可上报
		   ──────────────────────────────────────────────────────────────────────────────────
		   现场（用户原话）：「莫名其妙播放音频又出现了，我刚才什么都没动，他又开始播放了……
		   有时候可能响几声，有时候就没有了，听起来和非常卡顿一样（因为它不是连续播放的）」；
		   30s 连续采样（唯一媒体元素 `#mpw-bgVideo`，`paused=false, muted=true`、`play()` 只被我们调过一次）
		   **没抓到**那一刻 ⇒ 需要一个常驻、低开销、能自证"这条声音是从哪个元素、由谁、什么时候放出来的"的审计。
		   抓什么（都要带**调用栈摘要**与**当时状态**）：
		     · `HTMLMediaElement.prototype.play`（谁按的 play）
		     · `volume` / `muted` setter（谁把它从静音改成可听 —— "响几声"最常见的直接原因）
		     · `Audio` 构造、`AudioContext/webkitAudioContext` 构造与 `decodeAudioData`（绕过 `<audio>` 标签的声源）
		     · 我们自己的 `npPrimePlay` / `npApplyMute` / `npApplyVolume` / `npAudio`（对照面）
		   每条：`{t, kind, who(栈前 3 帧), el{tag,id,cls,connected,src摘要}, muted, volume, paused, currentTime, hidden, np{on,link,mute}}`。
		   环形缓冲 200 条（`window.__mpwAudioAudit = {list, clear, push, suspicious}`，有界、脱敏、截断）。
		   **自动上报**：命中"可疑"特征（`connected===false` 的元素在播 / **hidden 期间** play 或取消静音）
		   ⇒ 立刻 POST `/diag` 一条 `audio-audit` 信标（节流 5s/条）——用户"什么都没动却又响了"的
		   下一次就能直接从上报里读到来源。开关 `?npaudit=0` 完全不装（零开销）；缺省装。
		   ══════════════════════════════════════════════════════════════════════════════════ */
		const MPW_AUDIT_MAX = 200;
		let mpwAuditInstalled = false;
		let mpwAuditLastBeacon = 0;
		function mpwAuditOn() {
			try { return new URLSearchParams(location.search).get("npaudit") !== "0" } catch (e) { return true }
		}
		function mpwAuditWho() {
			try {
				const st = String(new Error().stack || "").split("\n").slice(2, 5).map((l) => l.trim().replace(/^at\s+/, "").slice(0, 90));
				return st.join(" <- ");
			} catch (e) { return "" }
		}
		/** ①(2026-09-20 彻查「静音下仍出声」) 这一条记录**来自哪个窗口**：
		 *  主页面 / 同源 iframe（壁纸帧、别的插件帧…）。跨源帧我们的钩子装不进去 ⇒ 如实标 `foreign:true`，
		 *  绝不把"看不到"说成"没有"（用户能据此知道"声音来自某个跨源帧，本插件管不到它的 WebAudio"）。 */
		function mpwAuditWin(win) {
			try {
				const w = win || window;
				const isTop = (() => { try { return w === w.top } catch (e) { return false } })();
				let url = "";
				try { url = String((w.location && w.location.href) || "") } catch (e) { url = "cross-origin" }
				return { top: isTop, url: url.replace(/token=[^&]*/, "token=…").slice(0, 120), foreign: url === "cross-origin" };
			} catch (e) { return { top: true, url: "", foreign: false } }
		}
		function mpwAuditEl(el) {
			try {
				if (!el) return null;
				const src = String(el.currentSrc || (el.getAttribute && el.getAttribute("src")) || "");
				return {
					tag: String(el.tagName || ""), id: String(el.id || ""), cls: String(el.className || "").slice(0, 60),
					connected: el.isConnected !== false,
					src: src.replace(/token=[^&]*/, "token=…").slice(0, 90),
					hasSrcAttr: !!(el.getAttribute && el.getAttribute("src")),
					/* 声音素材的**归属指纹**（用户问"这声音是谁放的"时，一眼能分类）：
					   ours = 我们自己的元素/接口；whale = 那个第三方鲸鱼插件；frame = 壁纸帧内；other = 未知 */
					owner: (() => {
						try {
							const own = (String(el.id || "") === "mpw-bgVideo") || /(^|\s)mpw[-_]/.test(String(el.className || "")) || /\/api\/mpkg-wallpaper\//.test(src);
							if (own) return "ours";
							if (/\/dsh-whale\//.test(src)) return "whale-widget";
							if (el.ownerDocument && el.ownerDocument !== document) return "frame";
							return "other";
		...
		function mpwAuditNp() {
			try {
				const s = readSection();
				return { on: s.npNowPlaying !== void 0 ? !!s.npNowPlaying : DEFAULT_NP_NOW_PLAYING, link: npLinkOn(s), mute: s.mute !== void 0 ? !!s.mute : true };
			} catch (e) { return null }
		}
		function mpwAuditPush(kind, el, extra, win) {
			try {
				if (!mpwAuditOn()) return null;
				const rec = Object.assign({
					t: Date.now(), kind: String(kind || ""), who: mpwAuditWho(), el: mpwAuditEl(el), win: mpwAuditWin(win),
					muted: el ? !!el.muted : null, volume: el && typeof el.volume === "number" ? Number(el.volume.toFixed(2)) : null,
					paused: el ? !!el.paused : null, currentTime: el && typeof el.currentTime === "number" ? Number(el.currentTime.toFixed(2)) : null,
					hidden: (() => { try { return document.hidden === true } catch (e) { return null } })(),
					np: mpwAuditNp(),
				}, extra || {});
				const store = window.__mpwAudioAudit;
				const list = store && Array.isArray(store.list) ? store.list : [];
				list.push(rec);
				if (list.length > MPW_AUDIT_MAX) list.splice(0, list.length - MPW_AUDIT_MAX);
				if (store) store.list = list;
				/* 「可疑」= 会**真的出声**的转变（家长口径）：
				   ① `!paused && !muted && volume>0`（可听播放）—— 无论是谁触发的；
				   ② 元素已从 DOM 摘除却仍在播（`isConnected=false`）；
				   ③ hidden 期间的 play / 取消静音。
				   命中 ⇒ 立刻把**审计窗口**（最近 12 条）POST 到 /diag（脱敏、截断、5s 节流）。 */
				const audible = !!(rec.el && rec.paused === false && rec.muted === false && Number(rec.volume) > 0);
				/* ①(2026-09-20 用户点名「静音状态下还是突然冒出来的声音」) **这一条**才是那次投诉的直接判据：
				   设置项 `mute === true`（面板写着静音），元素却处在**可听**状态（`!paused && !muted && volume>0`）
				   —— 不管是谁把它改成这样的（我们的哪条路径 / 壁纸帧内 / 别的插件），都必须立刻上报并带上**是谁改的**。
				   旧判据只有"可听播放"这一条泛判据（设置是开是关都报），既漏了"静音下可听"这个组合的强调，
				   也没把 `np.mute` 与可听状态**放在一起**判断。 */
				const muteOnAudible = !!(rec.np && rec.np.mute === true && audible);
				/* ①(2026-09-20 用户澄清后的补网) 用户明确："别人插件的音效必须我去交互他才会出现，并不会自己出现"
				   ⇒ 鲸鱼那条只解释了"交互时响"，**解释不了"什么都没动却响"**。真机上不需要交互就能出声、而我们
				   `muted` 又管不到的，只剩 **WebAudio**（Live2D 角色语音 / 壁纸自己用 AudioContext 播的声）：
				   它不碰任何 `<audio>/<video>`，所以 `audible` 这条永远抓不到它。判据：
				     · `webaudio-start`（`AudioBufferSourceNode.start()` —— 真的开始出声那一刻）；
				     · 或者 `AudioContext` 被 resume 到 running（自动播放解锁/作者主动 resume）；
				   只要**静音设置开着**（用户要的是安静）就立刻上报，并带上**是哪个窗口**（帧 URL）与 buffer 时长。 */
				const webaudioOnMuted = !!(rec.np && rec.np.mute === true)
					&& (kind === "webaudio-start" || kind === "audioctx-resume");
				const suspicious = audible
					|| muteOnAudible
					|| webaudioOnMuted
					|| !!(rec.el && rec.el.connected === false && rec.paused === false)
					|| (kind === "play" && rec.hidden === true)
					|| ((kind === "unmute" || kind === "volume") && rec.hidden === true && rec.muted === false);
				if (suspicious) {
					if (store) { try { store.suspicious = (store.suspicious || 0) + 1 } catch (e) {} }
					if (Date.now() - mpwAuditLastBeacon > 5000) {
						mpwAuditLastBeacon = Date.now();
						try {
							mpwWebDiag("audio-audit", {
								trigger: webaudioOnMuted ? "webaudio-on-muted"
									: (muteOnAudible ? "mute-on-but-audible"
										: (audible ? "audible-playback" : (rec.el && rec.el.connected === false ? "detached-playing" : "hidden-transition"))),
								record: rec, window: list.slice(-12), count: list.length,
								visibility: (() => { try { return document.visibilityState } catch (e) { return null } })(),
							});
						} catch (e) {}
						try { console.warn("[dsh-mpkg-wallpaper][audio-audit] 可疑出声:", JSON.stringify(rec).slice(0, 400)); } catch (e) {}
					}
				}
				return rec;
			} catch (e) { return null }
		}
		/** ①(2026-09-20 彻查「静音下仍出声」) 对**任意窗口**装审计钩子。
		 *  为什么必须能装进子窗口：真机上"突然响一声"的声源有两类是我们看不到的 ——
		 *    ① **壁纸帧内**的 `<audio>/<video>`（同源模式帧有**自己独立的 `HTMLMediaElement.prototype`**，
		 *       在主窗口上打补丁对它完全无效 ⇒ 旧审计对帧内声音是"瞎的"）；
		 *    ② 别的插件帧里的声音（同一原因）。
		 *  跨源帧装不进去（浏览器安全模型）⇒ 由 `mpwAuditWin()` 如实标 `foreign:true`，不谎报"没有声音"。
		 *  `who` 的栈里会带上帧 URL，用户/我们都能据此指认"这条声音是谁放的"。 */
		function mpwAuditPatchWin(win) {
			if (!win) return false;
			try { if (win.__mpwAuditedWin) return false } catch (e) { return false }
			try { win.__mpwAuditedWin = true } catch (e) { /* 只读窗口也继续装 */ }
			const push = (kind, el, extra) => { try { return mpwAuditPush(kind, el, extra, win) } catch (e) { return null } };
			/* ① `<audio>/<video>`：play / muted / volume */
			try {
				const P = win.HTMLMediaElement && win.HTMLMediaElement.prototype;
				if (P && typeof P.play === "function" && !P.__mpwAudited) {
					const origPlay = P.play;
					P.play = function () { push("play", this); return origPlay.apply(this, arguments) };
					for (const prop of ["muted", "volume"]) {
						try {
							const d = Object.getOwnPropertyDescriptor(P, prop);
							if (!d || !d.set || d.set.__mpwAudited) continue;
							const set = d.set, get = d.get;
							const wrapped = function (v) {
								const bMuted = !!this.muted, bVol = this.volume;
								set.call(this, v);
								const kind = prop === "muted" ? (this.muted ? "mute" : "unmute") : "volume";
								if (prop === "muted" ? bMuted !== !!this.muted : bVol !== this.volume) push(kind, this, { before: { muted: bMuted, volume: bVol } });
								return v;
							};
							wrapped.__mpwAudited = true;
							Object.defineProperty(P, prop, { configurable: true, enumerable: d.enumerable, get: get, set: wrapped });
						} catch (e) {}
					}
					P.__mpwAudited = true;
				}
			} catch (e) {}
			/* ② `new Audio(...)`（绕过 DOM 标签的声源，最常见的"响一声"） */
			try {
				if (typeof win.Audio === "function" && !win.Audio.__mpwAudited) {
					const OrigAudio = win.Audio;
					const Wrapped = function () { const a = new OrigAudio(...arguments); try { push("new-Audio", a, { args: Array.prototype.slice.call(arguments).join(",").slice(0, 80) }) } catch (e) {} return a };
					Wrapped.__mpwAudited = true;
					try { Wrapped.prototype = OrigAudio.prototype } catch (e) {}
					win.Audio = Wrapped;
				}
			} catch (e) {}
			/* ③ WebAudio：构造 / decodeAudioData / **真的出声那一刻**（AudioBufferSourceNode.start）
			   ④(2026-09-20 补) 只记"建了 AudioContext"是没用的：Live2D 角色语音那类声源
			   走 `AudioBufferSourceNode.start()` 出声，**不碰任何 `<audio>` 标签** ——
			   这正是"设置静音却仍然响一声"最可能的声源，必须记到"谁在什么时候 start 了它"。 */
			for (const key of ["AudioContext", "webkitAudioContext"]) {
				try {
					const Orig = win[key];
					if (typeof Orig !== "function" || Orig.__mpwAudited) continue;
					const Wrapped = function () { const c = new Orig(...arguments); try { push("AudioContext", null, { ctor: key, state: c && c.state }) } catch (e) {} return c };
					Wrapped.__mpwAudited = true;
			...
			try {
				...
				const proto = AC && AC.prototype;
				if (proto && typeof proto.decodeAudioData === "function" && !proto.__mpwAudited) {
					const orig = proto.decodeAudioData;
					proto.decodeAudioData = function () { push("decodeAudioData", null, { ctxState: this && this.state }); return orig.apply(this, arguments) };
					proto.__mpwAudited = true;
				}
				if (proto && typeof proto.resume === "function" && !proto.__mpwResumed) {
					const origResume = proto.resume;
					proto.resume = function () { push("audioctx-resume", null, { ctxState: this && this.state }); return origResume.apply(this, arguments) };
					proto.__mpwResumed = true;
				}
			} catch (e) {}
			for (const [nodeKey, nodeName] of [["AudioBufferSourceNode", "buffer-source"], ["OscillatorNode", "oscillator"], ["ConstantSourceNode", "constant"]]) {
				try {
					const Ctor = win[nodeKey];
					const np = Ctor && Ctor.prototype;
					if (np && typeof np.start === "function" && !np.__mpwAudited) {
						const origStart = np.start;
						np.start = function () {
							let ctxState = null, dur = null;
							try { ctxState = this.context && this.context.state } catch (e) {}
							try { dur = this.buffer && this.buffer.duration } catch (e) {}
							push("webaudio-start", null, { node: nodeName, ctxState: ctxState, bufferSec: dur == null ? null : Number(Number(dur).toFixed(3)), ourCtx: !!this.context });
							return origStart.apply(this, arguments)
						};
						np.__mpwAudited = true;
					}
				} catch (e) {}
			}
			/* ④ 递归同源子帧（帧内元素有自己的原型对象；跨源会抛 ⇒ 记一条"装不上"的痕） */
			try {
				for (let i = 0; i < win.frames.length; i++) {
					try { mpwAuditPatchWin(win.frames[i]) } catch (e) { try { mpwAuditPush("frame-cross-origin", null, { idx: i, reason: String(e && e.name || e).slice(0, 40) }, win) } catch (e2) {} }
				}
			} catch (e) {}
			return true;
		}
		/** 新出现的 iframe（壁纸帧常是晚一步插进来的）⇒ 重新扫一遍；每秒一次、上限 60 次，零长尾开销。 */
		let mpwAuditFrameScans = 0;
		function mpwAuditScanFrames() {
			try {
				if (mpwAuditFrameScans++ > 60) return;
				mpwAuditPatchWin(window);
			} catch (e) {}
		}
		function mpwInstallAudioAudit() {
			if (mpwAuditInstalled || !mpwAuditOn()) return false;
			mpwAuditInstalled = true;
			try {
				window.__mpwAudioAudit = window.__mpwAudioAudit || { list: [], suspicious: 0 };
				window.__mpwAudioAudit.push = mpwAuditPush;
				window.__mpwAudioAudit.clear = () => { try { window.__mpwAudioAudit.list.length = 0 } catch (e) {} };
				window.__mpwAudioAudit.scanFrames = () => { try { mpwAuditFrameScans = 0; mpwAuditScanFrames(); return true } catch (e) { return false } };
			} catch (e) {}
			try { mpwAuditPatchWin(window); } catch (e) {}
			/* ①(2026-09-20) 窗口清单：`webaudio-on-muted` 上报里只有 `win.url`，真机上还要能回答
			   "这个帧是什么"（壁纸帧？哪个插件帧？里面有没有 Live2D？）。纯只读、无副作用。 */
			try {
				window.__mpwAudioAudit.frames = () => {
					const out = [];
					const walk = (w, depth) => {
						try {
							let url = ""; try { url = String(w.location.href || "") } catch (e) { url = "cross-origin" }
							let live2d = null, media = 0;
							try {
								const d = w.document;
								media = d ? d.querySelectorAll("audio,video").length : 0;
								live2d = !!(d && (d.querySelector("canvas") && /live2d|model3\.json|loadJson\.json/i.test(String(d.documentElement && d.documentElement.innerHTML || "").slice(0, 200000))));
							} catch (e) { live2d = null }
							out.push({ depth, url: url.replace(/token=[^&]*/, "token=…").slice(0, 140), media: media, looksLive2D: live2d, top: depth === 0 });
						} catch (e) {}
						try { for (let i = 0; i < w.frames.length; i++) walk(w.frames[i], depth + 1) } catch (e) {}
					};
					walk(window, 0);
					return out;
				};
			} catch (e) {}
			try { setInterval(mpwAuditScanFrames, 1000); } catch (e) {}
			try { mpwTrace("audio-audit:installed", {}); } catch (e) {}
			return true;
		}
		try { mpwInstallAudioAudit(); } catch (e) {}

		/* ②④⑤⑥(2026-09-20) 壁纸生命周期钩子（tools/wallpaper-lifecycle-test.mjs 用；生产不引用）。
		   ⚠ 暴露的都是**生产实现本身**（不是副本）：换档成套重写 / 挂载裁决 / 形状绑定 /
		   预览候选链 / 弹层类污染自愈 / 武装前验活 / 沙箱降级 / 帧内音频归属。 */
		try {
			globalThis.__mpwLifecycleTest = {
				// ② 换档
				switchPatchFull: (patch) => { try { return mpwSwitchPatchFull(patch) } catch (e) { return { err: String(e && e.message || e) } } },
				normalizeSwitchPatch: (patch) => { try { return mpwNormalizeSwitchPatch(patch) } catch (e) { return { err: String(e && e.message || e) } } },
				switchKeys: MPW_SWITCH_SRC_KEYS.slice(),
				// A 交互音分类
				classifyWebAudio: (input) => { try { return mpwClassifyWebAudio(input) } catch (e) { return [] } },
				filterVoiceTracks: (tracks, table, mode) => { try { return mpwFilterVoiceTracks(tracks, table, mode) } catch (e) { return { err: String(e && e.message || e) } } },
				voiceMode: () => { try { return mpwVoiceMode() } catch (e) { return null } },
				// ①(NP-4 补) 联动开关切换的对齐状态机
				linkAlign: (linkOn, prevLinkOn, cardPaused) => { try { return mpwLinkAlign(!!linkOn, !!prevLinkOn, !!cardPaused) } catch (e) { return String(e && e.message || e) } },
				// ② 挂载裁决 / 形状
				pickMount: (s) => { try { return mpwPickMount(s || readSection()) } catch (e) { return { err: String(e && e.message || e) } } },
				srcConsistent: (s) => { try { return mpwSrcConsistent(s || readSection()) } catch (e) { return { err: String(e && e.message || e) } } },
				srcShapeOf: (u) => { try { return mpwSrcShapeOf(u) } catch (e) { return null } },
				srcRootOfKey: (k) => { try { return mpwSrcRootOfKey(k) } catch (e) { return null } },
				sourceFieldsFor: (entry, ctx) => { try { return mpwSourceFieldsFor(entry, ctx) } catch (e) { return { err: String(e && e.message || e) } } },
				srcMissing: (s) => { try { return mpwSrcMissing(s || readSection()) } catch (e) { return null } },
				// ① 预览候选链
				thumbCandidates: (s) => { try { return mpwThumbCandidates(s || readSection()) } catch (e) { return [] } },
				thumbNext: (el) => { try { return mpwThumbNext({ target: el }) } catch (e) { return String(e && e.message || e) } },
				// ⑥ 弹层类污染
				syncLayerHosts: () => { try { return mpwSyncLayerHosts() } catch (e) { return { error: String(e && e.message || e) } } },
				applyCompatBridges: (sec) => { try { applyCompatBridges(sec || readSection()); return true } catch (e) { return String(e && e.message || e) } },
				untagInjectedCompat: (why) => { try { return mpwUntagInjectedCompat(why || "test") } catch (e) { return -1 } },
				holdsLayer: (el) => { try { return mpwHoldsLayer(el) } catch (e) { return null } },
				looksLikeLayer: (el) => { try { return mpwLooksLikeLayer(el) } catch (e) { return null } },
				compatBridges: COMPAT_BRIDGES.map((b) => ({ sel: b[0], cls: b[1], mode: b[2] })),
				layerHostAttr: MPW_LAYER_HOST_ATTR,
				sideMarkAttr: MPW_SIDE_MARK_ATTR,
				// ④ 帧内音频归属
				frameSoundBlocked: () => { try { return npFrameSoundBlocked() } catch (e) { return String(e && e.message || e) } },
				publishFrameSound: () => { try { return npPublishFrameSound("test") } catch (e) { return null } },
				cardPaused: () => !!npCardPaused,
				framePausedByUs: () => !!npFramePausedByUs,
				applyFrameMute: () => { try { npApplyFrameMute(); return true } catch (e) { return String(e && e.message || e) } },
				transport: (op, section, value) => { try { npTransport(op, section || readSection(), value); return true } catch (e) { return String(e && e.message || e) } },
				applyNowPlaying: (s) => { try { applyNowPlaying(s || readSection()); return true } catch (e) { return String(e && e.message || e) } },
				// C 切页时的"任何重试不得起播"闸门
				// C2 音频审计
				audit: () => { try { return (window.__mpwAudioAudit && window.__mpwAudioAudit.list || []).slice(-20) } catch (e) { return [] } },
				auditPush: (kind, el, extra) => { try { return mpwAuditPush(kind, el, extra) } catch (e) { return null } },
				auditOn: () => { try { return mpwAuditOn() } catch (e) { return null } },
				gestureSeen: () => !!npGestureSeen,
				persistedPaused: () => { try { const s0 = readSection(); return s0.npPaused !== void 0 ? !!s0.npPaused : DEFAULT_NP_PAUSED } catch (e) { return null } },
				applyPersistedPause: (sec) => { try { return npApplyPersistedPause(sec || readSection()) } catch (e) { return false } },
				persistPaused: (v) => { try { return npPersistPaused(v) } catch (e) { return false } },
				cardPausedNow: () => !!npCardPaused,
				wantMuted: () => { try { return npWantMuted() } catch (e) { return null } },
				gestureUnlock: (by) => { try { return npGestureUnlock(by || "test") } catch (e) { return false } },
				npSource: () => { try { return { kind: npSourceKind(readSection()), id: npSourceId(readSection()), prev: npSrcIdPrev, audioPaused: npAudio ? !!npAudio.paused : null, audioSrc: npAudio ? String(npAudio.getAttribute('src') || '').slice(0, 60) : null } } catch (e) { return null } },
				hiddenAudioBlock: () => { try { return mpwHiddenAudioBlock() } catch (e) { return null } },
				hiddenPauseOn: () => { try { return mpwHiddenPauseOn() } catch (e) { return null } },
				ops: () => { try { return (window.__mpwNpOps || []).slice(-12) } catch (e) { return [] } },
				// ⑤ 可见性/省电
				defaultPowPauseHidden: DEFAULT_POW_PAUSE_HIDDEN,
				updatePowerPause: () => { try { updatePowerPause(); return true } catch (e) { return String(e && e.message || e) } },
				setupPowerSave: () => { try { setupPowerSave(); return true } catch (e) { return String(e && e.message || e) } },
				powState: () => ({ paused: !!powPaused, hidden: !!powHiddenNow, focused: !!powFocusedNow, hidNpWasPlaying: !!powHidNpWasPlaying, framePausedByUs: !!webFramePausedByUs }),
				// ② 武装前验活
				armProbe: (u) => mpwArmProbe(u),
				bgArmErrorSet: (kind, url, status, detail) => { try { return mpwBgArmErrorSet(kind, url, status, detail) } catch (e) { return null } },
				bgArmErrorClear: () => { try { mpwBgArmErrorClear(); return true } catch (e) { return String(e && e.message || e) } },
				armErrorState: () => { try { return window.__mpwBgArmError || null } catch (e) { return null } },
				verifyArm: (kind, url, onBad) => { try { mpwVerifyArm(kind, url, onBad); return true } catch (e) { return String(e && e.message || e) } },
				// ③ 沙箱降级
				webLooksPolicyBlocked: (d) => { try { return mpwWebLooksPolicyBlocked(d) } catch (e) { return null } },
				webSandboxFallback: (why, detail) => { try { return mpwWebSandboxFallback(why, detail) } catch (e) { return String(e && e.message || e) } },
				webShimUrl: (u) => { try { return mpwWebShimUrl(u) } catch (e) { return null } },
				compatAttrs: MPW_WEB_COMPAT_ATTR,
				// 通用
				read: () => { try { return readSection() } catch (e) { return {} } },
				writePartial: (next) => { try { writeSection(next || {}, true); return true } catch (e) { return String(e && e.message || e) } },
				apply: () => { try { applyFromStorage(); return true } catch (e) { return String(e && e.message || e) } },
				wrap: () => { try { return bgElements().wrap } catch (e) { return null } },
				frame: () => { try { return bgElements().frame } catch (e) { return null } },
				video: () => { try { return bgElements().video } catch (e) { return null } },
				audio: () => { try { return npAudio } catch (e) { return null } },
				ensureAudio: () => { try { return npEnsureAudio() } catch (e) { return null } },
				webShimUrlOf: (u) => { try { return mpwWebShimUrl(u) } catch (e) { return null } },
			};
		} catch {}
		try {
			...
			// ①(2026-09-18 §5 第3项) 诊断自证闭环的测试钩子（tools/diag-subsystem-test.mjs 用；生产不引用）。
			//   spec() 暴露字段表（id + provenance）⇒ "字段被删/被改名"能变成断言而不是靠肉眼；
			//   build/fit 走纯函数；send/manual/download 走**真实现**（fetch/Blob/URL 由测试桩提供）。
			// ①(2026-09-18) 设置写入的测试钩子（tools/better-sidebar-compat-test.mjs 用；生产不引用）：
			//   走的**就是**面板 toggle 的 commit 路径（含 writeSection 的"用户设过"打标记逻辑），
			//   这样"迁移后再手动关 ⇒ 保持关"能在假 DOM 上被真断言。
			/* ①(NP-3 2026-09-19) Now playing / 壁纸声音接线的测试钩子（tools/np-media-test.mjs 用；
			   生产不引用）。暴露的是**真实现**（同一批函数，不是复刻的副本）：
			     scope/scanUrl/trackUrl  —— 音轨清单作用域与两条宿主路由的拼法（纯函数）
			     activeVideo/videoAudio  —— "当前真的是谁在放"与"有没有音轨"
			     resolve                 —— 数据源快照（控件看到的那一份）
			     transport               —— 传输动作的真实落点（play/pause/prev/next/mute）
			     audio/load/idx/seedTracks —— 我们自己的播放器与曲目清单（夹具注入 / 读状态） */
			globalThis.__mpwNpTest = {
				scope: (section) => { try { return npAudioScope(section || readSection()) } catch (e) { return { err: String(e && e.message || e) } } },
				scanUrl: (section) => { try { return npScanUrl(npAudioScope(section || readSection())) } catch (e) { return "ERR " + (e && e.message || e) } },
				trackUrl: (section, p, src) => { try { return npTrackUrl(npAudioScope(section || readSection()), p, src) } catch (e) { return "ERR " + (e && e.message || e) } },
				activeVideo: (section) => { const v = npActiveVideo(section || readSection()); return v ? true : false },
				videoAudio: (section) => npVideoAudio(npActiveVideo(section || readSection())),
				resolve: (section) => { try { return npResolveMedia(section || readSection()) } catch (e) { return { err: String(e && e.message || e) } } },
				transport: (op, section, value) => { try { npTransport(op, section || readSection(), value); return true } catch (e) { return String(e && e.message || e) } },
				audio: () => npAudio,
				idx: () => npTrackIdx,
				load: (i) => { try { if (typeof i === "number") npTrackIdx = Math.max(0, i); return npLoadTrack(false) } catch (e) { return String(e && e.message || e) } },
				applyMute: () => { try { npApplyMute(); return true } catch (e) { return String(e && e.message || e) } },
				ownsSound: () => npAudioOwns(),
				/* 夹具：把"宿主扫描路由的应答"直接塞进缓存（等价于 npFetchTracks 的成功回调）。
				   入参是 **section**（不是 scope）：作用域由 npAudioScope 自己算 —— 与生产同一条口径。 */
				seedTracks: (section, tracks, source) => {
					try {
						const s = npAudioScope(section || readSection());
						/* ①(NP-4)④ 第三个入参 = 宿主扫描应答里的 `source`（'dir' | 'pkg'）：
						   'pkg' 表示音轨在 scene.pkg 里 ⇒ 只能列清单、不能播（判据见 A9a4/D 组）。 */
						npMediaCache = { key: npScopeKey(s), at: Date.now(), data: { count: (tracks || []).length, tracks: tracks || [], source: String(source || "") } };
						npTrackIdx = 0;
						return npMediaCache.key;
					} catch (e) { return String(e && e.message || e) }
				},
				cache: () => npMediaCache,
				/* 走**真 apply 路径**（applyNowPlaying 本身；测试/真机排障都用它手动重放一次） */
				apply: (section) => { try { applyNowPlaying(section || readSection()); return true } catch (e) { return String(e && e.message || e) } },
				dropAudio: () => { try { npDropAudio(); return true } catch (e) { return String(e && e.message || e) } },
				/* ══ ①(NP-4)① ② ③ 的新钩子（驱动的是**生产实现本身**，不是副本） ══
				   音量 / 联动 / 起播顺序 / 拖动落点这四件事的判据全走这里，见 tools/np-control-test.mjs。 */
				volume: () => { try { return npWantVolume() } catch (e) { return String(e && e.message || e) } },
				wantMuted: () => { try { return npWantMuted() } catch (e) { return String(e && e.message || e) } },
				link: (section) => { try { return npLinkOn(section || readSection()) } catch (e) { return String(e && e.message || e) } },
				applyVolume: (force) => { try { npApplyVolume(force === true); return true } catch (e) { return String(e && e.message || e) } },
				volumeSet: () => { try { return npVolumeSet() } catch (e) { return String(e && e.message || e) } },
				playBlocked: () => npPlayBlocked,
				primePlay: (section) => { try { const v = npActiveVideo(section || readSection()); return v ? npPrimePlay(v) : "no-active-video" } catch (e) { return String(e && e.message || e) } },
				primedMuted: () => !!npPrimedMuted,
				userPaused: () => npUserPausedOf(npActiveVideo(readSection())),
				soundBlocked: () => !!npSoundBlocked,
				/* ④(2026-09-20) 帧内音频归属：卡片暂停/我们在放音 ⇒ 帧内必须安静。
				   判据读的是**生产实现本身**（npFrameSoundBlocked/npPublishFrameSound）。 */
				frameSoundBlocked: () => { try { return npFrameSoundBlocked() } catch (e) { return String(e && e.message || e) } },
				frameSound: () => { try { return npPublishFrameSound("test") } catch (e) { return null } },
				cardPaused: () => !!npCardPaused,
				framePausedByUs: () => !!npFramePausedByUs,
				trackList: (section) => { try { const l = npTrackList(section || readSection()); return l ? { key: l.key, source: l.source, paths: l.tracks.map((t) => t.path) } : null } catch (e) { return String(e && e.message || e) } },
				step: (dir) => { try { return npStepTrack(dir, false) } catch (e) { return String(e && e.message || e) } },
				trackIdx: () => npTrackIdx,
			};
			globalThis.__mpwSectionTest = {
				read: () => { try { const s = readSection() || {}; return { bsCompat: s.bsCompat, bsCompatUserSet: s.bsCompatUserSet === true, headerFrostUserSet: s.headerFrostUserSet === true } } catch (e) { return { err: String(e && e.message || e) } } },
				write: (patch) => { try { const next = Object.assign({}, readSection(), patch || {}); writeSection(next, true); return true } catch (e) { return String(e && e.message || e) } },
				defaults: () => ({ bsCompat: DEFAULT_BS_COMPAT }),
			};
			globalThis.__mpwDiagTest = {
				schema: MPW_DIAG_SCHEMA,
				maxBytes: MPW_DIAG_MAX_BYTES,
				optional: MPW_DIAG_OPTIONAL.slice(),
				spec: () => { try { return MPW_DIAG_FIELDS.map((f) => ({ id: f.id, provenance: f.provenance })) } catch (e) { return [] } },
				subsystems: () => { try { return mpwDiagSubsystems() } catch (e) { return { err: String((e && e.message) || e) } } },
				collect: () => { try { return mpwDiagCollect() } catch (e) { return { err: String((e && e.message) || e) } } },
				fit: (data, cap) => { try { return mpwDiagFit(data, cap) } catch (e) { return { err: String((e && e.message) || e) } } },
				send: (why) => mpwDiagSendOnce(why),
				manual: () => mpwDiagManual(),
				download: (data) => { try { return mpwDiagDownload(data) } catch (e) { return { ok: false, error: String((e && e.message) || e) } } },
			};
...
// >>> MPW-NP-GEN-START (generated — do not edit by hand)
...
/* ── ①(NP-4)② 联动开关（"播放/暂停是否同时控制壁纸的播放/暂停"）的可查询落点 ────────
   开关本身在设置面板里（放 Now playing 之下，NP 关掉就撤掉那一行）；组件根上这个属性是
   真机探针/门禁读"当前是哪一档"的唯一入口（不靠读设置文件，避免两处口径）。 */
const NP_LINK_ATTR = "data-mpw-np-link";
...
/* ── ①(NP-4) 拖动：进度 seek + 音量，共用**一条**换算 ──────────────────────────
   为什么两条要合一条：两者都只是"指针横坐标 → 0..1"，差别只在把 0..1 交给谁。
   分成两份实现就会出现"一个按内边距盒算、一个按边框盒算"这种只在某一态偏几像素的
   分歧（上游那张样式表在别处也吃过同一种亏：同一个位置有两个意见）。
   `ratioOf` 是纯函数（只吃 rect + clientX）⇒ 无浏览器门禁可以直接喂矩形断言边界值。 */
const NP_SCRUB_PAD = 9;         /* 轨道只有 3px（RAIL_H），命中带上下各补 9px ⇒ 共 21px，够手指 */
const NP_VOL_W = 84;            /* 音量条的可见宽度（卡片 260 宽里时钟行放得下，见 §几何） */
/* ⑤ 悬浮态放大：容器能往两侧各"借"多少像素。
   上限 12 而不是"能借多少借多少"：借来的宽度是宿主自己的内边距（悬浮态 12px），
   贴到内边距盒边缘虽然不算裁切，但卡片会顶到宿主圆角卡片的边上。
   12 与本机实测的悬浮内边距相等 ⇒ 卡片的左右各留 1px 余量（判据见 np-control-live 的 G 组）。 */
const NP_BLEED_MAX = 12;
...
/* ══ ①(NP-4) 拖动（进度 seek + 音量）══
   两条都是自绘的（不用 input[type=range]）：本组件的每个位置都由补间逐帧写成内联样式，
   浏览器原生控件的高度/内边距/thum 尺寸在各家实现里都不一样，塞进来就是"第二个意见"。
   规则只写**材料**（颜色/圆角/命中带），位置与长度一律由 JS 内联写。 */
/* the scrubber —— 轨道只有 3px，命中带上下各补 9px（NP_SCRUB_PAD）。
   它绝对定位在 .mpw_np_bar 里（那条是 position:absolute ⇒ 自己是包含块），
   **不参与** 那一列的 flex 流 ⇒ 时钟/进度条的布局一个像素都不动（放大那件事不留回归）。 */
.mpw_np_scrub {
	position: absolute;
	left: 0;
	width: 100%;
...
/* 没有可 seek 的媒体时：命中带仍然在（判据要能量到它），但不给"能拖"的指针语义 */
.mpw_np_scrub[data-mpw-np-noseek] { cursor: default; }
/* the rail 的可见部分保持 3px；拖动时只把游标点亮，不改几何 */
.mpw_np_rail { position: relative; }
.mpw_np_scrub[data-mpw-np-drag] ~ .mpw_np_clock { color: var(--mpw-np-ink-3); }
/* the level —— 卡片里时钟行的中间那一段（左时刻 / 音量 / 右剩余）。
   收起态整行随 late 淡出，音量条与时钟同一条约定：只在卡片里存在。 */
.mpw_np_vol { flex: 1 1 auto; display: flex; align-items: center; justify-content: center; min-width: 0; }
.mpw_np_volHit {
	position: relative;
	display: block;
	width: 84px;              /* = NP_VOL_W，两端实测一致（css-matrix 有一条对拍） */
	height: 12px;             /* 命中带；可见轨道只有 2px，居中 */
	pointer-events: auto;
	cursor: pointer;
	touch-action: none;
}
.mpw_np_volRail {
	...
	border-radius: 999px; background: rgba(var(--mpw-np-ink-rgb), 0.16);
}
.mpw_np_volRun {
	position: absolute; left: 0; top: 50%; height: 2px; margin-top: -1px;
	border-radius: 999px; background: var(--mpw-np-ink);
}
.mpw_np_volKnob {
	position: absolute; top: 50%; width: 7px; height: 7px; margin: -3.5px 0 0 -3.5px;
	border-radius: 999px; background: var(--mpw-np-ink);
}
.mpw_np_volHit:focus-visible, .mpw_np_scrub:focus-visible {
	outline: none;
	box-shadow: 0 0 0 2px var(--mpw-np-card), 0 0 0 4px rgba(var(--mpw-np-ink-rgb), 0.35);
}
`;
...
/* ── ①(NP-4) 拖动换算与借宽判据（**模块作用域**的纯函数） ──────────────────────
   为什么放在模块作用域而不是工厂里：它们一行 DOM 都不碰（只吃矩形 + 一个计算样式函数），
   放在这里无浏览器门禁可以直接喂矩形断言边界值（tools/np-control-test.mjs 的 F 组），
   不必为了测"指针横坐标 → 0..1"去装一整个控制器。工厂里只留绑定 win / NP_BLEED_MAX 的薄壳。
   两条拖动条（进度 seek / 音量）共用同一条换算：两者都只是"指针横坐标 → 0..1"，
   差别只在把 0..1 交给谁。分两份实现就会出现"一个按内边距盒算、一个按边框盒算"这种
   只在某一态偏几像素的分歧（上游那张样式表在别处也吃过同一种亏：同一个位置有两个意见）。 */
const npClamp = (v, lo, hi) => Math.min(hi, Math.max(lo, v));
/** 指针横坐标 → 0..1（夹住：滑出条外仍然最左/最右，不回弹、不越界）。 */
function ratioOf(rect, clientX) {
  if (!rect) return 0;
  const w = Number(rect.width);
  if (!(w > 0)) return 0;
  const x = Number(clientX);
  if (!isFinite(x)) return 0;
  return npClamp((x - Number(rect.left)) / w, 0, 1);
}
/** 键盘步进（←/↓ 与 →/↑ 各一步、Home/End 到端点）；不是这些键返回 null（不吞别的键）。 */
function stepRatio(cur, key, step) {
  const st = Number(step) > 0 ? Number(step) : 0.02;
  const c = npClamp(Number(cur) || 0, 0, 1);
  if (key === "Home") return 0;
  if (key === "End") return 1;
  if (key === "ArrowLeft" || key === "ArrowDown") return npClamp(c - st, 0, 1);
  if (key === "ArrowRight" || key === "ArrowUp") return npClamp(c + st, 0, 1);
  return null;
}
/** 最近一个**会裁切**的祖先的内边距盒 = 硬边界（⑤ 的"不许被裁切"就是不许越过它）。
    量不到（假 DOM / 没有 getComputedStyle）⇒ null，调用方按"不放大"处理。 */
function clipPadBox(el, getCS) {
  try {
    let cur = el;
    /* 从容器自己开始往上走（容器是我们自己的节点，overflow 是 visible ⇒ 第一轮就跳过）；
       循环条件只判 `cur`：写成 `cur && cur.parentElement` 会把**最上面那个**祖先漏掉
       —— 真机上恰好是它（列元素）带 overflow:hidden，漏了就等于没有硬边界（借宽会越界）。 */
    while (cur) {
      const cs = getCS ? getCS(cur) : null;
      const ov = cs ? String(cs.overflow || "") + String(cs.overflowX || "") + String(cs.overflowY || "") : "";
      if (ov && ov.indexOf("visible") < 0) {
        const r = cur.getBoundingClientRect ? cur.getBoundingClientRect() : null;
        if (!r) return null;
        const pad = (v) => { const n = parseFloat(v); return isFinite(n) ? n : 0 };
        return {
          left: Number(r.left) + (cs ? pad(cs.paddingLeft) : 0),
          right: Number(r.right) - (cs ? pad(cs.paddingRight) : 0),
          node: cur,
        };
      }
...
/** ⑤ 悬浮态放大：容器能往左右各"借"多少像素。
    借的是**容器到最近裁切祖先内边距盒之间那段空档**（宿主自己的内边距，本来是空的）。
    `applied` 是当前已经借出去的量 —— 必须减掉再算，否则量到的是加宽后的盒子、
    算出来只剩 1px、下一帧又缩回去：**抖动**（"同一件事两个意见"在本文件里出现过一次）。
    借多少的硬上限是 NP_BLEED_MAX（见常量处），不贴宿主圆角卡片的边。 */
function bleedFor(container, applied, maxBleed, getCS) {
  if (!container || !container.getBoundingClientRect) return 0;
  const cs = getCS || (typeof getComputedStyle === "function" ? getComputedStyle : null);
  const clip = clipPadBox(container, cs);
  if (!clip) return 0;
  let r = null;
  try { r = container.getBoundingClientRect(); } catch (e) { return 0 }
  if (!r) return 0;
  const back = Number(applied) > 0 ? Number(applied) : 0;
  const baseL = Number(r.left) + back, baseR = Number(r.right) - back;
  const budgetL = baseL - clip.left, budgetR = clip.right - baseR;
  const b = Math.floor(Math.min(budgetL, budgetR));
  if (!(b > 0)) return 0;
  const cap = Number(maxBleed) > 0 ? Number(maxBleed) : NP_BLEED_MAX;
...
function createNowPlaying(deps) {
  ...
  const h = react.createElement;
    ...
    /* ①(NP-4) 音量（0..1）与"这条轨道能不能拖"都由控制器送进来（默认值 = 不改既有行为：
       音量 1、能不能拖跟"有没有已知时长"走）。 */
    const vol = M.clamp(Number(q.volume) >= 0 ? Number(q.volume) : (Number(media.volume) >= 0 ? Number(media.volume) : 1), 0, 1);
    const canSeek = q.canSeek === void 0 ? known : !!q.canSeek;
    ...
    return h("div", { className: "mpw_np" },
      h("div", {
        ...
        h("span", {
          ...
          /* ①(NP-4)③ 进度条可拖动：命中带绝对定位在 bar 上（**不进**那一列的 flex 流），
             所以时钟与轨道的布局一个像素都不动。轨道只有 3px，命中带上下各补 NP_SCRUB_PAD。
             落点由控制器交给 client.js 的接线（口径见 docs/NOW-PLAYING-DSH.md §7.8）：
             联动开 ⇒ 真的落到**当前媒体的 currentTime**；联动关 ⇒ 只落到我们自己的播放器；
             两样都没有 ⇒ 这条带子是只读的（data-mpw-np-noseek，不假装能拖）。 */
          h("span", {
            className: "mpw_np_scrub",
            "data-mpw-np-scrub": "",
            "data-mpw-np-noseek": canSeek ? void 0 : "",
            "data-mpw-np-drag": q.dragging ? "" : void 0,
            role: "slider",
            tabIndex: canSeek ? 0 : -1,
            "aria-label": t("np.seek"),
            "aria-valuemin": 0, "aria-valuemax": known ? media.total : 0,
            "aria-valuenow": known ? Math.round(at) : 0,
            "aria-disabled": canSeek ? void 0 : true,
            style: {
              /* ⚠ 这个 `top` 是**相对 `.mpw_np_bar`** 的（bar 自己已经在 `top: barTop(p)`），
                 所以只能是 `-NP_SCRUB_PAD`。写成 `barTop(p) - NP_SCRUB_PAD` 会把偏移**加两遍**
                 ⇒ 命中带落到卡片外面（真机读数：卡片 620..787，命中带 781.95..800.53，
                 中心点在卡片之下、被同一行的宿主元素接走 ⇒ 拖动事件根本到不了控件）。
                 几何不变量（门禁 F12 断言）：barTop(p)+RAIL_H+NP_SCRUB_PAD ≤ boxHeight(p)。 */
              top: -NP_SCRUB_PAD, left: 0, width: "100%",
              height: M.RAIL_H + NP_SCRUB_PAD * 2,
            },
            onPointerDown: (e) => { if (q.onScrubStart) q.onScrubStart(e); },
            onPointerMove: (e) => { if (q.onScrubMove) q.onScrubMove(e); },
            onPointerUp: (e) => { if (q.onScrubEnd) q.onScrubEnd(e); },
            onPointerCancel: (e) => { if (q.onScrubEnd) q.onScrubEnd(e); },
            onKeyDown: (e) => { if (q.onScrubKey) q.onScrubKey(e); },
          }),
          h("span", { className: "mpw_np_clock", style: { opacity: late } },
            h("span", null, M.clock(at)),
            /* ①(NP-4)① 音量：**与静音开关分开的第二件事**（用户原话："打开之后也不能调整它
               开启的音量"）。它只在卡片里存在 —— 与第四个键同一条 late 约定。 */
            showVolume ? h("span", { className: "mpw_np_vol" },
              h("span", {
                className: "mpw_np_volHit",
                "data-mpw-np-vol": "",
                role: "slider",
                tabIndex: media.canVolume ? 0 : -1,
                "aria-label": t("np.volume"),
                "aria-valuemin": 0, "aria-valuemax": 100,
                "aria-valuenow": Math.round(vol * 100),
                "aria-disabled": media.canVolume ? void 0 : true,
                onPointerDown: (e) => { if (q.onVolStart) q.onVolStart(e); },
                onPointerMove: (e) => { if (q.onVolMove) q.onVolMove(e); },
                onPointerUp: (e) => { if (q.onVolEnd) q.onVolEnd(e); },
                onPointerCancel: (e) => { if (q.onVolEnd) q.onVolEnd(e); },
                onKeyDown: (e) => { if (q.onVolKey) q.onVolKey(e); },
              },
                h("span", { className: "mpw_np_volRail" }),
                h("span", { className: "mpw_np_volRun", style: { width: (vol * 100) + "%" } }),
                h("span", { className: "mpw_np_volKnob", style: { left: (vol * 100) + "%" } }))) : null,
  ...
  const IDLE_MEDIA = {
    ...
    /* ①(NP-4) 空闲态也把这三项写全：谁读都不该读到 undefined（空闲态 = 不能拖、联动开）。 */
    canSeek: false, link: true,
  };
  ...
  const h = react.createElement;
    ...
    let hostWide = null;
    /* ── ①(NP-4) 拖动状态 ───────────────────────────────────────────────────────
       `drag` = 正在拖哪一条（"" / "seek" / "vol"）。拖动期间**媒体事件推来的进度不再回写**
       （否则手指按着不放，播放器每 250ms 的 timeupdate 会把游标拽回当前播放位置：
       真机表现就是"拖不动/拖了弹回去"）。松手（或 pointercancel）立刻恢复回写。 */
    let drag = "";
    /* ⑤ 当前已经借出去的左右像素（bleedFor 要用它把"量到的盒子"还原成"没借之前的盒子"）。 */
    let bleedNow = 0;
    const counts = { resize: 0, mutation: 0, raf: 0 };
    const state = { open: false, playing: false, at: 0, volume: null, canSeek: false, media: Object.assign({}, IDLE_MEDIA) };
    ...
    const h = react.createElement;
      ...
      try {
        root.render(h(NowPlaying, {
          ...
          volume: state.volume, canSeek: state.canSeek, dragging: drag === "seek",
          link: state.link,
          onToggle: () => {
          ...
          /* ①(NP-4)① 音量：0..1 交给接线落到真实元素（video / 我们的 audio / web 帧主音量）。
             与静音键是**两件事**：静音是开关（既有 mute 设置），这里是电平。 */
          onVolumeLevel: (ratio) => {
            if (!state.media.canVolume) { refuse("np.note.novolume"); return; }
            if (deps.onTransport) deps.onTransport("volume", M.clamp(Number(ratio) || 0, 0, 1));
            state.volume = M.clamp(Number(ratio) || 0, 0, 1);
            render();
          },
          /* ①(NP-4)③ 进度拖动。ratio 是**相对当前媒体时长**的 0..1；
             换算成秒、以及"联动了要不要同步壁纸"都在 client.js 的接线里（见 §7.8 口径）。 */
          onScrubStart: (e) => onDragStart("seek", e, (r) => {
            if (!state.canSeek) { refuse("np.note.noseek"); return; }
            state.at = r * (typeof state.media.total === "number" ? state.media.total : 0);
            if (deps.onTransport) deps.onTransport("seek", r);
            render();
          }),
          onScrubMove: (e) => onDragMove("seek", e, (r) => {
            if (!state.canSeek) return;
            state.at = r * (typeof state.media.total === "number" ? state.media.total : 0);
            if (deps.onTransport) deps.onTransport("seek", r);
            render();
          }),
          onScrubEnd: (e) => onDragEnd("seek", e, (r) => {
            if (!state.canSeek) return;
            if (deps.onTransport) deps.onTransport("seek", r);
          }),
          onScrubKey: (e) => onDragKey("seek", e, () => {
            const total = typeof state.media.total === "number" ? state.media.total : 0;
            return total > 0 ? state.at / total : 0;
          }, (r) => {
            if (!state.canSeek) { refuse("np.note.noseek"); return; }
            state.at = r * (typeof state.media.total === "number" ? state.media.total : 0);
            if (deps.onTransport) deps.onTransport("seek", r);
          ...
          onVolStart: (e) => onDragStart("vol", e, (r) => fireVol(r)),
          onVolMove: (e) => onDragMove("vol", e, (r) => fireVol(r)),
          onVolEnd: (e) => onDragEnd("vol", e, null),
          onVolKey: (e) => onDragKey("vol", e, () => (typeof state.volume === "number" ? state.volume : 1), (r) => fireVol(r)),
        }));
      } catch (err) { log.error(TAG + "render failed:", err); }
    }
    function fireVol(r) {
      if (!state.media.canVolume) { refuse("np.note.novolume"); return; }
      if (deps.onTransport) deps.onTransport("volume", M.clamp(Number(r) || 0, 0, 1));
      state.volume = M.clamp(Number(r) || 0, 0, 1);
      render();
    }

    /* ── ①(NP-4) 拖动的公共部分（seek 与音量共用；两条只是把 0..1 交给不同的落点） ──
       `hit` 是承载命中的那个元素（量矩形用它，不是用 e.target —— 指针落到填充/圆点上时
       target 会是子元素，rect 一样但语义上该以命中带为准）。捕获指针之后，手指滑出元素
       也还能收到 move/up（否则拖出条子就"卡住"）。 */
    function dragHitOf(e) {
      try { return (e && (e.currentTarget || e.target)) || null } catch (err) { return null }
    }
    function dragRectOf(e) {
      const el = dragHitOf(e);
      try { return el && el.getBoundingClientRect ? el.getBoundingClientRect() : null } catch (err) { return null }
    }
    function dragXOf(e) {
      try {
        if (e && typeof e.clientX === "number" && isFinite(e.clientX)) return e.clientX;
        if (e && e.nativeEvent && typeof e.nativeEvent.clientX === "number") return e.nativeEvent.clientX;
        if (e && e.touches && e.touches[0]) return e.touches[0].clientX;
      } catch (err) {}
      return NaN;
    }
    function onDragStart(which, e, apply) {
      drag = which;
      try { const el = dragHitOf(e); if (el && el.setPointerCapture && e && e.pointerId !== void 0) el.setPointerCapture(e.pointerId); } catch (err) {}
      try { if (e && e.preventDefault) e.preventDefault(); } catch (err) {}
      try { apply(ratioOf(dragRectOf(e), dragXOf(e))) } catch (err) {}
    }
    function onDragMove(which, e, apply) {
      if (drag !== which) return;                        /* 没按下的移动不算拖（悬停不算） */
      try { if (e && e.preventDefault) e.preventDefault(); } catch (err) {}
      try { apply(ratioOf(dragRectOf(e), dragXOf(e))) } catch (err) {}
    }
    function onDragEnd(which, e, apply) {
      if (drag !== which) return;
      drag = "";
      try { if (apply) apply(ratioOf(dragRectOf(e), dragXOf(e))) } catch (err) {}
      render();                                          /* 去掉 data-mpw-np-drag */
    }
    function onDragKey(which, e, curOf, apply) {
      const key = String((e && e.key) || "");
      const cur = curOf();
      const next = stepRatio(cur, key, which === "vol" ? 0.05 : 0.02);
      if (next === null) return;
      try { if (e && e.preventDefault) e.preventDefault(); } catch (err) {}
      try { apply(next) } catch (err) {}
    }
    ...
    /** ⑤ 悬浮态放大：把容器往两侧各借 bleed 像素（借的是宿主自己的内边距，本来是空的）。
        只写**我们自己容器**的内联样式，不碰宿主任何节点：
          · `width: calc(100% + 2b)` + 左右各 -b 的外边距 ⇒ 盒子在原处**居中变宽**；
          · 硬边界是最近裁切祖先的内边距盒（bleedFor 里算），所以借满也不会被切。
            真机读数（悬浮态）：列 256 / 内边距 12 ⇒ 内边距盒 24..256；容器原本 37..243（206），
            借 12 之后是 25..255（230），两边各留 1px。
        收敛性：bleedFor 用 `bleedNow` 把量到的盒子还原成"没借之前"，所以这是一步到位的；
        ResizeObserver 会为这次变宽回调一次，那一次算出来的值与现在相同 ⇒ 不再改样式、不再回调。 */
    function applyBleed() {
      if (!container) return 0;
      const getCS = win && win.getComputedStyle ? win.getComputedStyle.bind(win) : null;
      const want = bleedFor(container, bleedNow, NP_BLEED_MAX, getCS);
      if (want === bleedNow) return bleedNow;
      bleedNow = want;
      try {
        if (!want) {
          ...
          container.style.marginLeft = "-" + want + "px";
          container.style.marginRight = "-" + want + "px";
          container.style.width = "calc(100% + " + (want * 2) + "px)";
        }
      } catch (e) {}
      return bleedNow;
    }
    ...
    function mountInto(a) {
      try {
        ...
        /* ①(NP-4)② 联动开关的可查询落点（值由 setLink/applyNowPlaying 维护）。 */
        container.setAttribute(NP_LINK_ATTR, state.link === false ? "0" : "1");
        insertInto(a, true);
        enabled = true;
    ...
    function setEnabled(on) {
      ...
      if (!on) {
        ...
        drag = ""; bleedNow = 0;      /* ①(NP-4)/⑤：拖动与借宽都是"挂着才有"的状态 */
        return api;
      }
    ...
    const api = {
      ...
      setMedia(next) {
        ...
        /* ①(NP-4) 音量/可拖/联动三个派生量只在**接线送来**时才改（缺省保持"不改既有行为"：
           音量跟随媒体元素的 volume，能不能拖跟"有没有已知时长"，联动默认开）。 */
        if (next && next.volume !== void 0 && next.volume !== null && isFinite(Number(next.volume))) {
          state.volume = M.clamp(Number(next.volume), 0, 1);
        } else if (state.volume === null && state.media.volume !== void 0 && isFinite(Number(state.media.volume))) {
          state.volume = M.clamp(Number(state.media.volume), 0, 1);
        }
        if (next && next.canSeek !== void 0) state.canSeek = !!next.canSeek;
        else if (typeof state.media.total === "number" && state.media.total > 0) state.canSeek = true;
        if (next && next.link !== void 0) state.link = !!next.link;
        render();
        return api;
      },
      setPlaying(on) {
      ...
      /** 播放进度（媒体事件推来的）。①(NP-4)：**拖动期间直接丢弃** —— 手指按着的时候
          播放器自己的 timeupdate 会把游标拽回播放位置（真机表现："拖不动"）。
          拖动那一路由 onScrubMove 自己写 state.at，所以丢弃不会让游标停住。 */
      setProgress(at) { if (drag) return api; state.at = Number(at) || 0; render(); return api; },
      /** ①(NP-4) ② 联动开关状态（client.js 的 applyNowPlaying 送）——只影响根上的可查询属性，
          动作口径在接线里（见 docs/NOW-PLAYING-DSH.md §7.8）。 */
      setLink(on) {
        state.link = !!on;
        try { if (container) container.setAttribute(NP_LINK_ATTR, state.link === false ? "0" : "1"); } catch (e) {}
        render();
        return api;
      },
      /** ①(NP-4) 音量电平（0..1，接线送；拖动时也用它回写显示）。 */
      setVolume(level) {
        const v = Number(level);
        if (isFinite(v)) { state.volume = M.clamp(v, 0, 1); render(); }
        return api;
      },
      /** 给测试/排障看的一眼状态（不参与渲染）。 */
      inspect: () => ({
        ...
        /* ①(NP-4) 拖动/音量/联动/借宽的可查询状态（真机探针与门禁都读这里）。 */
        dragging: drag, volume: state.volume, canSeek: state.canSeek, link: state.link, bleed: bleedNow,
      }),
      ...
      inspectCollapse: () => ({ hostWide, hidden, fit: npFit(), width: measureAvailWidth(), colWidth: measureWidth(), hostCollapsed: hostSaysCollapsed(), settlePending: !!settleTimer, bleed: bleedNow }),
  ...
  return {
    NowPlaying, PlayMark, createController, createSlotAction, shouldHide, resolveAnchor, sidebarRoot, sidebarCol, occupantOf,
  };
}
...
module.exports = {
  ...
  NP_VOL_W,
  NP_BLEED_MAX,
  /* ①(NP-4) 拖动换算 + ⑤ 借宽判据（纯函数；无浏览器门禁直接断言）。 */
  ratioOf, stepRatio, clipPadBox, bleedFor,
  NP_CSS,
  NP_SIDEBAR_ROOT_SEL,
...
// <<< MPW-NP-GEN-END
		function buildCss(section) {
			...
			/* ②(2026-09-20 真机修复) `hasImage` 必须与**挂载裁决**同一口径：残留的 webUrl
			   （类型不符、或档案自相矛盾）不算"有源"，否则 `.mpw-bgWrap` 会被判为"有壁纸"而显示，
			   里面却一张必 404 的旧 web 帧 ⇒ 用户看到的就是黑屏（而不是"无壁纸"的正常语义）。 */
			const hasImage = !!(section && (section.image || (section.webUrl && mpwPickMount(section).mount === "web")));
			/* ①(NP-3 2026-09-19 真机) NP 段必须在**两条 return 路径**上都拼上 —— 计算提到前面来。
			   真机 bug：本函数末尾那条 return 有 `+ __npCss`，而下面的 `if (!hasImage)` 提前 return
			   忘了它 ⇒ 只要当前 section 没有 image/webUrl（无壁纸、或"只有宿主设置"的半残档，
			   真机上用户的档正是这种：mpkgKey 还在、image 没了），NP 控件就**整份样式丢失**：
			   `.mpw_np_abs` 全变 static ⇒ 传输键掉到卡片外面、点不到（现场"声音控制打不开"）。
			   现在两处共用同一个 `__npCss`（构建产物级判据见 tools/now-playing-test.mjs 的 B12）。 */
			const __npCss = (section && (section.npNowPlaying !== void 0 ? !!section.npNowPlaying : DEFAULT_NP_NOW_PLAYING)) ? MPW_NP_CSS : "";
			const opacity = section.opacity !== void 0 ? section.opacity : DEFAULT_OPACITY;
			...
			/* ①(2026-09-18 §5「功能静默无效」第 2 条 / 用户裁定候选 A)「新会话按钮跟随面板不透明度」：
			   开 = 跟随那条透明度（现状公式 U(panel)：统一虚化开=sidebarAlpha，关=panel）；
			   关 = **回到宿主原色** `var(--dsw-alias-button-elevated-fill)`（不混 transparent）。
			   裁定依据：设置页文案是给用户看的合同（"关 = 保持原按钮色"），与代码里旧注释冲突时以文案为准；
			   `DEFAULT_SESSION_FOLLOW = true` ⇒ 默认档行为与修复前逐字节一致。
			   修复前：全仓**没有任何地方读 section.sessionFollow**（开关点了没效果）。 */
			const sessionFollowOn = section.sessionFollow !== void 0 ? !!section.sessionFollow : DEFAULT_SESSION_FOLLOW;
			const newSessionBg = sessionFollowOn
				? `color-mix(in srgb, var(--dsw-alias-button-elevated-fill) ${U(panel)}%, transparent)`
...
`;
				...
				/* ①(NP-3) 无壁纸这条路径同样带上 NP 段（真机 bug：这里以前漏了 __npCss）。 */
				return emitSurfaceTokens(css + buildUiCss(section, false)) + __npCss;
			}
	...
	//   ①(WP-2 2026-09-19) 舞台加 `touch-action:none` + `user-select:none`：不写这两条时，
	//   触屏上的单指拖动被浏览器当成宿主页面滚动/缩放手势接管 → 收到 pointercancel、
	//   作者脚本只看到"按下就没了"（滑块/拖拽类壁纸在真机上完全不可用；桌面鼠标无此问题，所以只在触屏暴露）。
	css += `
.mpw-bgWrap .mpw-webInteract{position:fixed;inset:0;z-index:1;display:none;background:transparent;cursor:crosshair;pointer-events:auto;touch-action:none;user-select:none;-webkit-user-select:none;}
...
.hHd-Xa_newSession {
	/* ①(修正) 新会话按钮随 侧边栏/标题栏透明度（sidebarAlpha）：统一虚化开 → U(panel)=sidebarAlpha；
	   ...
	   ①(2026-09-18) 现在由「新会话按钮跟随面板不透明度」(sessionFollow) 门控：关 → 回到宿主原色。 */
...
`;
			// ①(2026-09-18 修复「功能静默无效」：纯 CSS/SVG 液态玻璃整块**从未执行**)
			//   真因：本块在 6352 行引用 `bdSupported`，而该 const 直到 6577 行才声明 —— 同一函数作用域里
			//   属于 **TDZ（暂时性死区）** ⇒ 每次调用都抛 `ReferenceError: Cannot access 'bdSupported'
			//   before initialization`，又被下面的 `catch { /* 液态玻璃失败不得影响其它样式 */ }` 吞掉。
			//   结果：lgCss 开与不开产物逐字节相同、永远没有 url(#mpw-lg-warp)（用户开了没效果、也不报错）。
			//   修法 = **把声明提到使用之前**（不是删 catch —— 那个 catch 的本意"液态玻璃失败不影响其它样式"
			//   必须保留，现在它只在真的失败时才起作用）。
			//   `bdSupported` 是纯表达式（读一个全局标志），提前求值没有任何副作用。
			...
			try {
				...
				// ①(2026-09-18 用户批准) `?lgcss=off` 一键回退：液态玻璃修好后是**首次真正启用**
				//   （SVG 位移折射玻璃，要起 feImage/feDisplacementMap），真机观感/性能未验 ⇒
				//   必须留一个不改设置就能整体关掉的口子（与 ?sbfill/?railink/?hdrfrost 同款写法）。
				const lgCssOff = (() => { try { return new URLSearchParams(location.search).get("lgcss") === "off" } catch { return false } })();
				if (lgCssOn && !lgCssOff && hasImage && bdSupported) {
					...
					// ①(2026-09-18 css-matrix 冲突修复，**不许放宽断言**) 顶栏的折射**绝不能加在
					//   `.wSkVaW_header` 本体**：本体一旦带 backdrop-filter 就成了 **backdrop root**
					//   ⇒ 顶栏内浮层（子代理展开面板 / 后台任务条）的 backdrop 采样范围被隔离
					//   ⇒ 浮层磨砂失效、背后文字**锐利**透出（真机历史回归；判据 tools/css-matrix.mjs:88-96，
					//   实测 40 个问题）。改放**伪元素**上：伪元素不是浮层的祖先 ⇒ 浮层照常采样，折射也保留
					//   （与既有磨砂层同一套路，见 buildUiCss 里 `.mpw-hdrFrost` 的层叠说明）。
					//   门控 `html body[data-mpw-hdr-frost-el]`：只在 JS **真的**注入了磨砂元素层时才这么做 ——
					//   那一层已经把宿主 header 的**直接子节点抬到 z-index:1**（仓库既有规则
					//   `.wSkVaW_header:has(> .mpw-hdrFrost) > :not(.mpw-hdrFrost)`），所以伪元素用
					//   `z-index:0` 正好"画在顶栏底色之上、内容之下"；而 `z-index:-1` 会被顶栏自己的
					//   半透明底色整片盖住（用户反复反馈过的"没有磨砂"）。
					//   JS 没注入时**不动顶栏**：宁可顶栏没有折射，也绝不把标题/按钮压到玻璃底下。
					//   与既有兜底规则 `:not([data-mpw-hdr-frost-el]) …::before` 互斥，两者永不叠加。
					css += `
...
.wSkVaW_scrollBody {
	...
	backdrop-filter: ${lgFilter} !important;
					...
					// ①(2026-09-18) 顶栏那一条**只在"顶栏本来就在磨砂"时才生成**，两边理由都成立：
					//   语义：折射要有东西可折 —— 用户关掉顶栏透出/磨砂时顶栏是不透明块，加玻璃没意义；
					//   判据：tools/css-matrix.mjs 的断言 7 按**源码文本**判"`.wSkVaW_header::before`
					//   带 backdrop-filter ⇒ 顶栏磨砂存在"，其口径是 `wantFrost = (headerBlur || unifyTint) && headerBg`。
					//   若在用户**关掉**顶栏磨砂时仍输出这条，就会被（正确地）判成"用户关掉的开关没生效"。
					//   **不许**放宽那条断言 ⇒ 这里与它同口径。
					if ((headerBlur || unifyTint) && headerBg) css += `
/* 顶栏：折射放**伪元素**（本体必须保持"不是 backdrop root"，否则浮层虚化失效） */
html body[data-mpw-hdr-frost-el] .wSkVaW_header::before {
	content: "" !important;
	...
	backdrop-filter: ${lgFilter} !important;
					...
					// 高光边缘（specular）：侧栏与顶栏共用（顶栏那条只在上面 headerGlassOn 成立时才伴随出现）
					css += `
...
`;
			...
			// ①(2026-09-18) `const bdSupported` 的声明已提到前面（液态玻璃块之前）——原来这里才声明，
			//   而液态玻璃块在它之前就用了 ⇒ TDZ ReferenceError 被 catch 吞掉，那一整块从未执行。
...
body[data-mpw-sblur] .pI_x6G_sidebarCol:not([data-mpw-holds-layer]),
body[data-mpw-sblur] [class*="sidebarCol"]:not([data-mpw-holds-layer]) {
...
/* ⑥(2026-09-20) :not([data-mpw-holds-layer])：**该节点是弹层的祖先时绝不给 backdrop-filter**
   —— 真机实测这条规则一旦落在 pI_x6G_sidebarCol 上（设置 overlay 的后代祖先链里有它），
   面板 rect 立刻从 {x:320,w:800} 变成 {x:13,w:254}（被压缩进左栏）。标记由 mpwSyncLayerHosts()
   按**结构**维护（不依赖 :has()、不依赖弹层可见性/动画进度）。 */
html body:not([data-mpw-sblur-off]).pI_x6G_sidebarCol:not([data-mpw-holds-layer]),
html body:not([data-mpw-sblur-off])[class*="sidebarCol"]:not([data-sidebar-right-panel]):not([data-mpw-holds-layer]) {
...
html body:not([data-mpw-sblur-off]) [class*="sidebarCol"]:not([data-sidebar-right-panel]):not([data-dockkit-strip]):not([data-mpw-holds-layer]) {
...
button[class*="newSession"] {
	/* ①(修正) 收起态新会话按钮同样随 sidebarAlpha（U(panel)）；①(2026-09-18) 由 sessionFollow 门控 */
	background-color: ${newSessionBg} !important;
	border-color: color-mix(in srgb, var(--dsw-alias-border-l2) 65%, transparent) !important;
}
...
`;
			...
			// ⑲(新) Aqua 实验模式：文字色/品牌蓝兜底 + 弹层模糊（token override 之外）
			// ①(2026-09-18 既有 bug 修复「功能静默无效」) 这一段原来把**三个互不相干**的开关
			//   一起包在 `if (aquaOn(section))` 里：
			//     · aquaOn（aquaMask‖aquaTint‖aquaInk）→ Aqua 文字/品牌色、标题栏 mask、弹层模糊；
			//     · 「配色」(accent)                  → `body[data-mpw-accent]` 品牌交互色 + 发送键；
			//     · 「深底文字可读增强」aquaTextEnhance → `body[data-mpw-aqua-text]` 文字描边 + 文字 token。
			//   后果：只开 accent（或只开 aquaTextEnhance）时**规则根本不生成** ⇒ 开关能点、
			//   没有任何效果、控制台也不报错（2026-09-18 用 613 组产物枚举发现；两段自己的
			//   注释都写着"不依赖 Aqua"，与实现矛盾）。
			//   三个门控属性本来就各自由 JS 维护（data-mpw-accent 在主题色/配色段设置、
			//   data-mpw-aqua-text 在 aquaOn 段设置），所以这里**按各自的开关分别生成**即可。
			//   ⚠ 分段顺序保持原样：发送键在 accent 与 aqua 下各有一条**同特异度 + 同 !important**
			//   的规则，原顺序是 accent 在前，合并会改变胜者；用 parts 数组逐字节保持原顺序。
			const accentConfigured = !!(section.accent && /^#[0-9a-fA-F]{6}$/.test(String(section.accent)));
			const textEnhanceOn = section.aquaTextEnhance !== void 0 ? !!section.aquaTextEnhance : DEFAULT_AQUA_TEXT_ENHANCE;
			const aquaCssParts = [];
			if (aquaOn(section)) aquaCssParts.push(`
...
`);
			if (accentConfigured) aquaCssParts.push(`/* ①(修正) 配色(accent) 不依赖 Aqua 也改品牌交互色/发送键(第4/5项):
...
`);
			if (aquaOn(section)) aquaCssParts.push(`/* ①(修正) 面板取色/统一雾时标题栏背景跟随 mask 色（用户实测只改侧边栏不改标题栏）。
...
`);
			if (textEnhanceOn) aquaCssParts.push(`/* ⑲(修正) 深底文字可读增强（aquaTextEnhance，独立开关）：之前只打 data-mpw-aqua-text
...
body[data-mpw-aqua-text] .mpw_dialog {
	text-shadow: none !important;
			...
			if (aquaCssParts.length) css += aquaCssParts.join("");
...
`;
			...
			css += buildThemeColorCss();
			/* ①(NP-1) Now playing：**只在开关开时**把那段规则拼进去（默认关 ⇒ 产物逐字节不变）。
			   ①(NP-3) `__npCss` 已挪到函数开头（两条 return 路径共用，真机 bug 见那里的注释）。 */
			/* ①(NP-1) 拼在 emitSurfaceTokens **之外**：那块会在末尾追加"表面 token SSOT"，
			   若把 NP 段塞进入参，产物就不再是"关档 + NP 段"（开关的纯增量性会被破坏，
			   tools/now-playing-test.mjs 的 B10 逐字节断言就是这条）。NP 规则只打我们自己的锚点，
			   位置在后面不影响任何宿主规则的特异性。 */
			return emitSurfaceTokens(css + buildUiCss(section, dlgFrosted)) + __npCss;
		}
...
${(section && (section.bsCompat !== void 0 ? !!section.bsCompat : DEFAULT_BS_COMPAT)) ? `
...
`;
		...
		function MpkgSectionImpl(props) {
			...
			// ①(2026-09-18 §5 第3项)「一键诊断上报」的 UI 状态：发送中 / 结果文案 / 结果是否成功 / 可复制的落点
			const [diagBusy, setDiagBusy] = react.useState(false);
			const [diagResult, setDiagResult] = react.useState("");
			const [diagResultOk, setDiagResultOk] = react.useState(false);
			const [diagResultPath, setDiagResultPath] = react.useState("");
			...
			const commit = (patch, instant, deferApply) => {
				// ②(2026-09-20 真机修复) 换壁纸 = **成套重写源字段**：调用点用 `undefined` 表达
				//   "这个字段不属于新壁纸"，而不变量Ⅰ规定 undefined = 不覆盖（粘性护栏还会把它带回来）
				//   ⇒ 旧 webUrl/sceneKey 永远删不掉，壁纸层就会"先走错路再回退"（真机黑屏级联）。
				//   这里统一转成显式删除（null = 唯一删除语义）；只转壁纸身份字段，见 MPW_SWITCH_SRC_KEYS。
				const next = Object.assign({}, readSection(), mpwSwitchPatchFull(patch));
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					enabled: DEFAULT_ENABLED, npNowPlaying: DEFAULT_NP_NOW_PLAYING, // ①(NP-1) 恢复默认同样关掉
					npLinkWallpaper: DEFAULT_NP_LINK_WALLPAPER, npVolume: DEFAULT_NP_VOLUME, // ①(NP-4) 两个新键一起复位（否则"恢复默认"留下旧电平）
					npPaused: DEFAULT_NP_PAUSED, // ①(NP-5) 恢复默认 = 回到"播放"（与"恢复默认后壁纸照常动"一致）
					powPauseHidden: DEFAULT_POW_PAUSE_HIDDEN, powPauseHiddenUserSet: true // ⑤(2026-09-20) 切页静音属新默认档：恢复默认也要回到它（并打"动过"标记，避免又被迁移改回去）
				}), true);
			...
			const BACKUP_FIELDS = [
				...
				"npNowPlaying", // ①(NP-1) Now playing 挂载开关（可随备份导出/导入）
				"npLinkWallpaper", "npVolume", "npPaused" // ①(NP-4)②① 联动开关 / 音量电平 + ①(NP-5) 用户暂停意图（同上，随备份走）
			];
			const exportBackup = () => {
				try {
					const s = readSection();
			...
			const clearBg = () => {
				...
				const s = readSection();
				/* ②(2026-09-20 真机修复) **清空必须真的清空**：旧写法 `webUrl: undefined` / `sceneKey: undefined`
				   在不变量Ⅰ（undefined = 不覆盖）下等于"没写"，`mpwStickySource` 还会把它从旧档**带回来**
				   ⇒ 点"清除壁纸"后名字/类型显示"无"，但 `webUrl` 还在 ⇒ applyFromStorage 又走 web 分支
				   把壁纸**重新加载了一遍**（用户原话：点清除后壁纸被重新加载）。`null` 才是删除语义。
				   同批一起收口 `timeVideos/timeConfig/info`（同样是"显式列出却写 undefined"= 等于没清）。 */
				writeSection(Object.assign({}, s, { image: "", source: "", fromMpkg: false, converted: "", mpkgKey: "", mpkgName: "", info: null, slot: null, timeVideos: null, timeConfig: null, activeSlot: null, timeSrc: null, timeOverride: null, webUrl: null, sceneKey: null }), true);
				setSection(readSection());
				setMpkgMeta(null);
			...
			/* ①(NP-4)② ② 那一行只在 NP 开着时渲染（渲染用派生量，与持久化默认值同源）。 */
			const npOnNow = section.npNowPlaying !== void 0 ? !!section.npNowPlaying : DEFAULT_NP_NOW_PLAYING;
			...
			try {
					...
					return h("div", { className: "mpw_field" }, [
						h("div", { style: { display: "flex", alignItems: "center", gap: "10px", width: "100%" } }, [
							...
							h("div", { className: "mpw_wallThumb" }, (() => {
								// ①(2026-09-20 真机修复) 预览走**候选链**（mpwThumbCandidates）：
								//   web 档不再落到"什么都没有"，耗尽候选显示类型占位（绝不显示破图图标）。
								const typePh = typeStr || t("none");
								const ph = h("span", { className: "mpw_hint", style: { margin: 0 }, "data-mpw-thumb-ph": "", hidden: has && mpwThumbCandidates(section).length ? "" : void 0 }, has ? typePh : t("none"));
								const cands = has ? mpwThumbCandidates(section) : [];
								if (!cands.length) return [ph];
								const first = cands[0];
								if (first.kind === "video") {
									return [h("video", { className: "mpw_thumbImg", src: first.src, muted: true, playsInline: true, preload: "metadata",
										onLoadedMetadata: (ev) => { try { const v = ev.target; if (v.duration && v.duration > 0.1) v.currentTime = 0.05 } catch {} },
										onError: (ev) => { try { ev.target.style.display = "none" } catch {}; try { const p = ev.target.parentElement && ev.target.parentElement.querySelector("[data-mpw-thumb-ph]"); if (p) p.style.display = "" } catch {} } }), ph];
								}
								const shown = cands.filter((c) => c.kind !== "video").map((c) => c.src);
								return [h("img", { className: "mpw_thumbImg", src: first.src, alt: "", loading: "lazy", decoding: "async",
									"data-mpw-thumb-list": JSON.stringify(shown), "data-mpw-thumb-idx": "0",
									onError: mpwThumbNext }), ph];
							})()),
				...
				// ①(NP-1 2026-09-19 用户第 1 条) Now playing：挂到左侧栏「设置」入口上方。
				//   默认关；关 = 一行 DOM 都不注入、一个观察者都不装（见 lib/now-playing.js）。
				//   左侧栏收起时组件自己隐藏（判据/阈值见 NP_COLLAPSE_MAX_W 的注释）。
				toggleRow(t("npNowPlaying"), t("npNowPlaying.desc"), "npNowPlaying", DEFAULT_NP_NOW_PLAYING),
				/* ①(NP-4)② 「播放/暂停是否同时控制壁纸的播放/暂停」——**放在 Now playing 之下**，
				   并且**只在 npNowPlaying 打开时显示**（NP 关掉 ⇒ 这一行跟着撤掉：一个不挂载的
				   组件不该还留着它的第二个开关）。判据见 tools/np-control-test.mjs 的 F 组
				   （两档：NP 开 ⇒ 这一行在；NP 关 ⇒ 这一行不在，且 DOM 里 0 个 mpw_np_* 节点）。
				   默认值 DEFAULT_NP_LINK_WALLPAPER = **true** ⇒ 默认档与改动前逐字节同行为。 */
				(npOnNow ? toggleRow(t("npLinkWallpaper"), t("npLinkWallpaper.desc"), "npLinkWallpaper", DEFAULT_NP_LINK_WALLPAPER) : null),
				...
				toggleRow(t("powPauseHidden"), t("powPauseHidden.desc"), "powPauseHidden", DEFAULT_POW_PAUSE_HIDDEN),
				...
				h("div", { style: { display: betterSidebar ? "" : "none" } }, [
					...
					toggleRow(t("bs.master"), t("bs.master.desc"), "bsCompat", DEFAULT_BS_COMPAT),
					h("div", { className: "mpw_props", style: { marginTop: 6, display: (section.bsCompat !== void 0 ? !!section.bsCompat : DEFAULT_BS_COMPAT) ? "" : "none" } }, [
				...
				toggleRow(t("todoBlur"), t("todoBlur.desc"), "todoBlur", DEFAULT_TODO_BLUR),
					...
					// ①(2026-09-18 §5 第3项)「一键诊断上报」：**一次点击**把关键子系统状态（磨砂/侧栏/时间线/
					//   壁纸类型与路径/shim 是否注入/视频解码/表面 token/场景健康）发回宿主，落 diag-*.json；
					//   宿主不可用（离线 / 路由未注册 / 403）⇒ 自动改成**下载 JSON 文件**，离线也能把状态发回来。
					//   字段表与来源见 docs/DIAGNOSTICS.md（每个字段都带 provenance，读不到就是 null+degraded）。
					h("div", { className: "mpw_field" }, [
						h("label", { className: "mpw_label" }, t("diagsend.sec")),
						h("p", { className: "mpw_hint" }, t("diagsend.desc")),
						h("div", { className: "mpw_inline" }, [
							h("button", { className: "mpw_button", type: "button", disabled: diagBusy,
								onClick: () => {
									if (diagBusy) return;
									setDiagBusy(true); setDiagResult(""); setDiagResultPath("");
									Promise.resolve().then(() => mpwDiagManual()).then((r) => {
										setDiagBusy(false); setDiagResultOk(!!r.ok);
										if (r.ok) { setDiagResult(t("diagsend.ok") + " " + r.file); setDiagResultPath(String(r.file || "")); }
										else if (r.download && r.download.ok) { setDiagResult(t("diagsend.offline") + " " + r.download.name + "（" + mpwTrim(r.error, 80) + "）"); setDiagResultPath(String(r.download.name || "")); }
										else { setDiagResult(t("diagsend.fail") + " " + mpwTrim((r.download && r.download.error) || r.error, 160)); }
										try { setHint(""); } catch (e) {}
									}).catch((e) => { setDiagBusy(false); setDiagResultOk(false); setDiagResult(t("diagsend.fail") + " " + mpwTrim(e && e.message, 160)); });
								} },
								diagBusy ? t("diagsend.busy") : t("diagsend.btn")),
							diagResultPath ? h("button", { className: "mpw_reset mpw_moreBtn", type: "button",
								onClick: () => mpwCopyText(diagResultPath, () => { try { setHint(t("diag.copied") + " " + diagResultPath); } catch (e) {} }) },
								t("diagsend.copy")) : null,
						]),
						diagResult ? h("p", { className: "mpw_hint", style: { color: diagResultOk ? "#7bd88f" : "#f1a17f", wordBreak: "break-all" } }, diagResult) : null,
					]),
		...
		const zh = {
			...
			"scnRender.extUrl.hint": "留空=不启用。填写后按 extbase 拼进场景渲染 iframe（见渲染器仓文档 EXTENSION-HOOKS.md）；点「测试」验证索引可达并显示钩子数量。",
			...
			"diag.copy": "复制",
			"diag.copied": "已复制",
				"diag.more": "全部开关见渲染器仓文档 README-DIAGNOSTICS.md（node diag-flag-check.mjs 校验代码与文档一致）。",
				"diagsend.sec": "一键诊断上报",
				"diagsend.desc": "把关键子系统状态（磨砂 / 侧栏 / 时间线是否被影响、壁纸类型与路径、shim 是否注入、视频解码、表面 token、场景健康）打包发回本机宿主，落在 ~/.dsh/.dsh-mpkg-wallpaper/diag-<时间戳>.json。每个字段都带来源；读不到的字段不会消失，而是标成 null + degraded 原因。宿主不可用时自动改成下载 JSON 文件（离线也能发回来）。",
				"diagsend.btn": "一键上报",
				"diagsend.busy": "收集中…",
				"diagsend.ok": "✓ 已上报，落点：",
				"diagsend.offline": "宿主不可用 → 已改为下载文件：",
				"diagsend.fail": "✗ 上报与下载都失败：",
				"diagsend.copy": "复制落点",
			...
			"powPauseHidden.desc": "最小化/切页时暂停壁纸的视频与声音（含网页壁纸自带的音频；解码归零），回来自动继续",
			...
			// ①(NP-1 2026-09-19 用户第 1 条) Now playing：开关 + 组件全部文案（zh/en 必须成对）
			"npNowPlaying": "Now playing（左侧栏「设置」上方）",
			"npNowPlaying.desc": "在左侧栏「设置」入口上方挂一个可展开的播放器（默认开；左侧栏收起时自动隐藏）。传输行就是壁纸声音的控制：上一首/下一首按壁纸目录里的音轨清单顺序，播放/暂停控真实媒体，静音/取消静音走既有静音设置。宿主同一位置已经有别的插件的元素时自动让位（不挂/撤下，状态见 data-mpw-np-yield）。数据源只有当前壁纸自己的音轨/声音，本插件没有系统媒体源（详见 docs/NOW-PLAYING-DSH.md §7.7）",
			"np.title": "正在播放",
			"np.idle": "未在播放",
			"np.open": "展开播放器",
			"np.collapse": "收起播放器",
			...
			"np.prev": "上一首",
			"np.next": "下一首",
			"np.noAudio": "该视频没有音轨（如实显示，不假装能出声）",
			"np.mute": "静音",
			"np.unmute": "取消静音",
			"np.src.video": "视频壁纸音轨",
			"np.src.track": "壁纸自带音轨",
			"np.src.web": "网页壁纸声音",
			"np.note.nosource": "当前壁纸没有可控制的音轨：本插件没有系统媒体源，播放控制不可用（详见 docs/NOW-PLAYING-DSH.md）",
			"np.note.nolist": "当前壁纸只有一条音轨，没有上一首/下一首（不假装有列表）",
			"np.note.novolume": "当前壁纸没有可控制的音量通道：只有视频/网页壁纸的声音可控",
			"np.note.decode": "音轨加载失败（浏览器不支持这个格式，或宿主取字节失败）",
			/* ①(NP-4) 新增词的落点（zh）。 */
			"np.volume": "音量",
			"np.seek": "播放进度（可拖动）",
			"np.note.noseek": "这条进度现在不可拖：没有可定位的媒体（联动关掉时壁纸的进度不受控件影响）",
			"np.note.pkgListOnly": "音轨在包内（scene.pkg），宿主没有能取出它的字节通道 ⇒ 只能列出清单，不能播放",
			"np.note.linkOff": "「播放/暂停同时控制壁纸」已关：控件只驱动本插件自己的播放器，壁纸本身的播放不受影响",
			"np.note.linkOffSoundOnly": "「播放/暂停同时控制壁纸」已关：播放/暂停只切换这条音轨的声音（画面继续播放）",
			"np.note.gesture": "为避免无声自动播放被浏览器拦截，首次出声需要你点一下页面（之后完全按设置）",
			"npLinkWallpaper": "播放/暂停同时控制壁纸",
			"npLinkWallpaper.desc": "开（默认）：Now playing 的播放/暂停与进度同时驱动壁纸自己的播放；关：只驱动本插件自己的播放器，壁纸的画面播放不受控件影响（壁纸媒体是唯一声源时控件如实禁用，不假装）。默认开 = 与改动前逐字节同行为（详见 docs/NOW-PLAYING-DSH.md §7.8）",
		};
		const en = {
			"nav": "MPKG Wallpaper",
			...
			"powPauseHidden.desc": "Pause the wallpaper video AND its audio when the page is hidden/in a background tab (including a web wallpaper's own sound; decoder to zero), resume on return",
			...
			"scnRender.extUrl.ph": "http://127.0.0.1:8899/ext",
			"scnRender.extUrl.hint": "Empty = disabled. When set, it is passed to the scene iframe as extbase (see the renderer repo docs EXTENSION-HOOKS.md); press Test to verify the index and show the hook count.",
			...
			"diag.copy": "Copy",
			"diag.copied": "Copied",
			"diag.more": "Full list: the renderer repo docs README-DIAGNOSTICS.md (node diag-flag-check.mjs verifies code/doc consistency).",
			"diagsend.sec": "One-click diagnostics report",
			"diagsend.desc": "Packages the key subsystem state (whether frosting / sidebar / timeline rail are affected, wallpaper type and path, whether the shim was injected, video decode state, surface tokens, scene health) and sends it back to the local host, landing in ~/.dsh/.dsh-mpkg-wallpaper/diag-<timestamp>.json. Every field carries its provenance; unreadable fields are not dropped but marked null + a degraded reason. If the host is unavailable it falls back to downloading the JSON file (works offline).",
			"diagsend.btn": "Send report",
			"diagsend.busy": "Collecting…",
			"diagsend.ok": "✓ Sent. Landed at:",
			"diagsend.offline": "Host unavailable → downloaded instead:",
			"diagsend.fail": "✗ Both sending and downloading failed:",
			"diagsend.copy": "Copy path",
			...
			// ①(NP-1) Now playing: switch + every string the component uses (zh/en stay in pairs)
			"npNowPlaying": "Now playing (above Settings in the sidebar)",
			"npNowPlaying.desc": "Mounts an expandable player above the Settings entry in the left sidebar (on by default; it hides itself when the sidebar is collapsed). Its transport row is the wallpaper-sound control: previous/next follow the wallpaper folder's audio-track list order, play/pause drives the real media element, mute drives the existing mute setting. It yields automatically when another plugin already occupies the same slot (unmounted, state in data-mpw-np-yield). The only source is the current wallpaper's own audio; this plugin has no system media source (see docs/NOW-PLAYING-DSH.md §7.7)",
			"np.title": "Now playing",
			"np.idle": "Nothing playing",
			"np.open": "Open the player",
			"np.collapse": "Collapse the player",
			"np.play": "Play",
			"np.pause": "Pause",
			"np.restart": "Restart",
			"np.prev": "Previous track",
			"np.next": "Next track",
			"np.noAudio": "This video has no audio track (shown as-is, no fake sound)",
			"np.mute": "Mute",
			"np.unmute": "Unmute",
			"np.src.video": "Video wallpaper audio",
			"np.src.track": "Audio track inside the wallpaper",
			"np.src.web": "Web wallpaper sound",
			"np.note.nosource": "This wallpaper has no controllable audio track: the plugin has no system media source, so the transport controls are unavailable (see docs/NOW-PLAYING-DSH.md)",
			"np.note.nolist": "This wallpaper has a single audio track, so there is no previous/next (no fake playlist)",
			"np.note.novolume": "This wallpaper has no controllable volume channel: only video/web wallpaper sound can be controlled",
			"np.note.decode": "The audio track failed to load (unsupported format, or the host could not serve the bytes)",
			/* ①(NP-4) added strings (en). */
			"np.volume": "Volume",
			"np.seek": "Playback position (draggable)",
			"np.note.noseek": "This position is not draggable right now: there is no seekable media (with the link switch off, the wallpaper's own position is left alone)",
			"np.note.pkgListOnly": "The audio lives inside the package (scene.pkg) and the host has no route to serve its bytes, so the list is real but playback is not possible",
			"np.note.linkOff": "\"Play/pause also controls the wallpaper\" is off: the transport only drives this plugin's own player, the wallpaper's own playback is untouched",
			"np.note.linkOffSoundOnly": "\"Play/pause also controls the wallpaper\" is off: play/pause only toggles this track's sound (the picture keeps playing)",
			"np.note.gesture": "To avoid blocked unmuted autoplay, the first sound needs one click on the page (afterwards the setting applies)",
			"npLinkWallpaper": "Play/pause also controls the wallpaper",
			"npLinkWallpaper.desc": "On (default): the Now playing transport also drives the wallpaper's own playback and position. Off: only this plugin's own player is driven and the wallpaper's playback is untouched (when the wallpaper media is the only sound source the controls are honestly disabled). On by default = byte-identical to the previous behaviour (see docs/NOW-PLAYING-DSH.md §7.8)",
		};
		...
		let sblurRecheckTimer = 0;   // ⑥ 延迟重算的合并句柄（动画期间不堆积定时器）
		function setupSblurObserver() {
			...
			const check = () => {
				try {
					...
					// ⑥(2026-09-20) **结构判据与可见性判据分开**：`data-mpw-holds-layer` 由"谁真的是弹层的祖先"
					//   决定（不依赖动画/透明度/高度阈值，也不依赖 Firefox 上不可靠的 :has()），
					//   `data-mpw-sblur-off` 仍按"看得见的弹层"决定（视觉档位）。两者一起用：
					//   前者保证"弹层祖先永远不是 containing block"，后者保证"弹层打开时侧栏不虚化"的观感。
					try { mpwSyncLayerHosts(); } catch {}
					try { mpwUntagInjectedCompat("sblur-check"); } catch {}
					if (hasDlg) {
			...
			// ⑥(2026-09-20) 动画/重渲染时序兜底：弹层刚插进 DOM 的那一刻，它可能还是
			//   `opacity:0`/高度 0（入场动画）—— 此刻 `isOpenOverlay` 判不出"可见"，于是
			//   `data-mpw-sblur-off` 会被摘掉；但 `data-mpw-holds-layer` 是**结构判据**，
			//   上面那次 check 已经打上了。为了不依赖"哪一次 mutation 恰好落在动画结束后"，
			//   这里再排一次延迟重算（合并去重，不堆积定时器）。
			const recheck = () => {
				if (sblurRecheckTimer) return;
				sblurRecheckTimer = setTimeout(() => { sblurRecheckTimer = 0; check(); }, 300);
			};
			try {
				sblurObserver = new MutationObserver(() => { check(); recheck(); });
				sblurObserver.observe(document.body, { childList: true, subtree: true });
			} catch { sblurObserver = null; }
			// 窗口尺寸变化会让"近全屏"阈值与面板 rect 都变（父对话的真机判据就带视口切换）⇒ 一并重算
			try { window.addEventListener("resize", recheck); } catch {}
			check();
		}
		...
		function applyInner(ctx) {
			// ①(NP-1) 模块级取词口：`t` 在设置面板里来自 props（`const { t } = props`），
			//   在模块作用域**不存在** ⇒ Now playing 的挂载路径（applyFromStorageInner 调用链）
			//   看不见它。这里存一份 `ctx.locale.bind(NS)`，与面板/词典注册同一个命名空间。
			try { mpwT = ctx.locale.bind(NS); } catch (e) { mpwT = (k) => k; }
			/* ①(2026-09-19 设置持久化轮) **boot 收尾闸门 + 启动合成**（顺序就是根因链的顺序）：
			   ① 先开闸门：这之后到宿主设置合并完之间的任何写都只置脏、不落盘 —— 否则 boot 那次
			      applyFromStorage() 会把"宿主字段还没合并进来"的残缺档同时写进两处存储；
			   ② 再**在渲染之前**把两处存储合成一次：旧写法要先渲染一次"无源"档
			      （→ `.mpw-bgWrap{display:none}`），等宿主 GET 回来才重渲染 ⇒ 现场表现就是
			      "刷新后壁纸先消失、再（可能）回来"；合成后第一次渲染就是完整的。
			   ③ 只在**第一次** apply 做（apply 会因 RTC 重连/热重注入重跑）。 */
			if (!window.__mpwBootReconciled) {
				window.__mpwBootReconciled = true;
				mpwBootPending = true;
				try {
					const rec0 = mpwReconcileStores(mpwLoadLocalRaw(), {});
					const carry0 = {};
					const localS0 = mpwStripMeta(mpwLoadLocalRaw());
					for (const k of MPW_SRC_KEYS) if (mpwVal(localS0, k) !== void 0 && mpwVal(rec0.merged, k) === void 0) carry0[k] = mpwVal(localS0, k);
					sectionCache = mpwNormalizeSection(mpwMergeSection(rec0.merged, carry0));
					mpwTrace("settings:boot-merge", {
						stage: "pre-host", winner: rec0.winner, newer: rec0.newer, reason: rec0.reason,
						localKeys: rec0.localKeys.length, hostKeys: rec0.hostKeys.length, halfArchive: !!rec0.half,
						srcKeys: MPW_SRC_KEYS.map((k) => (sectionCache[k] === void 0 ? "-" : (sectionCache[k] ? "set" : "empty")))
					});
				} catch (e) { mpwErr("applyInner(boot 合成)", e) }
			}
			// 背景 DOM 常驻
			ensureBgDom();
			...
			// ①(NP-1 2026-09-19 用户第 1 条) Now playing：占用宿主官方附加 slot
			//   `sidebar.footer.action`（list / scope root / replaceRisk none），
			//   它的出口就在「设置」入口**正前方**（`footArea > [footerActions, settingsArea]`）。
			//   宿主自带的插件开发 skill 明写：小的侧栏动作优先用这类附加 slot，
			//   **不要去操作宿主的硬编码 DOM 选择器** ⇒ 我们优先走 slot；
			//   slot 没被渲染（老版本/注册失败）时，控制器自己退回 DOM 插入（见 lib/now-playing.js 的 resolveAnchor）。
			//   开关关时组件返回 null ⇒ 零注入、零观察者（断言见 tools/now-playing-test.mjs 的 B 段）。
			ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({
				name: "sidebar.footer.action",
				id: "mpw-now-playing",
				order: 60,
				locale: NS,
			}, MpwNowPlayingSlotAction));
