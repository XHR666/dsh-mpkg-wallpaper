...
`;
		...
		function MpkgSection(props) {
			...
			// ①(修正) 恢复后**更新而非清空** ref（记录实际恢复后的新比例）——若清空，中途
			// 其他 setState（缩略图加载/better-sidebar 检测）触发重渲染时会漏恢复 → 跳顶
			// （用户实测：停下不动也跳、勾选也跳）。配合网格 overflow-anchor:auto 双重保障。
			react.useEffect(() => {
				if (!rotEditor) return;
					...
					requestAnimationFrame(() => {
						try {
							...
							if (max > 0) {
								...
								rotScrollRef.current = { ratio: el.scrollTop / (el.scrollHeight - el.clientHeight || 1) };
							}
						} catch {}
					});
				});
				...
				rotEditor && rotEdit ? h(MaskPortal, { onClick: () => setRotEditor(false) }, [
					h("div", { className: "mpw_dialog", style: { width: "min(560px, calc(100vw - 32px))" }, onClick: (ev) => ev.stopPropagation() }, [
						...
						wallList.length === 0
							...
							: h("div", { ref: rotGridRef, style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, maxHeight: 220, overflowY: "auto" }, overflowAnchor: "auto" }, wallList.filter((w) => rotFilter === "all" ? true : rotFilter === "steam" ? w.src === "steam" : !w.src || w.src !== "steam").map((w) => {
