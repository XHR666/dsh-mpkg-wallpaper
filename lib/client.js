window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function checkSceneVideo(key) {
			return fetch(sceneVideoCheckUrl(key), { method: "GET" })
				.then((r) => (r.ok ? r.json() : null))
				.then((d) => ({ has: !!(d && d.ok && d.has), mime: (d && d.mime) || "video/mp4" }))
				.catch(() => ({ has: false, mime: "video/mp4" }));
		}
		...
		const applySceneWallpaper = (opts) => {
			// opts: { key, image, source, mpkgKey, fallback } —— fallback(key, image) 执行原有链
			try {
				const { key, image, source, mpkgKey } = opts;
				checkSceneVideo(key).then((v) => {
					if (gen !== bgGen) return;
					if (v.has) {
						// 内嵌视频 → converted:"mp4" 走 video 播放（sv=1 标记 → scene-video 路由）
						const svImg = image + "&sv=1";
						commit({
							image: svImg, source, mpkgKey, mpkgName: source,
						...
						setMpkgMeta({ name: source, key: mpkgKey, info: null, entryName: "scene.pkg", slot: null });
						setHint(t("lib.applied") + "：" + source);
					} else {
						...
						vid.__mpwTransLoadingTimeout = setTimeout(() => {
							...
							try {
								...
								const poll = setInterval(() => {
									if (gen !== bgGen) { clearInterval(poll); return; }
...
`;
		...
		const zh = {
			...
			"sec.aqua": "Aqua 实验",
			"sec.aqua.desc": "外观方案来自 Bil812（PR #2）的构想，已采用并全部做成独立开关（默认全关，不影响原功能）：统一雾 = 全屏遮罩让所有表面共享一种雾色；自适应文字 = 文字色随背景亮度变化 + 蓝色清理。（面板取色开关已移到「外观」tab 的主题颜色区域）",
		...
		const en = {
			...
			"sec.aqua.desc": "Appearance ideas from Bil812 (PR #2), adopted and made into independent toggles (all OFF by default, original features untouched): unified fog = full-screen mask, every surface shares one fog color; adaptive ink = text follows background brightness + blue cleanup. (The panel-tint toggle moved to the Appearance tab, Theme-color area)",
