		...
		function MpkgSection(props) {
			...
			const SpeedBar = (props) => {
				...
				const pick = (clientX) => {
					...
					const idx = Math.min(SPEED_LEVELS.length - 1, Math.round(ratio * (SPEED_LEVELS.length - 1)));
					onChange(SPEED_LEVELS[idx]);
				};
				return h("div", { className: "mpw_speedBar", ref,
					style: { position: "relative", height: 30, margin: "6px 0 2px", touchAction: "none", cursor: "pointer" },
					...
					// 轨道
					h("div", { style: { position: "absolute", left: 0, right: 0, top: "50%", height: 6, marginTop: -3, borderRadius: 3, background: "color-mix(in srgb, var(--dsw-alias-interactive-bg-hover, #333a4a) 60%, transparent)" } }),
					// 分段竖线 + 填充进度
					h("div", { style: { position: "absolute", left: 0, top: "50%", height: 6, marginTop: -3, borderRadius: 3, background: "var(--dsw-alias-brand-primary, #4f8cff)", width: ((SPEED_LEVELS.indexOf(value) + 1) / SPEED_LEVELS.length * 100) + "%" } }),
					SPEED_LEVELS.map((rate, i) => h("div", { key: rate, style: {
						position: "absolute", top: "50%", marginTop: -9, left: (i / (SPEED_LEVELS.length - 1) * 100) + "%",
						width: 1, height: 18, background: "color-mix(in srgb, var(--dsw-alias-border-l2, #444b5c) 70%, transparent)"
					} })),
					// 当前档位标记
					h("div", { style: { position: "absolute", top: "50%", marginTop: -8, left: "calc(" + (SPEED_LEVELS.indexOf(value) / (SPEED_LEVELS.length - 1) * 100) + "% - 7px)", width: 14, height: 14, borderRadius: "50%", background: "var(--dsw-alias-brand-primary, #4f8cff)", border: "2px solid var(--dsw-alias-bg-base, #0e1420)" } }),
					// 档位标签
					SPEED_LEVELS.map((rate, i) => h("span", { key: rate, className: "mpw_hint", style: { position: "absolute", top: 32, left: (i / (SPEED_LEVELS.length - 1) * 100) + "%", transform: "translateX(-50%)" } }, (rate === 1 ? "1" : rate)))
				);
			};
			...
			return h("div", { className: "mpw_row" }, [
				...
				rotEditor && rotEdit ? h(MaskPortal, { onClick: () => setRotEditor(false) }, [
					h("div", { className: "mpw_dialog", style: { width: "min(560px, calc(100vw - 32px))" }, onClick: (ev) => ev.stopPropagation() }, [
						...
						h("input", { className: "mpw_input", type: "text", style: { width: "100%", boxSizing: "border-box", marginBottom: 6 }, placeholder: t("rot.namePh"), value: rotEdit.name, onChange: (ev) => setRotEdit(Object.assign({}, rotEdit, { name: ev.target.value })) }),
						h("div", { className: "mpw_inline", style: { marginBottom: 6 } }, [
							h("span", { className: "mpw_hint" }, t("lib.rotateMin")),
							h("input", { className: "mpw_input", type: "number", style: { width: 70 }, min: 1, max: 120, value: rotEdit.interval, onChange: (ev) => setRotEdit(Object.assign({}, rotEdit, { interval: Math.max(1, parseInt(ev.target.value || "5", 10)) })) }),
							h("span", { className: "mpw_hint" }, t("lib.minutes")),
						...
						h("div", { style: { maxHeight: 200, overflowY: "auto" } }, wallList.map((w) => h("label", { className: "mpw_row", key: w.key, style: { display: "flex", alignItems: "center", gap: 6, padding: "3px 0" } }, [
							h("input", { type: "checkbox", checked: (rotEdit.keys || []).includes(w.key),
								onChange: (ev) => {
									const keys = (rotEdit.keys || []).slice();
									if (ev.target.checked) { if (!keys.includes(w.key)) keys.push(w.key); }
									else { const i = keys.indexOf(w.key); if (i >= 0) keys.splice(i, 1); }
									setRotEdit(Object.assign({}, rotEdit, { keys }));
								} }),
							h("span", { className: "mpw_hint", style: { flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, w.title)
						]))),
						h("div", { className: "mpw_inline", style: { marginTop: 8 } }, [
							h("button", { className: "mpw_button", type: "button", onClick: () => {
								...
								const g = { id: rotEdit.id || ("rot" + Date.now()), name: rotEdit.name.trim() || t("rot.unnamed"), interval: Math.max(1, rotEdit.interval || 5), order: rotEdit.order === "random" ? "random" : "sequence", keys };
				...
				h("div", { className: "mpw_field" }, [
					h("label", { className: "mpw_label" }, t("speed")),
