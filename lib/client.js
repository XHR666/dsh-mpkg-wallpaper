	...
	factory: (require) => {
		...
		function showWebEl(url) {
			...
			frame.onload = () => {
				...
				// ①(修正) 隐藏面板/挂观察器**延迟到 canvas 创建后**：星野这类 WebGL 壁纸的
				// canvas 在 onload 之后才由 JS 创建，onload 立刻隐藏会因 containsCanvas 检测不到
				// canvas 而把壁纸可见容器整个藏掉（只剩底色）。延迟一帧让 canvas 先挂上。
				try {
		...
		function hideWebPanel(frame) {
			try {
				...
				const hidePanelChain = () => {
					...
					if (b) {
						// ①(修正) 只藏面板容器的**直接父层**（1 层即可），每层先过 containsCanvas 护栏。
						// 之前深度 4 太激进：星野这类 WebGL 壁纸若 canvas 未创建，护栏失效，
						// 会把壁纸可见容器（含 canvas 的祖先）整个藏掉 → 只剩底色。
