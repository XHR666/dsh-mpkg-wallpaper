window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			// ①(新) 主题颜色：body 门控 + CSS 变量（默认空 = DSH 默认品牌蓝）
			try {
			...
			try {
				...
				// ①(新) 液态玻璃（elysia395 方案）：设置窗口玻璃化总开关门控
				if (section.glassWindow !== void 0 ? !!section.glassWindow : false)
...
`;
			...
			// ①(新) 液态玻璃（参考 elysia395 的 glassColor/glassAlpha/glassWindow）：
			// 玻璃底色 + 白叠层强度 + 设置窗口玻璃化总开关；作用于插件设置卡片与弹窗。
			if (section.glassWindow !== void 0 ? !!section.glassWindow : false) {
				const gc = section.glassColor && /^#[0-9a-fA-F]{6}$/.test(section.glassColor)
					? section.glassColor
					: (document.body && document.body.hasAttribute("data-ds-dark-theme") ? "#0f1520" : "#ffffff");
				const ga = Math.max(0, Math.min(60, section.glassAlpha !== void 0 ? section.glassAlpha : 12));
				css += `
/* ── 液态玻璃（elysia395 方案）：设置卡片/弹窗玻璃化 ── */
body[data-mpw-glass] {
	--mpw-glass-color: ${gc};
	--mpw-glass-alpha: ${ga}%;
}
...
`;
			...
			// ①(新) 主题颜色（accent）：按钮/滑条/选中/链接/发送键跟随自定义主题色
			//（参照 elysia395 的 accent；放末尾保证覆盖 aquaInk 的品牌色）
			if (section.themeColor && /^#[0-9a-fA-F]{6}$/.test(section.themeColor)) {
				css += `
/* ── 主题颜色：品牌色统一为自定义主题色（可开关，空 = DSH 默认蓝） ── */
body[data-mpw-theme] {
	--dsw-alias-brand-primary: var(--mpw-theme-color, #3964fe);
	--dsw-alias-brand-text: var(--mpw-theme-color, #3964fe);
	--dsw-alias-button-info-fill: var(--mpw-theme-color, #3964fe);
	--dsw-alias-button-info-hover: var(--mpw-theme-color, #3964fe);
	--dsw-alias-state-business-primary: var(--mpw-theme-color, #3964fe);
	--dsw-alias-interactive-bg-selected: color-mix(in srgb, var(--mpw-theme-color, #3964fe) 16%, transparent);
}
`;
			}
...
`;
		...
		function MpkgSection(props) {
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					glassColor: "", glassAlpha: 12, glassWindow: false,
			...
			const SpeedBar = (props) => {
				...
				return h("div", { className: "mpw_speedBar", style: { position: "relative", height: 56, margin: "8px 0 0", touchAction: "none", cursor: "pointer" },
					ref,
					...
					// 统一坐标系：左右 12px 内边距，轨道/竖线/球/进度/数字标签全部用 pos(i)
					h("div", { style: { position: "absolute", left: PAD, right: PAD, top: 14, height: 6, borderRadius: 3, background: "color-mix(in srgb, var(--dsw-alias-interactive-bg-hover, #333a4a) 60%, transparent)" } }),
					h("div", { style: { position: "absolute", left: PAD, top: 14, height: 6, borderRadius: 3, background: "var(--dsw-alias-brand-primary, #4f8cff)", width: "calc(" + (idx / (SPEED_LEVELS.length - 1) * (100 - PAD * 2)) + "% - 0px)" } }),
					SPEED_LEVELS.map((rate, i) => h("div", { key: rate, style: {
						position: "absolute", top: 6, left: "calc(" + PAD + "px + " + (i / (SPEED_LEVELS.length - 1) * (100 - PAD * 2)) + "% - 1px)",
						width: 1, height: 22, background: "color-mix(in srgb, var(--dsw-alias-border-l2, #444b5c) 70%, transparent)"
					} })),
					h("div", { style: { position: "absolute", top: 8, left: "calc(" + PAD + "px + " + (idx / (SPEED_LEVELS.length - 1) * (100 - PAD * 2)) + "% - 7px)", width: 14, height: 14, borderRadius: "50%", background: "var(--dsw-alias-brand-primary, #4f8cff)", border: "2px solid var(--dsw-alias-bg-base, #0e1420)" } }),
					SPEED_LEVELS.map((rate, i) => h("button", { key: "b" + rate, type: "button", style: {
						position: "absolute", top: 32, left: pos(i), transform: "translateX(-50%)",
						padding: "4px 2px", fontSize: 11, border: 0, background: "transparent", cursor: "pointer", whiteSpace: "nowrap",
						color: i === idx ? "var(--dsw-alias-brand-primary, #4f8cff)" : "var(--dsw-alias-label-tertiary)",
						fontWeight: i === idx ? 700 : 400
					}, onClick: (ev) => { ev.stopPropagation(); onChange(SPEED_LEVELS[i]); } }, (rate === 1 ? "1" : rate) + "x"))
				);
			};
			...
			return h("div", { className: "mpw_row mpw_glassHost" }, [
				...
				toggleRow(t("glassWindow"), t("glassWindow.desc"), "glassWindow", false),
				// 配色（accent，复用 themeColor——驱动品牌色）
				h("div", { className: "mpw_field" }, [
					...
					h("div", { className: "mpw_inline" }, [
						...
						h("button", { className: "mpw_miniBtn", type: "button", onClick: () => commit({ themeColor: "" }, true) }, t("glass.reset"))
					]),
				...
				// 玻璃透明度（glassAlpha 0-60%）
				h("div", { style: { display: (section.glassWindow !== void 0 ? !!section.glassWindow : false) ? "" : "none" } },
		...
		const zh = {
			...
			"glass.accent.hint": "驱动按钮/滑条/选中/链接/发送键等品牌色（与外观 tab 的主题颜色同一个）",
			"glass.color": "玻璃颜色",
			...
			"themeColor": "主题颜色",
			"themeColor.hint": "驱动按钮/滑条/选中项/链接/发送键等品牌色（取色盘 + 预置）；空 = DSH 默认蓝",
			"speed": "视频倍速",
