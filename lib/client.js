window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			// ①(新) bsBottomAvoid：让 better-sidebar 底部面板避让 DSH 左 rail——
			// better-sidebar 底部面板 left 是 JS 内联写死（测量 [data-slot=conversation]
			// 的 rect.left），DSH 收起侧边栏时测量可能不含 rail 宽度 → 面板左缘覆盖
			// 收起的 rail（用户实测：悬浮+收起+底部面板展开重叠）。开启时读取 DSH
			// 左侧 sidebarCol 实际宽度写 --mpw-dsh-rail-w，CSS 偏移底部面板。
			try {
				const avoid = section.bsBottomAvoid !== void 0 ? !!section.bsBottomAvoid : false;
				if (avoid) {
					const sc = document.querySelector('[class*="sidebarCol"]');
					const w = sc ? sc.getBoundingClientRect().width : 0;
					document.documentElement.style.setProperty("--mpw-dsh-rail-w", w + "px");
				} else {
					document.documentElement.style.removeProperty("--mpw-dsh-rail-w");
				}
			} catch {}
...
/* 7) 底部面板避让 DSH 左 rail（bsBottomAvoid 独立开关）：better-sidebar 底部面板
   left 是 JS 内联写死（测 centerRect.left），悬浮+收起时 rail 宽度未排除 → 面板
   左缘覆盖收起的侧边栏（用户实测）。开启时 JS 读 sidebarCol 宽度写
   --mpw-dsh-rail-w，这里给底部面板加 margin-left 避让（left 是内联的，margin 可
   在其基础上叠加偏移）。 */
[data-dsh-better-sidebar] [class*="_bottomPanel"],
[data-dsh-better-sidebar] [data-dsh-bottom-panel] {
	margin-left: var(--mpw-dsh-rail-w, 0px) !important;
}
` : ""}
