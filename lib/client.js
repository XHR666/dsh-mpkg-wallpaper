	...
	factory: (require) => {
		...
		function buildCss(section) {
			...
			if (section.fontColorGray !== void 0 ? !!section.fontColorGray : false) {
				const gcGray = section.fontColorGrayColor && /^#[0-9a-fA-F]{6}$/.test(section.fontColorGrayColor) ? section.fontColorGrayColor : "#9aa4b2";
				css += `
...
`;
		...
		const TAB_ORDER = ["source", "wallpaper", "appearance", "unify", "blur", "show", "aqua", "other"];
		...
		function MpkgSection(props) {
			...
			const checkFfmpeg = () => {
				...
				fetch(HOST_BASE + "/ffmpeg-check", { method: "GET" }).then(async (r) => {
					...
					if (r.ok && d && d.ok && d.found) setFfmpegState({ ready: true, path: d.path, version: d.version });
			...
			if (!ffmpegState || ffmpegState.checking || ffmpegState.downloading) {
				ffStatus = ffmpegState && ffmpegState.downloading ? t("ffmpeg.downloading") : "…";
				...
				ffStatus = t("ffmpeg.ready") + (ffmpegState.path ? " (" + ffmpegState.path + (ffmpegState.version ? ": " + ffmpegState.version : "") + ")" : "");
			...
			return h("div", { className: "mpw_row mpw_glassHost" }, [
				...
				// ffmpeg 状态：显示已就绪(path: version) / 未安装；未安装时提供下载按钮（用户点击才联网）
				h("div", { className: "mpw_field" }, [
