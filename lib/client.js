window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** canvas 动态渲染：cover 适配 + 视差摆动 + 呼吸缩放。 */
		function showSceneEl(manifest) {
			...
			let started = false;
			const draw = () => {
				if (sceneAnim === null || !canvas || canvas.style.display === "none") return;
				...
				const t = Date.now();
				const swayX = Math.sin(t / 3200) * 12;
				const swayY = Math.cos(t / 3800) * 12;
				const breath = 1 + 0.012 * Math.sin(t / 2600);
				const cx = vw / 2, cy = vh / 2;
				c2.clearRect(0, 0, vw, vh);
				// ①(修正) 图层定位：清单坐标是场景包围盒内偏移（[0,W]×[0,H]），
				// 场景中心 = (W/2, H/2) 应对齐屏幕中心。之前少了中心偏移且符号反了
				// → 所有图层堆到左上角（用户实测凯尔希/Girl and Cat 叠在左上）。
				...
				for (let i = 0; i < layers.length; i++) {
					...
					const dx = l.px ? swayX * l.px : 0;
					const dy = l.py ? swayY * l.py : 0;
					const w = l.w * s, h = l.h * s;
					c2.drawImage(im, cx + (l.x - offX) * s - w / 2 + dx, cy + (l.y - offY) * s - h / 2 + dy, w, h);
				}
			...
			layers.forEach((l, i) => {
				...
				im.onload = () => { sceneImgs[i] = im; pending--; if (pending === 0 && !started) { started = true; sceneAnim = requestAnimationFrame(draw); } };
				im.onerror = () => { pending--; if (pending === 0 && !started) { started = true; sceneAnim = requestAnimationFrame(draw); } };
				im.src = l.url;
			});
			if (pending === 0 && !started) { started = true; sceneAnim = requestAnimationFrame(draw); }
		}
...
`;
		...
		function MpkgSection(props) {
			...
			const applyCustomWeb = (w) => {
				...
				setWebConfirm({ title: w.title, name: w.name, media: w.media || "index.html", src: "custom" });
			};
			...
			const applyLibraryWallpaper = (wp) => {
				...
				if (wp.type === "web") {
					...
					setWebConfirm({ title: wp.title || wp.ltoken, ltoken: wp.ltoken, media: wp.media, src: "library" });
					return;
				}
