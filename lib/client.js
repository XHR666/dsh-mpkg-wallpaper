...
[data-dsh-better-sidebar] [class*="_panel"],
[data-dsh-better-sidebar] [class*="_bottomPanel"] {
	background-color: color-mix(in srgb, var(--dsw-alias-bg-base) 62%, transparent) !important;
}
` : ""}
...
[data-dsh-better-sidebar] [class*="_pane"],
...
body[data-ds-dark-theme] [data-dsh-better-sidebar] [class*="_pane"],
...
` : ""}
/* 5) 底部面板避让（总开关开启即生效，不依赖子开关）：better-sidebar 底部面板是
   absolute bottom:0 全宽（z-index:40），只挤压了 DSH 中心列、**没有挤压左侧边栏**
   → 面板盖住 DSH 左侧边栏底部（设置按钮区），悬浮时因 sidebarCol 有 margin 空隙
   更显眼（用户实测「下侧边栏切掉收起侧边栏」）。这里补上左侧边栏的底部挤压：
   better-sidebar 展开底部面板时 --dsh-sidebar-height > 0，sidebarCol 同步让出
   高度，设置按钮区抬到面板上方，不再被盖。收起面板（变量=0）自动恢复。
   与 better-sidebar 对中心列的挤压同机制（同一 CSS 变量），不依赖其内部类名。 */
[class*="sidebarCol"] {
	margin-bottom: var(--dsh-sidebar-height, 0px);
	transition: margin-bottom var(--ds-transition-duration-slow, 0.2s) var(--ds-ease-in-out, ease);
}
/* 悬浮时 sidebarCol 自带 margin（12px/-12px），需与避让叠加：
   底部 margin = 悬浮 margin + 面板高度（收起态右负边距保持 rail 对齐） */
body[data-mpw-float] [class*="sidebarCol"] {
	margin-bottom: calc(12px + var(--dsh-sidebar-height, 0px)) !important;
}
body[data-mpw-float] [data-sidebar-collapsed] [class*="sidebarCol"],
body[data-mpw-float] [data-sidebar-collapsed] .pI_x6G_sidebarCol {
	margin: 12px -12px 12px 12px;
...
`;
		...
		const TAB_ORDER = ["source", "wallpaper", "appearance", "unify", "blur", "aqua", "other"];
		...
		function MpkgSection(props) {
			...
			// ①(新) 轮播勾选网格滚动保持：勾选/切换过滤会重建列表，浏览器把 scrollTop 归零
			// 跳顶（用户实测：新建轮播列表勾选全部壁纸后往下翻页回到顶部）。勾选前记录比例
			// （见下方卡片 onClick），这里在 keys/filter 变化渲染完成后按比例恢复（双 rAF
			// 确保新 DOM 已布局，与目录选择器 dirListRef 的恢复同款）。
			react.useEffect(() => {
				if (!rotEditor) return;
				const ratio = rotScrollRef.current && rotScrollRef.current.ratio;
				if (ratio === void 0 || ratio === null) return;
				requestAnimationFrame(() => {
					requestAnimationFrame(() => {
						try {
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					bsCompat: false, bsFloat: false, bsFont: false, bsReveal: false, bsAlpha: false,
			...
			const BACKUP_FIELDS = [
				...
				"fpsCap", "resMax", "bsCompat", "bsFloat", "bsFont", "bsReveal", "bsAlpha"
			];
			const exportBackup = () => {
				try {
					const s = readSection();
			...
			const toggleRow = (label, desc, field, def, disabled) => {
			...
			return h("div", { className: "mpw_row mpw_glassHost" }, [
				...
				rotEditor && rotEdit ? h(MaskPortal, { onClick: () => setRotEditor(false) }, [
					h("div", { className: "mpw_dialog", style: { width: "min(560px, calc(100vw - 32px))" }, onClick: (ev) => ev.stopPropagation() }, [
						...
						h("div", { className: "mpw_inline", style: { marginBottom: 6 } }, [
							h("button", { className: "mpw_miniBtn" + (rotFilter === "all" ? " mpw_on" : ""), type: "button", onClick: () => { try { const el = rotGridRef.current; if (el) { const max = el.scrollHeight - el.clientHeight; rotScrollRef.current = { ratio: max > 0 ? el.scrollTop / max : 0 }; } } catch {} setRotFilter("all"); } }, t("rot.all")),
							h("button", { className: "mpw_miniBtn" + (rotFilter === "custom" ? " mpw_on" : ""), type: "button", onClick: () => { try { const el = rotGridRef.current; if (el) { const max = el.scrollHeight - el.clientHeight; rotScrollRef.current = { ratio: max > 0 ? el.scrollTop / max : 0 }; } } catch {} setRotFilter("custom"); } }, t("rot.filterCustom")),
							h("button", { className: "mpw_miniBtn" + (rotFilter === "steam" ? " mpw_on" : ""), type: "button", onClick: () => { try { const el = rotGridRef.current; if (el) { const max = el.scrollHeight - el.clientHeight; rotScrollRef.current = { ratio: max > 0 ? el.scrollTop / max : 0 }; } } catch {} setRotFilter("steam"); } }, t("rot.filterSteam")),
							...
							return h("div", { className: "mpw_pickCard" + (on ? " mpw_on" : ""), key: w.key,
								...
								onClick: () => {
									...
									try {
										...
										if (el) {
											const max = el.scrollHeight - el.clientHeight;
											rotScrollRef.current = { ratio: max > 0 ? el.scrollTop / max : 0 };
										}
									} catch {}
				...
				// ①(新) dsh-better-sidebar 适配分类（总开关 + 子开关；仅检测到已安装时显示）
				betterSidebar ? h(react.Fragment, null, [
					...
					(section.bsCompat !== void 0 ? !!section.bsCompat : false) ? h("div", { className: "mpw_props", style: { marginTop: 6 } }, [
