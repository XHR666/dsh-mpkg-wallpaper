...
function readSection() {
		...
		const applySceneWallpaper = (opts) => {
			...
			try {
				...
				// ①(修复) 探测/合成是异步链（最长 4s+）：期间用户点了别的/清除了 →
				// 提交前比对签名，变了就丢弃，防"最后选择被覆盖/描述与画面错配"
				const __sig0 = sectionSigNow();
				...
				checkSceneVideo(key).then((v) => {
					if (sectionSigNow() !== __sig0) {
						try { console.warn("[dsh-mpkg-wallpaper] scene 探测完成时壁纸已切换，丢弃迟到应用"); } catch {}
						return;
					}
					if (v.has) {
					...
					if (sectionSigNow() === __sig0) { try { opts.fallback(key, image); } catch {} }
				});
		...
		function applyFromStorageInner() {
			bgGen++;
			const gen = bgGen;
			// ①(修复) 镜像当前代：共享 video error 监听器是首次挂载时的闭包（gen 陈旧），
			// 内部 "过期回调" 守卫改比 window 镜像，保证第 2 张起的 404 重试/自愈仍生效
			try { window.__mpwApplyGen = bgGen; } catch {}
			...
			if (typeof image === "string" && image.indexOf("host:") === 0) {
				...
				if (section.converted === "mp4") {
					...
					if (vid && !vid.__mpwErrWired) {
						...
						vid.addEventListener("error", () => {
							// ①(修复) 去掉 `gen !== bgGen` 陈旧守卫：video 是共享元素，error 只属于
							// 当前 src；旧守卫在第一次应用之后恒成立 → 404 重试/转码回退/
							// token 自愈对第 2 张起全部静默失效（黑屏不自愈）。死循环由
							// hostTries/transFallback 单次计数器兜底。
							...
							vid.__mpwHostRetryVideo = setTimeout(() => {
								if (window.__mpwApplyGen !== bgGen) return;
						...
						vid.__mpwTransLoadingTimeout = setTimeout(() => {
							...
							try {
								...
								const poll = setInterval(() => {
									if (window.__mpwApplyGen !== bgGen) { clearInterval(poll); return; }
				...
				idbGet("bg").then((v) => {
					if (window.__mpwApplyGen !== bgGen) return; // 过期回调（已有更新的应用）
				...
				idbGet("bg").then((v) => {
					if (window.__mpwApplyGen !== bgGen) return; // 过期回调
					if (!(v instanceof Blob)) return;
					const sig = v.size + ":" + v.type;
					if (lastBgSig && lastBgSig.sig === sig && lastBgSig.url) {
				...
				idbGet("bg").then((v) => { if (window.__mpwApplyGen !== bgGen) return; if (v && img.src !== v) { img.src = v; showImageEl(); } }).catch(() => {});
			} else {
...
`;
		...
		function MpkgSection(props) {
			...
			react.useEffect(() => {
				...
				return () => { try { if (window.__mpwRotTimer) clearInterval(window.__mpwRotTimer); window.__mpwRotTimer = null; } catch {} };
