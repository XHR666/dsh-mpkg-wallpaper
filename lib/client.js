...
// ①(2026-09-12 根因修复) 字符串字段归一化：持久化状态里 image 可能是布尔 true，
		...
		function mpwDeriveSourceFromKey(key) {
			...
			if (k.indexOf("custommpkg|") === 0) {
				...
				return mpwHostJson("/custom-mpkg?" + q).then((d) => {
		 ...
		 *  **只做一次**：同一把 key 只尝试一次（迟到那次成功即收敛，失败也不反复重试）。 */
		function mpwHealHalfArchive() {
			...
			try {
				...
				if (!key) return false;
				if (mpwHealSig === key && MPW_HEAL.attempted > 0) return false;   // 只一次
				...
				mpwTrace("settings:heal-attempt", { key });
		...
		function applyWebMute(frame) {
			try {
				const s = readSection();
				const mute = (s.mute !== void 0 ? !!s.mute : true) || npAudioOwns();
		...
		/** ①(NP-4) 联动的唯一判据（②）：开 ⇒ 传输动作同时驱动壁纸媒体；关 ⇒ 只驱动我们的播放器。 */
		...
		function npPrimePlay(video) {
			...
			if (wallUserPaused || powPaused || npUserPausedOf(video)) {
				npApplyMute(); npApplyVolume();
				npOpTrace("prime", null, "gated: wallUserPaused=" + wallUserPaused + " powPaused=" + powPaused + " userPaused=" + npUserPausedOf(video));
				return false;
			}
...
`;
		...
		function npAudioScope(section) {
			...
			if (s.folderName) return { mode: "custom", folder: String(s.folderName), src: "folderName" };
			...
			if (folder) return { mode: "custom", folder: folder, src: s.webUrl ? "webUrl" : "image" };
		...
		function npFetchTracks(section) {
			...
			try {
				fetch(url).then((r) => (r && r.ok ? r.json() : null)).then((d) => {
		...
		function npTransport(op, section, value) {
			...
			try {
				if (op === "play" || op === "pause") {
					...
					if (vid && link) {
						if (op === "play") { npSetUserPaused(vid, false); npPrimePlay(vid); did = "video.play"; }
						else { npWantPlay = false; npSetUserPaused(vid, true); vid.pause(); did = "video.pause"; }
						npApplyFrameMute();
					} else {
						...
						if (list && list.tracks.length) {
							...
							} else if (npAudio) { npAudio.pause(); did = "audio.pause"; }
						} else if (!vid) {
		...
		function applyNowPlaying(section) {
			...
			try {
				const vid = npActiveVideo(s);
				if (vid) { npWatchVideo(vid); if (vid.paused && !npPlayBlocked) npPrimePlay(vid); }
			} catch (e) {}
