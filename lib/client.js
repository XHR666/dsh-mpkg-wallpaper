	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			if (image) {
				...
				// ①(修正) 壁纸 blur 归谁管：
				//  - 统一虚化开 → unifyAmount 接管壁纸层 blur（整屏统一模糊，**无论
				//    chatFollow 开/关**——用户实测：chatFollow 关时 unifyAmount 完全
				//    不影响壁纸（bgBlur 走了磨砂条 frostBlur），「整屏虚化程度」条失效）
				//  - 统一虚化关 → 磨砂条控制
				//  chatFollow 只决定聊天区表面是否额外由磨砂条接管（见 scrollBody 规则）。
...
`;
			...
			// ①(修正) 统一虚化 + 聊天区不跟随的旧「侧边栏背后叠加 backdrop blur」规则已删除：
			// 现在 unifyTint 开时壁纸层 blur 恒 = unifyAmount（整屏统一模糊，见 applyFromStorage
			// 的 --mpw-bg-blur），侧边栏/标题栏/聊天区都透出模糊壁纸，无需再给侧边栏单独叠
			// backdrop blur——旧规则会与壁纸层 blur 双重叠加（视觉过度模糊），且 backdrop-filter
			// 会把 sidebarCol 变成 fixed 弹窗的 containing block（困住弹窗）。
