...
`;
			if (headerBlur && headerBg) {
				...
				// ①(修正) 标题栏雾底：**始终由「标题栏磨砂程度」条（headerBlurAmount）控制**——
				// 之前 unify 开时写死 0（全透明）导致统一虚化无法控制标题栏透明度（用户实测）；
				// 上次改成跟随 sidebarAlpha 后，用户调「标题栏磨砂程度」条仍无效（该条被忽略）。
				// 现在 unify 开/关都尊重 headerBlurAmount：0=透明透出壁纸，拉高=白雾。
