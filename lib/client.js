...
body[data-mpw-float] [data-sidebar-collapsed] [class*="sidebarCol"] {
	/* ①(修正) 用 padding 而非 margin 做左右悬浮边距：原 margin: 12px -12px 12px 12px
	   让 rail 卡片右缘溢出（与 better-sidebar 面板重叠）；改 margin: 12px 0 12px 12px
	   又窄 12px（rail 图标行差几个像素被裁——用户实测）。用 padding: 0 12px 让卡片
	   **宽度 = rail 轨道**（内容完整），左右 padding 内缩出悬浮边距感，右缘贴轨道
	   （不溢出、不重叠），内容不被裁。 */
	margin: 12px 0;
	padding: 0 12px;
	border-radius: 12px;
...
/* ①(修正) rail 卡片右缘**不向右溢出**（margin 已改为左右 0 + padding 内缩）。 */
