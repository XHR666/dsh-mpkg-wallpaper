...
`;
			...
			// ⑲(新) 深底文字可读增强（aquaTextEnhance，默认关）：全局文字加黑色细描边。
			// 说明：无法精确实现"只变经过深色处的文字"（需要逐字采样背景像素）。
			// 采用开源社区标准方案：多重 text-shadow 黑色描边——文字保留原色，
			// 黑色轮廓让深色/黑色背景上依然可读（比之前的白+黑双色描边干净，无模糊感）。
			if (section.aquaTextEnhance !== void 0 ? !!section.aquaTextEnhance : false) {
				css += `
/* ── 深底文字可读增强（实验，默认关）：黑色细描边保证任意背景可读 ── */
body[data-mpw-aqua-text] [data-conversation-scroll] :is(p, li, h1, h2, h3, h4, span, pre, code, td, th) {
	text-shadow: 1px 1px 1px rgba(0,0,0,.9), -1px -1px 1px rgba(0,0,0,.9), 0 0 2px rgba(0,0,0,.9) !important;
}
`;
			}
		...
		function MpkgSection(props) {
			...
			const openPicker = (key, currentColor) => {
				try {
					...
					hexInput.style.cssText = "flex:1;min-width:0;height:28px;border:1px solid var(--dsw-alias-border-l2);border-radius:6px;padding:0 8px;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-primary);font:inherit;font-size:12px;";
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					aquaMask: DEFAULT_AQUA_MASK, aquaTint: DEFAULT_AQUA_TINT, aquaInk: DEFAULT_AQUA_INK, aquaColor: "", aquaInkColor: "", aquaTextEnhance: DEFAULT_AQUA_TEXT_ENHANCE, todoBlur: DEFAULT_TODO_BLUR, aquaMaskAlpha: DEFAULT_AQUA_MASK_ALPHA,
