window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function initHostSettings() {
			try {
				fetch(HOST_BASE + "/settings", { method: "GET" }).then(async (r) => {
					...
					hostSettingsOk = !!(d && d.ok);
					if (!hostSettingsOk || !d.settings) return;
					// host 有数据 → 覆盖 localStorage（本地是旧的/空时也统一）
					try { localStorage.setItem(STORE_KEY, JSON.stringify(d.settings)); } catch {}
					sectionCache = d.settings;
					applyFromStorage();
				...
				if (hostSettingsOk) {
					try {
						fetch(HOST_BASE + "/settings", {
							...
							body: JSON.stringify(sectionCache || {})
						}).catch(() => {});
					} catch {}
				}
			}, instant ? 0 : 250);
		...
		function showVideoEdge(url, video) {
			...
			edgeDraw = { ctx: g, video, wrap, fit: readSection().zoom >= 100 ? "cover" : "contain" };
			...
			// 首帧前先画一帧（避免白屏）
			try { video.addEventListener("loadeddata", () => edgeDrawFrame(), { once: true }); } catch {}
		}
		 ...
		 *  壁纸在 iframe 内无法交互（用户实测），自带面板只能看不能点 → 直接 CSS 隐藏。
		 *  注入到 iframe 文档里（同源可访问），每次加载/切换都执行。 */
		...
		function applyFromStorage() {
			...
			if (typeof image === "string" && image.indexOf("host:") === 0) {
				...
				const hostUrl = hostBustTick ? hostUrl0 + (hostUrl0.indexOf("?") >= 0 ? "&" : "?") + "_t=" + hostBustTick : hostUrl0;
...
`;
		...
		function MpkgSection(props) {
			...
			react.useEffect(() => {
				...
				fetch(url).then((r) => (r.ok ? r.json() : null)).then((d) => {
					...
					setWebCfg({ skel: d.SettingModel.skel || "", model: d.SettingModel, languages: langs, customUrl: url, name });
			...
			const applyWebCfg = (patch) => {
				try {
					...
					const next = Object.assign({}, stored || {}, { SettingModel: merged });
					...
					try {
						...
						frame.src = u + (u.indexOf("?") >= 0 ? "&" : "?") + "_t=" + Date.now();
					} catch {}
			...
			const exportBackup = () => {
				try {
					...
					a.href = url;
					a.download = "dsh-mpkg-wallpaper-backup.json";
			...
			const importBackup = (file) => {
				...
				reader.onload = () => {
					try {
						...
						const s = readSection();
						const patch = {};
						for (const k of BACKUP_FIELDS) if (settings[k] !== void 0) patch[k] = settings[k];
			...
			return h("div", { className: "mpw_row mpw_glassHost" }, [
				...
				// ③(新) web 壁纸选项（L2D 类：分辨率 2k/4k/8k + 语言 + 音量）
				// 写入 iframe 同源 localStorage → 重载生效；与 mpkg 可调参数同一位置。
				webCfg ? h("div", { className: "mpw_field" }, [
					h("label", { className: "mpw_label" }, t("webcfg.title")),
					h("p", { className: "mpw_hint" }, t("webcfg.desc")),
					// 分辨率（2k/4k/8k 三选）
					webCfg.model && webCfg.model.resolution ? h("div", { className: "mpw_inline", style: { flexWrap: "wrap" } }, [
					...
					// 语言（中文/日本語/English/… 由 AnimationsTexts 键决定）
					webCfg.languages && webCfg.languages.length ? h("div", { className: "mpw_inline", style: { flexWrap: "wrap" } }, [
					...
					typeof webCfg.model.bgmVolume === "number" ? h("div", { className: "mpw_field" }, [
						...
						h("input", { className: "mpw_slider", type: "range", min: 0, max: 1, step: 0.01, value: webCfg.model.bgmVolume || 0,
							onChange: (ev) => applyWebCfg({ bgmVolume: Number(ev.target.value) }) })
					]) : null,
					typeof webCfg.model.talkVolume === "number" ? h("div", { className: "mpw_field" }, [
						...
						h("input", { className: "mpw_slider", type: "range", min: 0, max: 1, step: 0.01, value: webCfg.model.talkVolume || 0,
							onChange: (ev) => applyWebCfg({ talkVolume: Number(ev.target.value) }) })
					]) : null,
					// 其他开关（触摸区域框 / 文本框）
				...
				h("div", { className: "mpw_field" }, [
					h("label", { className: "mpw_label" }, t("backup.title")),
		...
		const zh = {
			...
			"backup.export": "导出备份",
			"backup.import": "导入备份",
			"backup.exported": "备份已导出（dsh-mpkg-wallpaper-backup.json）",
		...
		const en = {
			...
			"backup.exported": "Backup exported (dsh-mpkg-wallpaper-backup.json)",
