		...
		function MpkgSection(props) {
			...
			const SpeedBar = (props) => {
				...
				return h("div", { className: "mpw_speedBar", style: { position: "relative", height: 56, margin: "8px 0 0", touchAction: "none", cursor: "pointer" },
					...
					// ①(修正) 统一坐标系：左右 16px 内边距，轨道/竖线/球/进度/数字标签
					// 全部用 pos(i) = 16px + i/5*(100%-32px)——数字中心严格落在刻度上。
					// （之前 space-between 按钮宽度不同 → 中心不落在刻度；全宽又边缘被裁）
					// 轨道
					h("div", { style: { position: "absolute", left: 16, right: 16, top: 14, height: 6, borderRadius: 3, background: "color-mix(in srgb, var(--dsw-alias-interactive-bg-hover, #333a4a) 60%, transparent)" } }),
					// 填充进度
					h("div", { style: { position: "absolute", left: 16, top: 14, height: 6, borderRadius: 3, background: "var(--dsw-alias-brand-primary, #4f8cff)", width: "calc(" + (idx / (SPEED_LEVELS.length - 1) * (100 - 32)) + "% - 0px)" } }),
					// 分段竖线
					SPEED_LEVELS.map((rate, i) => h("div", { key: rate, style: {
						position: "absolute", top: 6, left: "calc(16px + " + (i / (SPEED_LEVELS.length - 1) * (100 - 32)) + "% - 1px)",
						width: 1, height: 22, background: "color-mix(in srgb, var(--dsw-alias-border-l2, #444b5c) 70%, transparent)"
					} })),
					// 当前档位球
					h("div", { style: { position: "absolute", top: 8, left: "calc(16px + " + (idx / (SPEED_LEVELS.length - 1) * (100 - 32)) + "% - 7px)", width: 14, height: 14, borderRadius: "50%", background: "var(--dsw-alias-brand-primary, #4f8cff)", border: "2px solid var(--dsw-alias-bg-base, #0e1420)" } }),
					// 数字标签也可点击
					SPEED_LEVELS.map((rate, i) => h("button", { key: "b" + rate, type: "button", style: {
						position: "absolute", top: 32, left: "calc(16px + " + (i / (SPEED_LEVELS.length - 1) * (100 - 32)) + "%)", transform: "translateX(-50%)",
					...
					}, onClick: () => onChange(SPEED_LEVELS[i]) }, (rate === 1 ? "1" : rate) + "x"))
				);
			};
					...
					h("div", { className: "mpw_dialog", style: { width: "min(560px, calc(100vw - 32px))" }, onClick: (ev) => ev.stopPropagation() }, [
						...
						wallList.length === 0
							...
							return h("div", { className: "mpw_pickCard" + (on ? " mpw_on" : ""), key: w.key,
								...
								// ①(修正) 固定 64px 高度容器：视频用首帧 <video>，图片用 <img>；
								// 加载失败只隐藏媒体（容器保留高度）→ 界面不再上下跳
								h("div", { style: { width: "100%", height: 64, position: "relative", background: "color-mix(in srgb, var(--dsw-alias-interactive-bg-hover, #333a4a) 30%, transparent)" } }, [
									turl && (w.type === "video" || w.converted === "mp4")
