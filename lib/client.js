		...
		function MpkgSection(props) {
			...
			const SelectBox = (props) => {
				...
				return h("div", { className: "mpw_select", style: { position: "relative", flex: 1, minWidth: 0 } }, [
					h("button", { className: "mpw_input mpw_selectBtn", type: "button",
						onClick: () => setSelectOpen(selectOpen === props.name ? "" : props.name) },
						h("span", { style: { flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, cur ? cur.label : placeholder),
						h("span", { className: "mpw_hint" }, selectOpen === props.name ? "▲" : "▼")
					),
					selectOpen === props.name
						? h("div", { className: "mpw_selectMenu", style: { position: "absolute", left: 0, right: 0, top: "100%", marginTop: 4, zIndex: 2600, maxHeight: 220, overflowY: "auto" } },
			...
			const SpeedBar = (props) => {
				...
				return h("div", { className: "mpw_speedBar" }, [
					h("div", { ref,
						style: { position: "relative", height: 30, margin: "8px 6px 2px", touchAction: "none", cursor: "pointer" },
						...
						// 轨道（左右留 6px 防端点被裁剪）
						h("div", { style: { position: "absolute", left: 6, right: 6, top: "50%", height: 6, marginTop: -3, borderRadius: 3, background: "color-mix(in srgb, var(--dsw-alias-interactive-bg-hover, #333a4a) 60%, transparent)" } }),
						// 填充进度（left:6 起，宽度相对容器 → 右边缘 = 6px + idx/(N-1)*(100-12)%）
						h("div", { style: { position: "absolute", left: 6, top: "50%", height: 6, marginTop: -3, borderRadius: 3, background: "var(--dsw-alias-brand-primary, #4f8cff)", width: (idx / (SPEED_LEVELS.length - 1) * (100 - 12)) + "%" } }),
						// 分段竖线：calc(6px + X%)——之前 6 没写单位被并进百分比 → 位置错
						SPEED_LEVELS.map((rate, i) => h("div", { key: rate, style: {
							position: "absolute", top: "50%", marginTop: -9, left: "calc(6px + " + (i / (SPEED_LEVELS.length - 1) * (100 - 12)) + "% - 1px)",
							width: 1, height: 18, background: "color-mix(in srgb, var(--dsw-alias-border-l2, #444b5c) 70%, transparent)"
						} })),
						// 当前档位标记：与进度右边缘严格对齐
						h("div", { style: { position: "absolute", top: "50%", marginTop: -8, left: "calc(6px + " + (idx / (SPEED_LEVELS.length - 1) * (100 - 12)) + "% - 7px)", width: 14, height: 14, borderRadius: "50%", background: "var(--dsw-alias-brand-primary, #4f8cff)", border: "2px solid var(--dsw-alias-bg-base, #0e1420)" } })
					),
					// ①(修正) 档位数字：正常流一行（可点击、当前档高亮），不再绝对定位重叠
					h("div", { className: "mpw_inline", style: { justifyContent: "space-between", margin: "2px 6px 0" } },
						SPEED_LEVELS.map((rate) => h("button", { key: rate, type: "button",
							className: "mpw_speedLabel" + (rate === value ? " mpw_on" : ""),
							style: { flex: "1 1 0", padding: "4px 0", fontSize: 12, textAlign: "center", border: 0, background: "transparent", cursor: "pointer",
								color: rate === value ? "var(--dsw-alias-brand-primary, #4f8cff)" : "var(--dsw-alias-label-tertiary)",
								fontWeight: rate === value ? 700 : 400 },
			...
			return h("div", { className: "mpw_row" }, [
				...
				rotEditor && rotEdit ? h(MaskPortal, { onClick: () => setRotEditor(false) }, [
					h("div", { className: "mpw_dialog", style: { width: "min(560px, calc(100vw - 32px))" }, onClick: (ev) => ev.stopPropagation() }, [
						...
						// ①(修正) 名称输入框与「图片链接」输入框同款（纯 mpw_input，无 inline 覆盖）
						h("input", { className: "mpw_input", type: "text", style: { width: "100%", boxSizing: "border-box", marginBottom: 10 }, placeholder: t("rot.namePh"), value: rotEdit.name, onChange: (ev) => setRotEdit(Object.assign({}, rotEdit, { name: ev.target.value })) }),
						...
						wallList.length === 0
							...
							: h("div", { style: { maxHeight: 200, overflowY: "auto" } }, wallList.map((w) => {
							const on = (rotEdit.keys || []).includes(w.key);
							return h("div", { className: "mpw_row mpw_pickRow" + (on ? " mpw_on" : ""), key: w.key,
								style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, cursor: "pointer", marginBottom: 2,
									background: on ? "color-mix(in srgb, var(--dsw-alias-brand-primary, #4f8cff) 16%, transparent)" : "transparent" },
								onClick: () => {
								...
								h("span", { style: { flex: "none", width: 20, height: 20, borderRadius: 6, border: "1px solid " + (on ? "var(--dsw-alias-brand-primary, #4f8cff)" : "var(--dsw-alias-border-l2, #444b5c)"), display: "flex", alignItems: "center", justifyContent: "center", color: on ? "var(--dsw-alias-brand-primary, #4f8cff)" : "transparent", fontSize: 14 } }, "✓"),
								h("span", { className: "mpw_hint", style: { flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--dsw-alias-label-primary)" } }, w.title)
							]);
						})),
