window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function showImageEl() {
			...
			if (frame) {
				try { if (frame.__mpwWebObs) { frame.__mpwWebObs.disconnect(); frame.__mpwWebObs = null; } } catch {}
		...
		function webMediaObserve(frame) {
			try {
				...
				try {
					...
					wrap.__mpwWebObsCleanup = () => { try { if (frame.__mpwWebObs) { frame.__mpwWebObs.disconnect(); frame.__mpwWebObs = null; } } catch {} };
				} catch {}
			} catch {}
		}
		 ...
		 *  面板显示由壁纸代码切换 #basetting 的祖父元素 display（F() 函数），
		 *  所以 CSS 隐藏 #basetting 之外，还要沿 DOM 向上把外层容器一起藏掉。 */
		function hideWebPanel(frame) {
			try {
				...
				// 沿 DOM 向上藏外层容器（面板 display 由祖父元素控制）
				const panel = doc.getElementById("basetting");
				if (panel) {
					let el = panel.parentElement && panel.parentElement.parentElement;
					let depth = 0;
					while (el && depth < 3) {
