...
function mpwTrace(ev, data) {
	try {
		...
		/* ①(NP-1 2026-09-19 用户第 1 条「now playing 挂载到 dsh 设置上面 有一个开关启用是否挂载
		   左边栏收起就隐藏」) Now playing 挂到 DSH 左侧栏「设置」入口**上方**。**默认关**：
		   不开 ⇒ 不注入任何 DOM、不装任何观察者、buildCss 产物里一行 NP 规则都没有
		   （三条都由 tools/now-playing-test.mjs 的 B 段断言）。
		   为什么默认关：它要往宿主侧栏里放东西（DOM + 样式 + 一个 ResizeObserver），
		   属于"看得见的行为改变"，与本仓 lgCss / sidebarBlur 同一条取向 —— 默认关、用户显式开。 */
		const DEFAULT_NP_NOW_PLAYING = false;
...
`;
		...
		//   · 开关 `npNowPlaying`（默认关）在「壁纸设置」tab 里；
		...
		//   开关关 ⇒ 这一整段一次都不跑：不建节点、不装观察者（setEnabled(false) 是纯拆除）。
		// ═══════════════════════════════════════════════════════════════════
		...
		/**
		 * 数据源：**只报我们真的知道的东西**。
		 * 情形① video 类壁纸：DSH 页面里就有那个 <video>（自带音轨）⇒ 播放/暂停/进度/音量**真能控**。
		 * 情形② scene/自定义壁纸自带音轨：音轨清单能从宿主扫描路由拿到（名字/mime/refs），
		 *        但**播放它的是场景渲染器**，插件没有那条控制通道，扫描结果里也没有时长 ⇒ 只显示、不假装。
		 * 情形③ web 类壁纸：声音走既有「静音」设置 + 宿主 /media-audio 契约（WP-1 新加）⇒ 能静音/调音量。
		 * 情形④ 无源：空闲态（曲名 = 未在播放），点播放**不做假动作**（面板一行说明 + console 一行）。
		 */
		function npResolveMedia(section) {
			...
			// 主 <video>（video 类壁纸 + hybrid 流式播放的落点）
			let vid = null;
			try { vid = document.querySelector("#mpw-bgWrap video, .mpw-bgVideo, video#mpw-bgVideo") || null; } catch {}
			if (vid && vid.tagName && String(vid.tagName).toUpperCase() === "VIDEO") {
				out.kind = "video";
				...
				out.canPlay = true; out.canVolume = true;
				return out;
			}
			if (id.kind === "web") {
				out.kind = "web";
				out.title = id.name;
				out.canVolume = true;       // 走既有 mute 设置 + /media-audio
				return out;
			}
			if (id.kind === "library" || id.kind === "custom") {
				...
				if (cached && cached.key === id.key) {
					if (cached.count > 0 && cached.tracks && cached.tracks.length) {
						out.title = cached.tracks[0].path ? String(cached.tracks[0].path).split(/[\\/]/).pop() : id.name;
						out.byline = id.name + " · " + (cached.tracks[0].mime || "audio");
					} else {
						out.kind = "none"; out.title = ""; out.byline = id.name;
					}
				} else {
					// 还没拿到清单：先按"未知"显示，拿到后 setMedia 覆盖（不猜）
					out.byline = mpwT("np.src.track");
				}
				return out;
			}
		...
		function npFetchTracks(section) {
			...
			let url = "";
			try {
				if (s.folderName) url = HOST_BASE + "/custom-scene-audio?refs=0&folder=" + encodeURIComponent(String(s.folderName));
				else if (s.mpkgKey) url = HOST_BASE + "/library-scene-audio?refs=0&ltoken=" + encodeURIComponent(String(s.mpkgKey));
			} catch {}
			if (!url) return;
			const id = npWallpaperIdentity(s);
			if (npMediaCache.key === id.key && npMediaCache.data) return;
			const seq = ++npMediaToken;
			try {
				fetch(url).then((r) => (r && r.ok ? r.json() : null)).then((d) => {
					if (seq !== npMediaToken) return;            // 迟到的应答丢掉（壁纸已经换了）
					npMediaCache = { key: id.key, at: Date.now(), data: { count: d && d.count || 0, tracks: (d && d.tracks) || [] } };
					applyNowPlaying(readSection());
				}).catch(() => {
					if (seq !== npMediaToken) return;
					npMediaCache = { key: id.key, at: Date.now(), data: { count: 0, tracks: [] } };
				});
			} catch (e) {}
		}
		...
		function npTransport(op, section) {
			...
			const vid = (() => { try { return document.querySelector("#mpw-bgWrap video, .mpw-bgVideo, video#mpw-bgVideo") || null; } catch { return null } })();
			try {
				if (op === "play" && vid) { const pr = vid.play(); if (pr && pr.catch) pr.catch(() => {}); }
				else if (op === "pause" && vid) vid.pause();
				else if (op === "restart" && vid) { try { vid.currentTime = 0; } catch {} }
				else if (op === "mute") {
		...
		function npEnsureCtl() {
			...
			try {
				const mod = MPW_NP.createNowPlaying({
					...
					react: react,
					createRoot: (node) => {
						try { return require("react-dom/client").createRoot(node); }
						catch (e) { mpwErr("np createRoot", e); return null; }
					},
				...
				try { window.__mpwNowPlaying = npCtl; window.__mpwNowPlayingSlotAction = npSlotAction; } catch {}
		...
		function applyNowPlaying(section) {
			...
			const ctl = npEnsureCtl();
			if (!ctl) return;
			if (!on) { ctl.setEnabled(false); return; }
			ctl.setEnabled(true);
			ctl.setMedia(npResolveMedia(s));
			try { if (ctl.isEnabled()) npFetchTracks(s); } catch {}
		}
		/** 静态渐变兜底背景（视频不可用时）。 */
...
function createNowPlaying(deps) {
  ...
  const h = react.createElement;
    ...
    const api = {
      ...
      inspectCollapse: () => ({ hostWide, hidden, fit: fitScale(measureWidth()), width: measureWidth(), hostCollapsed: hostSaysCollapsed(), settlePending: !!settleTimer }),
...
`;
			...
			/* ①(NP-1) Now playing：**只在开关开时**把那段规则拼进去（默认关 ⇒ 产物逐字节不变）。
			   拼在 emitSurfaceTokens 的**入参**里：它是我们自己的规则，不参与四表面 token 代换。 */
...
`;
		...
		const zh = {
			...
			"npNowPlaying.desc": "在左侧栏「设置」入口上方挂一个可展开的播放器；左侧栏收起时自动隐藏。默认关 = 不注入任何 DOM、不装任何观察者。数据源是当前壁纸自带的音轨/声音，本插件没有系统媒体源（详见 docs/NOW-PLAYING-DSH.md）",
			"np.title": "正在播放",
			"np.idle": "未在播放",
			"np.open": "展开播放器",
			"np.collapse": "收起播放器",
		...
		const en = {
			...
			"npNowPlaying.desc": "Mounts an expandable player above the Settings entry in the left sidebar; it hides itself when the sidebar is collapsed. Off by default = no DOM injected and no observer installed. The source is the current wallpaper's own audio; this plugin has no system media source (see docs/NOW-PLAYING-DSH.md)",
