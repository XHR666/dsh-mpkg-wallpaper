...
/* ⑳(修正) 顶部 Tab 条：dshmarket 风格（下划线高亮）+ 左右翻页动画 */
.mpw_tabBar {
	display: flex; gap: 2px; align-items: flex-end;
	border-bottom: 1px solid var(--dsw-alias-border-l2);
	margin-bottom: 8px; position: sticky; top: 0; z-index: 5;
	background: color-mix(in srgb, var(--dsw-static-neutral-bluish-00) 94%, transparent);
}
body[data-ds-dark-theme] .mpw_tabBar {
	background: color-mix(in srgb, var(--dsw-static-neutral-bluish-950) 94%, transparent);
}
.mpw_tab {
	flex: none; font: inherit; color: var(--dsw-alias-label-secondary);
	cursor: pointer; white-space: nowrap; background: 0 0; border: none;
	border-bottom: 2px solid transparent; padding: 7px 12px; font-size: 13px;
}
.mpw_tab:hover { color: var(--dsw-alias-label-primary); }
.mpw_tabActive {
	color: var(--dsw-alias-brand-primary, #4f6ef7);
	border-bottom-color: var(--dsw-alias-brand-primary, #4f6ef7);
	font-weight: 600;
}
/* 左右翻页动画：切换时内容从侧边滑入 + 淡入 */
.mpw_tabBody { min-height: 60px; }
.mpw_tabBody.mpw_slideRight { animation: mpwSlideRight .18s ease; }
.mpw_tabBody.mpw_slideLeft { animation: mpwSlideLeft .18s ease; }
@keyframes mpwSlideRight { from { opacity: .25; transform: translateX(26px); } to { opacity: 1; transform: none; } }
@keyframes mpwSlideLeft { from { opacity: .25; transform: translateX(-26px); } to { opacity: 1; transform: none; } }
...
`;
		...
		function MpkgSection(props) {
			...
			// ②(新) Tab 切换方向（左右翻页动画）：slideDir="right"/"left"，动画结束后清除
			const [slideDir, setSlideDir] = react.useState(null);
			const switchTab = (id) => {
				if (id === settingsTab) return;
				const order = ["source", "appearance", "unify", "blur", "show", "aqua", "other"];
				setSlideDir(order.indexOf(id) > order.indexOf(settingsTab) ? "Right" : "Left");
				setSettingsTab(id);
			};
			...
			return h("div", { className: "mpw_row" }, [
				// ①(修正) 标题/描述移到 Tab 条上方（用户要求）；Tab 仿 dshmarket 下划线式 + 左右翻页动画
				h("div", { className: "mpw_title" }, t("title")),
				h("p", { className: "mpw_desc" }, t("desc")),
				// ②(新) 顶部 Tab：分类切换，下划线高亮（dshmarket 风格），切换带滑入动画
				h("div", { className: "mpw_tabBar" }, [
				...
				// 内容区（滑动动画容器；tab div 始终挂载，hooks 稳定）
				h("div", { className: "mpw_tabBody" + (slideDir ? " mpw_slide" + slideDir : ""), onAnimationEnd: () => setSlideDir(null) }, [
				// source 组
				h("div", { style: { display: settingsTab === "source" ? "" : "none" } }, [
				// ═══ 背景来源 ═══
				...
				// ═══ 外观 ═══
				h("div", { style: { display: settingsTab === "appearance" ? "" : "none" } }, [
				...
				// ═══ 统一虚化（独立大标题分组）═══
				h("div", { style: { display: settingsTab === "unify" ? "" : "none" } }, [
				...
				// ═══ 界面虚化 ═══
				h("div", { style: { display: settingsTab === "blur" ? "" : "none" } }, [
				...
				// ═══ 透出壁纸 ═══
				h("div", { style: { display: settingsTab === "show" ? "" : "none" } }, [
				...
				// ═══ Aqua 实验（默认全关，不影响原功能）═══
				h("div", { style: { display: settingsTab === "aqua" ? "" : "none" } }, [
				...
				// ═══ 其他 ═══
				h("div", { style: { display: settingsTab === "other" ? "" : "none" } }, [
