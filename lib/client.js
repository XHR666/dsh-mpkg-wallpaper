...
// ①(2026-09-12 根因修复) 字符串字段归一化：持久化状态里 image 可能是布尔 true，
		...
		function showImageEl() {
			...
			disarmSceneWatchdog(true); // ①(批次15 B1) 切到图片壁纸：解除场景看门狗并清兜底视觉
			try { if (window.__mpwLnReset) window.__mpwLnReset(); } catch {} // ①(批次15 B3) 方向键守卫复位
			if (frame) {
		...
		function mpwSceneClearFallback() {
			try {
				const { img, wrap } = bgElements();
				if (wrap) wrap.classList.remove("mpw-scene-fallback");
				if (img) { try { img.removeAttribute("src"); } catch {} try { img.onerror = null; } catch {} }
			} catch {}
		}
		function disarmSceneWatchdog(clearVisual) {
			const wd = __mpwSceneWd;
			if (wd) { try { if (wd.checkTimer) clearInterval(wd.checkTimer); } catch {} }
			__mpwSceneWd = null;
			if (clearVisual) mpwSceneClearFallback();
		}
		...
		function showWebEl(url) {
			...
			try {
				...
				frame.setAttribute("sandbox", __sbStrict ? MPW_SANDBOX_STRICT_ATTR : MPW_SANDBOX_LEGACY_ATTR);
				frame.setAttribute("data-mpw-sandbox", __sbStrict ? "strict" : "legacy");
			...
			try { if (mpwStripBust(frame.getAttribute("src")) !== mpwEffUrl) frame.src = mpwEffUrl; } catch {}
			...
			try {
				if (mpwStripBust(frame.getAttribute("src")) === url) {
		 ...
		 */
		let lastFrostReason = '';   // 诊断用：磨砂为何开/为何关
		function ensureHeaderFrost(blurPx) {
			try {
				const hdr = document.querySelector(".wSkVaW_header, header[class*='_header_'], header");
				if (!hdr) return null;
				let el = hdr.querySelector(":scope > .mpw-hdrFrost");
				if (!el) {
		...
		function syncHeaderFrost() {
			try {
				...
				// ①(2026-09-13) 总开关关闭 → 撤掉注入的磨砂层与"自动半透明"属性（不留残留）
				...
				if (!enabled) {
					...
					try { const h = document.querySelector(".wSkVaW_header, header[class*='_header_'], header"); if (h) h.removeAttribute("data-mpw-hdr-translucent") } catch {}
					lastFrostReason = "插件总开关关闭";
					return;
				}
				...
				// ①(2026-09-13 用户第三次反馈"标题栏模糊还是没有" → 真因)
				//   旧判据 `(hb || un) && bg`：**顶栏透出(headerBg) 没开时磨砂永远是 0** ——
				//   而不透明的顶栏后面做模糊本来也看不见。用户真正要的是"磨砂玻璃顶栏"，
				//   所以现在：只要开了「标题栏磨砂」或「整屏虚化」，就**自动给顶栏半透明底 + 磨砂**；
				//   `?hdrfrost=off` 可关该自动透明（保留顶栏原本的不透明外观）。
				const autoTransparent = new URLSearchParams(location.search).get("hdrfrost") !== "off";
				// ①(2026-09-14 第四次反馈"标题栏磨砂还是没有" → 真机 diag 给出的真因)
				//   diag: `injected:false`（磨砂层压根没注入），而伪元素 computed 已是 blur(30px)。
				//   因为默认值 headerBlur=false + headerBg=false，而旧门控要求"磨砂或整屏虚化至少开一个"
				//   ⇒ 两个默认全关 → 永远不注入。用户的预期是**壁纸开着时顶栏就该是磨砂玻璃**，
				//   不该要求他先猜对三个开关。现规则（三条，互不冲突）：
				//     · 壁纸在显示 且 用户从未动过磨砂开关 → 默认磨砂（autoFrost）✅ 修本次反馈
				//     · 用户动过开关（writeSection 打 headerFrostUserSet）→ 完全按他的选择
				//     · `?hdrfrost=off` → 永远不自动半透明（保持旧逃生口语义）
				...
				const wanted = autoTransparent ? (hb || un || autoFrost) : ((hb || un) && bg);
				...
				try {
					const hdrEl = document.querySelector(".wSkVaW_header, header[class*='_header_'], header");
					if (hdrEl) {
						if (wanted && autoTransparent) {
							hdrEl.setAttribute("data-mpw-hdr-translucent", "");
							lastFrostReason = (hb || un)
								? ((bg ? "" : "顶栏透出未开→已自动半透明") + (own ? "独立强度" : (un ? "跟随整屏虚化" : "标题栏磨砂强度")))
								: "壁纸在显示且未显式配过磨砂→默认磨砂（?hdrfrost=off 可关）";
						} else {
							hdrEl.removeAttribute("data-mpw-hdr-translucent");
							lastFrostReason = !wanted ? "磨砂与整屏虚化都未开启" : "?hdrfrost=off（保留不透明顶栏）";
						}
					}
				} catch {}
				...
				ensureHeaderFrost(px);
			} catch {}
		}
		function applyFromStorageInner() {
			bgGen++;
			// ①(2026-09-13) 标题栏磨砂：注入/刷新真实磨砂层（设置一变立即生效）
			try { syncHeaderFrost(); } catch {}
			try {
				if (!window.__mpwHdrFrostGuard) {
					window.__mpwHdrFrostGuard = setInterval(() => { try { syncHeaderFrost(); } catch {} }, 3000);
				}
			} catch {}
			const gen = bgGen;
		...
		// ②(2026-09-11 改自动) dsh 认证会 303 清掉 query（?mpwdiag=1 丢失）——改为零操作自动上报：
		// 页面加载后 6s 上报一次；此后每当"设置面板/菜单"出现（MutationObserver）再上报，
		// 每会话上限 6 次（避免刷屏）。host 用 ?mpwdiag=0 可关（localStorage）。
		...
		try {
			if (localStorage.getItem('mpwdiag') !== '0') {
				let mpwDiagCount = 0;
...
html body [class*="sidebarCol"]:not([class*="_tab"]) {
	background-color: ${unifyTint
		...
		: `color-mix(in srgb, var(--dsw-static-neutral-bluish-00) ${Math.min(30, panel)}%, transparent)`} !important;
}
html body[data-ds-dark-theme] .pI_x6G_sidebarCol,
html body[data-ds-dark-theme] [class*="sidebarCol"]:not([class*="_tab"]) {
	background-color: ${unifyTint
		? `color-mix(in srgb, var(--dsw-static-neutral-bluish-950) ${U(panel)}%, transparent)`
		: `color-mix(in srgb, var(--dsw-static-neutral-bluish-950) ${Math.min(30, panel)}%, transparent)`} !important;
}` : `.pI_x6G_sidebarCol,
.hHd-Xa_root {
	background-color: color-mix(in srgb, var(--dsw-specific-sidebar-fill) 100%, transparent) !important;
}`}
...
html body:not([data-mpw-hdr-blur-element]) .wSkVaW_header::before {
	content: "" !important;
...
`;
			...
			if (!sidebar) {
				...
				css += `
html body {
	--dsw-specific-sidebar-fill: var(--dsw-static-neutral-bluish-00) !important;
}
html body[data-ds-dark-theme] {
	--dsw-specific-sidebar-fill: var(--dsw-static-neutral-bluish-950) !important;
}
`;
...
:root {
	--mpw-chrome-blur: ${chromeBlur}px;
	--mpw-chrome-alpha: ${chromeAlpha};
	--mpw-chrome-bg: ${chromeBg};
	--mpw-rs-blur: ${rsAmt2}px;
	--mpw-rs-alpha: ${rsAlpha2};
}
...
html body:not([data-mpw-sblur-off]) [class*="sidebarCol"]:not([data-sidebar-right-panel]):not([data-dockkit-strip]) {
	...
	background-color: rgba(var(--mpw-chrome-bg), var(--mpw-chrome-alpha)) !important;
}
...
html body[data-mpw-rsblur="on"] [data-dockkit-surface] {
	...
	background-color: rgba(${chromeBg === '255,255,255' ? '255,255,255' : chromeBg}, calc(var(--mpw-rs-alpha) * 0.55)) !important;
}
...
html body[data-ds-dark-theme][data-mpw-rsblur="on"] [data-dockkit-surface] {
	background-color: rgba(18, 22, 30, calc(var(--mpw-rs-alpha) * 0.75)) !important;
...
html body [data-rightbar-fullscreen] [class*="P3OORG_panel"] {
	...
	background-color: rgba(var(--mpw-chrome-bg, 255,255,255), calc(var(--mpw-rs-alpha, 0.45) * 0.55)) !important;
}
...
html body :is(${popSurfaceSel}) {
	background-color: rgba(${popSurfaceRgb}, var(--mpw-pop-alpha, ${(popSurfaceAlpha / 100).toFixed(2)})) !important;
}
html body[data-ds-dark-theme] :is(${popSurfaceSel}) {
	background-color: rgba(18, 22, 30, var(--mpw-pop-alpha, ${(popSurfaceAlpha / 100).toFixed(2)})) !important;
}
`
				: ''
...
body[data-mpw-aqua] {
	...
	/* ①(修正) 品牌交互色优先用「配色」(accent)：默认(空)=DSH 原色/回退 aqua-ink；
	   选了 accent 才切换成所选色(第4项: 默认不启用, 手动选才生效) */
	--dsw-alias-brand-primary: var(--mpw-accent-color, var(--mpw-aqua-ink, inherit));
	--dsw-alias-brand-text: var(--mpw-accent-color, var(--mpw-aqua-ink, inherit));
	--dsw-alias-state-business-primary: var(--mpw-accent-color, var(--mpw-aqua-ink-secondary, inherit));
}
...
body[data-mpw-aqua-text] {
	/* ①(修正 2026-09-13 第16项 token 漂移) DSH 0.1.5 用 label-* 家族；text-* 仅 0.1.4- 存在。
	   两套都写：旧版走 text-*，新版走 label-*（否则自适应文字色在 0.1.5 上完全不生效）。 */
	--dsw-alias-text-primary: var(--mpw-text-ink, var(--dsw-alias-text-primary)) !important;
	--dsw-alias-text-secondary: var(--mpw-text-ink-dim, var(--dsw-alias-text-secondary)) !important;
	--dsw-alias-text-tertiary: var(--mpw-text-ink-dim, var(--dsw-alias-text-tertiary)) !important;
	--dsw-alias-label-primary: var(--mpw-text-ink, var(--dsw-alias-label-primary)) !important;
	--dsw-alias-label-secondary: var(--mpw-text-ink-dim, var(--dsw-alias-label-secondary)) !important;
	--dsw-alias-label-tertiary: var(--mpw-text-ink-dim, var(--dsw-alias-label-tertiary)) !important;
}
...
/* ①(2026-09-13) 「标题栏磨砂」自动半透明底：只有半透明顶栏才看得见磨砂（属性由 JS 按设置挂上）。
   用 !important 压过宿主/主题的顶栏底色；关闭插件或 ?hdrfrost=off 时属性会被移除。 */
html body .wSkVaW_header[data-mpw-hdr-translucent],
html body header[class*="_header_"][data-mpw-hdr-translucent] {
	background-color: color-mix(in srgb, var(--dsw-static-neutral-bluish-00, #fff) 38%, transparent) !important;
	border-bottom: 1px solid transparent !important;
}
...
html body[data-ds-dark-theme] header[class*="_header_"][data-mpw-hdr-translucent] {
	background-color: color-mix(in srgb, var(--dsw-static-neutral-bluish-950, #12161e) 45%, transparent) !important;
}
...
`;
		...
		function MpkgSectionImpl(props) {
			...
			const dirScrollRef = react.useRef(null);    // { anchorIdx, anchorOff } 打开子目录前的锚定条目
			const dirClickLockRef = react.useRef(null); // ②(修复#9) pointerdown 锁定的目录条目（防重排点错）
			// ①(#6 用户实测) 目录列表"滚轮被锁到最顶/自动跳顶"：列表刷新触发的恢复逻辑与用户滚动打架。
			//   两道防线：a) 用户刚滚过（600ms 内）→ 恢复逻辑让位；b) 同一目录路径只恢复一次。
			const dirUserScrollAtRef = react.useRef(0);
			const dirRestoredKeyRef = react.useRef("");
			...
			const openDirPicker = async (path) => {
				...
				try {
					if (dirListRef.current) {
						// ②(修复#9) 记录"锚定条目 + 偏移"而非比例：内容高度变化时比例会指到别的行
						// （用户实测：滑动/点选时列表乱跳、回顶、选错文件夹）。
						const el = dirListRef.current;
						const rows = el.querySelectorAll(".mpw_prop");
						let anchorIdx = -1, anchorOff = 0;
						for (let i = 0; i < rows.length; i++) {
							const r = rows[i].getBoundingClientRect(), cr = el.getBoundingClientRect();
							if (r.bottom > cr.top + 1) { anchorIdx = i; anchorOff = r.top - cr.top; break; }
						}
						dirScrollRef.current = { anchorIdx, anchorOff, rows: rows.length };
					}
				} catch {}
			...
			react.useEffect(() => {
				...
				// #6：用户刚滚动过 → 不抢滚动位置（否则表现为"滚轮失效/被锁到最顶"）
				if (Date.now() - (dirUserScrollAtRef.current || 0) < 600) return;
				// #6：同一路径只做一次恢复（dirSubs 后台刷新不再重复套用旧位置）
				const key = String(dirPath || "") + "|" + (dirSubs ? dirSubs.length : 0);
				if (dirRestoredKeyRef.current === key) return;
				dirRestoredKeyRef.current = key;
				...
				requestAnimationFrame(() => {
					requestAnimationFrame(() => {
						try {
							const el = dirListRef.current;
							if (!el) return;
							const a = dirScrollRef.current;
							if (a && a.anchorIdx >= 0) {
								const rows2 = el.querySelectorAll(".mpw_prop");
								const target = rows2[Math.min(a.anchorIdx, rows2.length - 1)];
								if (target) {
									const cr = el.getBoundingClientRect(), tr = target.getBoundingClientRect();
									el.scrollTop += (tr.top - cr.top) - (a.anchorOff || 0);
								}
							} else {
			...
			// ①(#6) 目录列表用户滚动时间戳：wheel/touchmove/scroll 都算"用户在操作"
			react.useEffect(() => {
				const el = dirListRef.current;
				if (!el) return;
				const mark = () => { dirUserScrollAtRef.current = Date.now(); };
				el.addEventListener("wheel", mark, { passive: true });
				el.addEventListener("touchmove", mark, { passive: true });
				el.addEventListener("scroll", mark, { passive: true });
				return () => {
					try { el.removeEventListener("wheel", mark); el.removeEventListener("touchmove", mark); el.removeEventListener("scroll", mark); } catch {}
				};
			}, [dirPick, dirSubs]);
				...
				dirPick ? h(MaskPortal, { onClick: () => setDirPick(false) }, [
					h("div", { className: "mpw_dialog", onClick: (ev) => ev.stopPropagation() }, [
						...
						h("div", { className: "mpw_props", ref: dirListRef, style: { maxHeight: 240, overflowY: "auto" } }, [
							...
							dirSubs.length ? dirSubs.map((sd, sdi) => h("div", { className: "mpw_prop", key: sd, onPointerDown: () => { dirClickLockRef.current = { sd, sdi, at: Date.now() }; } }, [
								h("button", { className: "mpw_reset mpw_moreBtn", type: "button", onClick: () => {
									// ②(修复#9) 使用 pointerdown 时锁定的条目（列表重排时不点错）；
									// 超过 2s 的锁定视为过期（避免旧锁定影响后续正常点击）。
									const lock = dirClickLockRef.current;
									const use = (lock && lock.sd === sd) ? lock.sd : sd;
									dirClickLockRef.current = null;
									const sep = dirPath.includes("\\") ? "\\" : "/";
									openDirPicker((dirPath ? dirPath + sep : "") + use);
								} }, "📁 " + sd)
			...
			const report = (el, why) => {
				try {
					...
					fetch(HOST_BASE + '/diag', { method: 'POST', body: JSON.stringify({
						...
						headerFrost: (() => { try {
							const h = document.querySelector('.wSkVaW_header, header[class*="wSkVaW_header"], header[class*="_header_"]');
							if (!h) return null;
							const el = h.querySelector(':scope > .mpw-hdrFrost');
							const cs = el ? getComputedStyle(el) : null;
							const pcs = getComputedStyle(h, '::before');
							const r = el ? el.getBoundingClientRect() : null;
							return {
								injected: !!el,
								px: el ? (el.__mpwBlurPx | 0) : null,
								bdf: cs ? (cs.backdropFilter === 'none' ? 'none' : String(cs.backdropFilter).slice(0, 40)) : null,
								...
								headerRect: (() => { const hr = h.getBoundingClientRect(); return [Math.round(hr.x), Math.round(hr.y), Math.round(hr.width), Math.round(hr.height)] })(),
								pseudoBdf: pcs && pcs.backdropFilter && pcs.backdropFilter !== 'none' ? String(pcs.backdropFilter).slice(0, 40) : 'none',
								mode: new URLSearchParams(location.search).get('hdrblur') || 'child',
								reason: (typeof lastFrostReason === 'string' ? lastFrostReason : ''),
								translucent: (() => { try { const h = document.querySelector('.wSkVaW_header, header[class*="_header_"]'); return h ? h.hasAttribute('data-mpw-hdr-translucent') : null } catch { return null } })(),
							};
						} catch { return null } })(),
