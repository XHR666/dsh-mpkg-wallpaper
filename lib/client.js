...
// ①(2026-09-12 根因修复) 字符串字段归一化：持久化状态里 image 可能是布尔 true，
		...
		function applyVideoMute() {
			...
			try {
				const s = readSection();
				const want = (npPrimedMuted || npSoundBlocked) ? true : ((s.mute !== void 0 ? !!s.mute : true) || npAudioOwns());
		...
		function npPrimePlay(video) {
			...
			if (wallUserPaused || powPaused || npUserPausedOf(video) || mpwHiddenAudioBlock()) {
...
`;
		...
		let npCardPaused = false;      // 用户用卡片显式按过暂停（视频档与曲目档共用一条权威状态）
		...
		function npApplyMute() {
			...
			try { const v = npActiveVideo(s); const w = (npPrimedMuted || npSoundBlocked) ? true : mute; if (v && v.muted !== w) v.muted = w; } catch (e) {}
		...
		function npTransport(op, section, value) {
			...
			try {
				if (op === "play" || op === "pause") {
					...
					if (vid && link) {
						if (op === "play") { npCardPaused = false; npSetUserPaused(vid, false); npPrimePlay(vid); did = "video.play"; }
						else { npWantPlay = false; npCardPaused = true; npSetUserPaused(vid, true); vid.pause(); did = "video.pause"; }
						npApplyFrameMute();
					} else if (vid && !link) {
						...
						if (op === "play") { npCardPaused = false; try { vid.muted = false } catch (e) {} did = "video.unmute(link-off)"; }
						else { npCardPaused = true; try { vid.muted = true } catch (e) {} did = "video.mute(link-off)"; }
						...
						if (list && list.tracks.length) {
							...
							} else if (npAudio) { npCardPaused = true; npAudio.pause(); did = "audio.pause"; }
						} else if (!vid) {
		...
		function applyNowPlaying(section) {
			...
			try {
				...
				if (wk !== npCardPausedKey) { npCardPausedKey = wk; npCardPaused = false; npFramePausedByUs = false; }
			} catch (e) {}
...
`;
		...
		function MpkgSectionImpl(props) {
			...
			const BACKUP_FIELDS = [
				...
				"npLinkWallpaper", "npVolume" // ①(NP-4)②① 联动开关 / 音量电平（同上，随备份走）
			];
			const exportBackup = () => {
				try {
					const s = readSection();
