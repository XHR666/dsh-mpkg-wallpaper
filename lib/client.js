window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function handleVideoTexes(mpkg, vtex, file, t, showError, readEntry, projJson, hintFn) {
			try {
		...
		function showWebEl(url) {
			...
			// ①(新) 静音开关（默认开）：web 壁纸有声音时用
			try { frame.muted = !(readSection().mute !== void 0 ? !!readSection().mute : true); } catch {}
			frame.style.display = "";
		}
...
/* ①(新) 开关外置 ON/OFF 字母（uiverse 电源图标语义：开=ON 绿 / 关=OFF 红） */
.mpw_switchState {
	display: inline-flex; align-items: center; justify-content: center;
	font-size: 10px; font-weight: 800; letter-spacing: 1px;
	width: 30px; height: 20px; border-radius: 6px; flex: none;
	transition: background-color .3s ease, color .3s ease;
}
.mpw_switchState.mpw_on { background: rgba(31,173,62,.16); color: #1fad3e; }
.mpw_switchState:not(.mpw_on) { background: rgba(167,40,40,.14); color: #a72828; }
.mpw_switchWrap { display: inline-flex; align-items: center; gap: 6px; }
...
`;
		...
		function Toggle({ checked, onChange, disabled }) {
			...
			// ①(新) 新样式：开关 + 外置 ON/OFF 字母
			if (newStyle) {
				return h("span", { className: "mpw_switchWrap" }, [
					btn,
					h("span", { className: "mpw_switchState" + (checked ? " mpw_on" : "") }, checked ? "ON" : "OFF")
				]);
			}
			return btn;
		}
		...
		function MpkgSection(props) {
				...
				try {
					...
					if (vtex.length) {
						try {
							...
							const ok = await handleVideoTexes({ entries: d.entries }, vtex, { name: display, size: 0 }, t, showError, readEntry, null, setHint);
					...
					(async () => {
						try {
							...
							if (d.ok && d.selected && vtex.length) {
								...
								const ok = await handleVideoTexes({ entries: d.entries }, vtex, { name: w.title || w.name, size: 0 }, t, showError, readEntry, projJson, setHint);
								if (ok) {
			...
			const onMpkg = async (e) => {
				...
				try {
					...
					if (vtex.length) {
						const ok = await handleVideoTexes(mpkg, vtex, file, t, showError, readEntry, null, setHint);
						if (ok) {
