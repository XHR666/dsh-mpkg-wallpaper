...
.wSkVaW_header [class*="segmented"] button:hover {
	background-color: color-mix(in srgb, var(--dsw-alias-interactive-bg-hover) 70%, transparent) !important;
}
`;
			}
		...
		function MpkgSection(props) {
			...
			const SpeedBar = (props) => {
				...
				return h("div", { className: "mpw_speedBar" }, [
					// ①(修正) 全宽刻度：轨道/竖线/球/进度统一用 i/(N-1)*100%（容器全宽），
					// 数字行 flex space-between 首尾贴边 → 数字中心与刻度严格对齐。
					// 之前 6px 缩进 + calc 单位问题导致球/进度/数字互相错位。
					h("div", { ref,
						style: { position: "relative", height: 30, margin: "8px 0 0", touchAction: "none", cursor: "pointer" },
						...
						h("div", { style: { position: "absolute", left: 0, top: "50%", height: 6, marginTop: -3, borderRadius: 3, background: "var(--dsw-alias-brand-primary, #4f8cff)", width: (idx / (SPEED_LEVELS.length - 1) * 100) + "%" } }),
						...
						h("div", { style: { position: "absolute", top: "50%", marginTop: -8, left: "calc(" + (idx / (SPEED_LEVELS.length - 1) * 100) + "% - 7px)", width: 14, height: 14, borderRadius: "50%", background: "var(--dsw-alias-brand-primary, #4f8cff)", border: "2px solid var(--dsw-alias-bg-base, #0e1420)" } })
					),
					// 档位数字：space-between，首尾贴边 = 0%/100% 刻度，中间均分
					h("div", { className: "mpw_inline", style: { justifyContent: "space-between", margin: "2px 0 0" } },
						SPEED_LEVELS.map((rate) => h("button", { key: rate, type: "button",
							className: "mpw_speedLabel" + (rate === value ? " mpw_on" : ""),
							style: { flex: "none", padding: "4px 0", fontSize: 12, border: 0, background: "transparent", cursor: "pointer",
				...
				rotEditor && rotEdit ? h(MaskPortal, { onClick: () => setRotEditor(false) }, [
					h("div", { className: "mpw_dialog", style: { width: "min(560px, calc(100vw - 32px))" }, onClick: (ev) => ev.stopPropagation() }, [
						...
						wallList.length === 0
							...
							: h("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, maxHeight: 220, overflowY: "auto" } }, wallList.map((w) => {
							...
							return h("div", { className: "mpw_pickCard" + (on ? " mpw_on" : ""), key: w.key,
								...
								turl
									...
									: h("div", { style: { width: "100%", height: 64, background: "color-mix(in srgb, var(--dsw-alias-interactive-bg-hover, #333a4a) 40%, transparent)" } }),
