...
// ①(2026-09-12 根因修复) 字符串字段归一化：持久化状态里 image 可能是布尔 true，
		...
		/** ①(NP-4) 联动的唯一判据（②）：开 ⇒ 传输动作同时驱动壁纸媒体；关 ⇒ 只驱动我们的播放器。 */		/** ①(NP-4) 联动的唯一判据（②）：开 ⇒ 传输动作同时驱动壁纸媒体；关 ⇒ 只驱动我们的播放器。 */
...
`;
		...
		function npResolveMedia(section) {
			...
			if (vid) {
				...
				if (!link) out.byline = mpwT("np.note.linkOff");
		...
		function npDropAudio() {
			try { if (npAudio) { npAudio.pause(); npAudio.removeAttribute("src"); npAudio.remove(); } } catch (e) {}
			npAudio = null;
		}
