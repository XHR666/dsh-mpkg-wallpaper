...
body[data-mpw-float] [data-sidebar-collapsed] [class*="sidebarCol"] {
	...
	border-radius: 12px;
	border: 1px solid rgba(19,45,83,0.26);
	/* ①(修正) overflow: visible 让 rail 内容完整显示：DSH 外层 sidebarCol 是
	   overflow:hidden 会裁 rail 内容（用户实测：右 margin 改 0 后 rail 图标差几个
	   像素被裁）。rail 卡片设 overflow:visible 只解开卡片内裁切，rail 内容（竖排
	   36px 图标）在窄 12px 的卡片内仍完整（图标固定宽）。 */
	overflow: visible;
	/* ②(修正) 不设 position/z-index：设置面板 fixed overlay 在 sidebarCol 内，
	   设了会被困住叠在设置上（见区域5b 注释） */
	transition: margin 0.18s ease, padding 0.18s ease, border-radius 0.18s ease, border-color 0.18s ease;
}
/* ①(修正) rail 卡片右缘**不向右溢出**：原右 margin -12px 让 rail 卡片右缘
   超出 rail 轨道 12px 侵入 DSH 中心列，而 better-sidebar 底部面板按中心列左缘
   实时定位 → rail 右缘与面板左缘重叠（用户实测：悬浮+收起+底部面板展开重叠）。
   改右 margin 0（右缘贴轨道右缘，不侵入中心列）。若 rail 内容仍被外层 sidebarCol
   (overflow:hidden) 裁，配合 rail 卡片 overflow:visible + 内容固定宽显示。 */
...
/* ①(修正) 悬浮收起态外层 sidebarCol 也 overflow:visible（覆盖 DSH 默认 hidden）：
   rail 内容差几个像素被裁，根因是 DSH 的 .pI_x6G_sidebarCol overflow:hidden 裁掉
   rail 卡片(窄 12px)溢出的内容。悬浮时放行，rail 内容完整；rail 内容宽 ≤ rail
   轨道，不会右溢到中心列。 */
body[data-mpw-float] [data-sidebar-collapsed] .pI_x6G_sidebarCol,
body[data-mpw-float] [data-sidebar-collapsed] [class*="sidebarCol"] {
