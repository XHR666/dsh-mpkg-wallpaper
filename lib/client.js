...
body[data-mpw-sblur] .hHd-Xa_regionArea,
body[data-mpw-sblur] [class*="regionArea"] {
	backdrop-filter: ${sblFilter} !important;
	-webkit-backdrop-filter: ${sblFilter} !important;
}
html body[data-mpw-sblur][data-mpw-sblur-off] .hHd-Xa_regionArea,
html body[data-mpw-sblur][data-mpw-sblur-off] [class*="regionArea"] {
	backdrop-filter: none !important;
...
`;
			...
			// ①(重做) **blur 加在 regionArea（会话列表区）而不是 sidebarCol/header**：
			//   backdrop-filter 会把该元素变成 fixed 弹窗的 containing block。DSH 设置
			//   弹窗的 overlay 渲染在 sidebarCol > settingsArea 内——加在 sidebarCol 上会
			//   把弹窗"困"在侧边栏宽度里（用户实测：Via 打开设置被压缩进侧边栏）。
			//   regionArea 是 settingsArea 的兄弟（不含弹窗），模糊会话列表区即可保留
			//   "侧边栏有虚化"的观感，同时弹窗永远不被困 → 不再需要 keepBlurOnDialog
			//   摘除开关（原方案默认保留 blur 却在 Via 上困住弹窗，回归）。
			//   header 展开面板（data-mpw-header-blur-off）仍保留摘除，防困面板。
			if (unifyTint && !chatFollowInCss && unifyAmount > 0 && bdSupported) {
				css += `
/* ── 统一虚化 + 聊天区不跟随：会话列表区叠加整屏虚化度（blur 加在 regionArea，
   不碰 settingsArea，弹窗永远不被困；header 展开面板由 data-mpw-header-blur-off 摘除） ── */
html body:not([data-mpw-header-blur-off]) .hHd-Xa_regionArea,
html body:not([data-mpw-header-blur-off]) [class*="regionArea"] {
