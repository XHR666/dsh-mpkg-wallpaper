		...
		function buildCss(section) {
			...
			if (section && (section.lgTest !== void 0 ? !!section.lgTest : false)) {
				section = Object.assign({}, section, {
					blur: 0, dialogBlur: false, dialogAmount: 0, settingsBlur: false, settingsAmount: 0,
...
`;
			...
			if (unifyTint && !chatFollowInCss && unifyAmount > 0 && bdSupported) {
				// ①(修正) 设置弹窗打开时是否保留 G 块 blur（keepBlurOnDialog，默认开=保留）：
				// 原 data-mpw-sblur-off 会把侧边栏/标题栏的整屏虚化一并摘掉 → 打开设置后
				// 侧边栏/标题栏只剩半透明、虚化消失（用户实测）。默认保留虚化（弹窗实测
				// 基本居中，backdrop-filter 困弹窗的顾虑不成立）；若某些 DSH 版本弹窗错位，
				// 可关掉此开关恢复旧行为（弹窗打开时摘除）。header 展开面板
				// （data-mpw-header-blur-off）始终摘除，防困面板。
				const keepBlur = section.keepBlurOnDialog !== void 0 ? !!section.keepBlurOnDialog : true;
				const gSel = keepBlur
					? "html body:not([data-mpw-header-blur-off])"
					: "html body:not([data-mpw-sblur-off]):not([data-mpw-header-blur-off])";
				css += `
/* ── 统一虚化 + 聊天区不跟随：侧边栏/标题栏背后叠加整屏虚化度（标题栏展开面板
   出现时由 data-mpw-header-blur-off 摘除，防困住面板） ── */
${gSel} .pI_x6G_sidebarCol,
${gSel} [class*="sidebarCol"],
${gSel} .wSkVaW_header {
...
/* ②(重做) 遮罩虚化独立（maskBlur/maskAmount）：设置/弹层打开时全屏背景遮罩的虚化
   ①(修正) 同无壁纸分支：只作用于 .mpw_mask，避免宽匹配误伤 DSH 原生设置 overlay */
.mpw_mask {
...
`;
		...
		function MpkgSection(props) {
			...
			return h("div", { className: "mpw_row mpw_glassHost" }, [
				...
				(section.unifyTint !== void 0 ? !!section.unifyTint : DEFAULT_UNIFY_TINT)
					...
					: null,
				// ①(修正) 设置弹窗打开时保留侧边栏/标题栏虚化（默认开）：原弹窗打开
				// 即摘除整屏虚化 → 侧边栏/标题栏只剩半透明（用户实测）。开=保留虚化
				// （弹窗实测基本居中）；关=恢复旧行为（弹窗打开时摘除，个别 DSH 版本
				// 弹窗被 backdrop-filter 困住时用）。
				toggleRow(t("keepBlurOnDialog"), t("keepBlurOnDialog.desc"), "keepBlurOnDialog", true),
		...
		const zh = {
			...
			"keepBlurOnDialog": "设置打开时保留侧边栏/标题栏虚化",
			"keepBlurOnDialog.desc": "开（默认）：设置弹窗打开时侧边栏/标题栏的整屏虚化保留；关：弹窗打开时摘除（个别 DSH 版本弹窗被模糊层困住时用）",
		...
		const en = {
			...
			"keepBlurOnDialog": "Keep sidebar/title blur while settings open",
			"keepBlurOnDialog.desc": "ON (default): keep the unified blur on sidebar/title bar while the settings dialog is open; OFF: remove it while a dialog is open (use if the dialog is trapped by the blur layer on some DSH versions)",
