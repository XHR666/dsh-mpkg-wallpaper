	...
	factory: (require) => {
		...
		function refreshAqua() {
			try {
				...
				if (maskOn) {
					const panel = Math.max(0, Math.min(100, s.opacity !== void 0 ? s.opacity : DEFAULT_OPACITY)) / 100;
...
`;
			if (headerBlur && headerBg) {
				...
				// ①(修正) 标题栏雾底：与侧边栏一致（unify 开 → 跟随 sidebarAlpha，
				// 顶栏/侧边栏透明度同步可调——用户实测之前强制透明导致顶栏比侧边栏更透）；
				// unify 关 → 按「标题栏磨砂程度」条（默认 0=透明，需要雾底自行拉高）。
				// session log 白框 = 顶栏雾（一致视觉）；想要完全无白框 → sidebarAlpha 拉 0。
...
`;
			...
			// ⑲(新) 深底文字可读增强（aquaTextEnhance，默认关）：全局文字加轻度描边。
			// 说明：无法精确实现"只变经过深色处的文字"（需要逐字采样背景像素），
			// 近似方案 = 给聊天区文字加双色描边，任何背景（黑/白）下都保持可读。
			if (section.aquaTextEnhance !== void 0 ? !!section.aquaTextEnhance : false) {
				css += `
/* ── 深底文字可读增强（实验，默认关）：双色描边保证任意背景可读 ── */
body[data-mpw-aqua-text] [data-conversation-scroll] :is(p, li, h1, h2, h3, h4, span, pre, code, td, th) {
	text-shadow: 0 0 2px rgba(255,255,255,.6), 0 0 4px rgba(255,255,255,.35), 0 0 1px rgba(0,0,0,.35) !important;
}
`;
			}
			...
			if (section.todoBlur !== void 0 ? !!section.todoBlur : false) {
				css += `
/* ── 任务列表磨砂（实验，默认关）：todo 卡片背景模糊，收纳/展开统一 ── */
body[data-mpw-todo-blur] [data-tool="todo_write"] {
	backdrop-filter: blur(12px) !important;
	-webkit-backdrop-filter: blur(12px) !important;
	background-color: color-mix(in srgb, var(--dsw-static-neutral-bluish-00) 70%, transparent) !important;
}
body[data-mpw-todo-blur] body[data-ds-dark-theme] [data-tool="todo_write"] {
	background-color: color-mix(in srgb, var(--dsw-static-neutral-bluish-950) 70%, transparent) !important;
}
`;
			}
...
`;
		...
		function MpkgSection(props) {
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					aquaMask: DEFAULT_AQUA_MASK, aquaTint: DEFAULT_AQUA_TINT, aquaInk: DEFAULT_AQUA_INK, aquaColor: "", aquaInkColor: "", aquaTextEnhance: DEFAULT_AQUA_TEXT_ENHANCE, todoBlur: DEFAULT_TODO_BLUR,
			...
			return h("div", { className: "mpw_row" }, [
				...
				h("div", { className: "mpw_field" }, [
					...
					h("div", { className: "mpw_inline" }, [
						h("input", {
							className: "mpw_colorInput", type: "color",
							value: (section.aquaColor && /^#[0-9a-fA-F]{6}$/.test(section.aquaColor) ? section.aquaColor : "#808080"),
							onInput: (ev) => commit({ aquaColor: ev.target.value })
						}),
				...
				h("div", { className: "mpw_field" }, [
					...
					h("div", { className: "mpw_inline" }, [
						h("input", {
							className: "mpw_colorInput", type: "color",
							value: (section.aquaInkColor && /^#[0-9a-fA-F]{6}$/.test(section.aquaInkColor) ? section.aquaInkColor : "#808080"),
							onInput: (ev) => commit({ aquaInkColor: ev.target.value })
						}),
		...
		const zh = {
			...
			"sec.other": "其他",
			"sec.aqua": "Aqua 实验",
			"sec.aqua.desc": "借鉴社区 fork 的实验性外观模式，默认全关（不影响原功能），可单独开启体验：统一雾 = 全屏遮罩让所有表面共享一种雾色；面板取色 = 颜色跟随壁纸主色；自适应文字 = 文字色随背景亮度变化 + 蓝色清理",
			"aquaMask": "统一雾（全屏遮罩）",
			"aquaMask.desc": "所有表面（侧边栏/标题栏/聊天区/按钮）共享一种雾色（随「面板不透明度」条），不再分区白雾；关 = 保持现有分区雾",
			"aquaTint": "面板颜色匹配壁纸",
			"aquaTint.desc": "自动采样壁纸主色作为面板底色（视频/GIF 每 2 秒刷新）；关 = 使用主题中性色",
			"aquaInk": "自适应文字色 + 蓝色清理",
		...
		const en = {
			...
			"sec.aqua.desc": "Experimental appearance modes borrowed from a community fork, all OFF by default (original features untouched), enable individually: unified fog = full-screen mask so every surface shares one fog color; panel tint = colors follow the wallpaper's dominant color; adaptive ink = text color follows background brightness + blue cleanup",
			"aquaMask": "Unified fog (full-screen mask)",
			"aquaMask.desc": "Every surface (sidebar/title bar/chat/buttons) shares one fog color (follows the Panel opacity slider) instead of per-area fog; off = keep the current per-area fog",
			"aquaTint": "Panel colors match wallpaper",
			"aquaTint.desc": "Auto-samples the wallpaper's dominant color as the panel base (videos/GIFs refresh every 2s); off = theme neutral colors",
