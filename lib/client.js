...
`;
		...
		function mpwAuditPush(kind, el, extra) {
			try {
				if (!mpwAuditOn()) return null;
				const rec = Object.assign({
					t: Date.now(), kind: String(kind || ""), who: mpwAuditWho(), el: mpwAuditEl(el),
					...
					if (Date.now() - mpwAuditLastBeacon > 5000) {
						...
						try {
							mpwWebDiag("audio-audit", {
								trigger: audible ? "audible-playback" : (rec.el && rec.el.connected === false ? "detached-playing" : "hidden-transition"),
		...
		function mpwInstallAudioAudit() {
			...
			try {
				const P = globalThis.HTMLMediaElement && HTMLMediaElement.prototype;
				if (P && typeof P.play === "function" && !P.__mpwAudited) {
					const origPlay = P.play;
					P.play = function () { try { mpwAuditPush("play", this) } catch (e) {} return origPlay.apply(this, arguments) };
					for (const prop of ["muted", "volume"]) {
						try {
							...
							const wrapped = function (v) {
								...
								if (prop === "muted" ? bMuted !== !!this.muted : bVol !== this.volume) { try { mpwAuditPush(kind, this, { before: { muted: bMuted, volume: bVol } }) } catch (e) {} }
								return v;
							};
			...
			try {
				if (typeof window.Audio === "function" && !window.Audio.__mpwAudited) {
					const OrigAudio = window.Audio;
					const Wrapped = function () { const a = new OrigAudio(...arguments); try { mpwAuditPush("new-Audio", a, { args: Array.prototype.slice.call(arguments).join(",").slice(0, 80) }) } catch (e) {} return a };
					Wrapped.__mpwAudited = true;
			...
			for (const key of ["AudioContext", "webkitAudioContext"]) {
				try {
					...
					const Wrapped = function () { const c = new Orig(...arguments); try { mpwAuditPush("AudioContext", null, { ctor: key, state: c && c.state }) } catch (e) {} return c };
					Wrapped.__mpwAudited = true;
			...
			try {
				...
				if (proto && typeof proto.decodeAudioData === "function" && !proto.__mpwAudited) {
					...
					proto.decodeAudioData = function () { try { mpwAuditPush("decodeAudioData", null, { ctxState: this && this.state }) } catch (e) {} return orig.apply(this, arguments) };
