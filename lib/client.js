	...
	factory: (require) => {
		...
		function aquaTokenOverrides(section) {
			...
			if (inkOn) {
				...
				out["--dsw-alias-brand-primary"] = { light: info.ink, dark: info.ink };
				out["--dsw-alias-brand-text"] = { light: info.ink, dark: info.ink };
				out["--dsw-alias-button-info-fill"] = { light: tinted(0.82), dark: tinted(0.82) };
			}
			return out;
		}
...
`;
			if (headerBlur && headerBg) {
				...
				// ①(修正) 标题栏雾底：默认 0 = 透明；unify 开时**强制透明**——用户实测
				// 统一虚化开启时 header 白雾让 session log 按钮像被白色矩形框包住
				// （旧版本默认 80 已存入 localStorage 也不会随新默认值变化，必须强制）；
				// unify 关时按「标题栏磨砂程度」条（默认 0，需要雾底自行拉高）。
...
`;
			// ⑲(新) Aqua 实验模式：文字色/品牌蓝兜底（token override 之外，覆盖直接写死的颜色）
			if (aquaOn(section)) {
				css += `
		...
		function MpkgSection(props) {
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					aquaMask: DEFAULT_AQUA_MASK, aquaTint: DEFAULT_AQUA_TINT, aquaInk: DEFAULT_AQUA_INK,
