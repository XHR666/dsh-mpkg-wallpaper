...
/* ①(2026-09-24 包内/包旁 JSON 收口线) **宿主侧复用出口**：`lib/web-wallpaper.js`（WE 目录的 `project.json`）
   与 `lib/index.js`（Steam 库壁纸目录的 `project.json`）都要用**同一份**宽容解析与记账 —— 导出面只加名字，
   解析逻辑、计数口径、`swallowed` 语义**一字未动**（"包内 JSON 解析点都要记账"这条纪律由
   tools/we-json-tolerance-test.mjs 的静态反查看住）。客户端侧 `lib/client.js` 的同名块是**浏览器产物**的
   内联副本（页面没有模块图），两边一致性由同一条门禁的切片对拍看着 —— 所以这里不是"第三份实现"，
   而是把宿主这一份**接出来给宿主模块共用**。 */
export { mpwParseWeJson, mpwWeJsonStats, mpwWeJsonSwallowed };
