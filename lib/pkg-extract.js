...
/** 只解析 PKG 目录表：返回 { magic, dataStart, raw:[{path,offset(相对),length}] }。 */
...
function enumerateAudioTracks(src) {
  ...
  if (sceneJson === undefined && (!src || src.withRefs !== false)) {
    ...
    if (sjEntry) {
      try {
        ...
        if (bytes && bytes.length) { sceneJson = JSON.parse(textDecoder.decode(bytes)); sceneJsonRead = true; }
      } catch { sceneJson = null; }
    }
  }
  ...
  const tracks = order.map((rec) => {
    let mime = '';
    if (sniff && rec.entry) {
      const head = readHead(rec.entry, AUDIO_HEAD_BYTES);
      if (head && head.length) { headReads++; mime = sniffAudioMime(head); }
    }
    return { path: rec.path, size: rec.size, mime: mime || suffixAudioMime(rec.path), refs: rec.refs.slice() };
  });
...
function collectDirAudioTracks(dir, opts = {}) {
  ...
  try {
    ...
    if (existsSync(sj)) sceneJson = JSON.parse(readFileSync(sj, 'utf8'));
  } catch { sceneJson = null; }
...
const SCENE_VIDEO_SCAN_CACHE_MAX = 64;
const sceneVideoScanCounters = { hits: 0, misses: 0, entriesRead: 0, bytesRead: 0 };
function sceneVideoScanStats() {
  return { entries: sceneVideoScanCache.size, hits: sceneVideoScanCounters.hits,
    misses: sceneVideoScanCounters.misses, entriesRead: sceneVideoScanCounters.entriesRead,
    bytesRead: sceneVideoScanCounters.bytesRead, maxEntries: SCENE_VIDEO_SCAN_CACHE_MAX };
}
...
function scanSceneVideo(dirOrPath, opts = {}) {
  ...
  const rec = { video, indexEntries };
  if (useCache) {
    sceneVideoScanCache.set(key, rec);
    while (sceneVideoScanCache.size > SCENE_VIDEO_SCAN_CACHE_MAX) {
      const oldest = sceneVideoScanCache.keys().next().value;
      sceneVideoScanCache.delete(oldest);
    }
  }
  return { video, source, cacheHit: false, indexEntries, entriesRead, bytesRead, tableBytes: source === 'pkg' ? undefined : 0, ms };
}

export { extractSceneMainImage, extractSceneMainImageFromDir, extractSceneManifest, extractSceneLayer, extractTexVideoMp4, collectSceneVideoFiles, parsePkg, readPkgEntry, parseTex };
// ①(2026-09-15 音频扫描提速 / 2026-09-16 洁净室重写) 惰性音频索引：只读目录表 + 仅候选头 ≤16 字节。
//   名字与结构依据 docs/AUDIO-TRACK-SPEC.md（后缀判定 / 容器判定 / 枚举核心里外三层）。
export {
  isAudioPath, suffixAudioMime, sniffAudioMime, AUDIO_SUFFIX_MIME, AUDIO_CONTAINER_RULES,
