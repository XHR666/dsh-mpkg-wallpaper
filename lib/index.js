...
import { extractSceneMainImage, extractSceneMainImageFromDir, extractSceneManifest, extractSceneLayer, extractTexVideoMp4, collectSceneVideoFiles, parsePkg, readPkgEntry, scanSceneAudio, scanSceneVideo, suffixAudioMime, audioMimeFor, AUDIO_SUFFIX_MIME } from './pkg-extract.js';
        ...
        try {
          ...
          for (const e of readdirSync(dir, { withFileTypes: true })) {
            ...
            if (e.isDirectory() && !f.startsWith('.')) {
              ...
              if (type && type !== 'unknown' && media) {
                ...
                files.push({
                  ...
                  webHeavy, webExternal, kindReason: det.reason, declaredType: det.declared,
            ...
            if (exts.some((x) => f.toLowerCase().endsWith(x))) {
              ...
              const type = suffixAudioMime(f) ? 'audio' : /\.(mp4|webm|mov)$/i.test(f) ? 'video' : /\.mpkg$/i.test(f) ? 'mpkg' : 'image';
