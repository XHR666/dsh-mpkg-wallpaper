...
function spawnFfmpeg(ff, args) {
  return new Promise((resolve, reject) => {
    ...
    const runNext = () => {
      ...
      proc.on('error', (err) => settle(a.name + ' spawn error ' + (err && err.code ? err.code + ' ' + err.message : err)));
      proc.on('exit', (code) => {
        if (done) return;
        if (code === 0) {
          done = true;
        ...
        settle(a.name + ' exit ' + code + (timedOut ? ' (killed by timeout)' : ''));
      });
    };
    runNext();
  });
...
/** 跑一次抽帧转码：`-vf fps=<cap>` + CPU 编码（libx264 优先，libsvtav1 回退），-an 去音频。 */
async function runTranscode(srcFile, out, fps, job) {
  const ff = await ensureFfmpeg();
  if (!ff) {
    throw new Error('ffmpeg not found (set DSH_WE_FFMPEG, install ffmpeg to PATH, or click 下载 ffmpeg → /ffmpeg-download)');
  }
  job.ff = ff.path;
  const base = ['-y', '-hide_banner', '-loglevel', 'error', '-i', srcFile, '-vf', 'fps=' + fps];
  const encoders = [
...
/** 转码到 <fps>，磁盘缓存 keyed by 源标识|mtime|fps。返回缓存产物绝对路径。 */
async function transcodeToFps(src, fps) {
  ...
  const key = crypto.createHash('sha256')
    .update(srcId + '|' + Math.round(st.mtimeMs) + '|' + fps)
  ...
  const p = (async () => {
    ...
    try {
      ...
      await runTranscode(mat.file, tmp, fps, transcodeJobs.get(progKey) || {});
...
function apply(ctx) {
  ...
  if (!webServer || typeof webServer.register !== 'function') return;
  {
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          json(res, 200, { ok: true, found: !!r, path: r ? r.path : null, version: r ? r.version : null });
        } catch (err) {
          json(res, 200, { ok: false, found: false, path: null, version: null, error: String(err && err.message || err) });
        }
      },
    });
    ...
    // ①(新) 抽帧转码：把媒体转码到 fps 上限，Range 流式返回转码产物。
    // 用法：/transcode/<mpkgToken>?fps=60 或 /transcode?file=<绝对路径|customDir内文件名>&fps=60。
    // 缓存 DATA_DIR/transcodes/tc_<sha256(src|mtime|fps)>.mp4；转码失败返回 { ok:false }，
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const src = resolveTranscodeSource(file || token);
          if (!src) { json(res, 404, { ok: false, error: 'unknown source' }); return; }
          let out = null;
          let transcodeErr = null;
          try { out = await transcodeToFps(src, fps); } catch (err) { transcodeErr = err; } /* 转码失败 → 回退原片 */
          if (!out) {
