    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          req.on('end', () => {
            try {
              const dir = join(os.homedir(), '.dsh', '.dsh-mpkg-wallpaper');
          ...
          const scan = (root) => {
            ...
            for (const dir of dirs) {
              ...
              try {
                ...
                // ①(修复) ltoken 稳定化：按目录路径哈希（原随机→重启后已应用的 library 壁纸 404 无法自愈）
                const ltoken = crypto.createHash('sha256').update(resolve(fullPath)).digest('hex').slice(0, 16);
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          try { out = await transcodeToFps(src, fps, maxW, () => reqAborted); } catch (err) { transcodeErr = err; } /* 转码失败/取消 → 回退原片 */
