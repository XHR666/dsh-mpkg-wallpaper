...
function pruneTranscodeCache() {
  ...
  try {
    ...
    for (const n of readdirSync(dir)) {
      if (!n.startsWith('src_') || !n.endsWith('.bin')) continue;
      try {
...
async function runTranscode(srcFile, out, fps, maxW, job) {
  ...
  // ①(修正) 线程模式：多线程 → (futex 失败时) 单线程。单线程时 -threads 1 同时放
  // 输入侧(解码)与输出侧(编码)，并加 -filter_threads 1 限制滤镜线程。
  const threadModes = [
    { label: 'mt', extra: [] },
  ...
  for (const mode of threadModes) {
    ...
    for (const enc of encoders) {
      ...
      try {
        ...
        if (/futex/i.test(String(err && err.message || ''))) {
          futexSeen = true;
...
async function transcodeToFps(src, fps, maxW) {
  ...
  const p = (async () => {
    ...
    if (TRANSCODE_ACTIVE >= TRANSCODE_MAX_ACTIVE) {
      await new Promise((resolve) => TRANSCODE_WAITERS.push(resolve));
    }
    TRANSCODE_ACTIVE++;
    try {
