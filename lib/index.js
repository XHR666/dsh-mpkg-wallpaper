...
// ①(2026-09-21 音频格式补全) 后三个导出是**唯一的**扩展名→MIME 表与嗅探入口，见 docs/AUDIO-TRACK-SPEC.md §1.2/§3：
//   路由侧原先各自内联一份扩展名三元链 + 一份魔数嗅探，已与渲染器支持的后缀漂移（.oga/.m4a/.aac 缺失、ftyp 偏移写错）。
//   ⚠ 这条 import 必须**保持单行**：tools/bundle-equivalence-test.mjs 的具名导入解析器按行读（注释进括号会解析失败）。
import { extractSceneMainImage, extractSceneMainImageFromDir, extractSceneManifest, extractSceneLayer, extractTexVideoMp4, collectSceneVideoFiles, parsePkg, readPkgEntry, scanSceneAudio, scanSceneVideo, clearSceneVideoScanCache, suffixAudioMime, audioMimeFor, AUDIO_SUFFIX_MIME, mpwParseWeJson, mpwWeJsonSwallowed } from './pkg-extract.js';
...
// ①(WP-1 2026-09-19) 追加：帧内存储 facade 的持久化路由（/web-store）、宿主媒体音频控制
// （/media-audio；用户第 6 项 —— video 类壁纸默认静音**不变**，宿主显式打开才出声）、
// CSP 阻塞检测（不注入 + 留痕）、HTML 源级 file:/// 改写。契约见 docs/WEB-WALLPAPER.md §13。
import {
  detectWallpaperDir, isShimRequest, webPolicyFromQuery, buildSeedScript, rewriteWebEntryHtml,
  webAssetCorsHeaders, listWallpaperFiles, hasBlockingCsp, rewriteWebFileUrlsInHtml, webStoreFromQuery,
  sanitizeWebStore, webWallId, WEB_STORE_ROUTE, WEB_STORE_MAX_WALLS, WEB_STORE_MAX_BYTES, WEB_STORE_MAX_VALUE,
  MEDIA_AUDIO_ROUTE, MEDIA_AUDIO_DEFAULTS, normalizeMediaAudio, mediaAudioReport, mediaAudioPatchForEntry,
} from './web-wallpaper.js';

/* ①(MEDIA-1 接线 2026-09-20) 系统媒体会话（MPRIS / SMTC）：模块早就在（`media-session-test` 95/0），
   但一直**没接线** ⇒ NP 卡片拿不到系统曲目/封面/进度（喂它的是测试台自己的播放器状态）。
   接线口径见 docs/MEDIA-SESSION.md §6：宿主三条只读/控制路由 + 客户端异步补充路径，
   **不改** `npResolveMedia` 那条同步语义（它解的是"壁纸自己的媒体"）。 */
import { createMediaSession, CONTROL_OPS, SEEK_MAX_MS, MAX_ART_CHARS } from './media-session.js';
...
/** ①(2026-09-23 用户第 4 项 / 核验报告 G8) 容器目录表的**读取上限**：
 *  原实现固定只读前 2MiB 交给 parseMpkgHead ⇒ 目录表 >2MiB 的容器直接抛 DataView 的
 *  `The value of "offset" is out of range…`（用户看到的是这句 Node 原生英文）+ 500 + 落盘残骸。
 *  现在 2MiB 不够就 ×4 倍增（2→8→32MiB，`readMpkgHeadGrow`），到顶仍解析不出 ⇒
 *  **可读错误**（HTTP 413 + `code:'table-too-large'`）并清掉已落盘的上传物。
 *  为什么要有上限而不是无限读：条目数 × 单条对象都是 JS 堆内存，本机可用内存只有 ~4GB。 */
const MPKG_TABLE_MAX_BYTES = Math.max(HEAD_BYTES, Number(process.env.DSH_WE_MPKG_TABLE_MAX_BYTES) || 32 * 1024 * 1024);
/** ①(G9) 上传容器副本的**回收上限**：数量 + 合计字节双闸门、最旧先删（与 transcodes/ 同款口径）。
 *  一次导入就是一份整包副本（真机 .mpkg 830MB）⇒ 原实现永不回收、重启后还被 restoreFiles
 *  重新登记。env 可覆盖；当前壁纸仍在引用的 token（settings.json 里的 token=…）与
 *  最近 UPLOAD_PROTECT_MS 内被 /media 读过的 token **绝不删**。 */
const UPLOAD_KEEP = Math.max(1, Number(process.env.DSH_WE_UPLOAD_KEEP) || 4);
const UPLOAD_MAX_BYTES = Math.max(0, Number(process.env.DSH_WE_UPLOAD_MAX_BYTES) || 1024 * 1024 * 1024);
/** ①(G9) "最近还在被读"保护窗：/media 每命中一次就刷新 lastUsed，窗内不参与淘汰。 */
const UPLOAD_PROTECT_MS = Math.max(0, Number(process.env.DSH_WE_UPLOAD_PROTECT_MS) || 10 * 60 * 1000);
/** ①(G9) 上传物的文件名形状（`/upload` 用 crypto.randomBytes(16).toString('hex') 作 token）
 *  —— 只有这个形状才可能被回收：**用户自己放进 DATA_DIR 的文件一律不碰**。 */
const UPLOAD_NAME_RE = /^[0-9a-f]{32}\.mpkg$/;
...
function readMpkgHead(filePath, want) {
  const n = Math.max(1, Number(want) || HEAD_BYTES);
  const buf = Buffer.alloc(n);
  let fd = null;
  try {
    fd = openSync(filePath, 'r');
    const got = readSync(fd, buf, 0, n, 0);
    return got < n ? buf.subarray(0, got) : buf;
 ...
 *  ①(G8) 越界（头部字节不够装下整张目录表）⇒ 抛**带 code 的可读错误**，不再把
 *  DataView/Buffer 的原生 `offset is out of range` 直接透给用户；调用方按 code 分流：
 *  `readMpkgHeadGrow` 见到它就倍增重读，到顶则回可读的 413。 */
function parseMpkgHead(buf) {
  const need = (n, what) => {
    if (n > buf.length) {
      const e = new Error('容器目录表超出已读头部（需要 ' + n + ' 字节，只有 ' + buf.length + ' 字节；' + what + '）');
      e.code = 'table-too-large';
      throw e;
    }
  };
  let pos = 0;
  need(pos + 4, 'version_length');
  const versionLength = buf.readUInt32LE(pos); pos += 4;
  need(pos + versionLength, 'version');
  pos += versionLength; // 跳过 version 字符串
  need(pos + 4, '条目数');
  const fileTotal = buf.readUInt32LE(pos); pos += 4;
  const entries = [];
  for (let i = 0; i < fileTotal; i++) {
    need(pos + 4, '第 ' + (i + 1) + ' 条的 name_length');
    const nameLength = buf.readUInt32LE(pos); pos += 4;
    need(pos + nameLength + 8, '第 ' + (i + 1) + ' 条（name/index/size）');
...
/** ①(G8) 读容器头并解析：2MiB 不够就 ×4 倍增（到 MPKG_TABLE_MAX_BYTES 为止）。
 *  返回 { dataStart, entries, headBytes }；解析不出来 ⇒ 抛 code='table-too-large' 的可读错误。 */
function readMpkgHeadGrow(filePath) {
  let size = 0;
  try { size = statSync(filePath).size; } catch { /* 交给下面的读 */ }
  let lastErr = null;
  for (let cap = HEAD_BYTES; ; cap *= 4) {
    const want = Math.min(cap, MPKG_TABLE_MAX_BYTES, size > 0 ? size : cap);
    const buf = readMpkgHead(filePath, want);
    try {
      return Object.assign(parseMpkgHead(buf), { headBytes: want });
    } catch (e) { lastErr = e; }
    if (cap >= MPKG_TABLE_MAX_BYTES || (size > 0 && want >= size)) break;
  }
  const e = lastErr instanceof Error ? lastErr : new Error('容器目录表读不出来');
  e.code = 'table-too-large';
  throw e;
}

/** ①(G11 2026-09-23 / 核验报告) **压缩条目（LZ4 块链）的廉价识别**。
 *
 *  口径与 `lib/pkg-extract.js` 的 `probeCompressedEntry` **一致但更宽松**（只读 20 字节）：
 *    entry 开头 u64 = 解压后长度；必须 > 条目本身（压缩才可能变大）；
 *    随后 [i32 uncomp][i32 comp] 的第一块必须落在条目内。
 *  为什么是"廉价"：完整校验要逐块走完整个条目（几十~几百 MB 的多次随机读），本机语料
 *  508 条里压缩条目为 **0**（核验报告 §4.2e）——为它写解压是过度工程。这里只做
 *  "**如实报**"：命中就明确拒绝/标注，绝不把压缩字节当成图片/视频静默发出去。
 *  ⚠ 假阳性代价 = 一个本来能播的条目被拒（可换目录方式导入）；假阴性代价 = 静默坏掉。 */
function looksCompressedEntry(buf, entrySize) {
  try {
    if (!buf || buf.length < 16) return false;
    const originalSize = buf.readUInt32LE(0) + buf.readUInt32LE(4) * 4294967296;
    if (!(originalSize > entrySize) || originalSize > 2147483647) return false;
    const uncomp = buf.readInt32LE(8);
    const comp = buf.readInt32LE(12);
    if (uncomp <= 0 || comp <= 0) return false;
    if (8 + comp > entrySize) return false;
    return true;
  } catch { return false; }
}
...
/** ③ 自动发现：Steam 安装目录**候选**（纯函数：只产出字符串，是否存在由调用方 existsSync 决定）。
 *
 *  为什么必须是纯函数：平台分支（尤其 WSL 的盘符枚举）要能在**非 WSL 机器**上断言 ——
 *  "判断是契约，不是在我机器上能跑"。入参 `{ platform, release, homedir, exists, procVersion }`
 *  全部可注入，缺省分别取 `process.platform` / `os.release()` / `os.homedir()`；
 *  返回数组全是字符串、无重复、顺序稳定（同一入参两次调用逐项相同）。
 *
 *  ①(2026-09-21 跨平台轮) 修一个真实漏网：WSL 分支原来只写死 `/mnt/c/...` ⇒ Steam 装在 D:/E:/…
 *  的 WSL 用户**永远扫不到**（Windows 与 WSL 双装是常态，Steam 装在 D 盘也很常见）。
 *  现在按盘符枚举 `/mnt/d` … `/mnt/z` × 四个相对路径（见 WSL_STEAM_RELS）。
 *
 *  WSL 判据两条：`release` 小写含 `microsoft`（WSL2 内核固定带这个字样）；老 WSL1 内核 release
 *  里没有 ⇒ 再读 `/proc/version`（用 `exists` 先短路，读失败/权限拒绝一律吞掉，不抛）。
 *  只有 `platform === 'linux'` 才走这条分支：win32/darwin 上 release 出现 microsoft 属于不可能的
 *  输入，不去猜（否则等于到处乱探）。
 *
 *  ⚠ `STEAM_PROBE_DIRS` 与下面两条 `/mnt/c/...` 都是**历史遗留的不分平台候选项**（在 Windows/macOS
 *  上就是两个不存在的字符串，existsSync 自然为 false）。保持原样 ⇒ win32/darwin/linux 三个平台的
 *  候选集合与改动前**逐字节相同**；门禁 tools/cross-platform-test.mjs 逐项断言这一点。
 */
const WSL_DRIVE_LETTERS = 'defghijklmnopqrstuvwxyz';   // /mnt/d … /mnt/z（C 盘单独两条，历史上就有）
const WSL_STEAM_RELS = ['Program Files (x86)/Steam', 'Program Files/Steam', 'SteamLibrary', 'Steam'];
export function steamProbeDirs(env = {}) {
  const platform = typeof env.platform === 'string' ? env.platform : process.platform;
  const homedir = typeof env.homedir === 'string' ? env.homedir : homedirSafe();
  const release = typeof env.release === 'string' ? env.release : releaseSafe();
  // exists 只用来**短路** /proc/version 的读取（Windows/macOS 上省一次注定失败的 open）；
  // 传进来的实现抛异常也必须被吞掉（门禁里有一条"给会抛的 exists 也不许影响返回"）。
  const exists = typeof env.exists === 'function' ? env.exists : existsSync;
  const dirs = [...STEAM_PROBE_DIRS];
  if (platform === 'darwin') dirs.push(join(homedir, 'Library', 'Application Support', 'Steam'));
  if (platform === 'linux' || platform === 'android') dirs.push(join(homedir, '.local', 'share', 'Steam'));
  dirs.push('/mnt/c/Program Files (x86)/Steam', '/mnt/c/Program Files/Steam');
  if (platform === 'linux' && isWsl(release, env, exists)) {
    for (const letter of WSL_DRIVE_LETTERS) for (const rel of WSL_STEAM_RELS) dirs.push('/mnt/' + letter + '/' + rel);
  }
  return [...new Set(dirs)];
}
/** `os.homedir()` / `os.release()` 在个别残缺环境里会抛（部分容器没有 passwd 项）⇒ 吞掉给兜底值，
 *  保证 steamProbeDirs() 永不抛（判据里"退化入参也不许抛"包含本机 os 调用这一路）。 */
function homedirSafe() { try { return os.homedir() || '.'; } catch { return '.'; } }
function releaseSafe() { try { return os.release() || ''; } catch { return ''; } }
/** WSL 判定（判据见 steamProbeDirs 注释）。`env.procVersion` 可注入 ⇒ 不依赖本机 /proc/version。 */
function isWsl(release, env, exists) {
  if (String(release).toLowerCase().includes('microsoft')) return true;
  try {
    if (env.procVersion !== undefined) {
      const v = typeof env.procVersion === 'function' ? env.procVersion() : env.procVersion;
      return String(v == null ? '' : v).toLowerCase().includes('microsoft');
    }
    if (!exists('/proc/version')) return false;
 ...
 *  候选清单本身在纯函数 steamProbeDirs() 里（平台分支都在那里，可注入、可在非 WSL 机器上断言）；
 *  本函数只负责"注册表优先 → 逐个候选读 libraryfolders.vdf → 只认 wallpaper32.exe 存在的那一个"。
  ...
  try {
    ...
    for (const f of readdirSync(dir)) {
      ...
      try {
        ...
        // ①(G8) 用倍增读取：大目录表的容器重启后也能被登记（旧的固定 2MiB 会抛错被当成"损坏"跳过）
        const { dataStart, entries } = readMpkgHeadGrow(path);
    ...
    const walk = (d) => {
      ...
      for (const en of es) {
        ...
        try {
          ...
          const { dataStart, entries } = readMpkgHeadGrow(path);   // ①(G8) 同 restoreFiles
          files.set(token, { path, size: statSync(path).size, dataStart, entries });
        } catch { /* 跳过损坏文件 */ }
      }
    };
...
/** ①(2026-09-23 用户第 1 项 B「按屏幕物理尺寸预缩（ffmpeg lanczos）」档) 缩放滤波器 flags
 *  **白名单**（ffmpeg scale 的 `flags=` 取值）。客户端的预缩档传 `scale=lanczos`；
 *  不在表里 ⇒ **400**（不允许把任意串拼进 `-vf`）。空 = ffmpeg 默认（bicubic）⇒ 旧 URL 逐字不变。 */
const SCALE_FLAGS = ['fast_bilinear', 'bilinear', 'bicubic', 'experimental', 'nearest', 'area', 'bicublin', 'gauss', 'sinc', 'lanczos', 'spline'];
/** 预缩档要传的 flags；`null` = 非法（调用方回 400）。 */
function normalizeScaleFlag(raw) {
  const v = String(raw == null ? '' : raw).trim().toLowerCase();
  if (!v) return '';
  return SCALE_FLAGS.includes(v) ? v : null;
}
/** ①(B) `-vf` 链的**唯一构造点**（runTranscode 调它，门禁也直接调它做纯函数断言）：
 *  只在真的给了 maxW 时才带 scale（没上限就没有"缩"这回事，flags 会被忽略）——
 *  顺序仍是"先缩放再抽帧"（编码量更小）。 */
function buildScaleFilter(maxW, fps, scaleFlag) {
  const w = Number(maxW) > 0 ? Math.round(Number(maxW)) : 0;
  return w > 0
    ? "scale='min(" + w + ",iw)':-2" + (scaleFlag ? ':flags=' + scaleFlag : '') + ',fps=' + fps
    : 'fps=' + fps;
}
/** ①(B) 转码缓存键：**只含影响播放的字段**（源标识 | mtime | fps | maxW | 缩放 flags）。
 *  ⚠ 没有 flags 时与改动前**逐字节相同**（`|s:…` 只在 maxW>0 且给了 flags 时追加）
 *  ⇒ 既有产物继续命中，升级不重转（transcode-limit-test 的 B 组钉的就是这条）。 */
function transcodeKey(srcId, mtimeMs, fps, maxW, scaleFlag) {
  const w = Number(maxW) > 0 ? Math.round(Number(maxW)) : 0;
  return crypto.createHash('sha256')
    .update(srcId + '|' + Math.round(mtimeMs) + '|' + fps + '|' + w + ((w > 0 && scaleFlag) ? '|s:' + scaleFlag : ''))
...
/** ①(2026-09-23 资源审计 #1) `scanSceneVideo` 缓存（整段视频字节）当前归属的 scene 目录：
 *  换场景即调 `clearSceneVideoScanCache()`，见 `ensureSceneVideo`。初值 null ≠ 任何真实目录，
 *  所以第一次调用也会清一次（幂等；让审计前遗留的条目不可能跨场景活着）。 */
let sceneVideoScanOwnerDir = null;
/** ①(2026-09-23 静默失败审计 #3) "真·失败"标记（../docs/SILENT-FAILURE-AUDIT-20260923.md §A-3 表 #3/#4）：
 *  `ensureSceneVideo` 末尾的 `catch { return null }` 语义是"目录不在/读不到 ⇒ 当作没有视频"（路由据此
 *  回 404 / `has:false`），但**探测两级都抛**与**落盘失败**也落进同一个 catch，于是被伪装成"没有视频"：
 *  前者还顺手写 `{hash:null}` 负缓存 ⇒ 错误答案永久粘死；后者照写索引 + 照返回 `{path}` ⇒ 客户端拿到
 *  必然 404 的 `<video>`。这里用一个**显式标记**（而不是新错误类，blast radius 最小：本文件其它
 *  catch/调用点都不受影响）只在两条真失败上打标，外层 catch 只放行带此标记的 Error。 */
const SCENE_VIDEO_FAIL = Symbol('mpw.sceneVideoFail');
function sceneVideoFail(msg) {
  const e = new Error(msg);
  e[SCENE_VIDEO_FAIL] = true;   // ← 只有带这个标记的错误才允许穿过 ensureSceneVideo 的外层 catch
  return e;
}
...
function ensureSceneVideo(dir) {
  try {
    // ①(2026-09-23 资源审计 #1) `scanSceneVideo` 的缓存里存的是**整段视频字节**，所以它的生命周期
    //   跟着"当前在放哪个 scene"：换到另一个 scene 目录 ⇒ 上一个场景的字节在堆里已经没有任何消费者
    //   （落盘文件在上面 `sceneVideoIndex` 命中时即早退，`pkg-extract.js` 侧的预算/逐出是兜底），
    //   立刻清掉、释放引用。这是 `clearSceneVideoScanCache()` 在全仓的**第一个真实调用点**——
    //   审计时它零调用（docs/RESOURCE-AUDIT-20260923.md §2.1）。
    if (dir !== sceneVideoScanOwnerDir) {
      sceneVideoScanOwnerDir = dir;
    ...
    let video = null;
    let scanErr = null;     // 快路径（索引先行）的异常：只用于归因，单独抛**不**算失败
    let legacyErr = null;   // 兜底口径（整包 readFileSync + 全 TEX 解析）的异常
    try {
    ...
    if (legacyErr) {
      // ①(2026-09-23 静默失败审计 #3) 两级探测**都抛** ⇒ 这不是"没有视频"，是"探不出来"。改前此处
      //   `video = null`：下面会写 `{hash:null}` 负缓存 ⇒ /custom-scene-video-check 永久回
      //   200 `has:false`（错误答案粘死，且全程零日志）。所以：不写任何缓存、带真实原因抛出去
      //   （两条路由的外层 catch 已能回 500，见 serveSceneVideo / probeSceneVideo）。
      throw sceneVideoFail('scene 视频探测失败: ' + dir + ' — 快路径: ' + String((scanErr && scanErr.message) || scanErr) + '；兜底口径: ' + String((legacyErr && legacyErr.message) || legacyErr));
    }
    if (!video) {
      // 真·无视频（探测**成功**且结论如此）⇒ 保持改前语义：写负缓存，路由 200 has:false / 404。
    ...
    if (!existsSync(out)) {
      ...
      try { writeFileSync(tmp, video.bytes); renameSync(tmp, out); }
      catch (e) {
        try { unlinkSync(tmp); } catch { /* 忽略 */ }
        // ①(2026-09-23 静默失败审计 #3) 落盘失败且 `out` 确实不存在 ⇒ 不许写索引、不许返回 path。
        //   改前这一格静默吞掉，然后照样写 `{hash}` 索引 + 返回 `{path: out}` ⇒ 探测回 200 `has:true`，
        //   客户端却拿到一个必然 404（或永远不播）的 scene-video URL。`existsSync(out)` 复检只放过
        //   "并发请求已经写好"这一种情况（那种情况下文件真的在，返回它是正确的）。
        if (!existsSync(out)) throw sceneVideoFail('scene 视频落盘失败: ' + out + ' — ' + String((e && e.message) || e));
      }
    }
    ...
    // ①(2026-09-23 静默失败审计 #3) 只放行带 SCENE_VIDEO_FAIL 标记的**真失败**（探测两级都抛 / 落盘失败）。
    //   其余（`statSync` 目录不在或读不到、`sceneVideoDir()` 失败…）保持改前语义：`return null`
    //   ⇒ 路由 404 / `has:false` —— 那正是"这个目录没有视频"该有的答案，不能改成 500。
    if (err && err[SCENE_VIDEO_FAIL]) throw err;
    return null;
  }
}
/** scene.pkg 整包 → 找内嵌视频。
...
/** ①(2026-09-23 用户第 4 项 / 核验报告 G9) 上传容器副本的回收。
 *
 *  现场：一次导入就在 DATA_DIR 留一份 `<32hex>.mpkg` 整包副本（真机 830MB），
 *  **永不回收**，重启后还被 `restoreFiles()` 重新登记 ⇒ 会话内实测累积 4 个 / 4.97MB
 *  （真机就是 GB 级）。
 *
 *  回收口径（与 transcodes/ 同款，双闸门 + 最旧先删）：
 *    · 候选**只有** `/^[0-9a-f]{32}\.mpkg$/` 这一种文件名 —— 上传 token 的形状。
 *      **用户自己放进 DATA_DIR 的源文件一律不碰**（更别说用户库目录 customDir 里的东西）；
 *    · **当前壁纸仍在引用**的 token（settings.json 全文里的 `token=<hex32>`）**绝不动**；
 *    · 最近 UPLOAD_PROTECT_MS 内被 `/media` 读过的 token（正在播放的那一个）**绝不动**；
 *    · `protect` 传进来的路径（本次刚上传成功的那个）**绝不动**；
 *    · 其余按 mtime 从最旧开始删，直到「数量 ≤ UPLOAD_KEEP 且 合计 ≤ UPLOAD_MAX_BYTES」。
 *
 *  ⚠ 不注册 `process.on('exit')` 全量删：restoreFiles() 的"重启后壁纸自愈"依赖这些副本
 *  跨重启存在（删了 = 重启后壁纸 404）。回收只按上限做，不做退出清空。 */
function referencedUploadTokens() {
  const out = new Set();
  try {
    const f = join(DATA_DIR, 'settings.json');
    if (!existsSync(f)) return out;
    const raw = readFileSync(f, 'utf8');
    const re = /token=([0-9a-f]{32})/g;
    let m = null;
    while ((m = re.exec(raw))) out.add(m[1]);
  } catch { /* 读不到 → 只靠 lastUsed 保护 */ }
  return out;
}
function pruneUploads(protectPath) {
  const removed = { uploads: 0, freed: 0, keep: 0, bytes: 0 };
  try {
    const dir = DATA_DIR;
    if (!existsSync(dir)) return removed;
    const referenced = referencedUploadTokens();
    const now = Date.now();
    const rows = [];
    for (const n of readdirSync(dir)) {
      if (!UPLOAD_NAME_RE.test(n)) continue;          // 只可能回收上传物（用户文件一律不碰）
      const full = join(dir, n);
      let size = 0, mtime = 0;
      try { const st = statSync(full); size = st.size; mtime = st.mtimeMs; } catch { /* 竞态：刚被删 */ }
      rows.push({ n, full, token: n.slice(0, -5), size, mtime });
    }
    rows.sort((a, b) => b.mtime - a.mtime);           // 最新在前
    const protectResolved = protectPath ? resolve(protectPath) : '';
    const isLive = (r) => {
      if (protectResolved && resolve(r.full) === protectResolved) return true;
      if (referenced.has(r.token)) return true;
      const rec = files.get(r.token);
      if (rec && rec.lastUsed && (now - rec.lastUsed) < UPLOAD_PROTECT_MS) return true;
      return false;
    };
    let total = rows.reduce((s, r) => s + r.size, 0);
    let count = rows.length;
    for (let i = rows.length - 1; i >= 0; i--) {      // ← 从最旧（尾部）开始
      const r = rows[i];
      if (count <= UPLOAD_KEEP && total <= UPLOAD_MAX_BYTES) break;
      if (isLive(r)) { removed.keep++; continue; }    // 在用的（当前壁纸/刚上传/刚读过）跳过
      try {
        unlinkSync(r.full);
        files.delete(r.token);
        removed.uploads++; removed.freed += r.size; count--; total -= r.size;
      } catch { /* 单个删不掉不阻断其余 */ }
    }
    removed.bytes = total;
    if (removed.uploads) {
      console.log('[dsh-mpkg-wallpaper][prune] 上传容器副本：已删除 ' + removed.uploads + ' 个最旧文件，释放 '
        + (removed.freed / 1048576).toFixed(2) + ' MB（上限 ' + UPLOAD_KEEP + ' 个 / '
        + Math.round(UPLOAD_MAX_BYTES / 1048576) + ' MB；在用 ' + removed.keep + ' 个已保护）');
    }
  } catch { /* 忽略 */ }
  return removed;
}
/** ①(G9) 上传目录现场（数量 / 字节 / 在用），供启动日志、/probe 与门禁核对。 */
function uploadStat() {
  const out = { count: 0, bytes: 0, live: 0, keep: UPLOAD_KEEP, maxBytes: UPLOAD_MAX_BYTES };
  try {
    for (const n of readdirSync(DATA_DIR)) {
      if (!UPLOAD_NAME_RE.test(n)) continue;
      try { out.bytes += statSync(join(DATA_DIR, n)).size; out.count++; } catch { /* 竞态 */ }
      const rec = files.get(n.slice(0, -5));
      if (rec && rec.lastUsed && (Date.now() - rec.lastUsed) < UPLOAD_PROTECT_MS) out.live++;
    }
  } catch { /* 目录不在 */ }
  return out;
}
...
async function runTranscode(srcFile, out, fps, maxW, job, isCancelled, scaleFlag) {
  ...
  // ①(2026-09-23 B) vf 链构造收敛到 buildScaleFilter（预缩档 = maxW + flags=lanczos；
  //   没给 flags 时与改动前逐字相同）。
  const vf = buildScaleFilter(maxW, fps, scaleFlag);
 ...
 *  返回缓存产物绝对路径。 */
async function transcodeToFps(src, fps, maxW, isCancelled, scaleFlag) {
  const cancelled = () => { try { return typeof isCancelled === "function" && !!isCancelled(); } catch { return false; } };
  const srcPath = src.type === 'file' ? src.path : src.rec.path;
  const st = statSync(srcPath);
  const srcId = src.type === 'mpkg' ? 'mpkg:' + src.rec.path + ':' + src.entry.index : srcPath;
  const key = transcodeKey(srcId, st.mtimeMs, fps, maxW, scaleFlag);
  ...
  // ⓪(B) 进度 key 与缓存 key 同源：**带上缩放 flags**（否则预缩任务的进度永远查不到）。
  const progKey = srcId + '|' + fps + '|' + (maxW || 0) + ((Number(maxW) > 0 && scaleFlag) ? '|s:' + scaleFlag : '');
  const p = (async () => {
    ...
    try {
      ...
      try {
        ...
        await runTranscode(mat.file, tmp, fps, maxW, transcodeJobs.get(progKey) || {}, isCancelled, scaleFlag);
...
function handleTranscodeProgress(req, res) {
  try {
    ...
    // ①(B) 查询键与写入键一致（含缩放 flags）；未给 flags ⇒ 与改动前逐字相同的键。
    const scaleFlag = normalizeScaleFlag(url.searchParams.get('scale')) || '';
    let job = null;
    ...
    if (src) {
      ...
      const withFlags = srcId + '|' + fps + '|' + maxW + ((maxW > 0 && scaleFlag) ? '|s:' + scaleFlag : '');
      job = transcodeJobs.get(withFlags)
        || transcodeJobs.get(srcId + '|' + fps + '|' + maxW)
        || transcodeJobs.get(srcId + '|' + fps) || null;
...
/* ═══════════════════════════════════════════════════════════════════════════════════════
 * ①(2026-09-21 用户第 11 条) 网页壁纸风险预检 —— **唯一实现**（结构化数据，两处扫描 + 一条路由共用）
 * ───────────────────────────────────────────────────────────────────────────────────────
 * 判据（两个标记各自的来历，都是真机语料）：
 *   · `heavy`（重动画）：目录里存在骨骼动画资产 —— `.skel` / `.atlas` 后缀，或文件名含
 *     `spine` / `live2d` / `.l2d`（递归 ≤3 层，跳过点目录）。Spine/L2D 类网页壁纸在低配设备上
 *     会卡住界面（用户实测：昔涟类 webm 正常、Spine 类卡死）。
 *   · `external`（需外网）：入口 HTML（≤256 KB）里出现 `http(s)://` 引用，且**不是**本机回环地址
 *     （`localhost` / `127.0.0.1` / `[::1]`，插件自己的路由就是这些）。米哈游 SDK 类页面加载失败
 *     都是这一条（外网被墙/断网 ⇒ 白屏）。
 * `reasons` 是给人看的一句话（面板/测试台直接显示，不用自己拼文案）；`hits/refs` 是证据
 * （最多各 8 条，供悬停说明与探针断言）。
 * 契约（测试台/渲染器侧消费）：形状写进 docs/WEB-WALLPAPER.md「风险预检接口」一节。
 * ═══════════════════════════════════════════════════════════════════════════════════════ */
const WEB_PROBE_MAX_REFS = 8;
const WEB_PROBE_MAX_BYTES = 262144;
function probeWebWallpaper(dir, entryFile) {
  const out = {
    isWeb: false, heavy: false, external: false,
    heavyHits: [], externalRefs: [], htmlFile: null, scannedBytes: 0,
    reasons: [], limits: { depth: 3, bytes: WEB_PROBE_MAX_BYTES, refs: WEB_PROBE_MAX_REFS },
  };
  try {
    if (!dir || !existsSync(dir) || !statSync(dir).isDirectory()) { out.reasons.push('目录不可读'); return out; }
    const heavyHits = out.heavyHits;
    const walkHeavy = (d2, depth) => {
      if (depth > 3 || heavyHits.length >= WEB_PROBE_MAX_REFS) return;
      let ents = [];
      try { ents = readdirSync(d2, { withFileTypes: true }); } catch { return; }
      for (const en of ents) {
        if (heavyHits.length >= WEB_PROBE_MAX_REFS) return;
        if (en.isDirectory()) { if (!en.name.startsWith('.')) walkHeavy(join(d2, en.name), depth + 1); continue; }
        if (/\.(skel|atlas)$/i.test(en.name) || /spine|live2d|\.l2d/i.test(en.name)) heavyHits.push(en.name);
      }
    };
    walkHeavy(dir, 0);
    out.heavy = heavyHits.length > 0;
    if (out.heavy) out.reasons.push('骨骼动画资产 ' + heavyHits.length + ' 个（Spine/Live2D）⇒ 低配设备可能卡顿');
    // 入口 HTML：给了就用，没给就找第一个 .html/.htm（与 detectWallpaperDir 同一口径）
    let entry = entryFile ? String(entryFile) : '';
    if (!entry) {
      try { entry = readdirSync(dir).find((f) => /\.(html?|htm)$/i.test(f)) || ''; } catch { entry = ''; }
    }
    if (entry) {
      const htmlFile = resolve(join(dir, entry));
      out.htmlFile = entry;
      if (existsSync(htmlFile)) {
        let text = '';
        try { text = readFileSync(htmlFile, 'utf8').slice(0, WEB_PROBE_MAX_BYTES); } catch { text = ''; }
        out.scannedBytes = text.length;
        const re = /https?:\/\/[^\s"'\)<>]+/gi;
        const seen = new Set();
        let m;
        while ((m = re.exec(text)) && out.externalRefs.length < WEB_PROBE_MAX_REFS) {
          const raw = String(m[0] || '');
          let host = '';
          try { host = new URL(raw).hostname.toLowerCase(); } catch { host = ''; }
          if (!host || host === 'localhost' || host === '127.0.0.1' || host === '[::1]' || host === '::1' || /\.localhost$/.test(host)) continue;
          if (seen.has(raw)) continue;
          seen.add(raw);
          out.externalRefs.push(raw.length > 200 ? raw.slice(0, 200) : raw);
        }
        out.external = out.externalRefs.length > 0;
        if (out.external) out.reasons.push('入口 HTML 引用外网资源 ' + out.externalRefs.length + ' 处 ⇒ 断网/被墙时可能加载失败');
      } else {
        out.reasons.push('入口 HTML 不在磁盘上');
      }
    }
    out.isWeb = true;
  } catch (err) {
    // 预检失败**不阻塞**（既有语义）：如实把原因写进 reasons，两个标记保持 false。
    out.reasons.push('预检异常：' + String((err && err.message) || err));
  }
  return out;
}
...
function apply(ctx) {
  ...
  restoreFiles();
  // ①(G9) 启动回收一次上传容器副本（上限：数量 + 合计字节；当前壁纸引用的与最近读过的 token 受保护）。
  //  放在 restoreFiles() **之后**：先让 files Map 把 token → lastUsed 建起来，保护窗才判得准。
  try { console.log('[dsh-mpkg-wallpaper][limits] 转码缩放档：默认 flags=bicubic（ffmpeg 默认）；预缩档 `scale=lanczos`（白名单 '
    + SCALE_FLAGS.length + ' 项，见 /probe limits.scaleFlags）'); } catch { /* 忽略 */ }
  try { const up0 = pruneUploads(); console.log('[dsh-mpkg-wallpaper][limits] 上传容器副本：' + up0.bytes + 'B / 上限 '
    + UPLOAD_KEEP + ' 个 · ' + Math.round(UPLOAD_MAX_BYTES / 1048576) + 'MB（本次删除 ' + up0.uploads + ' 个，保护 ' + up0.keep + ' 个）'); } catch { /* 忽略 */ }
  restoreCustomDir();
  ...
  };
  {
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          // ①(2026-09-20 同类缺陷一并修) 超限原来走 `req.destroy()`：**先掐连接再写响应** ⇒ 客户端拿到的是
          //   ECONNRESET，看不到状态码（渲染器仓实测：curl `status=100 / exit=56`）。同一类缺陷在
          //   `we-scene-demo` 的四个 POST 接收端上已一并修掉，口径照那边：**继续读干净但不再缓存**，
          //   到 `end` 回 413 JSON。⚠ 不能用 `req.pause()`：请求体没读完 ⇒ `end` 永不触发 ⇒ 请求挂死
          //   （渲染器仓实测：pause 版让 curl 卡满 25s 超时，服务端一句话不回）。
          const DIAG_BODY_MAX = 8 * 1024 * 1024;
          let body = '';
          let tooBig = false;
          req.on('data', (c) => {
            if (tooBig) return;
            body += c;
            if (body.length > DIAG_BODY_MAX) { tooBig = true; body = ''; }
          });
          req.on('end', () => {
            if (tooBig) {
              json(res, 413, { ok: false, error: 'diag payload 超过上限 ' + DIAG_BODY_MAX + ' 字节（未落盘）' });
              return;
            }
            try {
      ...
      handler: async (req, res) => {
        try {
          ...
          if (method === 'PUT' || method === 'POST') {
            ...
            /* ①(2026-09-19 设置持久化轮) **合并不替换**（与客户端 writeSection 同一条契约）。
               旧实现把请求体**整串覆写**磁盘文件 ⇒ 客户端任何一次"内存里不完整的保存"都会把
               settings.json 里的其余字段一起抹掉（真机现场：`mpkgKey` 还在、`image`/`webUrl` 没了
               → 壁纸层被 display:none 藏掉，见 docs/SETTINGS-PERSIST.md）。
               现在：未出现的键**保持原值**；要删一个键必须显式传 `null`。
               `undefined` 在 JSON 里不可能出现 ⇒ 老客户端（不带某个键）天然受保护，无需版本协商。 */
            try {
              const prev = existsSync(filePath) ? JSON.parse(readFileSync(filePath, 'utf8')) : null;
              if (prev && typeof prev === 'object' && !Array.isArray(prev)) {
                const merged = Object.assign({}, prev);
                for (const k of Object.keys(settings)) {
                  if (settings[k] === null) { delete merged[k]; continue; }
                  merged[k] = settings[k];
                }
                settings = merged;
              }
            } catch { /* 旧文件损坏/不可读 ⇒ 退回整串覆写（不静默失败：下面照常落盘） */ }
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          // ①(G8 2026-09-23) 原来这里在流里攒 `head`（`Buffer.concat` 到满 2MiB）给 parseMpkgHead。
          //  现在改从**落盘文件**倍增读（readMpkgHeadGrow）⇒ 这 2MiB 累积不但没用，还对
          //  830MB 的上传做上千次 Buffer.concat（每次最多 2MiB 拷贝 ≈ 2GB 无谓复制 + 2MiB 常驻）。
          //  删掉；`size` 仍照记（响应里要回）。
          let size = 0;
          try {
          ...
          // ①(G8 2026-09-23) 头部读满 2MiB 仍不够 ⇒ ×4 倍增重读到能解析为止（上限 32MiB）。
          //  解析不出来 ⇒ **可读的 413**（而不是原生 RangeError 的 500），
          //  并且**把已落盘的上传物删掉**（原实现在这里留一个未登记残骸，实测 H8）。
          let parsed = null;
          try {
            parsed = readMpkgHeadGrow(filePath);
          } catch (e) {
            try { unlinkSync(filePath); } catch { /* 忽略 */ }
            json(res, 413, {
              ok: false,
              code: (e && e.code) || 'table-too-large',
              error: '容器目录表过大：已读 ' + HEAD_BYTES + 'B → 倍增到 ' + Math.round(MPKG_TABLE_MAX_BYTES / 1048576)
                + 'MB 仍读不出条目表（' + String(e && e.message || e) + '）。已删除本次上传的临时文件；'
                + '目录表超大的容器请改用「选择文件夹」方式导入',
              headBytes: HEAD_BYTES, tableMaxBytes: MPKG_TABLE_MAX_BYTES,
            });
            return;
          }
          const { dataStart, entries } = parsed;
          // ①(G11) 压缩条目（LZ4 块链）**如实报**：只探测将要被当成图片/视频播放的候选条目
          //  （每条只读 20 字节；完整解压校验见 lib/pkg-extract.js，本机语料 508 条里 0 条压缩）。
          const compressed = [];
          if (entries.length) {
            let fd = null;
            try {
              ...
              for (const e of entries) {
                if (compressed.length >= 8) break;
                if (!/\.(gif|png|jpe?g|webp|mp4|webm|mov)$/i.test(e.name)) continue;
                const probe = Buffer.alloc(20);
                let got = 0;
                try { got = readSync(fd, probe, 0, 20, dataStart + e.index); } catch { got = 0; }
                if (got >= 16 && looksCompressedEntry(probe.subarray(0, got), e.size)) compressed.push(e.name);
              }
            } catch { /* 探测失败不阻断导入 */ }
            finally { if (fd !== null) { try { closeSync(fd); } catch { /* 忽略 */ } } }
          }
          // ①(G9) 注意 lastUsed **不在这里写**：新上传的副本由 `pruneUploads(filePath)` 的
          //  protect 参数保护这一次；若这里就写 Now()，连续导入的每一份都会躺进"最近被读"保护窗
          //  ⇒ 上限永远不生效（回收形同虚设）。只有 `/media` 真的读过它，才算"在用"。
          files.set(token, { path: filePath, size, dataStart, entries, lastUsed: 0 });
          // ①(G9) 上传成功即按上限回收（本次产物受保护，正在播放的 token 受 lastUsed 保护）
          try { pruneUploads(filePath); } catch { /* 回收失败不影响导入 */ }
          json(res, 200, { ok: true, token, size, entries, compressed, uploads: uploadStat() });
        } catch (err) {
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          // ①(G9) 谁在被读 = 谁还活着：每次命中刷新 lastUsed（pruneUploads 的保护窗就是它）。
          try { rec.lastUsed = Date.now(); } catch { /* 忽略 */ }
          ...
          // ①(G11 2026-09-23) 压缩条目（LZ4 块链）**不解压**：直接发出去 = 用户拿到压缩字节
          //  （黑屏/花屏且没有任何解释）。这里如实拒绝 + 说明，客户端与日志都能看见。
          //  探测只读 20 字节；判据与 lib/pkg-extract.js 的 probeCompressedEntry 同源（更宽松）。
          if (off === 0 && seg >= 16) {
            try {
              const probe = Buffer.alloc(20);
              const fd = openSync(rec.path, 'r');
              let got = 0;
              try { got = readSync(fd, probe, 0, 20, start); } finally { closeSync(fd); }
              if (got >= 16 && looksCompressedEntry(probe.subarray(0, got), entry.size)) {
                console.warn('[dsh-mpkg-wallpaper][mpkg] 条目是压缩条目（LZ4），当前路径不解压：' + entry.name);
                json(res, 415, {
                  ok: false, code: 'entry-compressed', entry: entry.name,
                  error: '容器条目「' + entry.name + '」是压缩条目（LZ4 块链），当前 mpkg 播放路径不解压'
                    + '（渲染器侧的 pkg-extract 解析器支持解压）—— 请改用「选择文件夹」方式导入该壁纸',
                });
                return;
              }
            } catch { /* 探测失败不阻断（保持原行为） */ }
          ...
          // ①(2026-09-21 音频格式补全) 原三元链只补了 mp3/ogg/opus/wav/flac，缺 .oga/.m4a/.aac；
          //   改查统一表（AUDIO_SUFFIX_MIME，与渲染器逐项一致），音频项在图片/视频之前判。
          const mime = suffixAudioMime(name) ? suffixAudioMime(name)
    ...
    /* ═══ ①(MEDIA-1 接线 2026-09-20) 系统媒体会话三条路由（契约：docs/MEDIA-SESSION.md §6） ═══════════
       · GET  /media-session            ⇒ `snapshot()` 21 键**原样透传**（路由里不"顺手"改形状）
       · POST /media-control {op,arg}   ⇒ op 只允许 CONTROL_OPS 六项（坏 op 在模块里就被拒，这里再挡一道）
       · GET  /media-art?player=<名>    ⇒ MPRIS 的 `file://` 封面代理（浏览器读不到宿主磁盘；
                                            只允许**快照里出现过的** player + 那个 artUrl 路径）
       为什么是三条而不是一条：封面是二进制（不能塞进 JSON）、控制是副作用（不能混进只读快照）、
       快照要能被 2s 轮询（越简单越好）。 */
    const mediaSession = createMediaSession({ env: process.env });
    webServer.register({
      kind: 'exact', path: BASE + '/media-session',
      handler: async (req, res) => {
        try {
          const snap = await mediaSession.snapshot();
          /* 默认档**只回 21 键快照**（契约：路由里不改形状）。`?describe=1` 才附带模块自述
             （平台/适配器/超时/上限；纯函数、不触发任何命令），给诊断面板与探针用。 */
          let extra = {};
          try { if (new URL(req.url, 'http://localhost').searchParams.get('describe') === '1') extra.describe = mediaSession.describe(); } catch (e) {}
          json(res, 200, Object.assign({ ok: true }, snap, extra));
        } catch (e) { json(res, 500, { ok: false, error: String(e && e.message || e) }); }
      },
    });
    webServer.register({
      kind: 'exact', path: BASE + '/media-control',
      handler: async (req, res) => {
        try {
          if (req.method !== 'POST') { json(res, 405, { ok: false, error: '只接受 POST（body = {op,arg}）', allow: 'POST' }); return; }
          let body = '';
          for await (const c of req) { body += String(c); if (body.length > 64 * 1024) { json(res, 413, { ok: false, error: 'body 过大' }); return; } }
          let d = {};
          try { d = JSON.parse(body || '{}'); } catch { json(res, 400, { ok: false, error: 'bad json' }); return; }
          const op = String(d && d.op || '');
          if (!CONTROL_OPS.includes(op)) { json(res, 400, { ok: false, error: 'bad op', ops: CONTROL_OPS.slice() }); return; }
          let arg = d && d.arg;
          if (op === 'seek') {
            const n = Number(arg);
            if (!Number.isFinite(n) || n < 0) { json(res, 400, { ok: false, error: 'bad seek arg', maxMs: SEEK_MAX_MS }); return; }
            arg = Math.min(Math.floor(n), SEEK_MAX_MS);
          } else arg = null;
          const out = await mediaSession.control(op, arg);
          json(res, out && out.ok ? 200 : 200, Object.assign({ ok: !!(out && out.ok) }, out || {}));
        } catch (e) { json(res, 500, { ok: false, error: String(e && e.message || e) }); }
      },
    });
    webServer.register({
      kind: 'exact', path: BASE + '/media-art',
      handler: async (req, res) => {
        try {
          const u = new URL(req.url, 'http://localhost');
          const player = String(u.searchParams.get('player') || '');
          const snap = await mediaSession.snapshot();
          // 只服务**当前快照里**那个玩家的封面：调用方不能拿任意路径来读宿主文件。
          if (!snap || !snap.available || !player || player !== snap.player) { json(res, 404, { ok: false, error: 'no such player in current snapshot' }); return; }
          if (snap.artUrlKind !== 'file') { json(res, 409, { ok: false, error: 'artUrl 不是本地文件（直接用 artUrl 即可）', kind: snap.artUrlKind }); return; }
          const p = decodeURIComponent(String(snap.artUrl).replace(/^file:\/\//, ''));
          let st = null;
          try { st = statSync(p); } catch { json(res, 404, { ok: false, error: '封面文件不存在' }); return; }
          if (!st.isFile()) { json(res, 400, { ok: false, error: '不是文件' }); return; }
          if (st.size > MAX_ART_CHARS) { json(res, 413, { ok: false, error: '封面过大', bytes: st.size, maxBytes: MAX_ART_CHARS }); return; }
          const buf = readFileSync(p);
          const mime = /^\x89PNG/.test(buf.slice(0, 4).toString('latin1')) ? 'image/png'
            : (buf.slice(0, 3).toString('latin1') === 'GIF' ? 'image/gif'
              : (buf.slice(0, 2).toString('latin1') === '\xff\xd8' ? 'image/jpeg' : 'application/octet-stream'));
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          customDir = dir;
          persistCustomDir();
          // ①(2026-09-21 音频格式补全) 后缀取统一表 AUDIO_SUFFIX_MIME 的键（不再另写一份字面量：
          //   表外后缀进不来，表内 8 项与渲染器 **逐项一致**）。原来这里只列图片/视频/mpkg，
          //   自定义目录里的单段音轨（.flac/.mp3/.m4a…）根本不进清单 —— 用户侧的"扫不出来"。
          // ①(G5 2026-09-23 用户第 4 项) 白名单补 `.pkg`：原实现只有 `.mpkg` ⇒ 库目录**根**下的
          //   单文件 scene.pkg **根本不进清单**（同位置的 .mpkg 会进）——而"pkg 单文件"正是
          //   用户点名的路径。`.mpkg` 不会被子串误伤（"x.mpkg".endsWith('.pkg') === false）。
          const exts = ['.gif', '.png', '.jpg', '.jpeg', '.webp', '.mp4', '.webm', '.mov', '.pkg', '.mpkg',
            ...[...AUDIO_SUFFIX_MIME.keys()].map((s) => '.' + s)];
          // ①(安全) 只收图片/视频/mpkg/音频；排除 exe/bat/sh/scr 等可执行与脚本（防病毒注入）
          const files = [];
          ...
          for (const e of readdirSync(dir, { withFileTypes: true })) {
            ...
            if (e.isDirectory() && !f.startsWith('.')) {
              ...
              if (type && type !== 'unknown' && media) {
                // ①(2026-09-21 第 11 条) 网页壁纸风险预检：**唯一实现** probeWebWallpaper()（模块级）。
                //   两个标记 + 证据（hits/refs）+ 人话原因一起给出去（测试台/渲染器侧直接显示）。
                const wp = type === 'web' ? probeWebWallpaper(sub, media) : null;
                const webHeavy = !!(wp && wp.heavy), webExternal = !!(wp && wp.external);
                const pv = pickPreview(sub);
                // ①(G4 2026-09-23 用户第 4 项 / 核验报告) `detectWallpaperDir` 的三元只分
                //   `scene.pkg` vs 其它 ⇒ 任意名的容器（`my-wallpaper.pkg` / `xxx.mpkg`，entry
                //   就是容器路径）被标成 `scene-json`（"松散场景"），而 kindReason 会**原样下发**
                //   给界面 ⇒ 用户看到错误的判定依据。这里按 entry 的真实后缀改标 scene-container
                //   （判定口径与 lib/web-wallpaper.js 的 ANY_PKG_RE 一致：`/\.(pkg|mpkg)$/i`）。
                //   ⚠ 只改"依据文案"这一处消费点；未改 `lib/web-wallpaper.js`（非本线独占文件，
                //   避免与并行线写冲突——根因修法见核验报告 §3 G4）。
                const kindReason = (det.reason === 'scene-json' && /\.(pkg|mpkg)$/i.test(String(media || '')))
                  ? 'scene-container' : det.reason;
                files.push({
                  name: f, title: title || f, type, media, preview: pv, folder: true,
                  webHeavy, webExternal, kindReason, declaredType: det.declared,
                  /* 结构化预检数据（可选消费；形状见 docs/WEB-WALLPAPER.md「风险预检接口」） */
                  webProbe: wp ? { heavy: wp.heavy, external: wp.external, heavyHits: wp.heavyHits, externalRefs: wp.externalRefs, htmlFile: wp.htmlFile, reasons: wp.reasons } : null,
                });
                continue;
              }
              continue;
            ...
            if (exts.some((x) => f.toLowerCase().endsWith(x))) {
              // ①(2026-09-21 音频格式补全) 音频先判（用统一表，不看内容）：image/video/mpkg 的原有判定顺序不变。
              // ①(G5) 类型：`.pkg` 与 `.mpkg` 都是 PKG 容器 ⇒ 同一个 'mpkg' 类型（客户端按容器走
              //   /custom-mpkg，与子文件夹里的 scene.pkg 行为一致）。
              const type = suffixAudioMime(f) ? 'audio' : /\.(mp4|webm|mov)$/i.test(f) ? 'video' : /\.(mpkg|pkg)$/i.test(f) ? 'mpkg' : 'image';
              files.push({ name: f, type });
            }
          }
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          const { dataStart, entries } = readMpkgHeadGrow(filePath);   // ①(G8) 大目录表不再 500
          ...
          files.set(token, { path: filePath, size: statSync(filePath).size, dataStart, entries, lastUsed: Date.now() });
          ...
          // ①(G11) 选中条目若是**压缩条目**（LZ4 块链）⇒ 如实标出来（客户端据此报可读错误，
          //   而不是拿去播/画压缩字节）。只读 20 字节。
          try {
            const e = entries[sel.index];
            const off = dataStart + e.index + (sel.offset || 0);
            const probe = Buffer.alloc(20);
            const fd = openSync(filePath, 'r');
            let got = 0;
            try { got = readSync(fd, probe, 0, 20, off); } finally { closeSync(fd); }
            if (got >= 16 && looksCompressedEntry(probe.subarray(0, got), e.size)) {
              sel = Object.assign({}, sel, { compressed: true });
            }
          } catch { /* 探测失败不阻断 */ }
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          const filePath = folder ? join(customDir, folder, file) : join(customDir, file);
          // ①(G7 2026-09-23 用户第 4 项) 放宽到 `.pkg`：单文件 scene.pkg 与 .mpkg 是同一个
          //   容器格式，缩略图自然也该能从容器里取（实测原状：.mpkg→200 image/gif，.pkg→404）。
          //   缓存键与 token 本来就是文件名（含 folder），安全口径不变（下面仍是同目录 + 无穿越校验）。
          if (!existsSync(filePath) || !/\.(mpkg|pkg)$/i.test(file)) { json(res, 404, { ok: false, error: 'not found' }); return; }
          ...
          const { dataStart, entries } = readMpkgHeadGrow(filePath);   // ①(G8) 同 /custom-mpkg
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          // ①(2026-09-21 音频格式补全) 这条三元链**完全没有音频分支**：自定义目录里的单段音轨走
          //   /custom-media 时一律 octet-stream（浏览器 <audio> 不认）。补上统一表（audioMimeFor 的
          //   "扩展名兜底"一侧；此路由不进 magic 嗅探 —— 先读字节再决定 content-type 会拖慢视频首帧，
          //   带实际字节的路径是 /raw 与 /custom-folder）。
          const mime = suffixAudioMime(name) ? suffixAudioMime(name)
            : name.endsWith('.mp4') ? 'video/mp4' : name.endsWith('.webm') ? 'video/webm'
     ...
     *  ①(2026-09-21 音频格式补全) 嗅探与后缀判定**都收敛到 pkg-extract.js 的 audioMimeFor**
     *  （docs/AUDIO-TRACK-SPEC.md §3）。旧实现三处缺陷：① ISO-BMFF 分支写成
     *  `hex.startsWith('66427970')`（= ASCII `fByp`；`ftyp` 是 `66747970` 且位于**偏移 4**）
     *  ⇒ m4a/aac 永远落空；② 整个嗅探被 `n >= 12` 包住 ⇒ 小于 12 字节的合法文件
     *  （如 `fLaC` + STREAMINFO 块头的最小 FLAC）一条都判不出；③ 短头时 `buf[1]` 读的是
     *  未写入的 0 ⇒ 同步字规则在短头上给出错误结论。现在按签名各自的长度守卫，
     *  `fLaC`/`OggS`/`ID3` 只要 4/4/3 字节即可命中，判定优先级仍由 §3.2 的表决定。 */
    const AUDIO_MAGIC_MIME = (path, name) => {
      try {
        const fd = openSync(path, 'r');
        const buf = Buffer.alloc(16);
        let n = 0;
        try { n = readSync(fd, buf, 0, 16, 0); } finally { closeSync(fd); }
        const head = n > 0 ? buf.subarray(0, n) : null;       // 短文件只给实际字节，不留零尾
        const byMagic = audioMimeFor(name, head);
    ...
    const FOLDER_MIME = (name) => {
      ...
      // ①(2026-09-21 音频格式补全) 原先 6 条内联音频判定，与 pkg 条目 /media 的三元链、
      //   /raw 的二元判定各写一份 ⇒ 三份表漂移（.oga/.m4a/.aac 在别处缺失）。统一查 §1.2 的表。
      const audio = suffixAudioMime(n);
    ...
    // ①(WP-1 2026-09-19) ═══ 网页壁纸存储 facade 的宿主落点 + 宿主媒体音频控制 ═══
    //  两件事都是**宿主侧状态**（不是 UI）：UI 线只按 docs/WEB-WALLPAPER.md §13 的契约调用。
    //  · /web-store：帧内 localStorage facade 的持久化（不透明源下真 storage 抛 SecurityError）。
    //    键 = 壁纸 wallId（label+目录+入口的 sha1 前 12 位，**不含绝对路径**）；值全是字符串。
    //    上限与帧内同一套常量 + 每张壁纸条数上限；超上限的壁纸按最旧淘汰（updatedAt）。
    //  · /media-audio：用户第 6 项要的"把视频声音交给宿主"接口（setMediaVolume/setMuted/play/pause
    //    + 当前是否有声 audible 上报）。**默认仍静音**（MEDIA_AUDIO_DEFAULTS.muted=true）。
    // 落盘都走 DATA_DIR（与 settings/custom-dir 同目录，重启后仍在），写盘 debounce，失败只 warn。
    const WEB_STORE_FILE = join(DATA_DIR, 'web-store.json');
    const MEDIA_AUDIO_FILE = join(DATA_DIR, 'media-audio.json');
    const readJsonFile = (p, fallback) => {
      try { const o = JSON.parse(readFileSync(p, 'utf8')); return o && typeof o === 'object' ? o : fallback; } catch { return fallback; }
    };
    const writeJsonFile = (p, obj) => {
      try { mkdirSync(DATA_DIR, { recursive: true }); writeFileSync(p, JSON.stringify(obj)); return true; } catch (e) { console.warn('[dsh-mpkg-wallpaper] 写盘失败(' + p + '):', e && e.message); return false; }
    };
    let webStoreCache = null;                 // { [wallId]: { store:{k:v}, updatedAt } }
    let webStoreTimer = null;
    const webStoreAll = () => {
      if (!webStoreCache) webStoreCache = readJsonFile(WEB_STORE_FILE, {});
      return webStoreCache;
    };
    const webStoreSaveSoon = () => {
      if (webStoreTimer) return;
      webStoreTimer = setTimeout(() => { webStoreTimer = null; writeJsonFile(WEB_STORE_FILE, webStoreAll()); }, 400);
      try { if (webStoreTimer.unref) webStoreTimer.unref(); } catch { /* 忽略 */ }
    };
    const webStoreGet = (wall) => sanitizeWebStore((webStoreAll()[wall] || {}).store || {});
    const webStorePut = (wall, patch) => {
      const all = webStoreAll();
      const cur = all[wall] && typeof all[wall] === 'object' ? all[wall] : { store: {}, updatedAt: 0 };
      const next = Object.assign({}, sanitizeWebStore(cur.store || {}));
      if (patch && typeof patch.del === 'string' && patch.del) delete next[patch.del];
      else if (patch && typeof patch.k === 'string' && patch.k && typeof patch.v === 'string') {
        // 单值上限与帧内同一常量：超限直接拒（不落半截值，也不让磁盘被单键撑爆）
        if (patch.v.length > WEB_STORE_MAX_VALUE || patch.k.length > 256) return { ok: false, error: 'too large' };
        next[patch.k] = patch.v;
      } else return { ok: false, error: 'bad patch' };
      // 单壁纸总字节超限：丢弃最旧的键（不静默丢新写入的键）
      let bytes = Buffer.byteLength(JSON.stringify(next), 'utf8');
      const keys = Object.keys(next);
      while (bytes > WEB_STORE_MAX_BYTES && keys.length > 1) { delete next[keys.shift()]; bytes = Buffer.byteLength(JSON.stringify(next), 'utf8'); }
      all[wall] = { store: next, updatedAt: Date.now() };
      // 壁纸数量上限：最旧先删（与 diag/转码缓存同一策略）
      const ids = Object.keys(all);
      if (ids.length > WEB_STORE_MAX_WALLS) {
        ids.sort((a, b) => (Number(all[a].updatedAt) || 0) - (Number(all[b].updatedAt) || 0));
        for (const id of ids.slice(0, ids.length - WEB_STORE_MAX_WALLS)) delete all[id];
      }
      webStoreSaveSoon();
      return { ok: true, keys: Object.keys(next).length };
    };
    let mediaAudioCache = null;
    const mediaAudioState = () => {
      if (!mediaAudioCache) mediaAudioCache = normalizeMediaAudio(readJsonFile(MEDIA_AUDIO_FILE, null), null);
      return mediaAudioCache;
    };
    const mediaAudioSet = (patch) => {
      mediaAudioCache = normalizeMediaAudio(patch, mediaAudioState());
      writeJsonFile(MEDIA_AUDIO_FILE, mediaAudioCache);
      // 状态变化落实到**正在跑的网页壁纸帧**：客户端（UI 线）已有的 `policy` op 会把
      // muted/speed 推下去；音量是本次新增字段（shim 侧 policy.volume）。宿主不直接改帧内 DOM
      // （不透明源下不可达），只在种子脚本里下发首帧值 —— 见 serveWebAsset。
      return mediaAudioCache;
    };
    // ①(WP-1) `text/plain` 正文（帧内 facade 的 CORS 简单请求，不触发 preflight）
    const readSmallBody = async (req, limit = 128 * 1024) => {
      let body = '';
      try { for await (const c of req) { body += String(c); if (body.length > limit) break; } } catch { /* 读失败按空处理 */ }
      return body;
    };
    webServer.register({
      kind: 'exact', path: WEB_STORE_ROUTE,
      handler: async (req, res) => {
        const cors = webAssetCorsHeaders(req.headers.origin) || {};
        try {
          if (req.method === 'OPTIONS') { res.writeHead(204, cors); res.end(); return; }
          const url = new URL(req.url || '', 'http://localhost');
          if (req.method === 'GET') {
            const wall = url.searchParams.get('w') || '';
            if (!wall) { res.writeHead(400, Object.assign({ 'content-type': 'application/json' }, cors)); res.end(JSON.stringify({ ok: false, error: 'no wall' })); return; }
            const store = webStoreGet(wall);
            const body = Buffer.from(JSON.stringify({ ok: true, wall, keys: Object.keys(store).length, store }), 'utf8');
            res.writeHead(200, Object.assign({ 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-cache', 'content-length': body.length }, cors));
            res.end(body);
            return;
          }
          ...
          const wall = typeof patch.w === 'string' ? patch.w : '';
          if (!/^[A-Za-z0-9_.:-]{1,80}$/.test(wall)) { res.writeHead(400, Object.assign({ 'content-type': 'application/json' }, cors)); res.end(JSON.stringify({ ok: false, error: 'bad wall' })); return; }
          const out = webStorePut(wall, patch);
          const body = Buffer.from(JSON.stringify(out), 'utf8');
          res.writeHead(out.ok ? 200 : 400, Object.assign({ 'content-type': 'application/json; charset=utf-8', 'content-length': body.length }, cors));
          res.end(body);
        } catch (err) {
          try { json(res, 500, { ok: false, error: String(err && err.message || err) }); } catch { /* 忽略 */ }
        }
      },
    });
    webServer.register({
      kind: 'exact', path: MEDIA_AUDIO_ROUTE,
      handler: async (req, res) => {
        try {
          if (req.method === 'GET') { json(res, 200, Object.assign({ ok: true }, mediaAudioReport(mediaAudioState()))); return; }
          const raw = await readSmallBody(req, 16 * 1024);
          let patch = {};
          try { patch = JSON.parse(raw || '{}'); } catch { patch = {}; }
          mediaAudioSet(patch);
          json(res, 200, Object.assign({ ok: true }, mediaAudioReport(mediaAudioState())));
        } catch (err) {
          json(res, 500, { ok: false, error: String(err && err.message || err) });
        }
      },
    ...
    //   · ①(WP-1) 页面自带 CSP 会挡掉 inline shim（照抄上游 hasBlockingCsp 判定）⇒ **不注入**、
    //     加 `x-mpw-shim-skipped: csp` 留痕、原样返回；客户端既有的 2.5s「shim 未报到 → 去标记
    //     重载」兜底会接管（docs §7）。**绝不用"注入了但被 CSP 拒"这种半吊子状态**。
    const WEB_INJECT_MAX_BYTES = 8 * 1024 * 1024;
    const isHtmlName = (name) => /\.(x?html?)$/i.test(String(name || ''));
    /* ②(2026-09-20 真机修复) **文档类请求的失败绝不用 JSON 回**。
       现场：用户点了一个源形状不对/目录已换的 web 档 ⇒ `/custom-folder/<x>/index.html` 回
       404 `{"ok":false,"error":"not found"}` ⇒ 浏览器把这段 **JSON 原样渲染进 iframe**，
       用户看到的"壁纸"就是一坨 `{"ok":false,…}`（真机截图级现场）。
       判据：目标是 .html/.htm，或请求头 `Sec-Fetch-Dest` 是 iframe/document/frame/embed/object
       ⇒ 回一个**极小的空 HTML**（404 状态码照旧；探针/日志仍能看出是 404）。
       为什么不是"始终回 HTML"：`/custom-folder/<x>/assets/*.js` 这类子资源请求的失败应当是
       明确的 JSON（前端 fetch 会读 `ok:false`），改掉反而让诊断变差。 */
    const looksLikeDocumentReq = (req, relFile) => {
      try {
        if (isHtmlName(relFile)) return true;
        const d = String((req && req.headers && req.headers['sec-fetch-dest']) || '').toLowerCase();
        return d === 'iframe' || d === 'document' || d === 'frame' || d === 'embed' || d === 'object';
      } catch { return false }
    };
    const docNotFound = (res, cors) => {
      const body = Buffer.from('<!doctype html><meta charset="utf-8"><title>404</title><body style="margin:0;background:#101216"></body>', 'utf8');
      res.writeHead(404, Object.assign({ 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-cache', 'content-length': body.length }, cors || {}));
      res.end(body);
    };
    const serveWebAsset = (req, res, baseDir, relFile, label, wallKey) => {
      try {
        ...
        if (!existsSync(target) || !statSync(target).isFile()) {
          // ②(2026-09-20) 文档类请求 ⇒ HTML（避免把 JSON 渲染成壁纸）；其它子资源保持 JSON
          if (looksLikeDocumentReq(req, relFile)) { docNotFound(res, cors); return; }
        ...
        if (isHtmlName(relFile) && isShimRequest(req.url)) {
          ...
          // ①(WP-1) 页面自带 CSP 挡 inline script ⇒ 不注入（注了必被拒），原样返回 + 留痕。
          //   客户端既有的 2.5s「shim 未报到 → 去标记重载」兜底（docs §7）接管，绝不白屏。
          if (hasBlockingCsp(html)) {
            try { console.warn('[dsh-mpkg-wallpaper] 网页壁纸自带 CSP 挡 inline shim，跳过注入（' + (label || '') + '/' + relFile + '）'); } catch { /* 忽略 */ }
            const body = Buffer.from(html, 'utf8');
            res.writeHead(200, Object.assign({
              'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-cache',
              'content-length': body.length, 'x-mpw-shim-skipped': 'csp',
            }, cors || {}));
            res.end(body);
            return;
          }
          const policy = webPolicyFromQuery(req.url) || { muted: true, speed: 1, paused: false };
          // ①(WP-1) 宿主音量接管（**只有显式写过 /media-audio 才生效**；默认档一个字段都不加）
          const audioPatch = mediaAudioPatchForEntry(mediaAudioState());
          if (audioPatch) { policy.muted = audioPatch.muted; policy.volume = audioPatch.volume; }
          // ①(WP-1) 帧内存储 facade：种子给 wallId + 回写 URL + 已存快照；
          //   `?mpwstore=0` 显式关（种子写 `store:false`，帧内不装 facade = 旧行为）。
          const storeMode = webStoreFromQuery(req.url);
          const wall = webWallId(label, wallKey, relFile);
          //   `?mpwstore=0` ⇒ extras 只留 `{store:false}`（**不含 wall**）：帧内不装 facade = 改动前行为，
          //   且种子与旧版的差异**只有**这一个字段（测试断言"剥掉它 == 旧种子字面量"）。
          const extras = storeMode.on
            ? { wall, store: { id: wall, url: WEB_STORE_ROUTE, persist: !!storeMode.persist, snap: storeMode.persist ? webStoreGet(wall) : {} } }
            : { store: false };
          // ①(WP-1) 源级 file:/// 改写：只覆盖运行时钩子够不到的形态（HTML 里写死的属性 / url()）
          const fileUrlBase = label === 'library' ? (BASE + '/library-web/' + wallKey + '/') : (BASE + '/custom-folder/' + wallKey + '/');
          const out = rewriteWebEntryHtml(html, {
            seedScript: buildSeedScript(policy, relFile, extras),
            fileUrlBase,
          });
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          if (!folder || !file || folder.includes('..') || folder.includes('/') || folder.includes('\\') || folder.startsWith('.')) {
            // ②(2026-09-20) 形状不对的**文档请求**（如把库 token 当 folder）回 HTML，不回 JSON
            if (looksLikeDocumentReq(req, file)) { docNotFound(res); return; }
            json(res, 403, { ok: false, error: 'forbidden' }); return;
          }
          // ①(2026-09-16 I 项) 统一走 serveWebAsset：入口 HTML 注入 WE shim（?mpwshim=1），其余原样
          serveWebAsset(req, res, join(customDir, folder), file, 'custom', folder);   // ①(WP-1) wallKey=目录名（存储隔离键，不含绝对路径）
        } catch (err) { json(res, 500, { ok: false, error: String(err && err.message || err) }); }
      },
    });
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          const rec = library.get(lt);
          if (!rec || !rec.dir || !file || file.includes('..') || file.startsWith('/') || file.includes('\\')) {
            // ②(2026-09-20 审计第 3 条) 库清单为空/未注册的 token ⇒ 403（语义如实）；
            //   但**文档请求**回 HTML：否则 iframe 里又是一坨 JSON。
            if (looksLikeDocumentReq(req, file)) { docNotFound(res); return; }
            json(res, 403, { ok: false, error: 'forbidden' }); return;
          }
          // ①(2026-09-16 I 项) 同上：入口 HTML 注入 WE shim（?mpwshim=1）
          serveWebAsset(req, res, rec.dir, file, 'library', lt);   // ①(WP-1) wallKey=库 token（存储隔离键）
        } catch (err) { json(res, 500, { ok: false, error: String(err && err.message || err) }); }
      },
    });
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          const st = statSync(filePath);
          // ①(2026-09-21 音频格式补全) 除 .mpkg/.pkg/.json 外原先**一律 octet-stream** ⇒ 客户端
          //   npTrackUrl 对自定义目录里的单段音轨走的正是 /raw（`?custom=1&folder=X&file=song.flac`），
          //   .flac/.mp3/.m4a 拿到 octet-stream 后浏览器 <audio> 不认（用户侧"目录里扫不出/放不响"）。
          //   音频改走统一表 + magic 双判（audioMimeFor：头字节判定优先、后缀兜底；
          //   ID3v2 前缀 + .flac 后缀的特例让带标签的 FLAC 不被误报成 audio/mpeg）。
          //   路径本身没有 audioMimeFor 的契约，但 §3 的嗅探只需 16 字节且这条路由已经 stat 过文件。
          const mime = /\.mpkg$/i.test(file) || /\.pkg$/i.test(file) ? 'application/octet-stream'
            : /\.json$/i.test(file) ? 'application/json'
            : (() => {
              // 非音频后缀不必读头：audioMimeFor 在后缀不在 §1.2 表内时必然返回 ''，读头是白花的 IO。
              if (!suffixAudioMime(file)) return 'application/octet-stream';
              let head = null;
              try {
                const fd = openSync(filePath, 'r');
                const buf = Buffer.alloc(16);
                let n = 0;
                try { n = readSync(fd, buf, 0, 16, 0); } finally { closeSync(fd); }
                head = n > 0 ? buf.subarray(0, n) : null;
              } catch { head = null; }   // 读头失败 → audioMimeFor 回落后缀兜底（规格 §5.4 同款口径）
              return audioMimeFor(file, head) || 'application/octet-stream';
            })();
    ...
    /* ①(2026-09-21 用户第 11 条) 「网页壁纸风险预检」**独立查询接口**（结构化数据）：
       GET /web-probe?folder=<自定义目录下的子目录>      → 自定义目录里的那张网页壁纸
       GET /web-probe?ltoken=<库 token>                → 本地库/Steam 里那张
       返回 { ok, probe:{ heavy, external, heavyHits[], externalRefs[], htmlFile, reasons[], limits } }。
       为什么单开一条：测试台（:8902）要在导入/选择前把"重动画 / 需外网"两个标记显示出来，
       而扫描接口（/custom-dir、/steam-inventory）只在**扫描那一刻**给一次、且条目结构各不相同。
       判定与两处扫描**共用同一实现** probeWebWallpaper()，不另写一份（否则迟早漂移）。
       形状契约见 docs/WEB-WALLPAPER.md「风险预检接口」。 */
    webServer.register({
      kind: 'exact', path: BASE + '/web-probe',
      handler: (req, res) => {
        try {
          const url = new URL(req.url || '', 'http://localhost');
          const folder = url.searchParams.get('folder') || '';
          ...
          if (ltoken) {
            ...
            dir = rec.dir;
            entry = rec.media ? String(rec.media).split(/[\\/]/).pop() : null;
          } else if (folder) {
            if (bad(folder)) { json(res, 403, { ok: false, error: 'forbidden' }); return }
            if (!customDir) restoreCustomDir();
            if (!customDir) { json(res, 404, { ok: false, error: 'no dir' }); return }
            dir = resolve(join(customDir, folder));
            if (!existsSync(dir) || !statSync(dir).isDirectory()) { json(res, 404, { ok: false, error: 'not found' }); return }
            const det = detectWallpaperDir(dir);
            entry = det.entry || null;
          } else {
            json(res, 400, { ok: false, error: 'need folder or ltoken' }); return;
          }
          const probe = probeWebWallpaper(dir, entry);
          json(res, 200, { ok: true, target: ltoken ? ('library|' + ltoken) : ('custom|' + folder), probe });
        } catch (err) { json(res, 500, { ok: false, error: String(err && err.message || err) }) }
      },
    });
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          /* ②(2026-09-20 真机修复·根因 B) **目录不存在 ⇒ 404，不是 500**。
             真机现场：客户端档里的 `folder` 是"当前自定义目录下的子目录"，而宿主侧 customDir
             还停在旧目录（用户换过目录）⇒ 拼出来的路径不存在 ⇒ `scanSceneAudio` 抛 ENOENT
             ⇒ 外层 catch 一律回 500 `{error:"ENOENT: no such file or directory, stat '…/dd/小鸟游星野'"}`。
             500 会被读成"宿主坏了"，而真相是"这个目录不在当前库里"（客户端据此重同步目录并重试一次）。
             语义与相邻路由统一：参数非法 403 / 解析不到 404 / 内部错误才 500。 */
          const audioDir = resolve(join(customDir, folder));
          if (!existsSync(audioDir) || !statSync(audioDir).isDirectory()) { json(res, 404, { ok: false, error: 'not found' }); return }
          serveSceneAudio(req, res, audioDir);
        } catch (err) { json(res, 500, { ok: false, error: String(err && err.message || err) }) }
      },
    });
    webServer.register({
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          // ②(2026-09-20 根因 B 同口径) 目录不在当前库里 ⇒ 404（不是 ENOENT 抛出的 500）
          {
            const frDir = resolve(join(customDir, folder));
            if (!existsSync(frDir) || !statSync(frDir).isDirectory()) { json(res, 404, { ok: false, error: 'not found' }); return }
            serveSceneFrame(req, res, frDir, null);
          }
        } catch (err) { json(res, 500, { ok: false, error: String(err && err.message || err) }); }
      },
    });
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          // ②(2026-09-20 根因 B 同口径) 目录不在当前库里 ⇒ 404（不是 ENOENT 抛出的 500）
          {
            const svDir = resolve(join(customDir, folder));
            if (!existsSync(svDir) || !statSync(svDir).isDirectory()) { json(res, 404, { ok: false, error: 'not found' }); return }
            serveSceneVideo(req, res, svDir);
          }
        } catch (err) { json(res, 500, { ok: false, error: String(err && err.message || err) }); }
      },
    });
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          // ①(2026-09-20 同类缺陷一并修) 同 `/diag`：超限不再 `req.destroy()`（那样跨源渲染器只会看到
          //   连接被掐、拿不到 413），改为**读干净但不再缓存**后回 413 + CORS 头（跨源也能读到原因）。
          let body = '';
          let tooBig = false;
          req.on('data', (c) => {
            if (tooBig) return;
            body += c;
            if (body.length > THUMB_BODY_MAX) { tooBig = true; body = ''; }
          });
          req.on('end', () => {
            if (tooBig) { jsonCors(413, { ok: false, error: 'scene thumb payload 超过上限 ' + THUMB_BODY_MAX + ' 字节（未落盘）' }); return; }
            try {
              let d = null;
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          const st = statSync(filePath);
          // ①(2026-09-21 音频格式补全) 这条同样只有视频/HTML 分支（本地壁纸库里选的单段音轨拿到
          //   octet-stream）。音频改查统一表：库媒体按文件名选，magic 嗅探留给 /raw 与 /custom-folder。
          const libAudio = suffixAudioMime(filePath);
          const mime = libAudio ? libAudio
        ...
        try {
          ...
          const scan = (root) => {
            ...
            for (const dir of dirs) {
              ...
              try {
                /* ①(2026-09-24 包内/包旁 JSON 收口线) **包旁** JSON：Steam 库里 `<壁纸目录>/project.json`
                   （与 scene.pkg 同级，官方工坊布局）⇒ 走**宿主侧同一份**宽容解析（尾逗号/注释/BOM；
                   官方 `assets/effects/fluidsimulation/effect.json` 第 402 行自己就带尾逗号）。
                   语义与改动前一致：读不到/真坏 JSON ⇒ 仍然抛（下面的既有 catch 照旧跳过该壁纸并 console.warn）。
                   多出来的是**记账**：解析器计数（`mpwWeJsonStats().failed`）+ 下面 catch 里的
                   `mpwWeJsonSwallowed`（计数 + 一行诊断；同 where+原因只吵一次）—— 原来只有一句 warn，
                   "坏在哪一步/坏了几次"看不出来。**注意只在解析这一段**调用记账：`readdirSync`/探测
                   失败不该被记成"JSON 坏"（归因不能糊）。 */
                const meta = (() => {
                  try { return mpwParseWeJson(readFileSync(proj, 'utf8')); }
                ...
                if (type === 'video') {
                  ...
                  if (h) media = join(p, h);
                  // ①(2026-09-21 第 11 条) 与 /custom-dir 共用同一个 probeWebWallpaper（不再各写一份递归）
                  const wp = probeWebWallpaper(p, h || null);
                  webHeavy = wp.heavy; webExternal = wp.external;
                }
      ...
      handler: (req, res) => {
        try {
          ...
          const limits = {
            ...
            // ①(2026-09-23 B) 缩放 flags 白名单（`/transcode?scale=`）。预缩档
            //   「按屏幕物理尺寸预缩」= 客户端按屏幕物理宽传 maxW + `scale=lanczos`；
            //   默认档（不传）= ffmpeg 默认 bicubic，与改动前逐字节相同。
            //   ⚠ 登记点本应在 docs（本仓 docs/TRANSCODE-RESOURCE.md 与渲染器仓
            //   docs/README-DIAGNOSTICS.md 都属并行线/禁碰），本线只在**代码内**登记（此处 + 启动日志）。
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const maxWRaw = Number(url.searchParams.get('maxW'));
          const maxW = Number.isFinite(maxWRaw) && maxWRaw > 0 ? Math.min(3840, Math.round(maxWRaw)) : 0;
          // ①(2026-09-23 B) 预缩档：`scale=lanczos`（白名单外 ⇒ 400，不把任意串拼进 -vf）。
          //  它只影响**缩放滤波器**；目标宽仍由 maxW 决定（客户端按屏幕物理宽算 maxW）。
          const scaleFlag = normalizeScaleFlag(url.searchParams.get('scale'));
          if (scaleFlag === null) { json(res, 400, { ok: false, error: 'invalid scale (allowed: ' + SCALE_FLAGS.join('/') + ')' }); return; }
          ...
          try { out = await transcodeToFps(src, fps, effMaxW, () => reqAborted, scaleFlag); } catch (err) { transcodeErr = err; } /* 转码失败/取消 → 回退原片 */
...
export const __mpwTest = {
  ...
  // ①(2026-09-23 用户第 4 项) pkg/mpkg 导入链的测试出口（tools/pkg-import-test.mjs 直接调真实现）：
  //   容器头倍增读取 / 越界可读错误 / 压缩条目识别 / 上传物回收（上限 + 在用保护）。
  readMpkgHeadGrow, parseMpkgHead, looksCompressedEntry, pruneUploads, uploadStat, referencedUploadTokens,
  // ①(2026-09-23 用户第 1 项 B) 预缩档的纯函数出口（tools/transcode-prescale-test.mjs 直接断言
  //  "lanczos 只进 vf/键，不进既有默认档"——不需要真跑 ffmpeg）：
  normalizeScaleFlag, buildScaleFilter, transcodeKey, SCALE_FLAGS,
  ...
  // ①(WP-1) 网页壁纸轮（存储 facade / 宿主音量 / CSP / 源级 URL 改写）的纯函数出口：
  //   `tools/web-wallpaper-test.mjs` 的 H/I/J 段直接调它们断言语义，K 段再走真路由端到端。
  //   **只读/纯函数**，不改变插件在宿主里的注册面（宿主只用 `apply`）。
  hasBlockingCsp, rewriteWebFileUrlsInHtml, webStoreFromQuery, sanitizeWebStore, webWallId,
  MEDIA_AUDIO_DEFAULTS, normalizeMediaAudio, mediaAudioReport, mediaAudioPatchForEntry,
  webStoreFile: () => WEB_STORE_FILE, mediaAudioFile: () => MEDIA_AUDIO_FILE,
  limits: {
    ...
    HEAD_BYTES, MPKG_TABLE_MAX_BYTES, UPLOAD_KEEP, UPLOAD_MAX_BYTES, UPLOAD_PROTECT_MS,
