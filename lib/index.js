...
let customDir = null;
/** ①(新) mpkg preview 缓存：key=文件名 → { mtimeMs, dataStart, entries, previewIndex, bytes }。
 *  扫描列表缩略图会请求每个 mpkg 的 preview——不缓存则每次都重新解析头部（2MB 读）+ 打开大文件（用户实测慢）。 */
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          const cached = mpkgPreviewCache.get(file);
          ...
          if (e.size <= 12 * 1024 * 1024) {
            ...
            mpkgPreviewCache.set(file, { mtimeMs: st.mtimeMs, mime, bytes: buf });
