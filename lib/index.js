    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          const img = entries.findIndex((e) => /preview.*\.(gif|png|jpe?g|webp)$/i.test(e.name))
            >= 0 ? entries.findIndex((e) => /preview.*\.(gif|png|jpe?g|webp)$/i.test(e.name))
            : entries.findIndex((e) => /\.(gif|png|jpe?g|webp)$/i.test(e.name));
