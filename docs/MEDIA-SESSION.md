...
## 6. 如何接进 NP 控件（**已于 2026-09-20 接线**，本节保留接线口径与取证行号）

> 状态更新（2026-09-20）：本节描述的接线**已经落地**（宿主 `lib/index.js` 的 `/media-session`、`/media-control`、
> `/media-art` 三条路由 + 客户端 `lib/client.js` 的 `npSystemMedia*` 补充路径），判据 = `tools/media-session-wiring-test.mjs`
> （**24 断言 + 3 组变异自证**，已进 `tools/check.sh` 第 2 步）。下面 §6.0 的"留给下一个代理"是**当时的**记录，
> 保留原文以便对照；实际实现与本节建议的差异只有三处（都在客户端）：
>   ① 轮询不用 `setInterval(2000)`，改成**自适应单链**：有系统媒体 2s、不可用/失败 30s（一次 `snapshot()` = 宿主起一个进程，
>      本机没会话总线时每 2s 探一次纯属浪费）；
>   ② 定时器统一走 `npSysTimerSet`（有 `unref` 就 unref）—— 否则桩 DOM 门禁"跑完了却不结束"（实测踩到）；
>   ③ 连续失败 ≥3 次**降到 30s 慢探**而不是彻底停（后端起来了还能自己接上）。
> 另外 `?describe=1` 才在快照响应里附带 `mediaSession.describe()`（默认档保持 21 键形状不变）。
