...
> Version scope: this document describes the implementation shipped as **`3.7.3`** in `package.json`. The publish surface is **14 files** (`lib/` runtime files + `icon.svg`, `cordis.patch.yml`, `README.md`, `README.en.md`, `THIRD-PARTY.md`, `LICENSE`); `lib/liquid-glass/**`, `lib/liquid-glass-bundle.js`, `dist/`, `tools/` and `docs/` never enter the npm package (`package.json:8-21`).

---

## What changed in this version

Baseline = the version currently in `package.json`, **3.7.3**. The per-item release notes, rollback switches and measured values live in [`docs/RELEASE.md`](docs/RELEASE.md) §3.7.3→next and [`docs/RELEASE-READY-3.8.0.md`](docs/RELEASE-READY-3.8.0.md).

- **Now playing mounted in the left sidebar**: new setting `npNowPlaying` (default **off**), placed in the *Wallpaper* tab directly under the existing `mute` toggle (`lib/client.js:13126`). While off it injects nothing — no DOM, no observers, not a single NP rule in the `buildCss` output (`lib/now-playing.js:38-51`, group B of `tools/now-playing-test.mjs`). The component itself is a **line-by-line port of Bencho's "Now playing" (MIT)** with the upstream comments kept verbatim; attribution is registered in `THIRD-PARTY.md` §6. Pure math and component are split into `lib/now-playing-math.js` / `lib/now-playing.js` and **inlined byte-for-byte** into the generated region of `lib/client.js` (`MPW-NP-GEN-START/END`, `lib/client.js:6240`–`7884`) by `tools/build-now-playing.mjs`, guarded by two independent drift gates. Mounting and collapse criteria: see [Now playing widget](#now-playing-widget).
- **Host-side adapter for the system media session**: new `lib/media-session.js` — **MPRIS** on Linux (`playerctl` first, `dbus-send` as fallback), **SMTC** on Windows (`powershell.exe` + WinRT), explicitly `unsupported-platform` on macOS; with no desktop session bus it returns `available:false / no-session-bus` — it **never throws, never invents data and never fakes an action** (`lib/media-session.js:1-68`). ⚠ **The module is not wired up yet**: nothing imports it (`lib/index.js`, `lib/client.js` and `tools/build-bundle.mjs` all lack an import) and no host route exposes it, so the settings panel shows no system media information at all. Gate: `node tools/media-session-test.mjs` (95 assertions = 89 main + 6 mutations, not part of `tools/check.sh`).
- **Web-wallpaper rendering / API coverage (WP-1)**: an in-frame **storage facade** (under an opaque origin, touching real `localStorage` throws `SecurityError`) plus the host `/web-store` route persisting per wallpaper; **master volume = host volume × the author page's volume**; HTML **source-level** `file:///` rewriting; shim injection is **skipped** when a blocking CSP is detected (original bytes are served, never a 500); host `/media-audio` control contract (**still muted by default**). Rollback: the "Compatibility mode (same-origin)" button in the confirm dialog, or `?mpwstore=0`. Evidence: `lib/web-wallpaper.js:1-30`, `docs/WEB-WALLPAPER.md` §5.4/§6/§13.
- **Web-wallpaper touch (WP-2)**: a `postMessage` `op:'touch'` protocol plus real in-frame **`TouchEvent`s**, so single-finger drags and multi-touch sequences reach the author scripts, and "drag counted as a click" is fixed. The default mode is unchanged (`mpwWebIxMode()` still returns `pointer`). Rollback: setting `webInteraction="off"` or the URL `?mpwinteract=off|0` (`lib/web-interaction.js:99-107`, `lib/client.js:2622-2633`).
- **Several on-device fixes**: Now playing no longer mistakes the expanded 256px state for collapsed on the frame right after slot rendering (so the widget no longer disappears after a refresh) — the criterion is now **physical width first** (`lib/now-playing.js:420-425`, `:744-752`, threshold `NP_COLLAPSE_MAX_W=96`); the header-frost `ReferenceError` and the transparent right-side timeline rail (see [Historical ledger](#historical-ledger)); the `lgCss` block that never executed because of a TDZ error; `sessionFollow` that had a toggle but no reader; the folder picker scrolling back to the top.
- **Gate expansion**: 615-setting CSS matrix, style-scope guard, surface-token namespace equivalence, switch-wiring audit and a second-scale pre-commit hook (all in the [historical ledger](#historical-ledger) and [Gates](#gates)).

> This round **changes no default behaviour**: `webInteraction` still defaults to `pointer` (already the case in the published 3.7.3), `npNowPlaying` is off by default and injects nothing, and WP-1 completes existing web-wallpaper capability (still muted by default).
...
| **Now playing (above Settings in the sidebar)** | `npNowPlaying` | **off** | Mounts an expandable player in the left sidebar; off injects nothing | off / restore defaults |
...
- **Mount point**: the host slot `sidebar.footer.action` (the plugin registers `createSlotAction` with `id:"mpw-now-playing"`, `order:60`, `lib/client.js:15411-15416`). When the slot is unavailable it walks a three-step fallback chain — before `[data-slot="sidebar.settings"]` → before `[class*="settingsArea"]` → first position in `[class*="footArea"]`; if none is found it **creates no node at all** and logs a `console.warn` (`lib/now-playing.js:464-488`, `:863-867`).
- **Hidden when the sidebar is collapsed**: `data-mpw-np-hidden` + CSS `display:none` (`lib/now-playing.js:174`). The criterion is **physical width first** (`shouldHide()` reads `[class*="sidebarCol"]`, threshold `NP_COLLAPSE_MAX_W=96`, strict `<`); host signals (slot `wide` / `data-sidebar-collapsed` / root class `collapsed`) are only a fallback when the width cannot be measured. When space is tight the whole widget scales by `--mpw-np-fit = clamp(avail/260, 0.5, 1)` (`lib/now-playing.js:106-115`, `:420-425`, `:744-769`).
- **Shape**: one pill that expands into a card — artwork, title/byline, progress rail + clock, a full-card hit target and three transport buttons (restart / play-pause / mute). Expanding is a self-stopping 0→1 tween (no resident rAF). Play/pause is not a swapped icon but the pair of eight-point quads (`lib/now-playing.js:506-529`, `:559-627`).
- **Four data-source cases** (`lib/client.js:5198-5278`):

| Current wallpaper | What NP shows | What it can control |
|---|---|---|
| Video wallpaper (embedded mp4 / video texture) | Real playback state; duration and progress available | Play / pause / restart / mute (directly on that `<video>`) |
| Web wallpaper | Has a sound channel | Mute (through the existing `mute` setting + host `/media-audio`) |
| Scene / custom-folder wallpaper with its own audio | Track list (`/custom-scene-audio`, `/library-scene-audio`) | **Display only, no control** (there is no author player to drive) |
| Static image / no audio at all | Idle state | No "does nothing when clicked" buttons are shown |

- **What it cannot do** (listed explicitly; no fake actions, `docs/NOW-PLAYING-DSH.md` §4): there is no system media source (that is what `lib/media-session.js` above is for, and it is not wired up yet); scene/custom-wallpaper audio has no transport and no duration; web wallpapers have no 0..1 numeric volume; no playlists, no previous/next; the heart button is not rendered; no seeking, no waveform, no keyboard shortcuts.
- **Rollback**: turn `npNowPlaying` off (or "Restore all defaults") ⇒ back to zero injection. Regression: `node tools/now-playing-test.mjs` (79 assertions = 73 main + 6 mutations, wired into step 2 of `tools/check.sh`); live probe `node tools/np-sidebar-live-probe.mjs` (12 live checks + `--selftest` with 4 browserless checks, not part of the gate).
- **Attribution**: the component is a **line-by-line port of Bencho's "Now playing" (MIT)** with the upstream comments kept verbatim; the sidebar mount controller, self-drawn icons, token mapping, data wiring and gates are written here. Registered in `THIRD-PARTY.md` §6.
...
```
dsh-mpkg-wallpaper/
├── package.json      # version 3.7.3; dsh.bundle + dsh.client declarations; files allow-list = publish surface (14 files)
...
│   ├── now-playing.js      # Now playing component + sidebar mount controller (Bencho MIT port + this repo's controller)
...
│                     #   token-namespace-test.mjs / now-playing-test.mjs + build-now-playing.mjs (inline generation + drift gates) /
│                     #   media-session-test.mjs (not in the gate) / web-wallpaper-test.mjs / web-interaction-test.mjs /
...
The 12 steps of `tools/check.sh`: ① syntax ② panel smoke + P-66 + folder picker + wallpaper-layer visibility + persistence + NP (both drift gates) ③ CSS matrix (all 512 combinations + 600 random + boundaries) ④ scene watchdog/debug params ⑤ sandbox and scene tokens + web-wallpaper shim + transcode resources ⑥ publish integrity self-check ⑦ audio-scan speed-up ⑧ scene video indexing ⑨ on-device replica A/B ⑩ better-sidebar compatibility ⑪ single-file bundle equivalence ⑫ style-scope guard + surface token namespace.
