...
面板 tab 顺序固定为 `TAB_ORDER = ["source","wallpaper","appearance","unify","blur","other","liquid"]`（`lib/client.js:10730`），界面文案：**背景来源 / 壁纸设置 / 外观 / 界面统一 / 界面虚化 / 其他 / 测试项**。
...
插件已发布到 npm（`dsh-mpkg-wallpaper`）。四种装载方式：
...
更新：`dsh plugin --profile web update dsh-mpkg-wallpaper`（+ 重启 `dsh web` + Ctrl+F5）。走 `latest` 标签。
...
与方式一同源，只是不经 `dsh plugin` 包装；更新同样走依赖表。
...
> 方式三不写依赖表 ⇒ 市场不显示「已安装」（只影响显示，不影响功能）；更新用 `git pull`。
...
node tools/build-bundle.mjs          # 产物：dist/dsh-mpkg-wallpaper.bundle.mjs（实测 442 317 B / 432.0KB）
...
| 场景 | 真实行为（含代码位置） |
|---|---|
| 没装 WE（或找不到 `wallpaper32.exe`） | `GET /api/mpkg-wallpaper/steam-inventory` 返回 **200 `{ok:true, installDir:null, wallpapers:[]}`**（不是错误、不抛 500）—— `lib/index.js:3120-3180` |
| 点「扫描本地壁纸库」 | 列表空 + 顶部报错条「未检测到壁纸引擎安装（需 Windows + Steam 版 Wallpaper Engine）」（判 `!d.installDir`，`lib/client.js:8781`/`:11555`）；空列表另有提示「未发现可用的壁纸（或不是 Windows 环境）」。**扫描本身不报错** |
| 装了 WE 但素材目录都不存在 | 每个根目录单独判存在（`if (!existsSync(root)) return`）⇒ 清单为空、**且不弹「未检测到安装」**（`installDir` 非空）；界面只剩空提示。**未实现**：没有"WE 装了但素材目录缺失"的**专用**提示 |
| 非 Windows / 移动端 | 同"没装 WE"（注册表分支直接在非 win32 返回 null）；Steam 目录探测串都是纯字符串、`existsSync` 自然为 false，无副作用 |
| WE 原生播放列表 | 只在 `installDir` 存在且 `config.json` 可解析时才有；否则客户端首次扫描后写 `rotSeeded:true` 并**不再重播种**（`lib/client.js:8782-8793`），避免覆盖自定义轮换 |
| **没装 WE 时仍然可用的** | ① 手动选目录（`/list-dirs` + `/custom-dir`）；② 直接导入 `.mpkg`（hybrid 无 600MB 上限）；③ 网页/视频类壁纸走 URL 或本地文件；④ 场景提取、音频清单、设置持久化、诊断上报全都不依赖 WE |
| 宿主端整体不可用（方式四没装/端口不通） | 客户端探测 `/ping` 失败 ⇒ 回退**纯浏览器模式**：状态行「宿主端不可用（已回退纯浏览器模式，600MB 上限）」（`lib/client.js:10203`/`:11603`）；超过 600MB 的素材无法处理 |
| `ffmpeg` 缺失 | `GET /api/mpkg-wallpaper/ffmpeg-check` 返回 **200 `{ok:true, found:false, source:null, path:null, version:null}`**（`lib/index.js:3245-3259`），面板显示"未安装"并只在用户点按后才走下载链；可直读的视频不做转码 |
...
| Firefox | ⭐⭐ 中 | 功能全支持（`backdrop-filter` 103+、编码不支持自动转码兜底）；三点减分：`backdrop-filter` 性能弱于 Chromium、`iframe.muted` 不支持（有声 web 壁纸首次可能被自动播放策略拦停首帧）、`color-mix` 需 113+ |
