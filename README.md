...
当前版本 = `package.json` 的 **3.12.0**（本轮发布线 3.8.0 → 3.8.1 → 3.8.2 → 3.9.0 → 3.9.1 → 3.10.0 → 3.10.1 → 3.11.0 → **3.12.0**）。发布前置/命令/回滚见 [`docs/RELEASE.md`](docs/RELEASE.md)，本轮改动清单见 [`docs/RELEASE-READY-3.8.0.md`](docs/RELEASE-READY-3.8.0.md)，Now playing / 壁纸声音的根因与真机读数见 [`docs/NOW-PLAYING-DSH.md`](docs/NOW-PLAYING-DSH.md) §7.7。

- **3.12.0：WSL 上 Steam 装在别的盘也能扫到（安装目录候选抽成可注入纯函数 + 盘符枚举）+ 插件侧跨平台静态门禁** ——
  ①真漏网：WSL 分支原来只写死 `/mnt/c/...`，Steam 装在 D:/E:/… 的 WSL 用户**永远扫不到**（Windows 与 WSL 双装、Steam 装 D 盘都很常见）。现在 `lib/index.js` 的 `steamProbeDirs(env)`（**纯函数**，`platform`/`release`/`homedir`/`exists`/`procVersion` 全可注入）在 `platform === 'linux'` 且判为 WSL 时枚举 `/mnt/d` … `/mnt/z` × `Program Files (x86)/Steam`、`Program Files/Steam`、`SteamLibrary`、`Steam` 四个相对路径；WSL 判据两条（`os.release()` 含 `microsoft`，老 WSL1 再读 `/proc/version`，读失败一律吞掉不抛）。非 WSL 的 linux/darwin **不**产出 `/mnt/d…z`；win32/darwin/linux/android 四个平台的候选集合与改动前**逐字节相同**（门禁逐项断言）。
  ②新增 `tools/cross-platform-test.mjs`（**63 断言 / 1.2s**，无浏览器无网络；登记进 `tools/check.sh` 的既有第 6 步，**不新增 step 编号**）：WSL 分支可判（在非 WSL 机器上用注入入参断言 —— 含 `/mnt/d/SteamLibrary`，换普通内核后 0 条 `/mnt/d…z`）、三平台各自成根且互不串台、纯函数纪律（确定性/无重复/退化入参不抛）、代码里的 `/tmp` 字面量与五类宿主绝对路径必须"同行可覆盖"（存量 25 处走逐条带理由的账本，**每条反查必须仍命中**，腐烂即判红）、shell 可移植（9 种 bash 4+ 写法 + shebang 与语法匹配）、文件名可移植、BOM/CRLF、G 段逐类合成反例必红 + 干净样本零发现，以及两处**变异自证**（删掉盘符枚举 ⇒ 判红；把路径写死 ⇒ 判红）。
